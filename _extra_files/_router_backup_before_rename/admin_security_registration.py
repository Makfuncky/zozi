...
def _find_user(db: Session, email: str | None, username: str | None) -> User | None:
    ...
    if username and '@' in username and (not email):
        return db_read_query(db, User).filter(User.email == username).first()
    return None
...
def register(payload: RegisterRequest, db: Session=Depends(get_db)):
    try:
        if db_read_query(db, User).filter(User.email == payload.email).first():
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail='Email already registered')
        if payload.username and db_read_query(db, User).filter(User.username == payload.username).first():
...
def refresh(payload: RefreshRequest | None=None, request: Request=None, db: Session=Depends(get_db)):
    ...
    user = db_read_query(db, User).filter(User.id == int(decoded.get('sub'))).first()
...
@router.post('/logout')
def logout(credentials: HTTPAuthorizationCredentials | None=Depends(bearer_scheme), current_user: User=Depends(get_current_user)):
    if credentials:
        try:
