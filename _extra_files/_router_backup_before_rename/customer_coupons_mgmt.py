...
@router.get("")
def list_coupons(_: dict = Depends(require_admin), db: Session = Depends(get_db), page: int = Query(1, ge=1), page_size: int = Query(20, ge=1, le=100)):
...
def delete_coupon(coupon_id: str, _: dict = Depends(require_admin), db: Session = Depends(get_db)):
