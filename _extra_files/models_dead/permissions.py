...
class PermissionCategory(Base):
    ...
    __table_args__ = (Index("ix_permission_categories_country_created", "country_code", "created_at"),)
    ...
    country_code = Column(String(3), nullable=False, server_default="OM")
...
class Permission(Base):
    ...
    __table_args__ = (Index("ix_permissions_country_created", "country_code", "created_at"),)
    id = Column(Integer, primary_key=True, index=True)
...
class RolePermissionAssignment(Base):
    ...
    __table_args__ = (
        UniqueConstraint("role_name", "permission_id", "country_code", name="uq_role_permission_country") + (Index("ix_role_permission_assignments_country_created", "country_code", "created_at"),))
    id = Column(Integer, primary_key=True, index=True)
    ...
    country_code = Column(String(3), ForeignKey("country_configs.code", ondelete="RESTRICT"), nullable=True)
...
class UserPermissionOverride(Base):
    ...
    __table_args__ = (
        UniqueConstraint("user_id", "permission_id", "country_code", name="uq_user_perm_override_country") + (Index("ix_user_permission_overrides_country_created", "country_code", "created_at"),))
    id = Column(Integer, primary_key=True, index=True)
...
class PermissionAuditLog(Base):
    ...
    __table_args__ = (Index("ix_permission_audit_log_country_created", "country_code", "created_at"),)
    id = Column(Integer, primary_key=True, index=True)
