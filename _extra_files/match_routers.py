...
def mods(d):
    return {f[:-3] for f in os.listdir(d) if f.endswith(".py") and f != "__init__.py"}

router_files = mods(routers_dir)
controller_files = mods(controllers_dir)

# missing names from prior analysis
...
for line in missing:
    ...
    # direct controller match by name
    direct = name in controller_files
    # similarity to controllers
    sims = []
    for c in controller_files:
        # token overlap
        t1, t2 = tokens(name), tokens(c)
        overlap = len(t1 & t2) / max(1, len(t1 | t2))
        ratio = SequenceMatcher(None, name, c).ratio()
        score = max(overlap, ratio)
        if score > 0.3:
            sims.append((round(score,2), c))
    sims.sort(reverse=True)
    print(f"{name:28s} direct_ctrl={direct}  top: {sims[:4]}")
