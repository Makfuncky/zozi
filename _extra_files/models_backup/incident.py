...
class IncidentThread(Base):
    ...
    __table_args__ = ({"schema": "communication"},)
    id = Column(Integer, primary_key=True, index=True)
    war_room_id = Column(Integer, ForeignKey("incident_war_rooms.id", ondelete="RESTRICT"), nullable=False, index=True)
...
class IncidentActionItem(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    war_room_id = Column(Integer, ForeignKey("incident_war_rooms.id", ondelete="RESTRICT"), nullable=False, index=True)
    assignee_id = Column(Integer, ForeignKey("users.id", ondelete="RESTRICT"), nullable=True, index=True)
    title = Column(String(200), nullable=False)
