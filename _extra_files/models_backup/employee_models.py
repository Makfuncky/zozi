...
class PhysicalIDCard(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    employee_id = Column(Integer, ForeignKey("employees.id", ondelete="RESTRICT"), unique=True, nullable=False, index=True)
...
class OrgUnit(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    name = Column(String(200), nullable=False)
    parent_id = Column(Integer, ForeignKey("org_units.id", ondelete="RESTRICT"), nullable=True, index=True)
...
class Employee(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    user_id = Column(Integer, ForeignKey("users.id", ondelete="RESTRICT"), unique=True)
    ...
    reporting_manager_id = Column(Integer, ForeignKey("employees.id", ondelete="RESTRICT"), nullable=True, index=True)
    hiring_manager_id = Column(Integer, ForeignKey("users.id", ondelete="RESTRICT"), nullable=True, index=True)
    authority_level = Column(Integer, nullable=True)
    org_unit_id = Column(Integer, ForeignKey("org_units.id", ondelete="RESTRICT"), nullable=True, index=True)
    created_at = Column(DateTime, default=_utcnow)
...
class EmployeeRelation(Base):
    ...
    internal_employee_id = Column(Integer, ForeignKey("employees.id", ondelete="RESTRICT"), nullable=True, index=True)
    created_at = Column(DateTime, default=_utcnow)
    updated_at = Column(DateTime, default=_utcnow, onupdate=_utcnow)
    country_code = Column(String(10), nullable=True, index=True)
...
class EmployeeAddress(Base):
    ...
    country_code = Column(String(10), ForeignKey("country_configs.code", ondelete="RESTRICT"), nullable=False, index=True)
...
class EmployeeActivityLog(Base):
    """Append-only audit trail of employee actions (HCM security/audit log)."""
    __tablename__ = "employee_activity_logs"
    __table_args__ = (
        Index("ix_employee_activity_actor", "actor_employee_id"),
        Index("ix_employee_activity_created", "created_at"),
        {"schema": "logistics"},
    )
    id = Column(Integer, primary_key=True, index=True)
