...
from db.mixins import VersionMixin, SoftDeleteMixin
from models.comms.communication import EmailFolder, InternalEmail  # re-exported for seed_all + data layer
...
class TrainingModule(Base, VersionMixin, SoftDeleteMixin):
    ...
    uuid = Column(Uuid, default=uuid4, unique=True, nullable=False, index=True)
    version = Column(Integer, nullable=False, default=1)
    created_by = Column(Integer, nullable=True, index=True)
    updated_by = Column(Integer, nullable=True, index=True)


class EmployeeTraining(Base, VersionMixin, SoftDeleteMixin):
    ...
    employee_id = Column(Integer, ForeignKey("hr.employees.id", ondelete="RESTRICT"), nullable=False)
    module_id = Column(String(100), ForeignKey("hr.training_modules.module_id", ondelete="RESTRICT"), nullable=False)
...
class Office(Base, VersionMixin, SoftDeleteMixin):
    __tablename__ = "offices"
    __table_args__ = (
        Index("ix_offices_country_created", "country_code", "created_at"),
        {"schema": "hr"},
    )
    id = Column(Integer, primary_key=True, index=True)
...
class PhysicalIDCard(Base, VersionMixin, SoftDeleteMixin):
    __tablename__ = "physical_id_cards"
    __table_args__ = ({"schema": "hr"},)
    id = Column(Integer, primary_key=True, index=True)
    employee_id = Column(Integer, ForeignKey("hr.employees.id", ondelete="RESTRICT"), unique=True, nullable=False)
...
class DynamicQRSession(Base, VersionMixin, SoftDeleteMixin):
...
class EmployeeBiometric(Base, VersionMixin, SoftDeleteMixin):
...
class GeoFenceLog(Base, VersionMixin, SoftDeleteMixin):
    __tablename__ = "geo_fence_logs"
    __table_args__ = ({"schema": "hr"},)
    id = Column(Integer, primary_key=True, index=True)
    employee_id = Column(Integer, ForeignKey("hr.employees.id", ondelete="RESTRICT"), nullable=False, index=True)
...
class EmployeeRole(Base, VersionMixin, SoftDeleteMixin):
    __tablename__ = "employee_roles"
    __table_args__ = (
        Index("ix_employee_roles_permissions_gin", "permissions", postgresql_using="gin"),
        {"schema": "hr"},
    )
    id = Column(Integer, primary_key=True, index=True)
...
class OrgUnit(Base, VersionMixin, SoftDeleteMixin):
    __tablename__ = "org_units"
    __table_args__ = (
        Index("ix_org_units_country_created", "country_code", "created_at"),
        {"schema": "hr"},
    )
    id = Column(Integer, primary_key=True, index=True)
    name = Column(String(200), nullable=False)
    parent_id = Column(Integer, ForeignKey("hr.org_units.id", ondelete="RESTRICT"), nullable=True, index=True)
    country_code = Column(String(10), nullable=True, index=True)
...
class Employee(Base, VersionMixin, SoftDeleteMixin):
    __tablename__ = "employees"
    __table_args__ = (
        Index("ix_employees_user_id", "user_id"),
        Index("ix_employees_office", "office_id"),
        Index("ix_employees_country_created", "country_code", "created_at"),
        {"schema": "hr"})
    id = Column(Integer, primary_key=True, index=True)
    user_id = Column(Integer, ForeignKey("security.users.id", ondelete="RESTRICT"), unique=True)
    ...
    country_code = Column(String(10), ForeignKey("country.country_configs.code", ondelete="RESTRICT"), nullable=True, index=True)
    ...
    reporting_manager_id = Column(Integer, ForeignKey("hr.employees.id", ondelete="RESTRICT"), nullable=True, index=True)
    hiring_manager_id = Column(Integer, ForeignKey("security.users.id", ondelete="RESTRICT"), nullable=True, index=True)
    authority_level = Column(Integer, nullable=True)
    org_unit_id = Column(Integer, ForeignKey("hr.org_units.id", ondelete="RESTRICT"), nullable=True, index=True)
    created_at = Column(DateTime, default=_utcnow)
    updated_at = Column(DateTime, default=_utcnow, onupdate=_utcnow)
    uuid = Column(Uuid, default=uuid4, unique=True, nullable=False, index=True)
    version = Column(Integer, nullable=False, default=1)
...
class EmployeeAttendance(Base, VersionMixin, SoftDeleteMixin):
    ...
    __table_args__ = (UniqueConstraint("employee_id", "date", name="uq_attendance_employee_date"), {"schema": "hr"})


class EmployeeWorkLog(Base, VersionMixin, SoftDeleteMixin):
...
class EmployeeLeaveRequest(Base, VersionMixin, SoftDeleteMixin):
    ...
    approved_by = Column(Integer, ForeignKey("security.users.id", ondelete="RESTRICT"), nullable=True, index=True)
...
class EmployeeLeaveLedger(Base, VersionMixin, SoftDeleteMixin):
    __tablename__ = "employee_leave_ledgers"
    __table_args__ = (UniqueConstraint("employee_id", "leave_type", "year", name="uq_leave_ledger_employee_type_year"), {"schema": "hr"})
    id = Column(Integer, primary_key=True, index=True)
    employee_id = Column(Integer, ForeignKey("hr.employees.id", ondelete="RESTRICT"), nullable=False, index=True)
    leave_type = Column(String(50), nullable=False)
...
class EmployeeShiftRoster(Base, VersionMixin, SoftDeleteMixin):
...
class EmployeeAsset(Base, VersionMixin, SoftDeleteMixin):
...
class EmployeeCertification(Base, VersionMixin, SoftDeleteMixin):
...
class EmployeeDocument(Base, VersionMixin, SoftDeleteMixin):
    ...
    expiry_date = Column(Date, nullable=True)
    verified_by = Column(Integer, ForeignKey("security.users.id", ondelete="RESTRICT"), nullable=True, index=True)
...
class EmployeeDependent(Base, VersionMixin, SoftDeleteMixin):
...
class EmployeeRelation(Base, VersionMixin, SoftDeleteMixin):
    ...
    internal_employee_id = Column(Integer, ForeignKey("hr.employees.id", ondelete="RESTRICT"), nullable=True, index=True)
    created_at = Column(DateTime, default=_utcnow)
    updated_at = Column(DateTime, default=_utcnow, onupdate=_utcnow)
    country_code = Column(String(10), nullable=True, index=True)
    uuid = Column(Uuid, default=uuid4, unique=True, nullable=False, index=True)
...
class EmployeeAddress(Base, VersionMixin, SoftDeleteMixin):
    __tablename__ = "employee_addresses"
    __table_args__ = (
        Index("ix_employee_addresses_country_created", "country_code", "created_at"),
        {"schema": "hr"},
    )
    id = Column(Integer, primary_key=True, index=True)
    employee_id = Column(Integer, ForeignKey("hr.employees.id", ondelete="RESTRICT"), nullable=False, index=True)
    ...
    country_code = Column(String(10), ForeignKey("country.country_configs.code", ondelete="RESTRICT"), nullable=False, index=True)
...
class COIReport(Base, VersionMixin, SoftDeleteMixin):
...
class TravelRequest(Base, VersionMixin, SoftDeleteMixin):
    __tablename__ = "employee_travel_requests"
    __table_args__ = (
        Index("ix_employee_travel_requests_per_diem_gin", "per_diem_json", postgresql_using="gin"),
        {"schema": "hr"},
    )
    id = Column(Integer, primary_key=True, index=True)
    employee_id = Column(Integer, ForeignKey("hr.employees.id", ondelete="RESTRICT"), nullable=False, index=True)
...
class AlumniNetwork(Base, VersionMixin, SoftDeleteMixin):
...
class DisciplinaryCase(Base, VersionMixin, SoftDeleteMixin):
...
class OffboardingCase(Base, VersionMixin, SoftDeleteMixin):
...
class PayrollRecord(Base, VersionMixin, SoftDeleteMixin):
    __tablename__ = "payroll_records"
    __table_args__ = ({"schema": "hr"},)
    id = Column(Integer, primary_key=True, index=True)
    employee_id = Column(Integer, ForeignKey("hr.employees.id", ondelete="RESTRICT"), nullable=True, index=True)
    country_code = Column(String(10), nullable=True, index=True)
