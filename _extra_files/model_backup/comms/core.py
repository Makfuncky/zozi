...
class Address(Base, TenantMixin):
    ...
    user_id = Column(Integer, ForeignKey("security.users.id"), nullable=False)
...
class AuditLog(VersionMixin, Base):
...
class CityDistanceMatrix(VersionMixin, Base):
...
class UserBrowsingHistory(VersionMixin, Base):
...
class SystemHealthEvent(VersionMixin, Base):
...
class CommandCenterView(VersionMixin, Base):
...
class NewsSource(VersionMixin, Base):
...
class InternalNotice(VersionMixin, Base):
...
class PredictiveSimulation(VersionMixin, Base):
...
class AlertEscalationRule(VersionMixin, Base):
...
class EntityChatThread(VersionMixin, Base):
...
class EntityChatMessage(VersionMixin, Base):
...
class VideoRoomParticipant(VersionMixin, Base):
...
class VideoRoomRecording(VersionMixin, Base):
...
class DirectChatMessage(VersionMixin, Base):
...
class GroupChatMember(VersionMixin, Base):
...
class GroupChatMessage(VersionMixin, Base):
...
class ShiftHandoverTask(VersionMixin, Base):
...
class EscalationSLALog(VersionMixin, Base):
