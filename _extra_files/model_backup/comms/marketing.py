...
class EmailTemplate(VersionMixin, Base):
...
class NewsletterSubscriber(VersionMixin, Base):
...
class EmailCampaignLog(VersionMixin, Base):
...
class CampaignRecipient(VersionMixin, Base):
...
class EmailDeliveryEvent(VersionMixin, Base):
...
class EmailSuppression(VersionMixin, Base):
...
class EmailRuntimeConfig(VersionMixin, Base):
...
class PointsTransaction(VersionMixin, Base):
...
class UserPoints(VersionMixin, Base):
    __tablename__ = "user_points"
    __table_args__ = ({"schema": "commerce"},)
    id = Column(Integer, primary_key=True, index=True)
    user_id = Column(Integer, ForeignKey("security.users.id", ondelete="RESTRICT"), nullable=False, unique=True)
    points = Column(Numeric(12, 2), nullable=False, default=0)
