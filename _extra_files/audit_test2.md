...
> **Generated:** 2026-08-05T05:13:24.256614+00:00  
> **Repo:** `D:\Projects\10- E-COMMERCE WEBSITE\zozi`  
> **Result:** 🔴 16 violations · 🟡 983 advisories · 🟢 33 info  
> **Architecture Debt Score:** **21474**  
...
graph TD
    ...
    controllers_flat["21 flat .py files"]
    controllers --> controllers_flat
    data["data/"]
    ROOT --> data
    data_flat["77 flat .py files"]
    data --> data_flat
    db["db/"]
    ROOT --> db
    db_sql["sql/"]
...
| A2 | 2 | 🟡 ADVISORY | possibly dead/orphan module (no inbound imports; not an entrypoint) |
...
| CIR1 | 2 | 🔴 VIOLATION | circuit violation: import is outside the allowed layer circuit |
| CIR2 | 110 | 🟡 ADVISORY | circuit bypass: import skips the preferred layer (migration warning) |
| D1 | 6 | 🟡 ADVISORY | duplicate module basename within backend (import-shadow) |
| DG2 | 21 | 🟡 ADVISORY | circular dependency detected |
| DG3 | 2 | 🔴 VIOLATION | cross-domain import violates explicit bounded-context ownership |
...
| DOM6 | 7 | 🟢 INFO | new domain candidate auto-detected |
...
| F4 | 11 | 🟡 ADVISORY | committed cache/build/artifact present (bloat) |
...
| LC1 | 4 | 🔴 VIOLATION | layer contract violation: forbidden operation in layer |
...
| MV1 | 16 | 🟡 ADVISORY | flat layer file should be moved into its detected domain folder |
...
| PERF2 | 1 | 🟡 ADVISORY | possible DB query inside loop (N+1 risk) |
| PF1 | 1 | 🟢 INFO | required project file missing |
| PF2 | 8 | 🟡 ADVISORY | required scope document missing |
| Q1 | 20 | 🟡 ADVISORY | controller/router reads via db.query (delegate) |
| QUAL1 | 45 | 🟡 ADVISORY | weak exception handling (bare except / swallowed exception) |
| QUAL2 | 4 | 🟡 ADVISORY | TODO/FIXME technical debt marker |
| QUAL3 | 77 | 🟡 ADVISORY | oversized file or function (scaling/maintainability risk) |
| QUAL4 | 10 | 🟡 ADVISORY | print/debug output in application code |
| RN1 | 8 | 🟡 ADVISORY | flat router filename must be comprehensive: {surface}_{domain}_{operation}.py |
| S2 | 1 | 🟡 ADVISORY | overlapping service stems (ownership ambiguity) |
| SYM1 | 100 | 🟡 ADVISORY | symbol defined but never used (dead symbol) |
| SYM2 | 100 | 🟡 ADVISORY | duplicate symbol definition across modules |
| W1 | 3 | 🔴 VIOLATION | controller/router writes to DB (must be a service) |
| W4 | 47 | 🟡 ADVISORY | controller imports another controller (shared logic -> service/util) |
...
|---|---|---|---|---|---|
| 🔴 | CIR1 | backend | `backend\routers\api_geography_communications.py:34` | circuit violation: routers -> db (db.models_country_control) is outside the allowed circuit | routers may import only: controllers, data, dependencies, utils |
| 🔴 | CIR1 | backend | `backend\routers\effective_permissions.py:34` | circuit violation: routers -> db (db.models_country_control) is outside the allowed circuit | routers may import only: controllers, data, dependencies, utils |
| 🔴 | DG2 | backend | `domain-graph` | circular domain dependency: cash_management_service -> logistics -> cash_management_service | redefine bounded-context boundaries; introduce explicit service contracts / events |
| 🔴 | DG2 | backend | `domain-graph` | circular domain dependency: cash_management_service -> communication -> logistics -> cash_management_service | redefine bounded-context boundaries; introduce explicit service contracts / events |
| 🔴 | DG3 | backend | `backend\services\cash_management_service.py:46` | cross-domain import cash_management_service -> logistics violates explicit ownership rules | declare allowed imports in layer_rules.yaml or route via logistics service facade |
| 🔴 | DG3 | backend | `backend\services\cash_management_service.py:47` | cross-domain import cash_management_service -> communication violates explicit ownership rules | declare allowed imports in layer_rules.yaml or route via communication service facade |
| 🔴 | F4 | backend | `backend\test.db` | must not sit at backend (damages structure/scale) | delete + add to .gitignore |
| 🔴 | F4 | backend | `backend\zozi.db` | must not sit at backend (damages structure/scale) | delete + add to .gitignore |
| 🔴 | F4 | backend | `backend\_probe.db` | must not sit at backend (damages structure/scale) | delete + add to .gitignore |
| 🔴 | LC1 | routers | `backend\routers\admin_commerce_routes.py:521` | layer contract violation: 'db\.query' found in routers/ | routers must not perform this operation; move it to the appropriate layer |
| 🔴 | LC1 | routers | `backend\routers\api_comms_command.py:128` | layer contract violation: 'db\.query' found in routers/ | routers must not perform this operation; move it to the appropriate layer |
| 🔴 | LC1 | routers | `backend\routers\api_geography_communications.py:16` | layer contract violation: 'db\.query' found in routers/ | routers must not perform this operation; move it to the appropriate layer |
| 🔴 | LC1 | routers | `backend\routers\effective_permissions.py:16` | layer contract violation: 'db\.query' found in routers/ | routers must not perform this operation; move it to the appropriate layer |
| 🔴 | W1 | backend | `backend\controllers\treasury\admin_payouts_controller.py` | 1 session write(s) in this file; move writes into services/<domain>/ (lines: 83) | a services/<domain>/*_service.py method |
| 🔴 | W1 | backend | `backend\controllers\country\country_controller.py` | 1 session write(s) in this file; move writes into services/<domain>/ (lines: 1429) | a services/<domain>/*_service.py method |
| 🔴 | W1 | backend | `backend\routers\api_comms_realtime.py` | 4 session write(s) in this file; move writes into services/<domain>/ (lines: 51, 60, 84, 91) | a services/<domain>/*_service.py method |
| 🟡 | A1 | backend | `backend\controllers\supplier\supplier_controller.py` | architecture hotspot: fan_in=2, fan_out=21, instability=0.91 | reduce coupling; split responsibilities or introduce an abstraction layer |
...
| 🟡 | A1 | backend | `backend\data\models.py` | architecture hotspot: fan_in=302, fan_out=1, instability=0.00 | reduce coupling; split responsibilities or introduce an abstraction layer |
| 🟡 | A1 | backend | `backend\data\models_employee_models.py` | architecture hotspot: fan_in=42, fan_out=1, instability=0.02 | reduce coupling; split responsibilities or introduce an abstraction layer |
| 🟡 | A1 | backend | `backend\data\schemas.py` | architecture hotspot: fan_in=54, fan_out=1, instability=0.02 | reduce coupling; split responsibilities or introduce an abstraction layer |
...
| 🟡 | A1 | backend | `backend\models\__init__.py` | architecture hotspot: fan_in=46, fan_out=2, instability=0.04 | reduce coupling; split responsibilities or introduce an abstraction layer |
...
| 🟡 | A1 | backend | `backend\services\__init__.py` | architecture hotspot: fan_in=125, fan_out=1, instability=0.01 | reduce coupling; split responsibilities or introduce an abstraction layer |
...
| 🟡 | A1 | backend | `backend\utils\config.py` | architecture hotspot: fan_in=58, fan_out=0, instability=0.00 | reduce coupling; split responsibilities or introduce an abstraction layer |
...
| 🟡 | API2 | backend | `backend\services\communication\chat_enrichment.py:19` | private symbol '_ALLOWED_TABLES' used in 4 external module(s) | make it public (remove _) or keep internal and refactor external usages |
...
| 🟡 | API2 | backend | `backend\services\supplier\supplier_badge_service.py:47` | private symbol '_BADGE_AMOUNT_QUANT' used in 2 external module(s) | make it public (remove _) or keep internal and refactor external usages |
| 🟡 | API2 | backend | `backend\services\supplier\supplier_badge_service.py:39` | private symbol '_BADGE_THRESHOLDS' used in 4 external module(s) | make it public (remove _) or keep internal and refactor external usages |
| 🟡 | API2 | backend | `backend\services\supplier\supplier_badge_service.py:45` | private symbol '_FULFILLED_ORDER_STATUSES' used in 3 external module(s) | make it public (remove _) or keep internal and refactor external usages |
| 🟡 | API2 | backend | `backend\services\media\free_image_tools.py:31` | private symbol '_HAS_CV2' used in 32 external module(s) | make it public (remove _) or keep internal and refactor external usages |
| 🟡 | API2 | backend | `backend\services\supplier\supplier_badge_service.py:46` | private symbol '_MANUAL_BADGE_LEVELS' used in 7 external module(s) | make it public (remove _) or keep internal and refactor external usages |
...
| 🟡 | API2 | backend | `backend\controllers\supplier\supplier_controller.py:4153` | private symbol '_PERIOD_DAYS' used in 4 external module(s) | make it public (remove _) or keep internal and refactor external usages |
...
| 🟡 | API2 | backend | `backend\controllers\supplier\supplier_controller.py:140` | private symbol '_UNSET' used in 8 external module(s) | make it public (remove _) or keep internal and refactor external usages |
...
| 🟡 | API2 | backend | `backend\services\finance\payments_gateway_service.py:567` | private symbol '_apply_stripe_runtime_key' used in 1 external module(s) | make it public (remove _) or keep internal and refactor external usages |
...
| 🟡 | API2 | backend | `backend\services\orders\orders_router_service.py:72` | private symbol '_as_float' used in 14 external module(s) | make it public (remove _) or keep internal and refactor external usages |
| 🟡 | API2 | backend | `backend\services\treasury\treasury_adapter.py:71` | private symbol '_audit' used in 5 external module(s) | make it public (remove _) or keep internal and refactor external usages |
| 🟡 | API2 | backend | `backend\services\supplier\supplier_badge_service.py:598` | private symbol '_badge_for_score' used in 1 external module(s) | make it public (remove _) or keep internal and refactor external usages |
| 🟡 | API2 | backend | `backend\services\supplier\supplier_badge_service.py:81` | private symbol '_badge_period_bounds' used in 2 external module(s) | make it public (remove _) or keep internal and refactor external usages |
| 🟡 | API2 | backend | `backend\services\supplier\supplier_badge_service.py:111` | private symbol '_badge_tier_meets_metrics' used in 3 external module(s) | make it public (remove _) or keep internal and refactor external usages |
| 🟡 | API2 | backend | `backend\routers\admin_commerce_routes.py:766` | private symbol '_banner_to_dict' used in 6 external module(s) | make it public (remove _) or keep internal and refactor external usages |
| 🟡 | API2 | backend | `backend\utils\error_handler.py:81` | private symbol '_before_send' used in 2 external module(s) | make it public (remove _) or keep internal and refactor external usages |
...
| 🟡 | API2 | backend | `backend\providers\hr\bg_remover.py:83` | private symbol '_bytes_to_image' used in 4 external module(s) | make it public (remove _) or keep internal and refactor external usages |
| 🟡 | API2 | backend | `backend\services\security\effective_permissions.py:151` | private symbol '_cache_key' used in 2 external module(s) | make it public (remove _) or keep internal and refactor external usages |
...
| 🟡 | API2 | backend | `backend\services\supplier\supplier_badge_service.py:117` | private symbol '_compute_badge_threshold_metrics' used in 3 external module(s) | make it public (remove _) or keep internal and refactor external usages |
...
| 🟡 | API2 | backend | `backend\services\supplier\supplier_badge_service.py:220` | private symbol '_create_badge_billing_record' used in 3 external module(s) | make it public (remove _) or keep internal and refactor external usages |
| 🟡 | API2 | backend | `backend\services\treasury\payout_batch_service.py:184` | private symbol '_create_payout_batch' used in 1 external module(s) | make it public (remove _) or keep internal and refactor external usages |
...
| 🟡 | API2 | backend | `backend\services\country\country_detection.py:113` | private symbol '_default_country' used in 6 external module(s) | make it public (remove _) or keep internal and refactor external usages |
...
| 🟡 | API2 | backend | `backend\services\supplier\supplier_badge_service.py:54` | private symbol '_ensure_supplier_profile_record' used in 3 external module(s) | make it public (remove _) or keep internal and refactor external usages |
...
| 🟡 | API2 | backend | `backend\providers\ai\text.py:348` | private symbol '_extract_product_name' used in 1 external module(s) | make it public (remove _) or keep internal and refactor external usages |
| 🟡 | API2 | backend | `backend\providers\ai\text.py:356` | private symbol '_extract_tags' used in 1 external module(s) | make it public (remove _) or keep internal and refactor external usages |
| 🟡 | API2 | backend | `backend\controllers\catalog\search_controller.py:173` | private symbol '_extract_terms' used in 2 external module(s) | make it public (remove _) or keep internal and refactor external usages |
| 🟡 | API2 | backend | `backend\providers\ai\text.py:284` | private symbol '_extract_variant_from_text' used in 1 external module(s) | make it public (remove _) or keep internal and refactor external usages |
...
| 🟡 | API2 | backend | `backend\services\country\country_ai_research.py:562` | private symbol '_fallback' used in 1 external module(s) | make it public (remove _) or keep internal and refactor external usages |
| 🟡 | API2 | backend | `backend\services\hr\hierarchy_service.py:68` | private symbol '_fetch_employees_by_ids' used in 1 external module(s) | make it public (remove _) or keep internal and refactor external usages |
| 🟡 | API2 | backend | `backend\services\supplier\supplier_badge_service.py:197` | private symbol '_find_existing_badge_billing' used in 1 external module(s) | make it public (remove _) or keep internal and refactor external usages |
| 🟡 | API2 | backend | `backend\services\orders\orders_router_service.py:83` | private symbol '_first_non_none' used in 6 external module(s) | make it public (remove _) or keep internal and refactor external usages |
...
| 🟡 | API2 | backend | `backend\services\payments\payment_engine.py:167` | private symbol '_get_adapter' used in 1 external module(s) | make it public (remove _) or keep internal and refactor external usages |
...
| 🟡 | API2 | backend | `backend\services\treasury\payout_engine.py:41` | private symbol '_get_country' used in 5 external module(s) | make it public (remove _) or keep internal and refactor external usages |
...
| 🟡 | API2 | backend | `backend\controllers\catalog\products_controller.py:198` | private symbol '_get_product_cache_version' used in 1 external module(s) | make it public (remove _) or keep internal and refactor external usages |
...
| 🟡 | API2 | backend | `backend\utils\migration_helpers.py:29` | private symbol '_index_names' used in 1 external module(s) | make it public (remove _) or keep internal and refactor external usages |
| 🟡 | API2 | backend | `backend\utils\migration_helpers.py:21` | private symbol '_inspector' used in 9 external module(s) | make it public (remove _) or keep internal and refactor external usages |
| 🟡 | API2 | backend | `backend\alembic\versions\20260731_0012_partition_journal_entries.py:27` | private symbol '_is_postgres' used in 6 external module(s) | make it public (remove _) or keep internal and refactor external usages |
...
| 🟡 | API2 | backend | `backend\services\supplier\supplier_badge_service.py:90` | private symbol '_load_active_badge_tiers' used in 2 external module(s) | make it public (remove _) or keep internal and refactor external usages |
...
| 🟡 | API2 | backend | `backend\controllers\supplier\supplier_controller.py:203` | private symbol '_load_shipments_for_orders' used in 2 external module(s) | make it public (remove _) or keep internal and refactor external usages |
| 🟡 | API2 | backend | `backend\services\treasury\payout_batch_service.py:308` | private symbol '_log_automation' used in 3 external module(s) | make it public (remove _) or keep internal and refactor external usages |
| 🟡 | API2 | backend | `backend\services\hr\employee_communication_service.py:655` | private symbol '_log_comm_event' used in 1 external module(s) | make it public (remove _) or keep internal and refactor external usages |
...
| 🟡 | API2 | backend | `backend\services\supplier\supplier_badge_service.py:272` | private symbol '_maybe_create_recurring_badge_billing' used in 1 external module(s) | make it public (remove _) or keep internal and refactor external usages |
| 🟡 | API2 | backend | `backend\services\country\country_ai_research.py:524` | private symbol '_merge_ai_output' used in 5 external module(s) | make it public (remove _) or keep internal and refactor external usages |
...
| 🟡 | API2 | backend | `backend\controllers\supplier\supplier_controller.py:163` | private symbol '_normalize_product_visibility_regions' used in 1 external module(s) | make it public (remove _) or keep internal and refactor external usages |
...
| 🟡 | API2 | backend | `backend\providers\ai\text.py:80` | private symbol '_ollama_vision_chat' used in 2 external module(s) | make it public (remove _) or keep internal and refactor external usages |
...
| 🟡 | API2 | backend | `backend\services\country\country_ai_research.py:510` | private symbol '_parse_json_text' used in 4 external module(s) | make it public (remove _) or keep internal and refactor external usages |
| 🟡 | BC1 | backend | `backend\services\cash_management_service.py:46` | cross-domain import cash_management_service → logistics bypasses event/facade boundary | route through events/ or a logistics service facade; declare in layer_rules.yaml if intentional |
| 🟡 | BC3 | backend | `backend\services\audit\ediscovery.py:16` | bounded context leakage: audit service directly imports treasury models | use treasury service API or events instead of direct model access |
...
| 🟡 | BC3 | backend | `backend\services\hr\employee_communication_service.py:390` | bounded context leakage: hr service directly imports communication models | use communication service API or events instead of direct model access |
| 🟡 | BC3 | backend | `backend\services\treasury\treasury_engine.py:380` | bounded context leakage: treasury service directly imports orders models | use orders service API or events instead of direct model access |
...
| 🟡 | CA2 | services | `backend\services\cash_management_service.py` | file contains signals for 7 domains: finance(22), treasury(16), logistics(13), supplier(10), core(3), audit(2), orders(2) — split candidate | split this file into domain-specific modules; each file should serve one domain |
...
| 🟡 | CA2 | services | `backend\services\treasury\payout_admin_service.py` | file contains signals for 5 domains: treasury(14), orders(3), finance(2), geography(2), supplier(2) — split candidate | split this file into domain-specific modules; each file should serve one domain |
...
| 🟡 | CA2 | providers | `backend\providers\bg_remover.py` | file contains signals for 2 domains: hr(2), media(2) — split candidate | split this file into domain-specific modules; each file should serve one domain |
...
| 🟡 | CIR2 | backend | `backend\routers\admin_commerce_routes.py:12` | circuit bypass: routers -> services (services.commerce.promotion_engine_service) | routers should call controllers; direct router -> service usage skips the orchestration layer |
...
| 🟡 | CIR2 | backend | `backend\routers\admin_finance_routes.py:12` | circuit bypass: routers -> services (services.commission_write_service) | routers should call controllers; direct router -> service usage skips the orchestration layer |
...
| 🟡 | CIR2 | backend | `backend\routers\admin_treasury_routes.py:10` | circuit bypass: routers -> services (services.cash_management_write_service) | routers should call controllers; direct router -> service usage skips the orchestration layer |
...
| 🟡 | CIR2 | backend | `backend\routers\api_comms_command.py:33` | circuit bypass: routers -> services (services.write_helpers) | routers should call controllers; direct router -> service usage skips the orchestration layer |
...
| 🟡 | CIR2 | backend | `backend\routers\api_hr_processing.py:13` | circuit bypass: routers -> services (services.finance.payroll_read_service) | routers should call controllers; direct router -> service usage skips the orchestration layer |
...
| 🟡 | CIR2 | backend | `backend\routers\api_media_bulk.py:34` | circuit bypass: routers -> services (services.storage) | routers should call controllers; direct router -> service usage skips the orchestration layer |
...
| 🟡 | CIR2 | backend | `backend\routers\api_treasury_approval.py:29` | circuit bypass: routers -> services (services.treasury.treasury_router_service) | routers should call controllers; direct router -> service usage skips the orchestration layer |
| 🟡 | CIR2 | backend | `backend\routers\api_treasury_routes.py:31` | circuit bypass: routers -> services (services.treasury.treasury_router_service) | routers should call controllers; direct router -> service usage skips the orchestration layer |
| 🟡 | CIR2 | backend | `backend\routers\api_treasury_routes_2.py:4` | circuit bypass: routers -> services (services.treasury_service) | routers should call controllers; direct router -> service usage skips the orchestration layer |
...
| 🟡 | CIR2 | backend | `backend\routers\treasury_api.py:11` | circuit bypass: routers -> services (services.finance.treasury_query_service) | routers should call controllers; direct router -> service usage skips the orchestration layer |
...
| 🟡 | D1 | backend | `config.py` | sensitive module name in 3 dirs (import-shadow): backend\utils\config.py, backend\providers\hr\config.py, backend\providers\configuration\config.py | keep the canonical copy (utils/config.py); delete the shadows |
...
| 🟡 | DG2 | backend | `backend\models\__init__.py` | circular module dependency: models -> models._exports -> models.communication.core -> models.communication -> models | break the cycle by extracting shared logic into a lower layer (utils/service interface) |
...
| 🟡 | DG2 | backend | `backend\models\__init__.py` | circular module dependency: models -> models._exports -> models.country.country_enhancements -> models.country -> models | break the cycle by extracting shared logic into a lower layer (utils/service interface) |
...
| 🟡 | DG2 | backend | `backend\models\__init__.py` | circular module dependency: models -> models._exports -> models.treasury.finance -> models.treasury -> models | break the cycle by extracting shared logic into a lower layer (utils/service interface) |
| 🟡 | DG2 | backend | `backend\controllers\__init__.py` | circular module dependency: controllers -> controllers.supplier -> controllers.supplier.supplier_controller -> controllers | break the cycle by extracting shared logic into a lower layer (utils/service interface) |
...
| 🟡 | DG5 | backend | `backend\services\cash_management_service.py:533` | dynamic execution/import obscures dependency graph (import_module:None) | avoid eval/exec/dynamic import_module for layer-critical code paths |
...
| 🟡 | DOM2 | services | `backend/services/` | 2 file(s) are in the wrong backend/services/ sub-folder; detected domain: 'treasury' | mkdir -p backend/services/treasury; move: backend\services\finance\payouts_write_service.py, backend\services\supplier\supplier_payouts_service.py (detected from payouts) |
...
| 🟢 | DOM6 | backend | `backend/services|models/service` | new domain candidate auto-detected: 'service' | create backend/<layer>/service/ and group related files; or merge into nearest existing domain if this is not a real bounded context. Examples: backend\services\bank_transaction_service.py, backend\services\cash_management_service.py, backend\services\cash_management_write_service.py, backend\services\commission_write_service.py, backend\services\credit_control_service.py, backend\services\customer\customer_health_service.py, backend\services\customer\customer_router_service.py, backend\services\supplier\supplier_badge_service.py |
...
| 🟡 | F4 | backend | `backend\test.db-shm` | must not sit at backend (damages structure/scale) | delete + add to .gitignore |
| 🟡 | F4 | backend | `backend\test.db-wal` | must not sit at backend (damages structure/scale) | delete + add to .gitignore |
...
| 🟡 | F4 | backend | `backend\_probe.db-shm` | must not sit at backend (damages structure/scale) | delete + add to .gitignore |
| 🟡 | F4 | backend | `backend\_probe.db-wal` | must not sit at backend (damages structure/scale) | delete + add to .gitignore |
...
| 🟡 | H1 | backend | `backend\utils\analyze_fks.py:4` | sys.path manipulation detected | remove sys.path.insert/append; fix package structure and use proper imports |
...
| 🟢 | MET1 | repo | `architecture-debt` | architecture debt score = 21474 | track this number down over time; lower is healthier |
...
| 🟡 | MET2 | backend | `backend\controllers\orders\admin_orders_controller.py` | high instability: I=0.92 (Ca=1, Ce=12) | module is very fragile; add abstractions or reduce outgoing dependencies |
| 🟡 | MET2 | backend | `backend\controllers\orders\orders_controller.py` | high instability: I=0.94 (Ca=1, Ce=17) | module is very fragile; add abstractions or reduce outgoing dependencies |
...
| 🟡 | MET2 | backend | `backend\controllers\supplier\supplier_controller.py` | high instability: I=0.91 (Ca=2, Ce=21) | module is very fragile; add abstractions or reduce outgoing dependencies |
...
| 🟢 | MET3 | middleware | `backend/middleware/` | no abstract classes in middleware/ (A=0.00, 39 classes) | consider adding interfaces/ABCs for dependency inversion |
| 🟢 | MET3 | controllers | `backend/controllers/` | no abstract classes in controllers/ (A=0.00, 13 classes) | consider adding interfaces/ABCs for dependency inversion |
...
| 🟡 | MV1 | services | `backend/services/` | 2 'comms' domain file(s) at backend/services/ root should be moved to backend/services/comms/ | mkdir -p backend/services/comms; move: backend\services\cash_management_service.py, backend\services\video_conferencing.py (detected from communication, email) |
| 🟡 | MV1 | services | `backend/services/` | 1 'finance' domain file(s) at backend/services/ root should be moved to backend/services/finance/ | mkdir -p backend/services/finance; move: backend\services\commission_write_service.py (detected from commission) |
...
| 🟡 | MV1 | services | `backend/services/` | 3 'treasury' domain file(s) at backend/services/ root should be moved to backend/services/treasury/ | mkdir -p backend/services/treasury; move: backend\services\auto_payout_scheduler.py, backend\services\bank_transaction_service.py, backend\services\cash_management_write_service.py (detected from payout) |
...
| 🟡 | PERF2 | backend | `backend\utils\rls_interceptor.py` | 2 possible DB query inside loop (N+1 risk) in this file; batch queries / use joins / preload relationships (lines: 1049, 1055) | batch the query / use joins / preload relationships instead of querying per item |
...
| 🟡 | Q1 | backend | `backend\controllers\supplier\supplier_controller.py` | 37 DB read(s) via .query() in this file; delegate reads to a service (lines: 298, 612, 620, 989, 1541, 1580, 2060, 2073, 2088, 2106 +27 more) | service layer |
...
| 🟡 | Q1 | backend | `backend\controllers\orders\returns_controller.py` | 1 DB read(s) via .query() in this file; delegate reads to a service (lines: 150) | service layer |
...
| 🟡 | Q1 | backend | `backend\routers\admin_commerce_routes.py` | 1 DB read(s) via .query() in this file; delegate reads to a service (lines: 521) | service layer |
| 🟡 | Q1 | backend | `backend\routers\api_comms_command.py` | 21 DB read(s) via .query() in this file; delegate reads to a service (lines: 128, 176, 200, 231, 257, 284, 346, 359, 385, 401 +11 more) | service layer |
| 🟡 | Q1 | backend | `backend\routers\api_geography_communications.py` | 4 DB read(s) via .query() in this file; delegate reads to a service (lines: 16, 35, 55, 77) | service layer |
| 🟡 | Q1 | backend | `backend\routers\effective_permissions.py` | 4 DB read(s) via .query() in this file; delegate reads to a service (lines: 16, 35, 55, 77) | service layer |
...
| 🟡 | QUAL1 | backend | `backend\utils\auth.py` | 9 weak exception handling location(s) in this file; log or re-raise instead of swallowing exceptions (lines: 93, 111, 132, 155, 176, 226, 237, 255, 266) | log or re-raise; silent swallowing hides bugs |
...
| 🟡 | QUAL1 | backend | `backend\services\supplier\supplier_orders_service.py` | 1 weak exception handling location(s) in this file; log or re-raise instead of swallowing exceptions (lines: 36) | log or re-raise; silent swallowing hides bugs |
...
| 🟡 | QUAL1 | backend | `backend\services\media\free_image_tools.py` | 1 weak exception handling location(s) in this file; log or re-raise instead of swallowing exceptions (lines: 97) | log or re-raise; silent swallowing hides bugs |
| 🟡 | QUAL1 | backend | `backend\services\hr\performance_service.py` | 2 weak exception handling location(s) in this file; log or re-raise instead of swallowing exceptions (lines: 190, 460) | log or re-raise; silent swallowing hides bugs |
| 🟡 | QUAL1 | backend | `backend\services\finance\finance_automation.py` | 1 weak exception handling location(s) in this file; log or re-raise instead of swallowing exceptions (lines: 439) | log or re-raise; silent swallowing hides bugs |
...
| 🟡 | QUAL1 | backend | `backend\services\country\country_detection.py` | 3 weak exception handling location(s) in this file; log or re-raise instead of swallowing exceptions (lines: 98, 123, 142) | log or re-raise; silent swallowing hides bugs |
...
| 🟡 | QUAL1 | backend | `backend\routers\api_comms_command.py` | 1 weak exception handling location(s) in this file; log or re-raise instead of swallowing exceptions (lines: 505) | log or re-raise; silent swallowing hides bugs |
...
| 🟡 | QUAL1 | backend | `backend\providers\hr\bg_remover.py` | 3 weak exception handling location(s) in this file; log or re-raise instead of swallowing exceptions (lines: 297, 1265, 1298) | log or re-raise; silent swallowing hides bugs |
...
| 🟡 | QUAL1 | backend | `backend\middleware\country_detection.py` | 1 weak exception handling location(s) in this file; log or re-raise instead of swallowing exceptions (lines: 46) | log or re-raise; silent swallowing hides bugs |
...
| 🟡 | QUAL1 | backend | `backend\controllers\supplier\supplier_controller.py` | 2 weak exception handling location(s) in this file; log or re-raise instead of swallowing exceptions (lines: 594, 1768) | log or re-raise; silent swallowing hides bugs |
...
| 🟡 | QUAL1 | backend | `backend\controllers\orders\returns_controller.py` | 1 weak exception handling location(s) in this file; log or re-raise instead of swallowing exceptions (lines: 319) | log or re-raise; silent swallowing hides bugs |
| 🟡 | QUAL1 | backend | `backend\controllers\country\country_controller.py` | 1 weak exception handling location(s) in this file; log or re-raise instead of swallowing exceptions (lines: 1430) | log or re-raise; silent swallowing hides bugs |
...
| 🟡 | QUAL3 | backend | `backend\services\auto_payout_scheduler.py:72` | oversized function 'run_auto_payout_sweep' (270 lines) | extract smaller functions / service methods; long functions hide side effects |
| 🟡 | QUAL3 | backend | `backend\services\auto_payout_scheduler.py:347` | oversized function 'run_auto_logistics_payout_sweep' (269 lines) | extract smaller functions / service methods; long functions hide side effects |
| 🟡 | QUAL3 | backend | `backend\services\cash_management_service.py` | oversized file (1248 lines) | split by domain/responsibility; large files become change bottlenecks |
...
| 🟡 | QUAL3 | backend | `backend\services\orders\cart_shipping_service.py:158` | oversized function 'quote_supplier_groups' (195 lines) | extract smaller functions / service methods; long functions hide side effects |
| 🟡 | QUAL3 | backend | `backend\services\media\media_router_service.py:249` | oversized function 'process_ai_upload_job' (142 lines) | extract smaller functions / service methods; long functions hide side effects |
| 🟡 | QUAL3 | backend | `backend\services\media\media_router_service.py:393` | oversized function 'batch_publish_products' (206 lines) | extract smaller functions / service methods; long functions hide side effects |
...
| 🟡 | QUAL3 | backend | `backend\routers\api_comms_command.py:524` | oversized function 'get_comprehensive_dashboard' (341 lines) | extract smaller functions / service methods; long functions hide side effects |
...
| 🟡 | QUAL3 | backend | `backend\routers\api_media_bulk.py:134` | oversized function 'batch_analyze_products' (127 lines) | extract smaller functions / service methods; long functions hide side effects |
...
| 🟡 | QUAL3 | backend | `backend\controllers\supplier\supplier_controller.py` | oversized file (4737 lines) | split by domain/responsibility; large files become change bottlenecks |
| 🟡 | QUAL3 | backend | `backend\controllers\supplier\supplier_controller.py:979` | oversized function 'get_supplier_orders' (145 lines) | extract smaller functions / service methods; long functions hide side effects |
| 🟡 | QUAL3 | backend | `backend\controllers\supplier\supplier_controller.py:1237` | oversized function 'get_supplier_label_payload' (143 lines) | extract smaller functions / service methods; long functions hide side effects |
| 🟡 | QUAL3 | backend | `backend\controllers\supplier\supplier_controller.py:1382` | oversized function 'upload_supplier_parcel_proof' (141 lines) | extract smaller functions / service methods; long functions hide side effects |
| 🟡 | QUAL3 | backend | `backend\controllers\supplier\supplier_controller.py:1691` | oversized function 'create_supplier_product_upload' (132 lines) | extract smaller functions / service methods; long functions hide side effects |
| 🟡 | QUAL3 | backend | `backend\controllers\supplier\supplier_controller.py:2760` | oversized function 'get_supplier_reports' (147 lines) | extract smaller functions / service methods; long functions hide side effects |
| 🟡 | QUAL3 | backend | `backend\controllers\supplier\supplier_controller.py:2916` | oversized function 'bulk_upload_products' (302 lines) | extract smaller functions / service methods; long functions hide side effects |
...
| 🟡 | QUAL3 | backend | `backend\controllers\orders\returns_controller.py:425` | oversized function 'update_return_request' (147 lines) | extract smaller functions / service methods; long functions hide side effects |
...
| 🟡 | QUAL3 | backend | `backend\controllers\country\country_controller.py` | oversized file (1698 lines) | split by domain/responsibility; large files become change bottlenecks |
| 🟡 | QUAL3 | backend | `backend\controllers\country\country_controller.py:358` | oversized function 'create_admin_country' (175 lines) | extract smaller functions / service methods; long functions hide side effects |
| 🟡 | QUAL3 | backend | `backend\controllers\country\country_controller.py:732` | oversized function '_apply_version_payload' (143 lines) | extract smaller functions / service methods; long functions hide side effects |
...
| 🟡 | QUAL4 | backend | `backend\utils\analyze_fks.py` | 4 print/debug output location(s) in this file; use structured logging instead of print() (lines: 24, 60, 70, 71) | use structured logging instead of print() |
...
| 🟡 | RN1 | routers | `backend\routers\treasury_api.py` | flat router filename 'treasury_api.py' is not comprehensive; missing operation | rename to {surface}_{domain}_{operation}.py, e.g. admin_orders_management.py, supplier_orders_fulfillment.py, customer_orders_tracking.py |
...
| 🟡 | SYM1 | routers | `backend\routers\api_ai_generation.py:15` | symbol 'AIImageAnalysisResponse' (class) defined but never referenced outside its module | verify usage; delete if dead code |
| 🟡 | SYM1 | routers | `backend\routers\api_ai_routes_2.py:27` | symbol 'AIResearchRequest' (class) defined but never referenced outside its module | verify usage; delete if dead code |
| 🟡 | SYM1 | routers | `backend\routers\api_ai_routes_2.py:36` | symbol 'AIResearchResponse' (class) defined but never referenced outside its module | verify usage; delete if dead code |
...
| 🟡 | SYM1 | services | `backend\services\supplier\onboarding_pipeline.py:97` | symbol 'AIVerifier' (class) defined but never referenced outside its module | verify usage; delete if dead code |
| 🟡 | SYM1 | routers | `backend\routers\api_finance_integration.py:172` | symbol 'APBillCreate' (class) defined but never referenced outside its module | verify usage; delete if dead code |
| 🟡 | SYM1 | routers | `backend\routers\api_finance_routes.py:529` | symbol 'APPayableBody' (class) defined but never referenced outside its module | verify usage; delete if dead code |
| 🟡 | SYM1 | routers | `backend\routers\api_finance_integration.py:184` | symbol 'APPayment' (class) defined but never referenced outside its module | verify usage; delete if dead code |
| 🟡 | SYM1 | routers | `backend\routers\api_finance_routes.py:549` | symbol 'APPaymentBody' (class) defined but never referenced outside its module | verify usage; delete if dead code |
| 🟡 | SYM1 | routers | `backend\routers\api_finance_routes.py:458` | symbol 'ARInvoiceBody' (class) defined but never referenced outside its module | verify usage; delete if dead code |
| 🟡 | SYM1 | routers | `backend\routers\api_finance_integration.py:113` | symbol 'ARInvoiceCreate' (class) defined but never referenced outside its module | verify usage; delete if dead code |
| 🟡 | SYM1 | routers | `backend\routers\api_finance_routes.py:478` | symbol 'ARPaymentBody' (class) defined but never referenced outside its module | verify usage; delete if dead code |
| 🟡 | SYM1 | routers | `backend\routers\api_finance_integration.py:125` | symbol 'ARReceipt' (class) defined but never referenced outside its module | verify usage; delete if dead code |
| 🟡 | SYM1 | routers | `backend\routers\api_finance_automation.py:70` | symbol 'AccountCreate' (class) defined but never referenced outside its module | verify usage; delete if dead code |
| 🟡 | SYM1 | routers | `backend\routers\api_finance_integration.py:65` | symbol 'AccountUpdate' (class) defined but never referenced outside its module | verify usage; delete if dead code |
| 🟡 | SYM1 | routers | `backend\routers\api_finance_automation.py:290` | symbol 'AccrualCreate' (class) defined but never referenced outside its module | verify usage; delete if dead code |
| 🟡 | SYM1 | routers | `backend\routers\api_treasury_routes.py:43` | symbol 'ActionRequest' (class) defined but never referenced outside its module | verify usage; delete if dead code |
| 🟡 | SYM1 | routers | `backend\routers\admin_core_console.py:995` | symbol 'AdminDisputeBulkActionBody' (class) defined but never referenced outside its module | verify usage; delete if dead code |
| 🟡 | SYM1 | routers | `backend\routers\admin_treasury_routes_3.py:41` | symbol 'AdminTreasuryActionRequest' (class) defined but never referenced outside its module | verify usage; delete if dead code |
| 🟡 | SYM1 | utils | `backend\utils\schema_audit.py:129` | symbol 'AlembicInfo' (class) defined but never referenced outside its module | verify usage; delete if dead code |
| 🟡 | SYM1 | routers | `backend\routers\api_comms_command.py:103` | symbol 'AlertResponse' (class) defined but never referenced outside its module | verify usage; delete if dead code |
| 🟡 | SYM1 | providers | `backend\providers\analytics\analytics.py:13` | symbol 'AnalyticsProviderSettings' (class) defined but never referenced outside its module | verify usage; delete if dead code |
| 🟡 | SYM1 | middleware | `backend\middleware\behavioral_analytics.py:24` | symbol 'AnomalyDetector' (class) defined but never referenced outside its module | verify usage; delete if dead code |
| 🟡 | SYM1 | routers | `backend\routers\api_geography_registry.py:682` | symbol 'ArchivePayload' (class) defined but never referenced outside its module | verify usage; delete if dead code |
...
| 🟡 | SYM1 | services | `backend\services\hr\offboarding.py:22` | symbol 'AssetRecoveryTask' (class) defined but never referenced outside its module | verify usage; delete if dead code |
| 🟡 | SYM1 | routers | `backend\routers\api_geography_registry.py:581` | symbol 'AssignStaffBody' (class) defined but never referenced outside its module | verify usage; delete if dead code |
| 🟡 | SYM1 | utils | `backend\utils\audit_log.py:23` | symbol 'AuditLogAction' (class) defined but never referenced outside its module | verify usage; delete if dead code |
| 🟡 | SYM1 | utils | `backend\utils\schema_audit.py:166` | symbol 'AuditReport' (class) defined but never referenced outside its module | verify usage; delete if dead code |
| 🟡 | SYM1 | services | `backend\services\audit\audit_service.py:13` | symbol 'AuditService' (class) defined but never referenced outside its module | verify usage; delete if dead code |
| 🟡 | SYM1 | services | `backend\services\audit\audit_trail_service.py:119` | symbol 'AuditTrailDataResidencyService' (class) defined but never referenced outside its module | verify usage; delete if dead code |
| 🟡 | SYM1 | controllers | `backend\controllers\security\auth_controller.py:137` | symbol 'AuthForgotPasswordRequest' (class) defined but never referenced outside its module | verify usage; delete if dead code |
| 🟡 | SYM1 | controllers | `backend\controllers\security\auth_controller.py:159` | symbol 'AuthLoginRequest' (class) defined but never referenced outside its module | verify usage; delete if dead code |
| 🟡 | SYM1 | controllers | `backend\controllers\security\auth_controller.py:141` | symbol 'AuthResetPasswordRequest' (class) defined but never referenced outside its module | verify usage; delete if dead code |
| 🟡 | SYM1 | routers | `backend\routers\api_core_ingestion.py:90` | symbol 'AutoAllocateInput' (class) defined but never referenced outside its module | verify usage; delete if dead code |
| 🟡 | SYM1 | routers | `backend\routers\api_geography_registry.py:499` | symbol 'AutoPopulateBody' (class) defined but never referenced outside its module | verify usage; delete if dead code |
...
| 🟡 | SYM1 | utils | `backend\utils\backup.py:30` | symbol 'BackupManager' (class) defined but never referenced outside its module | verify usage; delete if dead code |
| 🟡 | SYM1 | routers | `backend\routers\api_finance_routes_2.py:44` | symbol 'BadgeTierBody' (class) defined but never referenced outside its module | verify usage; delete if dead code |
...
| 🟡 | SYM1 | controllers | `backend\controllers\core\banner_controller.py:148` | symbol 'BannerControllerCreate' (class) defined but never referenced outside its module | verify usage; delete if dead code |
...
| 🟡 | SYM1 | services | `backend\services\media\free_image_tools.py:730` | symbol 'BatchResult' (class) defined but never referenced outside its module | verify usage; delete if dead code |
| 🟡 | SYM1 | services | `backend\services\hr\succession_service.py:20` | symbol 'BenchStrengthScore' (class) defined but never referenced outside its module | verify usage; delete if dead code |
| 🟡 | SYM1 | services | `backend\services\security\triple_auth.py:79` | symbol 'BiometricValidator' (class) defined but never referenced outside its module | verify usage; delete if dead code |
...
| 🟡 | SYM1 | providers | `backend\providers\legacy\br_05.py:153` | symbol 'Br05BackgroundRemover' (class) defined but never referenced outside its module | verify usage; delete if dead code |
| 🟡 | SYM1 | providers | `backend\providers\legacy\br_05.py:105` | symbol 'Br05CleanEdgeRefiner' (class) defined but never referenced outside its module | verify usage; delete if dead code |
| 🟡 | SYM1 | providers | `backend\providers\legacy\br_05.py:33` | symbol 'Br05ColoredFormatter' (class) defined but never referenced outside its module | verify usage; delete if dead code |
| 🟡 | SYM1 | providers | `backend\providers\legacy\br_05.py:56` | symbol 'Br05Deps' (class) defined but never referenced outside its module | verify usage; delete if dead code |
| 🟡 | SYM1 | providers | `backend\providers\legacy\br_06.py:281` | symbol 'Br06BackgroundRemover' (class) defined but never referenced outside its module | verify usage; delete if dead code |
| 🟡 | SYM1 | providers | `backend\providers\legacy\br_06.py:33` | symbol 'Br06ColoredFormatter' (class) defined but never referenced outside its module | verify usage; delete if dead code |
| 🟡 | SYM1 | providers | `backend\providers\legacy\br_06.py:53` | symbol 'Br06Deps' (class) defined but never referenced outside its module | verify usage; delete if dead code |
| 🟡 | SYM1 | providers | `backend\providers\legacy\br_06.py:243` | symbol 'Br06EdgeRefiner' (class) defined but never referenced outside its module | verify usage; delete if dead code |
| 🟡 | SYM1 | providers | `backend\providers\legacy\br_06.py:110` | symbol 'Br06HandRemover' (class) defined but never referenced outside its module | verify usage; delete if dead code |
| 🟡 | SYM1 | providers | `backend\providers\legacy\br_06.py:153` | symbol 'Br06HoleFiller' (class) defined but never referenced outside its module | verify usage; delete if dead code |
| 🟡 | SYM1 | providers | `backend\providers\legacy\br_06.py:219` | symbol 'Br06HumanPreserver' (class) defined but never referenced outside its module | verify usage; delete if dead code |
| 🟡 | SYM1 | providers | `backend\providers\legacy\br_06.py:98` | symbol 'Br06SceneAnalyzer' (class) defined but never referenced outside its module | verify usage; delete if dead code |
| 🟡 | SYM1 | providers | `backend\providers\legacy\br_06.py:194` | symbol 'Br06ThinPartHandler' (class) defined but never referenced outside its module | verify usage; delete if dead code |
| 🟡 | SYM1 | providers | `backend\providers\legacy\br_08.py:101` | symbol 'Br08ColorSpaceUtils' (class) defined but never referenced outside its module | verify usage; delete if dead code |
| 🟡 | SYM1 | providers | `backend\providers\legacy\br_08.py:463` | symbol 'Br08Exporter' (class) defined but never referenced outside its module | verify usage; delete if dead code |
| 🟡 | SYM1 | providers | `backend\providers\legacy\br_08.py:384` | symbol 'Br08HoleFiller' (class) defined but never referenced outside its module | verify usage; delete if dead code |
| 🟡 | SYM1 | providers | `backend\providers\legacy\br_08.py:128` | symbol 'Br08ImageLoader' (class) defined but never referenced outside its module | verify usage; delete if dead code |
| 🟡 | SYM1 | providers | `backend\providers\legacy\br_08.py:32` | symbol 'Br08MemoryManager' (class) defined but never referenced outside its module | verify usage; delete if dead code |
| 🟡 | SYM1 | providers | `backend\providers\legacy\br_08.py:191` | symbol 'Br08ModelSelector' (class) defined but never referenced outside its module | verify usage; delete if dead code |
| 🟡 | SYM1 | providers | `backend\providers\legacy\br_08.py:215` | symbol 'Br08MultiModelSegmenter' (class) defined but never referenced outside its module | verify usage; delete if dead code |
| 🟡 | SYM1 | providers | `backend\providers\legacy\br_08.py:74` | symbol 'Br08ProcessingConfig' (class) defined but never referenced outside its module | verify usage; delete if dead code |
| 🟡 | SYM1 | providers | `backend\providers\legacy\br_08.py:146` | symbol 'Br08QualityAnalyzer' (class) defined but never referenced outside its module | verify usage; delete if dead code |
| 🟡 | SYM1 | providers | `backend\providers\legacy\br_08.py:157` | symbol 'Br08SubjectDetector' (class) defined but never referenced outside its module | verify usage; delete if dead code |
| 🟡 | SYM1 | providers | `backend\providers\legacy\br_08.py:408` | symbol 'Br08WoodBackgroundRemover' (class) defined but never referenced outside its module | verify usage; delete if dead code |
| 🟡 | SYM1 | providers | `backend\providers\legacy\br_08.py:493` | symbol 'Br08ZoziBackgroundRemover' (class) defined but never referenced outside its module | verify usage; delete if dead code |
| 🟡 | SYM1 | providers | `backend\providers\legacy\br_11.py:60` | symbol 'Br11AISegmenter' (class) defined but never referenced outside its module | verify usage; delete if dead code |
| 🟡 | SYM1 | providers | `backend\providers\legacy\br_11.py:198` | symbol 'Br11ArtifactIsolator' (class) defined but never referenced outside its module | verify usage; delete if dead code |
| 🟡 | SYM1 | providers | `backend\providers\legacy\br_11.py:132` | symbol 'Br11EdgeShaver' (class) defined but never referenced outside its module | verify usage; delete if dead code |
| 🟡 | SYM1 | providers | `backend\providers\legacy\br_11.py:218` | symbol 'Br11Exporter' (class) defined but never referenced outside its module | verify usage; delete if dead code |
| 🟡 | SYM1 | providers | `backend\providers\legacy\br_11.py:152` | symbol 'Br11GlobalBackgroundBleeder' (class) defined but never referenced outside its module | verify usage; delete if dead code |
| 🟡 | SYM1 | providers | `backend\providers\legacy\br_11.py:54` | symbol 'Br11MemoryManager' (class) defined but never referenced outside its module | verify usage; delete if dead code |
| 🟡 | SYM1 | providers | `backend\providers\legacy\br_11.py:47` | symbol 'Br11ProcessingConfig' (class) defined but never referenced outside its module | verify usage; delete if dead code |
| 🟡 | SYM1 | providers | `backend\providers\legacy\br_12.py:54` | symbol 'Br12AISegmenter' (class) defined but never referenced outside its module | verify usage; delete if dead code |
| 🟡 | SYM1 | providers | `backend\providers\legacy\br_12.py:196` | symbol 'Br12BottomTextEraser' (class) defined but never referenced outside its module | verify usage; delete if dead code |
| 🟡 | SYM1 | providers | `backend\providers\legacy\br_12.py:238` | symbol 'Br12EdgeShaver' (class) defined but never referenced outside its module | verify usage; delete if dead code |
| 🟡 | SYM1 | providers | `backend\providers\legacy\br_12.py:277` | symbol 'Br12Exporter' (class) defined but never referenced outside its module | verify usage; delete if dead code |
| 🟡 | SYM1 | providers | `backend\providers\legacy\br_12.py:140` | symbol 'Br12FloatingArtifactRemover' (class) defined but never referenced outside its module | verify usage; delete if dead code |
| 🟡 | SYM1 | providers | `backend\providers\legacy\br_12.py:248` | symbol 'Br12GlobalBackgroundBleeder' (class) defined but never referenced outside its module | verify usage; delete if dead code |
| 🟡 | SYM1 | providers | `backend\providers\legacy\br_12.py:48` | symbol 'Br12MemoryManager' (class) defined but never referenced outside its module | verify usage; delete if dead code |
| 🟡 | SYM1 | providers | `backend\providers\legacy\br_12.py:41` | symbol 'Br12ProcessingConfig' (class) defined but never referenced outside its module | verify usage; delete if dead code |
| 🟡 | SYM1 | providers | `backend\providers\legacy\br_13.py:53` | symbol 'Br13AISegmenter' (class) defined but never referenced outside its module | verify usage; delete if dead code |
| 🟡 | SYM1 | providers | `backend\providers\legacy\br_13.py:216` | symbol 'Br13BottomTextEraser' (class) defined but never referenced outside its module | verify usage; delete if dead code |
| 🟡 | SYM1 | providers | `backend\providers\legacy\br_13.py:138` | symbol 'Br13EdgeShaver' (class) defined but never referenced outside its module | verify usage; delete if dead code |
| 🟡 | SYM1 | providers | `backend\providers\legacy\br_13.py:241` | symbol 'Br13Exporter' (class) defined but never referenced outside its module | verify usage; delete if dead code |
| 🟡 | SYM1 | providers | `backend\providers\legacy\br_13.py:176` | symbol 'Br13FloatingArtifactRemover' (class) defined but never referenced outside its module | verify usage; delete if dead code |
| 🟡 | SYM1 | providers | `backend\providers\legacy\br_13.py:148` | symbol 'Br13GlobalBackgroundBleeder' (class) defined but never referenced outside its module | verify usage; delete if dead code |
| 🟡 | SYM1 | providers | `backend\providers\legacy\br_13.py:47` | symbol 'Br13MemoryManager' (class) defined but never referenced outside its module | verify usage; delete if dead code |
| 🟡 | SYM1 | providers | `backend\providers\legacy\br_13.py:40` | symbol 'Br13ProcessingConfig' (class) defined but never referenced outside its module | verify usage; delete if dead code |
| 🟡 | SYM1 | routers | `backend\routers\api_finance_integration.py:333` | symbol 'BudgetSet' (class) defined but never referenced outside its module | verify usage; delete if dead code |
| 🟡 | SYM1 | routers | `backend\routers\admin_core_console.py:194` | symbol 'BulkDeleteUsersBody' (class) defined but never referenced outside its module | verify usage; delete if dead code |
| 🟡 | SYM1 | routers | `backend\routers\api_geography_registry.py:686` | symbol 'BulkIdsPayload' (class) defined but never referenced outside its module | verify usage; delete if dead code |
| 🟡 | SYM1 | routers | `backend\routers\admin_core_console.py:397` | symbol 'BulkOrderDeleteBody' (class) defined but never referenced outside its module | verify usage; delete if dead code |
| 🟡 | SYM2 | backend | `providers.legacy.br_05:96, providers.legacy.br_06:90` | class 'Config' defined in 2 modules | consolidate into one canonical definition |
...
| 🟡 | SYM2 | backend | `services.write_helpers:7, services.supplier.suppliers_write_service:173, services.orders.orders_write_service:15, services.logistics.logistics_write_service:121, services.core.write_helpers:15` | public function 'add_and_flush' defined in 5 modules | consolidate or rename to avoid confusion |
...
| 🟡 | SYM2 | backend | `services.supplier.suppliers_write_service:178, services.finance.payments_write_service:219` | public function 'add_notification' defined in 2 modules | consolidate or rename to avoid confusion |
...
| 🟡 | SYM2 | backend | `services.supplier.suppliers_write_service:186, services.finance.payments_write_service:204, services.country.country_write_service:415` | public function 'add_to_session' defined in 3 modules | consolidate or rename to avoid confusion |
...
| 🟡 | SYM2 | backend | `services.supplier.supplier_badge_service:702, controllers.supplier.supplier_controller:4196` | public function 'admin_set_supplier_badge' defined in 2 modules | consolidate or rename to avoid confusion |
...
| 🟡 | SYM2 | backend | `services.supplier.supplier_countries_service:1013, routers.api_geography_registry:388, controllers.country.country_controller:706` | public function 'approve_country_version' defined in 3 modules | consolidate or rename to avoid confusion |
...
| 🟡 | SYM2 | backend | `services.supplier.supplier_countries_service:1412, services.country.country_staff_service:64, controllers.country.country_controller:1438` | public function 'assign_staff_to_country' defined in 3 modules | consolidate or rename to avoid confusion |
| 🟡 | SYM2 | backend | `services.hr.lms_write_service:36, routers.api_hr_training:22, controllers.hr.lms_controller:33` | public function 'assign_training' defined in 3 modules | consolidate or rename to avoid confusion |
...
| 🟡 | SYM2 | backend | `services.media.media_router_service:393, routers.api_media_bulk:97` | public function 'batch_publish_products' defined in 2 modules | consolidate or rename to avoid confusion |
...
| 🟡 | SYM2 | backend | `routers.supplier_supplier_routes:255, controllers.supplier.supplier_controller:2916` | public function 'bulk_upload_products' defined in 2 modules | consolidate or rename to avoid confusion |
...
| 🟡 | SYM2 | backend | `services.hr.payroll_engine:527, services.hr.payroll_service:13` | public function 'calculate_monthly_payroll' defined in 2 modules | consolidate or rename to avoid confusion |
...
| 🟡 | SYM2 | backend | `services.media.media_router_service:237, routers.api_ai_ingestion:161` | public function 'cancel_ai_upload_job' defined in 2 modules | consolidate or rename to avoid confusion |
...
| 🟡 | SYM2 | backend | `services.hr.lms_read_service:11, controllers.hr.lms_controller:57` | public function 'check_permission_lock' defined in 2 modules | consolidate or rename to avoid confusion |
...
| 🟡 | SYM2 | backend | `services.orders.orders_router_service:304, controllers.orders.cart_controller:208` | public function 'clear_cart' defined in 2 modules | consolidate or rename to avoid confusion |
| 🟡 | SYM2 | backend | `utils.rls_interceptor:765, middleware.country_context:218` | public function 'clear_rls_context' defined in 2 modules | consolidate or rename to avoid confusion |
...
| 🟡 | SYM2 | backend | `utils.migration_helpers:67, alembic.migration_helpers:67` | public function 'column_exists' defined in 2 modules | consolidate or rename to avoid confusion |
...
| 🟡 | SYM2 | backend | `services.write_helpers:30, services.supplier.suppliers_write_service:199, services.orders.orders_write_service:22, services.logistics.logistics_partner_write_service:377, services.finance.payments_write_service:199` | public function 'commit_and_refresh' defined in 7 modules | consolidate or rename to avoid confusion |
| 🟡 | SYM2 | backend | `services.write_helpers:26, services.supplier.suppliers_write_service:165, services.orders.orders_write_service:29, services.logistics.logistics_write_service:126, services.finance.payments_write_service:195` | public function 'commit_only' defined in 6 modules | consolidate or rename to avoid confusion |
| 🟡 | SYM2 | backend | `services.supplier.suppliers_write_service:194, services.logistics.logistics_partner_write_service:372` | public function 'commit_session' defined in 2 modules | consolidate or rename to avoid confusion |
| 🟡 | SYM2 | backend | `services.supplier.supplier_badge_service:525, controllers.supplier.supplier_controller:3912` | public function 'compute_credibility_score' defined in 2 modules | consolidate or rename to avoid confusion |
...
| 🟡 | SYM2 | backend | `utils.migration_helpers:75, alembic.migration_helpers:75` | public function 'constraint_exists' defined in 2 modules | consolidate or rename to avoid confusion |
...
| 🟡 | SYM2 | backend | `services.supplier.supplier_countries_service:251, routers.api_geography_registry:268, controllers.country.country_controller:358` | public function 'create_admin_country' defined in 3 modules | consolidate or rename to avoid confusion |
...
| 🟡 | SYM2 | backend | `services.hr.employee_communication_service:227, routers.internal_core_channels:15` | public function 'create_channel' defined in 2 modules | consolidate or rename to avoid confusion |
...
| 🟡 | SYM2 | backend | `services.supplier.supplier_countries_service:595, routers.api_geography_registry:295, controllers.country.country_controller:610` | public function 'create_commission_draft' defined in 3 modules | consolidate or rename to avoid confusion |
| 🟡 | SYM2 | backend | `services.supplier.supplier_countries_service:948, routers.api_geography_registry:372, controllers.country.country_controller:1313` | public function 'create_commission_tiers_draft' defined in 3 modules | consolidate or rename to avoid confusion |
...
| 🟡 | SYM2 | backend | `routers.api_comms_command:306, routers.api_comms_console:299` | public function 'create_executive_news' defined in 2 modules | consolidate or rename to avoid confusion |
...
| 🟡 | SYM2 | backend | `services.treasury.treasury_service:119, services.finance.treasury_query_service:101, routers.api_finance_domain:68, routers.api_finance_routes:98, routers.api_finance_routes_3:98` | public function 'create_journal_entry' defined in 5 modules | consolidate or rename to avoid confusion |
| 🟡 | SYM2 | backend | `services.hr.employee_write_service:237, services.hr.ess_service:31` | public function 'create_leave_request' defined in 2 modules | consolidate or rename to avoid confusion |
| 🟡 | SYM2 | backend | `services.supplier.supplier_countries_service:777, routers.api_geography_registry:332, controllers.country.country_controller:1197` | public function 'create_legal_rules_draft' defined in 3 modules | consolidate or rename to avoid confusion |
| 🟡 | SYM2 | backend | `services.supplier.supplier_countries_service:554, routers.api_geography_registry:290, controllers.country.country_controller:570` | public function 'create_logistics_draft' defined in 3 modules | consolidate or rename to avoid confusion |
...
| 🟡 | SYM2 | backend | `services.supplier.supplier_countries_service:726, routers.api_geography_registry:322, controllers.country.country_controller:1159` | public function 'create_logistics_providers_draft' defined in 3 modules | consolidate or rename to avoid confusion |
| 🟡 | W4 | backend | `backend\controllers\__init__.py:9` | controller imports another controller ('controllers.country') | extract shared logic into a service or util; controllers stay thin |
| 🟡 | W4 | backend | `backend\controllers\__init__.py:10` | controller imports another controller ('controllers.finance') | extract shared logic into a service or util; controllers stay thin |
| 🟡 | W4 | backend | `backend\controllers\__init__.py:11` | controller imports another controller ('controllers.orders') | extract shared logic into a service or util; controllers stay thin |
| 🟡 | W4 | backend | `backend\controllers\__init__.py:12` | controller imports another controller ('controllers.security') | extract shared logic into a service or util; controllers stay thin |
| 🟡 | W4 | backend | `backend\controllers\__init__.py:13` | controller imports another controller ('controllers.supplier') | extract shared logic into a service or util; controllers stay thin |
| 🟡 | W4 | backend | `backend\controllers\__init__.py:14` | controller imports another controller ('controllers.communication') | extract shared logic into a service or util; controllers stay thin |
| 🟡 | W4 | backend | `backend\controllers\__init__.py:15` | controller imports another controller ('controllers.hr') | extract shared logic into a service or util; controllers stay thin |
...
| 🟡 | W4 | backend | `backend\controllers\admin_controller.py:99` | controller imports another controller ('controllers.treasury.admin_payouts_controller') | extract shared logic into a service or util; controllers stay thin |
...
| 🟡 | W4 | backend | `backend\controllers\iam_controller.py:2` | controller imports another controller ('controllers.security.iam_controller') | extract shared logic into a service or util; controllers stay thin |
...
| 🟡 | W4 | backend | `backend\controllers\risk_controller.py:2` | controller imports another controller ('controllers.security.risk_controller') | extract shared logic into a service or util; controllers stay thin |
...
| 🟡 | W4 | backend | `backend\controllers\supplier\__init__.py:6` | controller imports another controller ('controllers.supplier.supplier_controller') | extract shared logic into a service or util; controllers stay thin |
| 🟡 | W4 | backend | `backend\controllers\supplier\supplier_controller.py:1485` | controller imports another controller ('controllers.product_verification_controller') | extract shared logic into a service or util; controllers stay thin |
| 🟡 | W4 | backend | `backend\controllers\supplier\supplier_controller.py:273` | controller imports another controller ('controllers.country_controller') | extract shared logic into a service or util; controllers stay thin |
...
- 🟢 **I4** `move-map` — 55 file move suggestions generated → *see 'File Move Suggestions' section in this report*
...
- 🟢 **I1** `.` — backend models=51 routers=152 controllers=70 services=269 middleware=22
- 🟢 **I2** `documents/scope/` — rules loaded from: YAML policy (documents/scope/ or governance/)
- 🟢 **I3** `backend/` — module graph: modules=887, edges=2456, classes=1256
- 🟢 **MET1** `architecture-debt` — architecture debt score = 21474 → *track this number down over time; lower is healthier*

### BACKEND (640 findings)
...
- 🟢 **DOM8** `backend/` — 186 scanned file(s) are already in the correct domain folder → *keep these placements; do not move them*
...
- 🟡 **D1** `config.py` — sensitive module name in 3 dirs (import-shadow): backend\utils\config.py, backend\providers\hr\config.py, backend\providers\configuration\config.py → *keep the canonical copy (utils/config.py); delete the shadows*
...
- 🔴 **F4** `backend\test.db` — must not sit at backend (damages structure/scale) → *delete + add to .gitignore*
- 🟡 **F4** `backend\test.db-shm` — must not sit at backend (damages structure/scale) → *delete + add to .gitignore*
- 🟡 **F4** `backend\test.db-wal` — must not sit at backend (damages structure/scale) → *delete + add to .gitignore*
...
- 🔴 **F4** `backend\_probe.db` — must not sit at backend (damages structure/scale) → *delete + add to .gitignore*
- 🟡 **F4** `backend\_probe.db-shm` — must not sit at backend (damages structure/scale) → *delete + add to .gitignore*
- 🟡 **F4** `backend\_probe.db-wal` — must not sit at backend (damages structure/scale) → *delete + add to .gitignore*
...
- 🟡 **W4** `backend\controllers\__init__.py:9` — controller imports another controller ('controllers.country') → *extract shared logic into a service or util; controllers stay thin*
- 🟡 **W4** `backend\controllers\__init__.py:10` — controller imports another controller ('controllers.finance') → *extract shared logic into a service or util; controllers stay thin*
- 🟡 **W4** `backend\controllers\__init__.py:11` — controller imports another controller ('controllers.orders') → *extract shared logic into a service or util; controllers stay thin*
- 🟡 **W4** `backend\controllers\__init__.py:12` — controller imports another controller ('controllers.security') → *extract shared logic into a service or util; controllers stay thin*
- 🟡 **W4** `backend\controllers\__init__.py:13` — controller imports another controller ('controllers.supplier') → *extract shared logic into a service or util; controllers stay thin*
- 🟡 **W4** `backend\controllers\__init__.py:14` — controller imports another controller ('controllers.communication') → *extract shared logic into a service or util; controllers stay thin*
- 🟡 **W4** `backend\controllers\__init__.py:15` — controller imports another controller ('controllers.hr') → *extract shared logic into a service or util; controllers stay thin*
...
- 🟡 **W4** `backend\controllers\admin_controller.py:99` — controller imports another controller ('controllers.treasury.admin_payouts_controller') → *extract shared logic into a service or util; controllers stay thin*
...
- 🟡 **W4** `backend\controllers\iam_controller.py:2` — controller imports another controller ('controllers.security.iam_controller') → *extract shared logic into a service or util; controllers stay thin*
...
- 🟡 **W4** `backend\controllers\risk_controller.py:2` — controller imports another controller ('controllers.security.risk_controller') → *extract shared logic into a service or util; controllers stay thin*
...
- 🟡 **W4** `backend\controllers\supplier\__init__.py:6` — controller imports another controller ('controllers.supplier.supplier_controller') → *extract shared logic into a service or util; controllers stay thin*
- 🟡 **W4** `backend\controllers\supplier\supplier_controller.py:1485` — controller imports another controller ('controllers.product_verification_controller') → *extract shared logic into a service or util; controllers stay thin*
- 🟡 **W4** `backend\controllers\supplier\supplier_controller.py:273` — controller imports another controller ('controllers.country_controller') → *extract shared logic into a service or util; controllers stay thin*
...
- 🔴 **DG3** `backend\services\cash_management_service.py:46` — cross-domain import cash_management_service -> logistics violates explicit ownership rules → *declare allowed imports in layer_rules.yaml or route via logistics service facade*
- 🔴 **DG3** `backend\services\cash_management_service.py:47` — cross-domain import cash_management_service -> communication violates explicit ownership rules → *declare allowed imports in layer_rules.yaml or route via communication service facade*
...
- 🟡 **CIR2** `backend\routers\admin_commerce_routes.py:12` — circuit bypass: routers -> services (services.commerce.promotion_engine_service) → *routers should call controllers; direct router -> service usage skips the orchestration layer*
...
- 🟡 **CIR2** `backend\routers\admin_finance_routes.py:12` — circuit bypass: routers -> services (services.commission_write_service) → *routers should call controllers; direct router -> service usage skips the orchestration layer*
...
- 🟡 **CIR2** `backend\routers\admin_treasury_routes.py:10` — circuit bypass: routers -> services (services.cash_management_write_service) → *routers should call controllers; direct router -> service usage skips the orchestration layer*
...
- 🟡 **CIR2** `backend\routers\api_comms_command.py:33` — circuit bypass: routers -> services (services.write_helpers) → *routers should call controllers; direct router -> service usage skips the orchestration layer*
...
- 🔴 **CIR1** `backend\routers\api_geography_communications.py:34` — circuit violation: routers -> db (db.models_country_control) is outside the allowed circuit → *routers may import only: controllers, data, dependencies, utils*
...
- 🟡 **CIR2** `backend\routers\api_hr_processing.py:13` — circuit bypass: routers -> services (services.finance.payroll_read_service) → *routers should call controllers; direct router -> service usage skips the orchestration layer*
...
- 🟡 **CIR2** `backend\routers\api_media_bulk.py:34` — circuit bypass: routers -> services (services.storage) → *routers should call controllers; direct router -> service usage skips the orchestration layer*
...
- 🟡 **CIR2** `backend\routers\api_treasury_approval.py:29` — circuit bypass: routers -> services (services.treasury.treasury_router_service) → *routers should call controllers; direct router -> service usage skips the orchestration layer*
- 🟡 **CIR2** `backend\routers\api_treasury_routes.py:31` — circuit bypass: routers -> services (services.treasury.treasury_router_service) → *routers should call controllers; direct router -> service usage skips the orchestration layer*
- 🟡 **CIR2** `backend\routers\api_treasury_routes_2.py:4` — circuit bypass: routers -> services (services.treasury_service) → *routers should call controllers; direct router -> service usage skips the orchestration layer*
- 🔴 **CIR1** `backend\routers\effective_permissions.py:34` — circuit violation: routers -> db (db.models_country_control) is outside the allowed circuit → *routers may import only: controllers, data, dependencies, utils*
...
- 🟡 **CIR2** `backend\routers\treasury_api.py:11` — circuit bypass: routers -> services (services.finance.treasury_query_service) → *routers should call controllers; direct router -> service usage skips the orchestration layer*
...
- 🟡 **DG2** `backend\models\__init__.py` — circular module dependency: models -> models._exports -> models.communication.core -> models.communication -> models → *break the cycle by extracting shared logic into a lower layer (utils/service interface)*
...
- 🟡 **DG2** `backend\models\__init__.py` — circular module dependency: models -> models._exports -> models.country.country_enhancements -> models.country -> models → *break the cycle by extracting shared logic into a lower layer (utils/service interface)*
...
- 🟡 **DG2** `backend\models\__init__.py` — circular module dependency: models -> models._exports -> models.treasury.finance -> models.treasury -> models → *break the cycle by extracting shared logic into a lower layer (utils/service interface)*
- 🟡 **DG2** `backend\controllers\__init__.py` — circular module dependency: controllers -> controllers.supplier -> controllers.supplier.supplier_controller -> controllers → *break the cycle by extracting shared logic into a lower layer (utils/service interface)*
...
- 🔴 **DG2** `domain-graph` — circular domain dependency: cash_management_service -> logistics -> cash_management_service → *redefine bounded-context boundaries; introduce explicit service contracts / events*
- 🔴 **DG2** `domain-graph` — circular domain dependency: cash_management_service -> communication -> logistics -> cash_management_service → *redefine bounded-context boundaries; introduce explicit service contracts / events*
...
- 🟡 **A1** `backend\controllers\supplier\supplier_controller.py` — architecture hotspot: fan_in=2, fan_out=21, instability=0.91 → *reduce coupling; split responsibilities or introduce an abstraction layer*
...
- 🟡 **A1** `backend\data\models.py` — architecture hotspot: fan_in=302, fan_out=1, instability=0.00 → *reduce coupling; split responsibilities or introduce an abstraction layer*
- 🟡 **A1** `backend\data\models_employee_models.py` — architecture hotspot: fan_in=42, fan_out=1, instability=0.02 → *reduce coupling; split responsibilities or introduce an abstraction layer*
- 🟡 **A1** `backend\data\schemas.py` — architecture hotspot: fan_in=54, fan_out=1, instability=0.02 → *reduce coupling; split responsibilities or introduce an abstraction layer*
...
- 🟡 **A1** `backend\models\__init__.py` — architecture hotspot: fan_in=46, fan_out=2, instability=0.04 → *reduce coupling; split responsibilities or introduce an abstraction layer*
...
- 🟡 **A1** `backend\services\__init__.py` — architecture hotspot: fan_in=125, fan_out=1, instability=0.01 → *reduce coupling; split responsibilities or introduce an abstraction layer*
...
- 🟡 **A1** `backend\utils\config.py` — architecture hotspot: fan_in=58, fan_out=0, instability=0.00 → *reduce coupling; split responsibilities or introduce an abstraction layer*
...
- 🟡 **H1** `backend\utils\analyze_fks.py:4` — sys.path manipulation detected → *remove sys.path.insert/append; fix package structure and use proper imports*
...
- 🟡 **SYM2** `providers.legacy.br_05:96, providers.legacy.br_06:90` — class 'Config' defined in 2 modules → *consolidate into one canonical definition*
...
- 🟡 **SYM2** `services.write_helpers:7, services.supplier.suppliers_write_service:173, services.orders.orders_write_service:15, services.logistics.logistics_write_service:121, services.core.write_helpers:15` — public function 'add_and_flush' defined in 5 modules → *consolidate or rename to avoid confusion*
...
- 🟡 **SYM2** `services.supplier.suppliers_write_service:178, services.finance.payments_write_service:219` — public function 'add_notification' defined in 2 modules → *consolidate or rename to avoid confusion*
...
- 🟡 **SYM2** `services.supplier.suppliers_write_service:186, services.finance.payments_write_service:204, services.country.country_write_service:415` — public function 'add_to_session' defined in 3 modules → *consolidate or rename to avoid confusion*
...
- 🟡 **SYM2** `services.supplier.supplier_badge_service:702, controllers.supplier.supplier_controller:4196` — public function 'admin_set_supplier_badge' defined in 2 modules → *consolidate or rename to avoid confusion*
...
- 🟡 **SYM2** `services.supplier.supplier_countries_service:1013, routers.api_geography_registry:388, controllers.country.country_controller:706` — public function 'approve_country_version' defined in 3 modules → *consolidate or rename to avoid confusion*
...
- 🟡 **SYM2** `services.supplier.supplier_countries_service:1412, services.country.country_staff_service:64, controllers.country.country_controller:1438` — public function 'assign_staff_to_country' defined in 3 modules → *consolidate or rename to avoid confusion*
- 🟡 **SYM2** `services.hr.lms_write_service:36, routers.api_hr_training:22, controllers.hr.lms_controller:33` — public function 'assign_training' defined in 3 modules → *consolidate or rename to avoid confusion*
...
- 🟡 **SYM2** `services.media.media_router_service:393, routers.api_media_bulk:97` — public function 'batch_publish_products' defined in 2 modules → *consolidate or rename to avoid confusion*
...
- 🟡 **SYM2** `routers.supplier_supplier_routes:255, controllers.supplier.supplier_controller:2916` — public function 'bulk_upload_products' defined in 2 modules → *consolidate or rename to avoid confusion*
...
- 🟡 **SYM2** `services.hr.payroll_engine:527, services.hr.payroll_service:13` — public function 'calculate_monthly_payroll' defined in 2 modules → *consolidate or rename to avoid confusion*
...
- 🟡 **SYM2** `services.media.media_router_service:237, routers.api_ai_ingestion:161` — public function 'cancel_ai_upload_job' defined in 2 modules → *consolidate or rename to avoid confusion*
...
- 🟡 **SYM2** `services.hr.lms_read_service:11, controllers.hr.lms_controller:57` — public function 'check_permission_lock' defined in 2 modules → *consolidate or rename to avoid confusion*
...
- 🟡 **SYM2** `services.orders.orders_router_service:304, controllers.orders.cart_controller:208` — public function 'clear_cart' defined in 2 modules → *consolidate or rename to avoid confusion*
- 🟡 **SYM2** `utils.rls_interceptor:765, middleware.country_context:218` — public function 'clear_rls_context' defined in 2 modules → *consolidate or rename to avoid confusion*
...
- 🟡 **SYM2** `utils.migration_helpers:67, alembic.migration_helpers:67` — public function 'column_exists' defined in 2 modules → *consolidate or rename to avoid confusion*
...
- 🟡 **SYM2** `services.write_helpers:30, services.supplier.suppliers_write_service:199, services.orders.orders_write_service:22, services.logistics.logistics_partner_write_service:377, services.finance.payments_write_service:199` — public function 'commit_and_refresh' defined in 7 modules → *consolidate or rename to avoid confusion*
- 🟡 **SYM2** `services.write_helpers:26, services.supplier.suppliers_write_service:165, services.orders.orders_write_service:29, services.logistics.logistics_write_service:126, services.finance.payments_write_service:195` — public function 'commit_only' defined in 6 modules → *consolidate or rename to avoid confusion*
- 🟡 **SYM2** `services.supplier.suppliers_write_service:194, services.logistics.logistics_partner_write_service:372` — public function 'commit_session' defined in 2 modules → *consolidate or rename to avoid confusion*
- 🟡 **SYM2** `services.supplier.supplier_badge_service:525, controllers.supplier.supplier_controller:3912` — public function 'compute_credibility_score' defined in 2 modules → *consolidate or rename to avoid confusion*
...
- 🟡 **SYM2** `utils.migration_helpers:75, alembic.migration_helpers:75` — public function 'constraint_exists' defined in 2 modules → *consolidate or rename to avoid confusion*
...
- 🟡 **SYM2** `services.supplier.supplier_countries_service:251, routers.api_geography_registry:268, controllers.country.country_controller:358` — public function 'create_admin_country' defined in 3 modules → *consolidate or rename to avoid confusion*
...
- 🟡 **SYM2** `services.hr.employee_communication_service:227, routers.internal_core_channels:15` — public function 'create_channel' defined in 2 modules → *consolidate or rename to avoid confusion*
...
- 🟡 **SYM2** `services.supplier.supplier_countries_service:595, routers.api_geography_registry:295, controllers.country.country_controller:610` — public function 'create_commission_draft' defined in 3 modules → *consolidate or rename to avoid confusion*
- 🟡 **SYM2** `services.supplier.supplier_countries_service:948, routers.api_geography_registry:372, controllers.country.country_controller:1313` — public function 'create_commission_tiers_draft' defined in 3 modules → *consolidate or rename to avoid confusion*
...
- 🟡 **SYM2** `routers.api_comms_command:306, routers.api_comms_console:299` — public function 'create_executive_news' defined in 2 modules → *consolidate or rename to avoid confusion*
...
- 🟡 **SYM2** `services.treasury.treasury_service:119, services.finance.treasury_query_service:101, routers.api_finance_domain:68, routers.api_finance_routes:98, routers.api_finance_routes_3:98` — public function 'create_journal_entry' defined in 5 modules → *consolidate or rename to avoid confusion*
- 🟡 **SYM2** `services.hr.employee_write_service:237, services.hr.ess_service:31` — public function 'create_leave_request' defined in 2 modules → *consolidate or rename to avoid confusion*
- 🟡 **SYM2** `services.supplier.supplier_countries_service:777, routers.api_geography_registry:332, controllers.country.country_controller:1197` — public function 'create_legal_rules_draft' defined in 3 modules → *consolidate or rename to avoid confusion*
- 🟡 **SYM2** `services.supplier.supplier_countries_service:554, routers.api_geography_registry:290, controllers.country.country_controller:570` — public function 'create_logistics_draft' defined in 3 modules → *consolidate or rename to avoid confusion*
...
- 🟡 **SYM2** `services.supplier.supplier_countries_service:726, routers.api_geography_registry:322, controllers.country.country_controller:1159` — public function 'create_logistics_providers_draft' defined in 3 modules → *consolidate or rename to avoid confusion*
- 🟡 **API2** `backend\services\communication\chat_enrichment.py:19` — private symbol '_ALLOWED_TABLES' used in 4 external module(s) → *make it public (remove _) or keep internal and refactor external usages*
...
- 🟡 **API2** `backend\services\supplier\supplier_badge_service.py:47` — private symbol '_BADGE_AMOUNT_QUANT' used in 2 external module(s) → *make it public (remove _) or keep internal and refactor external usages*
- 🟡 **API2** `backend\services\supplier\supplier_badge_service.py:39` — private symbol '_BADGE_THRESHOLDS' used in 4 external module(s) → *make it public (remove _) or keep internal and refactor external usages*
- 🟡 **API2** `backend\services\supplier\supplier_badge_service.py:45` — private symbol '_FULFILLED_ORDER_STATUSES' used in 3 external module(s) → *make it public (remove _) or keep internal and refactor external usages*
- 🟡 **API2** `backend\services\media\free_image_tools.py:31` — private symbol '_HAS_CV2' used in 32 external module(s) → *make it public (remove _) or keep internal and refactor external usages*
- 🟡 **API2** `backend\services\supplier\supplier_badge_service.py:46` — private symbol '_MANUAL_BADGE_LEVELS' used in 7 external module(s) → *make it public (remove _) or keep internal and refactor external usages*
...
- 🟡 **API2** `backend\controllers\supplier\supplier_controller.py:4153` — private symbol '_PERIOD_DAYS' used in 4 external module(s) → *make it public (remove _) or keep internal and refactor external usages*
...
- 🟡 **API2** `backend\controllers\supplier\supplier_controller.py:140` — private symbol '_UNSET' used in 8 external module(s) → *make it public (remove _) or keep internal and refactor external usages*
...
- 🟡 **API2** `backend\services\finance\payments_gateway_service.py:567` — private symbol '_apply_stripe_runtime_key' used in 1 external module(s) → *make it public (remove _) or keep internal and refactor external usages*
...
- 🟡 **API2** `backend\services\orders\orders_router_service.py:72` — private symbol '_as_float' used in 14 external module(s) → *make it public (remove _) or keep internal and refactor external usages*
- 🟡 **API2** `backend\services\treasury\treasury_adapter.py:71` — private symbol '_audit' used in 5 external module(s) → *make it public (remove _) or keep internal and refactor external usages*
- 🟡 **API2** `backend\services\supplier\supplier_badge_service.py:598` — private symbol '_badge_for_score' used in 1 external module(s) → *make it public (remove _) or keep internal and refactor external usages*
- 🟡 **API2** `backend\services\supplier\supplier_badge_service.py:81` — private symbol '_badge_period_bounds' used in 2 external module(s) → *make it public (remove _) or keep internal and refactor external usages*
- 🟡 **API2** `backend\services\supplier\supplier_badge_service.py:111` — private symbol '_badge_tier_meets_metrics' used in 3 external module(s) → *make it public (remove _) or keep internal and refactor external usages*
- 🟡 **API2** `backend\routers\admin_commerce_routes.py:766` — private symbol '_banner_to_dict' used in 6 external module(s) → *make it public (remove _) or keep internal and refactor external usages*
- 🟡 **API2** `backend\utils\error_handler.py:81` — private symbol '_before_send' used in 2 external module(s) → *make it public (remove _) or keep internal and refactor external usages*
...
- 🟡 **API2** `backend\providers\hr\bg_remover.py:83` — private symbol '_bytes_to_image' used in 4 external module(s) → *make it public (remove _) or keep internal and refactor external usages*
- 🟡 **API2** `backend\services\security\effective_permissions.py:151` — private symbol '_cache_key' used in 2 external module(s) → *make it public (remove _) or keep internal and refactor external usages*
...
- 🟡 **API2** `backend\services\supplier\supplier_badge_service.py:117` — private symbol '_compute_badge_threshold_metrics' used in 3 external module(s) → *make it public (remove _) or keep internal and refactor external usages*
...
- 🟡 **API2** `backend\services\supplier\supplier_badge_service.py:220` — private symbol '_create_badge_billing_record' used in 3 external module(s) → *make it public (remove _) or keep internal and refactor external usages*
- 🟡 **API2** `backend\services\treasury\payout_batch_service.py:184` — private symbol '_create_payout_batch' used in 1 external module(s) → *make it public (remove _) or keep internal and refactor external usages*
...
- 🟡 **API2** `backend\services\country\country_detection.py:113` — private symbol '_default_country' used in 6 external module(s) → *make it public (remove _) or keep internal and refactor external usages*
...
- 🟡 **API2** `backend\services\supplier\supplier_badge_service.py:54` — private symbol '_ensure_supplier_profile_record' used in 3 external module(s) → *make it public (remove _) or keep internal and refactor external usages*
...
- 🟡 **API2** `backend\providers\ai\text.py:348` — private symbol '_extract_product_name' used in 1 external module(s) → *make it public (remove _) or keep internal and refactor external usages*
- 🟡 **API2** `backend\providers\ai\text.py:356` — private symbol '_extract_tags' used in 1 external module(s) → *make it public (remove _) or keep internal and refactor external usages*
- 🟡 **API2** `backend\controllers\catalog\search_controller.py:173` — private symbol '_extract_terms' used in 2 external module(s) → *make it public (remove _) or keep internal and refactor external usages*
- 🟡 **API2** `backend\providers\ai\text.py:284` — private symbol '_extract_variant_from_text' used in 1 external module(s) → *make it public (remove _) or keep internal and refactor external usages*
...
- 🟡 **API2** `backend\services\country\country_ai_research.py:562` — private symbol '_fallback' used in 1 external module(s) → *make it public (remove _) or keep internal and refactor external usages*
- 🟡 **API2** `backend\services\hr\hierarchy_service.py:68` — private symbol '_fetch_employees_by_ids' used in 1 external module(s) → *make it public (remove _) or keep internal and refactor external usages*
- 🟡 **API2** `backend\services\supplier\supplier_badge_service.py:197` — private symbol '_find_existing_badge_billing' used in 1 external module(s) → *make it public (remove _) or keep internal and refactor external usages*
- 🟡 **API2** `backend\services\orders\orders_router_service.py:83` — private symbol '_first_non_none' used in 6 external module(s) → *make it public (remove _) or keep internal and refactor external usages*
...
- 🟡 **API2** `backend\services\payments\payment_engine.py:167` — private symbol '_get_adapter' used in 1 external module(s) → *make it public (remove _) or keep internal and refactor external usages*
...
- 🟡 **API2** `backend\services\treasury\payout_engine.py:41` — private symbol '_get_country' used in 5 external module(s) → *make it public (remove _) or keep internal and refactor external usages*
...
- 🟡 **API2** `backend\controllers\catalog\products_controller.py:198` — private symbol '_get_product_cache_version' used in 1 external module(s) → *make it public (remove _) or keep internal and refactor external usages*
...
- 🟡 **API2** `backend\utils\migration_helpers.py:29` — private symbol '_index_names' used in 1 external module(s) → *make it public (remove _) or keep internal and refactor external usages*
- 🟡 **API2** `backend\utils\migration_helpers.py:21` — private symbol '_inspector' used in 9 external module(s) → *make it public (remove _) or keep internal and refactor external usages*
- 🟡 **API2** `backend\alembic\versions\20260731_0012_partition_journal_entries.py:27` — private symbol '_is_postgres' used in 6 external module(s) → *make it public (remove _) or keep internal and refactor external usages*
...
- 🟡 **API2** `backend\services\supplier\supplier_badge_service.py:90` — private symbol '_load_active_badge_tiers' used in 2 external module(s) → *make it public (remove _) or keep internal and refactor external usages*
...
- 🟡 **API2** `backend\controllers\supplier\supplier_controller.py:203` — private symbol '_load_shipments_for_orders' used in 2 external module(s) → *make it public (remove _) or keep internal and refactor external usages*
- 🟡 **API2** `backend\services\treasury\payout_batch_service.py:308` — private symbol '_log_automation' used in 3 external module(s) → *make it public (remove _) or keep internal and refactor external usages*
- 🟡 **API2** `backend\services\hr\employee_communication_service.py:655` — private symbol '_log_comm_event' used in 1 external module(s) → *make it public (remove _) or keep internal and refactor external usages*
...
- 🟡 **API2** `backend\services\supplier\supplier_badge_service.py:272` — private symbol '_maybe_create_recurring_badge_billing' used in 1 external module(s) → *make it public (remove _) or keep internal and refactor external usages*
- 🟡 **API2** `backend\services\country\country_ai_research.py:524` — private symbol '_merge_ai_output' used in 5 external module(s) → *make it public (remove _) or keep internal and refactor external usages*
...
- 🟡 **API2** `backend\controllers\supplier\supplier_controller.py:163` — private symbol '_normalize_product_visibility_regions' used in 1 external module(s) → *make it public (remove _) or keep internal and refactor external usages*
...
- 🟡 **API2** `backend\providers\ai\text.py:80` — private symbol '_ollama_vision_chat' used in 2 external module(s) → *make it public (remove _) or keep internal and refactor external usages*
...
- 🟡 **API2** `backend\services\country\country_ai_research.py:510` — private symbol '_parse_json_text' used in 4 external module(s) → *make it public (remove _) or keep internal and refactor external usages*
...
- 🟡 **DG5** `backend\services\cash_management_service.py:533` — dynamic execution/import obscures dependency graph (import_module:None) → *avoid eval/exec/dynamic import_module for layer-critical code paths*
...
- 🟡 **QUAL3** `backend\services\auto_payout_scheduler.py:72` — oversized function 'run_auto_payout_sweep' (270 lines) → *extract smaller functions / service methods; long functions hide side effects*
- 🟡 **QUAL3** `backend\services\auto_payout_scheduler.py:347` — oversized function 'run_auto_logistics_payout_sweep' (269 lines) → *extract smaller functions / service methods; long functions hide side effects*
- 🟡 **QUAL3** `backend\services\cash_management_service.py` — oversized file (1248 lines) → *split by domain/responsibility; large files become change bottlenecks*
...
- 🟡 **QUAL3** `backend\services\orders\cart_shipping_service.py:158` — oversized function 'quote_supplier_groups' (195 lines) → *extract smaller functions / service methods; long functions hide side effects*
- 🟡 **QUAL3** `backend\services\media\media_router_service.py:249` — oversized function 'process_ai_upload_job' (142 lines) → *extract smaller functions / service methods; long functions hide side effects*
- 🟡 **QUAL3** `backend\services\media\media_router_service.py:393` — oversized function 'batch_publish_products' (206 lines) → *extract smaller functions / service methods; long functions hide side effects*
...
- 🟡 **QUAL3** `backend\routers\api_comms_command.py:524` — oversized function 'get_comprehensive_dashboard' (341 lines) → *extract smaller functions / service methods; long functions hide side effects*
...
- 🟡 **QUAL3** `backend\routers\api_media_bulk.py:134` — oversized function 'batch_analyze_products' (127 lines) → *extract smaller functions / service methods; long functions hide side effects*
...
- 🟡 **QUAL3** `backend\controllers\supplier\supplier_controller.py` — oversized file (4737 lines) → *split by domain/responsibility; large files become change bottlenecks*
- 🟡 **QUAL3** `backend\controllers\supplier\supplier_controller.py:979` — oversized function 'get_supplier_orders' (145 lines) → *extract smaller functions / service methods; long functions hide side effects*
- 🟡 **QUAL3** `backend\controllers\supplier\supplier_controller.py:1237` — oversized function 'get_supplier_label_payload' (143 lines) → *extract smaller functions / service methods; long functions hide side effects*
- 🟡 **QUAL3** `backend\controllers\supplier\supplier_controller.py:1382` — oversized function 'upload_supplier_parcel_proof' (141 lines) → *extract smaller functions / service methods; long functions hide side effects*
- 🟡 **QUAL3** `backend\controllers\supplier\supplier_controller.py:1691` — oversized function 'create_supplier_product_upload' (132 lines) → *extract smaller functions / service methods; long functions hide side effects*
- 🟡 **QUAL3** `backend\controllers\supplier\supplier_controller.py:2760` — oversized function 'get_supplier_reports' (147 lines) → *extract smaller functions / service methods; long functions hide side effects*
- 🟡 **QUAL3** `backend\controllers\supplier\supplier_controller.py:2916` — oversized function 'bulk_upload_products' (302 lines) → *extract smaller functions / service methods; long functions hide side effects*
...
- 🟡 **QUAL3** `backend\controllers\orders\returns_controller.py:425` — oversized function 'update_return_request' (147 lines) → *extract smaller functions / service methods; long functions hide side effects*
...
- 🟡 **QUAL3** `backend\controllers\country\country_controller.py` — oversized file (1698 lines) → *split by domain/responsibility; large files become change bottlenecks*
- 🟡 **QUAL3** `backend\controllers\country\country_controller.py:358` — oversized function 'create_admin_country' (175 lines) → *extract smaller functions / service methods; long functions hide side effects*
- 🟡 **QUAL3** `backend\controllers\country\country_controller.py:732` — oversized function '_apply_version_payload' (143 lines) → *extract smaller functions / service methods; long functions hide side effects*
...
- 🟡 **MET2** `backend\controllers\orders\admin_orders_controller.py` — high instability: I=0.92 (Ca=1, Ce=12) → *module is very fragile; add abstractions or reduce outgoing dependencies*
- 🟡 **MET2** `backend\controllers\orders\orders_controller.py` — high instability: I=0.94 (Ca=1, Ce=17) → *module is very fragile; add abstractions or reduce outgoing dependencies*
...
- 🟡 **MET2** `backend\controllers\supplier\supplier_controller.py` — high instability: I=0.91 (Ca=2, Ce=21) → *module is very fragile; add abstractions or reduce outgoing dependencies*
...
- 🟡 **BC1** `backend\services\cash_management_service.py:46` — cross-domain import cash_management_service → logistics bypasses event/facade boundary → *route through events/ or a logistics service facade; declare in layer_rules.yaml if intentional*
- 🟡 **BC3** `backend\services\audit\ediscovery.py:16` — bounded context leakage: audit service directly imports treasury models → *use treasury service API or events instead of direct model access*
...
- 🟡 **BC3** `backend\services\hr\employee_communication_service.py:390` — bounded context leakage: hr service directly imports communication models → *use communication service API or events instead of direct model access*
- 🟡 **BC3** `backend\services\treasury\treasury_engine.py:380` — bounded context leakage: treasury service directly imports orders models → *use orders service API or events instead of direct model access*
...
- 🔴 **W1** `backend\controllers\treasury\admin_payouts_controller.py` — 1 session write(s) in this file; move writes into services/<domain>/ (lines: 83) → *a services/<domain>/*_service.py method*
- 🟡 **Q1** `backend\controllers\supplier\supplier_controller.py` — 37 DB read(s) via .query() in this file; delegate reads to a service (lines: 298, 612, 620, 989, 1541, 1580, 2060, 2073, 2088, 2106 +27 more) → *service layer*
...
- 🟡 **Q1** `backend\controllers\orders\returns_controller.py` — 1 DB read(s) via .query() in this file; delegate reads to a service (lines: 150) → *service layer*
...
- 🔴 **W1** `backend\controllers\country\country_controller.py` — 1 session write(s) in this file; move writes into services/<domain>/ (lines: 1429) → *a services/<domain>/*_service.py method*
...
- 🟡 **Q1** `backend\routers\admin_commerce_routes.py` — 1 DB read(s) via .query() in this file; delegate reads to a service (lines: 521) → *service layer*
- 🟡 **Q1** `backend\routers\api_comms_command.py` — 21 DB read(s) via .query() in this file; delegate reads to a service (lines: 128, 176, 200, 231, 257, 284, 346, 359, 385, 401 +11 more) → *service layer*
- 🔴 **W1** `backend\routers\api_comms_realtime.py` — 4 session write(s) in this file; move writes into services/<domain>/ (lines: 51, 60, 84, 91) → *a services/<domain>/*_service.py method*
- 🟡 **Q1** `backend\routers\api_geography_communications.py` — 4 DB read(s) via .query() in this file; delegate reads to a service (lines: 16, 35, 55, 77) → *service layer*
- 🟡 **Q1** `backend\routers\effective_permissions.py` — 4 DB read(s) via .query() in this file; delegate reads to a service (lines: 16, 35, 55, 77) → *service layer*
- 🟡 **PERF2** `backend\utils\rls_interceptor.py` — 2 possible DB query inside loop (N+1 risk) in this file; batch queries / use joins / preload relationships (lines: 1049, 1055) → *batch the query / use joins / preload relationships instead of querying per item*
...
- 🟡 **QUAL1** `backend\utils\auth.py` — 9 weak exception handling location(s) in this file; log or re-raise instead of swallowing exceptions (lines: 93, 111, 132, 155, 176, 226, 237, 255, 266) → *log or re-raise; silent swallowing hides bugs*
...
- 🟡 **QUAL1** `backend\services\supplier\supplier_orders_service.py` — 1 weak exception handling location(s) in this file; log or re-raise instead of swallowing exceptions (lines: 36) → *log or re-raise; silent swallowing hides bugs*
...
- 🟡 **QUAL1** `backend\services\media\free_image_tools.py` — 1 weak exception handling location(s) in this file; log or re-raise instead of swallowing exceptions (lines: 97) → *log or re-raise; silent swallowing hides bugs*
- 🟡 **QUAL1** `backend\services\hr\performance_service.py` — 2 weak exception handling location(s) in this file; log or re-raise instead of swallowing exceptions (lines: 190, 460) → *log or re-raise; silent swallowing hides bugs*
- 🟡 **QUAL1** `backend\services\finance\finance_automation.py` — 1 weak exception handling location(s) in this file; log or re-raise instead of swallowing exceptions (lines: 439) → *log or re-raise; silent swallowing hides bugs*
...
- 🟡 **QUAL1** `backend\services\country\country_detection.py` — 3 weak exception handling location(s) in this file; log or re-raise instead of swallowing exceptions (lines: 98, 123, 142) → *log or re-raise; silent swallowing hides bugs*
...
- 🟡 **QUAL1** `backend\routers\api_comms_command.py` — 1 weak exception handling location(s) in this file; log or re-raise instead of swallowing exceptions (lines: 505) → *log or re-raise; silent swallowing hides bugs*
...
- 🟡 **QUAL1** `backend\providers\hr\bg_remover.py` — 3 weak exception handling location(s) in this file; log or re-raise instead of swallowing exceptions (lines: 297, 1265, 1298) → *log or re-raise; silent swallowing hides bugs*
...
- 🟡 **QUAL1** `backend\middleware\country_detection.py` — 1 weak exception handling location(s) in this file; log or re-raise instead of swallowing exceptions (lines: 46) → *log or re-raise; silent swallowing hides bugs*
...
- 🟡 **QUAL1** `backend\controllers\supplier\supplier_controller.py` — 2 weak exception handling location(s) in this file; log or re-raise instead of swallowing exceptions (lines: 594, 1768) → *log or re-raise; silent swallowing hides bugs*
...
- 🟡 **QUAL1** `backend\controllers\orders\returns_controller.py` — 1 weak exception handling location(s) in this file; log or re-raise instead of swallowing exceptions (lines: 319) → *log or re-raise; silent swallowing hides bugs*
- 🟡 **QUAL1** `backend\controllers\country\country_controller.py` — 1 weak exception handling location(s) in this file; log or re-raise instead of swallowing exceptions (lines: 1430) → *log or re-raise; silent swallowing hides bugs*
...
- 🟡 **QUAL4** `backend\utils\analyze_fks.py` — 4 print/debug output location(s) in this file; use structured logging instead of print() (lines: 24, 60, 70, 71) → *use structured logging instead of print()*
...
- 🟡 **SYM1** `backend\controllers\security\auth_controller.py:137` — symbol 'AuthForgotPasswordRequest' (class) defined but never referenced outside its module → *verify usage; delete if dead code*
- 🟡 **SYM1** `backend\controllers\security\auth_controller.py:159` — symbol 'AuthLoginRequest' (class) defined but never referenced outside its module → *verify usage; delete if dead code*
- 🟡 **SYM1** `backend\controllers\security\auth_controller.py:141` — symbol 'AuthResetPasswordRequest' (class) defined but never referenced outside its module → *verify usage; delete if dead code*
- 🟡 **SYM1** `backend\controllers\core\banner_controller.py:148` — symbol 'BannerControllerCreate' (class) defined but never referenced outside its module → *verify usage; delete if dead code*
- 🟢 **MET3** `backend/controllers/` — no abstract classes in controllers/ (A=0.00, 13 classes) → *consider adding interfaces/ABCs for dependency inversion*

### MIDDLEWARE (2 findings)

- 🟡 **SYM1** `backend\middleware\behavioral_analytics.py:24` — symbol 'AnomalyDetector' (class) defined but never referenced outside its module → *verify usage; delete if dead code*
- 🟢 **MET3** `backend/middleware/` — no abstract classes in middleware/ (A=0.00, 39 classes) → *consider adding interfaces/ABCs for dependency inversion*
...
- 🟡 **SYM1** `backend\providers\analytics\analytics.py:13` — symbol 'AnalyticsProviderSettings' (class) defined but never referenced outside its module → *verify usage; delete if dead code*
...
- 🟡 **SYM1** `backend\providers\legacy\br_05.py:153` — symbol 'Br05BackgroundRemover' (class) defined but never referenced outside its module → *verify usage; delete if dead code*
- 🟡 **SYM1** `backend\providers\legacy\br_05.py:105` — symbol 'Br05CleanEdgeRefiner' (class) defined but never referenced outside its module → *verify usage; delete if dead code*
- 🟡 **SYM1** `backend\providers\legacy\br_05.py:33` — symbol 'Br05ColoredFormatter' (class) defined but never referenced outside its module → *verify usage; delete if dead code*
- 🟡 **SYM1** `backend\providers\legacy\br_05.py:56` — symbol 'Br05Deps' (class) defined but never referenced outside its module → *verify usage; delete if dead code*
- 🟡 **SYM1** `backend\providers\legacy\br_06.py:281` — symbol 'Br06BackgroundRemover' (class) defined but never referenced outside its module → *verify usage; delete if dead code*
- 🟡 **SYM1** `backend\providers\legacy\br_06.py:33` — symbol 'Br06ColoredFormatter' (class) defined but never referenced outside its module → *verify usage; delete if dead code*
- 🟡 **SYM1** `backend\providers\legacy\br_06.py:53` — symbol 'Br06Deps' (class) defined but never referenced outside its module → *verify usage; delete if dead code*
- 🟡 **SYM1** `backend\providers\legacy\br_06.py:243` — symbol 'Br06EdgeRefiner' (class) defined but never referenced outside its module → *verify usage; delete if dead code*
- 🟡 **SYM1** `backend\providers\legacy\br_06.py:110` — symbol 'Br06HandRemover' (class) defined but never referenced outside its module → *verify usage; delete if dead code*
- 🟡 **SYM1** `backend\providers\legacy\br_06.py:153` — symbol 'Br06HoleFiller' (class) defined but never referenced outside its module → *verify usage; delete if dead code*
- 🟡 **SYM1** `backend\providers\legacy\br_06.py:219` — symbol 'Br06HumanPreserver' (class) defined but never referenced outside its module → *verify usage; delete if dead code*
- 🟡 **SYM1** `backend\providers\legacy\br_06.py:98` — symbol 'Br06SceneAnalyzer' (class) defined but never referenced outside its module → *verify usage; delete if dead code*
- 🟡 **SYM1** `backend\providers\legacy\br_06.py:194` — symbol 'Br06ThinPartHandler' (class) defined but never referenced outside its module → *verify usage; delete if dead code*
- 🟡 **SYM1** `backend\providers\legacy\br_08.py:101` — symbol 'Br08ColorSpaceUtils' (class) defined but never referenced outside its module → *verify usage; delete if dead code*
- 🟡 **SYM1** `backend\providers\legacy\br_08.py:463` — symbol 'Br08Exporter' (class) defined but never referenced outside its module → *verify usage; delete if dead code*
- 🟡 **SYM1** `backend\providers\legacy\br_08.py:384` — symbol 'Br08HoleFiller' (class) defined but never referenced outside its module → *verify usage; delete if dead code*
- 🟡 **SYM1** `backend\providers\legacy\br_08.py:128` — symbol 'Br08ImageLoader' (class) defined but never referenced outside its module → *verify usage; delete if dead code*
- 🟡 **SYM1** `backend\providers\legacy\br_08.py:32` — symbol 'Br08MemoryManager' (class) defined but never referenced outside its module → *verify usage; delete if dead code*
- 🟡 **SYM1** `backend\providers\legacy\br_08.py:191` — symbol 'Br08ModelSelector' (class) defined but never referenced outside its module → *verify usage; delete if dead code*
- 🟡 **SYM1** `backend\providers\legacy\br_08.py:215` — symbol 'Br08MultiModelSegmenter' (class) defined but never referenced outside its module → *verify usage; delete if dead code*
- 🟡 **SYM1** `backend\providers\legacy\br_08.py:74` — symbol 'Br08ProcessingConfig' (class) defined but never referenced outside its module → *verify usage; delete if dead code*
- 🟡 **SYM1** `backend\providers\legacy\br_08.py:146` — symbol 'Br08QualityAnalyzer' (class) defined but never referenced outside its module → *verify usage; delete if dead code*
- 🟡 **SYM1** `backend\providers\legacy\br_08.py:157` — symbol 'Br08SubjectDetector' (class) defined but never referenced outside its module → *verify usage; delete if dead code*
- 🟡 **SYM1** `backend\providers\legacy\br_08.py:408` — symbol 'Br08WoodBackgroundRemover' (class) defined but never referenced outside its module → *verify usage; delete if dead code*
- 🟡 **SYM1** `backend\providers\legacy\br_08.py:493` — symbol 'Br08ZoziBackgroundRemover' (class) defined but never referenced outside its module → *verify usage; delete if dead code*
- 🟡 **SYM1** `backend\providers\legacy\br_11.py:60` — symbol 'Br11AISegmenter' (class) defined but never referenced outside its module → *verify usage; delete if dead code*
- 🟡 **SYM1** `backend\providers\legacy\br_11.py:198` — symbol 'Br11ArtifactIsolator' (class) defined but never referenced outside its module → *verify usage; delete if dead code*
- 🟡 **SYM1** `backend\providers\legacy\br_11.py:132` — symbol 'Br11EdgeShaver' (class) defined but never referenced outside its module → *verify usage; delete if dead code*
- 🟡 **SYM1** `backend\providers\legacy\br_11.py:218` — symbol 'Br11Exporter' (class) defined but never referenced outside its module → *verify usage; delete if dead code*
- 🟡 **SYM1** `backend\providers\legacy\br_11.py:152` — symbol 'Br11GlobalBackgroundBleeder' (class) defined but never referenced outside its module → *verify usage; delete if dead code*
- 🟡 **SYM1** `backend\providers\legacy\br_11.py:54` — symbol 'Br11MemoryManager' (class) defined but never referenced outside its module → *verify usage; delete if dead code*
- 🟡 **SYM1** `backend\providers\legacy\br_11.py:47` — symbol 'Br11ProcessingConfig' (class) defined but never referenced outside its module → *verify usage; delete if dead code*
- 🟡 **SYM1** `backend\providers\legacy\br_12.py:54` — symbol 'Br12AISegmenter' (class) defined but never referenced outside its module → *verify usage; delete if dead code*
- 🟡 **SYM1** `backend\providers\legacy\br_12.py:196` — symbol 'Br12BottomTextEraser' (class) defined but never referenced outside its module → *verify usage; delete if dead code*
- 🟡 **SYM1** `backend\providers\legacy\br_12.py:238` — symbol 'Br12EdgeShaver' (class) defined but never referenced outside its module → *verify usage; delete if dead code*
- 🟡 **SYM1** `backend\providers\legacy\br_12.py:277` — symbol 'Br12Exporter' (class) defined but never referenced outside its module → *verify usage; delete if dead code*
- 🟡 **SYM1** `backend\providers\legacy\br_12.py:140` — symbol 'Br12FloatingArtifactRemover' (class) defined but never referenced outside its module → *verify usage; delete if dead code*
- 🟡 **SYM1** `backend\providers\legacy\br_12.py:248` — symbol 'Br12GlobalBackgroundBleeder' (class) defined but never referenced outside its module → *verify usage; delete if dead code*
- 🟡 **SYM1** `backend\providers\legacy\br_12.py:48` — symbol 'Br12MemoryManager' (class) defined but never referenced outside its module → *verify usage; delete if dead code*
- 🟡 **SYM1** `backend\providers\legacy\br_12.py:41` — symbol 'Br12ProcessingConfig' (class) defined but never referenced outside its module → *verify usage; delete if dead code*
- 🟡 **SYM1** `backend\providers\legacy\br_13.py:53` — symbol 'Br13AISegmenter' (class) defined but never referenced outside its module → *verify usage; delete if dead code*
- 🟡 **SYM1** `backend\providers\legacy\br_13.py:216` — symbol 'Br13BottomTextEraser' (class) defined but never referenced outside its module → *verify usage; delete if dead code*
- 🟡 **SYM1** `backend\providers\legacy\br_13.py:138` — symbol 'Br13EdgeShaver' (class) defined but never referenced outside its module → *verify usage; delete if dead code*
- 🟡 **SYM1** `backend\providers\legacy\br_13.py:241` — symbol 'Br13Exporter' (class) defined but never referenced outside its module → *verify usage; delete if dead code*
- 🟡 **SYM1** `backend\providers\legacy\br_13.py:176` — symbol 'Br13FloatingArtifactRemover' (class) defined but never referenced outside its module → *verify usage; delete if dead code*
- 🟡 **SYM1** `backend\providers\legacy\br_13.py:148` — symbol 'Br13GlobalBackgroundBleeder' (class) defined but never referenced outside its module → *verify usage; delete if dead code*
- 🟡 **SYM1** `backend\providers\legacy\br_13.py:47` — symbol 'Br13MemoryManager' (class) defined but never referenced outside its module → *verify usage; delete if dead code*
- 🟡 **SYM1** `backend\providers\legacy\br_13.py:40` — symbol 'Br13ProcessingConfig' (class) defined but never referenced outside its module → *verify usage; delete if dead code*
- 🟡 **CA2** `backend\providers\bg_remover.py` — file contains signals for 2 domains: hr(2), media(2) — split candidate → *split this file into domain-specific modules; each file should serve one domain*
...
- 🔴 **LC1** `backend\routers\admin_commerce_routes.py:521` — layer contract violation: 'db\.query' found in routers/ → *routers must not perform this operation; move it to the appropriate layer*
- 🔴 **LC1** `backend\routers\api_comms_command.py:128` — layer contract violation: 'db\.query' found in routers/ → *routers must not perform this operation; move it to the appropriate layer*
- 🔴 **LC1** `backend\routers\api_geography_communications.py:16` — layer contract violation: 'db\.query' found in routers/ → *routers must not perform this operation; move it to the appropriate layer*
- 🔴 **LC1** `backend\routers\effective_permissions.py:16` — layer contract violation: 'db\.query' found in routers/ → *routers must not perform this operation; move it to the appropriate layer*
...
- 🟡 **RN1** `backend\routers\treasury_api.py` — flat router filename 'treasury_api.py' is not comprehensive; missing operation → *rename to {surface}_{domain}_{operation}.py, e.g. admin_orders_management.py, supplier_orders_fulfillment.py, customer_orders_tracking.py*
- 🟡 **SYM1** `backend\routers\api_ai_generation.py:15` — symbol 'AIImageAnalysisResponse' (class) defined but never referenced outside its module → *verify usage; delete if dead code*
- 🟡 **SYM1** `backend\routers\api_ai_routes_2.py:27` — symbol 'AIResearchRequest' (class) defined but never referenced outside its module → *verify usage; delete if dead code*
- 🟡 **SYM1** `backend\routers\api_ai_routes_2.py:36` — symbol 'AIResearchResponse' (class) defined but never referenced outside its module → *verify usage; delete if dead code*
- 🟡 **SYM1** `backend\routers\api_finance_integration.py:172` — symbol 'APBillCreate' (class) defined but never referenced outside its module → *verify usage; delete if dead code*
- 🟡 **SYM1** `backend\routers\api_finance_routes.py:529` — symbol 'APPayableBody' (class) defined but never referenced outside its module → *verify usage; delete if dead code*
- 🟡 **SYM1** `backend\routers\api_finance_integration.py:184` — symbol 'APPayment' (class) defined but never referenced outside its module → *verify usage; delete if dead code*
- 🟡 **SYM1** `backend\routers\api_finance_routes.py:549` — symbol 'APPaymentBody' (class) defined but never referenced outside its module → *verify usage; delete if dead code*
- 🟡 **SYM1** `backend\routers\api_finance_routes.py:458` — symbol 'ARInvoiceBody' (class) defined but never referenced outside its module → *verify usage; delete if dead code*
- 🟡 **SYM1** `backend\routers\api_finance_integration.py:113` — symbol 'ARInvoiceCreate' (class) defined but never referenced outside its module → *verify usage; delete if dead code*
- 🟡 **SYM1** `backend\routers\api_finance_routes.py:478` — symbol 'ARPaymentBody' (class) defined but never referenced outside its module → *verify usage; delete if dead code*
- 🟡 **SYM1** `backend\routers\api_finance_integration.py:125` — symbol 'ARReceipt' (class) defined but never referenced outside its module → *verify usage; delete if dead code*
- 🟡 **SYM1** `backend\routers\api_finance_automation.py:70` — symbol 'AccountCreate' (class) defined but never referenced outside its module → *verify usage; delete if dead code*
- 🟡 **SYM1** `backend\routers\api_finance_integration.py:65` — symbol 'AccountUpdate' (class) defined but never referenced outside its module → *verify usage; delete if dead code*
- 🟡 **SYM1** `backend\routers\api_finance_automation.py:290` — symbol 'AccrualCreate' (class) defined but never referenced outside its module → *verify usage; delete if dead code*
- 🟡 **SYM1** `backend\routers\api_treasury_routes.py:43` — symbol 'ActionRequest' (class) defined but never referenced outside its module → *verify usage; delete if dead code*
- 🟡 **SYM1** `backend\routers\admin_core_console.py:995` — symbol 'AdminDisputeBulkActionBody' (class) defined but never referenced outside its module → *verify usage; delete if dead code*
- 🟡 **SYM1** `backend\routers\admin_treasury_routes_3.py:41` — symbol 'AdminTreasuryActionRequest' (class) defined but never referenced outside its module → *verify usage; delete if dead code*
- 🟡 **SYM1** `backend\routers\api_comms_command.py:103` — symbol 'AlertResponse' (class) defined but never referenced outside its module → *verify usage; delete if dead code*
- 🟡 **SYM1** `backend\routers\api_geography_registry.py:682` — symbol 'ArchivePayload' (class) defined but never referenced outside its module → *verify usage; delete if dead code*
- 🟡 **SYM1** `backend\routers\api_geography_registry.py:581` — symbol 'AssignStaffBody' (class) defined but never referenced outside its module → *verify usage; delete if dead code*
- 🟡 **SYM1** `backend\routers\api_core_ingestion.py:90` — symbol 'AutoAllocateInput' (class) defined but never referenced outside its module → *verify usage; delete if dead code*
- 🟡 **SYM1** `backend\routers\api_geography_registry.py:499` — symbol 'AutoPopulateBody' (class) defined but never referenced outside its module → *verify usage; delete if dead code*
- 🟡 **SYM1** `backend\routers\api_finance_routes_2.py:44` — symbol 'BadgeTierBody' (class) defined but never referenced outside its module → *verify usage; delete if dead code*
- 🟡 **SYM1** `backend\routers\api_finance_integration.py:333` — symbol 'BudgetSet' (class) defined but never referenced outside its module → *verify usage; delete if dead code*
- 🟡 **SYM1** `backend\routers\admin_core_console.py:194` — symbol 'BulkDeleteUsersBody' (class) defined but never referenced outside its module → *verify usage; delete if dead code*
- 🟡 **SYM1** `backend\routers\api_geography_registry.py:686` — symbol 'BulkIdsPayload' (class) defined but never referenced outside its module → *verify usage; delete if dead code*
- 🟡 **SYM1** `backend\routers\admin_core_console.py:397` — symbol 'BulkOrderDeleteBody' (class) defined but never referenced outside its module → *verify usage; delete if dead code*
...
- 🟡 **MV1** `backend/services/` — 2 'comms' domain file(s) at backend/services/ root should be moved to backend/services/comms/ → *mkdir -p backend/services/comms; move: backend\services\cash_management_service.py, backend\services\video_conferencing.py (detected from communication, email)*
...
- 🟡 **MV1** `backend/services/` — 1 'finance' domain file(s) at backend/services/ root should be moved to backend/services/finance/ → *mkdir -p backend/services/finance; move: backend\services\commission_write_service.py (detected from commission)*
...
- 🟡 **DOM2** `backend/services/` — 2 file(s) are in the wrong backend/services/ sub-folder; detected domain: 'treasury' → *mkdir -p backend/services/treasury; move: backend\services\finance\payouts_write_service.py, backend\services\supplier\supplier_payouts_service.py (detected from payouts)*
...
- 🟡 **SYM1** `backend\services\supplier\onboarding_pipeline.py:97` — symbol 'AIVerifier' (class) defined but never referenced outside its module → *verify usage; delete if dead code*
- 🟡 **SYM1** `backend\services\hr\offboarding.py:22` — symbol 'AssetRecoveryTask' (class) defined but never referenced outside its module → *verify usage; delete if dead code*
- 🟡 **SYM1** `backend\services\audit\audit_service.py:13` — symbol 'AuditService' (class) defined but never referenced outside its module → *verify usage; delete if dead code*
- 🟡 **SYM1** `backend\services\audit\audit_trail_service.py:119` — symbol 'AuditTrailDataResidencyService' (class) defined but never referenced outside its module → *verify usage; delete if dead code*
...
- 🟡 **SYM1** `backend\services\media\free_image_tools.py:730` — symbol 'BatchResult' (class) defined but never referenced outside its module → *verify usage; delete if dead code*
- 🟡 **SYM1** `backend\services\hr\succession_service.py:20` — symbol 'BenchStrengthScore' (class) defined but never referenced outside its module → *verify usage; delete if dead code*
- 🟡 **SYM1** `backend\services\security\triple_auth.py:79` — symbol 'BiometricValidator' (class) defined but never referenced outside its module → *verify usage; delete if dead code*
...
- 🟡 **CA2** `backend\services\cash_management_service.py` — file contains signals for 7 domains: finance(22), treasury(16), logistics(13), supplier(10), core(3), audit(2), orders(2) — split candidate → *split this file into domain-specific modules; each file should serve one domain*
...
- 🟡 **CA2** `backend\services\treasury\payout_admin_service.py` — file contains signals for 5 domains: treasury(14), orders(3), finance(2), geography(2), supplier(2) — split candidate → *split this file into domain-specific modules; each file should serve one domain*
...
- 🟡 **SYM1** `backend\utils\schema_audit.py:129` — symbol 'AlembicInfo' (class) defined but never referenced outside its module → *verify usage; delete if dead code*
- 🟡 **SYM1** `backend\utils\audit_log.py:23` — symbol 'AuditLogAction' (class) defined but never referenced outside its module → *verify usage; delete if dead code*
- 🟡 **SYM1** `backend\utils\schema_audit.py:166` — symbol 'AuditReport' (class) defined but never referenced outside its module → *verify usage; delete if dead code*
- 🟡 **SYM1** `backend\utils\backup.py:30` — symbol 'BackupManager' (class) defined but never referenced outside its module → *verify usage; delete if dead code*
...
| 2 | `backend\services\auto_payout_scheduler.py` | `backend/services/treasury/auto_payout_scheduler.py` | domain | 52% |
| 3 | `backend\services\bank_transaction_service.py` | `backend/services/treasury/bank_transaction_service.py` | domain | 86% |
| 4 | `backend\services\cash_management_service.py` | `backend/services/comms/cash_management_service.py` | domain | 53% |
| 5 | `backend\services\cash_management_write_service.py` | `backend/services/treasury/cash_management_write_service.py` | domain | 86% |
| 6 | `backend\services\catalog\wishlist_read_service.py` | `backend/services/commerce/wishlist_read_service.py` | domain | 86% |
| 7 | `backend\services\commerce\cart_write_service.py` | `backend/services/orders/cart_write_service.py` | domain | 86% |
| 8 | `backend\services\commerce\cross_border_tracker.py` | `backend/services/geography/cross_border_tracker.py` | domain | 96% |
| 9 | `backend\services\commission_write_service.py` | `backend/services/finance/commission_write_service.py` | domain | 86% |
| 10 | `backend\services\customer\customer_router_service.py` | `backend/services/commerce/customer_router_service.py` | domain | 94% |
| 11 | `backend\services\finance\automation_read_service.py` | `backend/services/ai/automation_read_service.py` | domain | 86% |
| 12 | `backend\services\finance\payouts_write_service.py` | `backend/services/treasury/payouts_write_service.py` | domain | 86% |
| 13 | `backend\services\finance\payroll_read_service.py` | `backend/services/hr/payroll_read_service.py` | domain | 86% |
| 14 | `backend\services\orders\cart_shipping_service.py` | `backend/services/logistics/cart_shipping_service.py` | domain | 74% |
| 15 | `backend\services\orders\order_payment_functions.py` | `backend/services/finance/order_payment_functions.py` | domain | 67% |
| 16 | `backend\services\supplier\supplier_finance_service.py` | `backend/services/finance/supplier_finance_service.py` | domain | 86% |
| 17 | `backend\services\supplier\supplier_payouts_service.py` | `backend/services/treasury/supplier_payouts_service.py` | domain | 86% |
| 18 | `backend\services\supplier\supplier_profile_service.py` | `backend/services/customer/supplier_profile_service.py` | domain | 86% |
| 19 | `backend\services\video_conferencing.py` | `backend/services/comms/video_conferencing.py` | domain | 74% |
| 20 | `backend\models\_exports.py` | `backend/models/geography/_exports.py` | domain | 59% |
| 21 | `backend\controllers\admin_controller.py` | `backend/controllers/security/admin_controller.py` | domain | 53% |
| 22 | `backend\controllers\banner_controller.py` | `backend/controllers/core/banner_controller.py` | domain | 93% |
| 23 | `backend\controllers\cart_controller.py` | `backend/controllers/orders/cart_controller.py` | domain | 93% |
| 24 | `backend\controllers\chatbot_controller.py` | `backend/controllers/ai/chatbot_controller.py` | domain | 91% |
| 25 | `backend\controllers\comm_controller.py` | `backend/controllers/comms/comm_controller.py` | domain | 93% |
| 26 | `backend\controllers\coupons_controller.py` | `backend/controllers/commerce/coupons_controller.py` | domain | 93% |
| 27 | `backend\controllers\disputes_controller.py` | `backend/controllers/orders/disputes_controller.py` | domain | 93% |
| 28 | `backend\controllers\flash_sale_controller.py` | `backend/controllers/catalog/flash_sale_controller.py` | domain | 62% |
| 29 | `backend\controllers\iam_controller.py` | `backend/controllers/security/iam_controller.py` | domain | 93% |
| 30 | `backend\controllers\logistics_partner_controller.py` | `backend/controllers/logistics/logistics_partner_controller.py` | domain | 91% |
| 31 | `backend\controllers\product_verification_controller.py` | `backend/controllers/catalog/product_verification_controller.py` | domain | 96% |
| 32 | `backend\controllers\products_controller.py` | `backend/controllers/catalog/products_controller.py` | domain | 96% |
| 33 | `backend\controllers\promotion_controller.py` | `backend/controllers/commerce/promotion_controller.py` | domain | 93% |
| 34 | `backend\controllers\returns_controller.py` | `backend/controllers/orders/returns_controller.py` | domain | 93% |
| 35 | `backend\controllers\risk_controller.py` | `backend/controllers/security/risk_controller.py` | domain | 93% |
| 36 | `backend\controllers\search_controller.py` | `backend/controllers/catalog/search_controller.py` | domain | 93% |
| 37 | `backend\controllers\sub_ledger_controller.py` | `backend/controllers/finance/sub_ledger_controller.py` | domain | 93% |
| 38 | `backend\providers\bg_remover.py` | `backend/providers/media/bg_remover.py` | domain | 89% |
| 39 | `backend\providers\catalog\text.py` | `backend/providers/ai/text.py` | domain | 91% |
| 40 | `backend\providers\hr\config.py` | `backend/providers/configuration/config.py` | domain | 91% |
| 41 | `backend\providers\image.py` | `backend/providers/media/image.py` | domain | 93% |
| 42 | `backend\providers\ocr.py` | `backend/providers/ai/ocr.py` | domain | 91% |
| 43 | `backend\providers\vision.py` | `backend/providers/ai/vision.py` | domain | 91% |
| 44 | `backend\providers\voice_to_text.py` | `backend/providers/ai/voice_to_text.py` | domain | 95% |
| 45 | `backend\_fix_syntax.py` | `backend/utils/_fix_syntax.py` | backend-root | 60% |
| 46 | `backend\check_app.py` | `backend/utils/check_app.py` | backend-root | 60% |
| 47 | `backend\check_auth_head.py` | `backend/services/security/check_auth_head.py` | backend-root | 60% |
| 48 | `backend\events.py` | `backend/utils/events.py` | backend-root | 60% |
| 49 | `backend/controllers/communication/` | `backend/controllers/comms/` | rename-folder | 100% |
| 50 | `backend/models/communication/` | `backend/models/comms/` | rename-folder | 100% |
| 51 | `backend/models/country/` | `backend/models/geography/` | rename-folder | 100% |
| 52 | `backend/providers/country/` | `backend/providers/geography/` | rename-folder | 100% |
| 53 | `backend/services/communication/` | `backend/services/comms/` | rename-folder | 100% |
| 54 | `backend/services/country/` | `backend/services/geography/` | rename-folder | 100% |
| 55 | `backend/services/payments/` | `backend/services/finance/` | rename-folder | 100% |
...
- **Architecture Debt Score:** 21474
- **Modules scanned:** 887
- **Dependency edges:** 2456
- **Classes found:** 1256
- **Layer counts:** `_fix_syntax=1`, `_triage=1`, `alembic=32`, `check_app=1`, `check_auth_head=1`, `controllers=70`, `data=78`, `db=10`, `dependencies=3`, `events=1`, `lifespan=1`, `main=1`, `middleware=22`, `models=51`, `providers=45`, `routers=152`, `run_server=1`, `scripts=3`, `services=269`, `tests=74`, `utils=70`
...
| `models._exports` | 33 |
| `tests.conftest` | 22 |
| `controllers.supplier.supplier_controller` | 21 |
| `routers.admin_core_console` | 19 |
| `controllers.security.auth_controller` | 17 |
| `controllers.orders.orders_controller` | 17 |
| `providers` | 15 |
...
- **Features discovered:** 681
- **Frontend features:** 9
- **Backend top-level dirs:** 19
- **Cross-domain edges:** 74
