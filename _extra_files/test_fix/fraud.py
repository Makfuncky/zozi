...
class FraudEvent(Base):
    ...
    __table_args__ = (
                         ...
                         Index("ix_fraud_events_details", "details", postgresql_using="gin"),
                         Index("ix_fraud_events_country_created", "country_code", "created_at"),
                         {"schema": "security"},
                     )
...
class FraudRule(Base):
    ...
    __table_args__ = (
                         Index("ix_fraud_rule_active", "is_active"),
                         Index("ix_fraud_rules_country_created", "country_code", "created_at"),
                         {"schema": "security"},
                     )
...
class ManualReviewQueue(Base):
    ...
    assigned_to = Column(Integer, ForeignKey("users.id", ondelete="RESTRICT"), nullable=True, index=True)
...
class IPReputation(Base):
    ...
    __table_args__ = (
                         Index("ix_ip_reputation_ip", "ip_address"),
                         Index("ix_ip_reputations_country_created", "country_code", "created_at"),
                         {"schema": "security"},
                     )
...
class SupplierFraudIndicator(Base):
    ...
    __table_args__ = (
                         Index("ix_supplier_fraud_indicators_country_created", "country_code", "created_at"),
                         {"schema": "supplier"},
                     )
    id = Column(Integer, primary_key=True, index=True)
...
class LogisticsFraudIndicator(Base):
    ...
    __table_args__ = (
                         Index("ix_logistics_fraud_indicators_country_created", "country_code", "created_at"),
                         {"schema": "logistics"},
                     )
    id = Column(Integer, primary_key=True, index=True)
    partner_id = Column(Integer, ForeignKey("logistics_partners.id", ondelete="RESTRICT"), nullable=False, index=True)
    indicator_type = Column(String(50), nullable=False)
    value = Column(String, nullable=True)
    is_active = Column(Boolean, default=True)
    created_at = Column(DateTime, default=_utcnow)
...
class FraudAlert(Base):
    ...
    __table_args__ = (
                         Index("ix_fraud_alerts_country_created", "country_code", "created_at"),
                         {"schema": "security"},
                     )
    id = Column(Integer, primary_key=True, index=True)
...
class FraudScoringLog(Base):
    ...
    __table_args__ = (
                         ...
                         Index("ix_fraud_scoring_logs_triggered_rules", "triggered_rules", postgresql_using="gin"),
                         Index("ix_fraud_scoring_logs_metadata_json", "metadata_json", postgresql_using="gin"),
                         Index("ix_fraud_scoring_logs_country_created", "country_code", "created_at"),
                         {"schema": "security"},
                     )
...
class FraudCase(Base):
    ...
    __table_args__ = (
                         ...
                         Index("ix_fraud_cases_country_created", "country_code", "created_at"),
                         {"schema": "security"},
                     )
...
class FraudCaseAssignment(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    case_id = Column(Integer, ForeignKey("fraud_cases.id", ondelete="RESTRICT"), nullable=False, index=True)
    assigned_to = Column(Integer, ForeignKey("users.id", ondelete="RESTRICT"), nullable=False, index=True)
    assigned_by = Column(Integer, ForeignKey("users.id", ondelete="RESTRICT"), nullable=True, index=True)
...
class DLPViolation(Base):
    ...
    sender_id = Column(Integer, ForeignKey("users.id", ondelete="RESTRICT"), nullable=True, index=True)
...
class MeetingTranscript(Base):
    ...
    __table_args__ = (Index("ix_transcript_room", "room_id"), Index("ix_meeting_transcripts_segments", "segments", postgresql_using="gin"), Index("ix_meeting_transcripts_action_items", "action_items", postgresql_using="gin"), {"schema": "security"})
...
class MeetingActionItem(Base):
    ...
    __table_args__ = (Index("ix_action_item_meeting", "meeting_id"), Index("ix_action_item_status", "status"), Index("ix_meeting_action_items_metadata_json", "metadata_json", postgresql_using="gin"), {"schema": "security"})
    
    id = Column(Integer, primary_key=True, index=True)
    meeting_id = Column(Integer, ForeignKey("meeting_transcripts.id", ondelete="RESTRICT"), nullable=False)
    entity_type = Column(String(50), nullable=True)
    entity_id = Column(Integer, nullable=True)
...
class MeetingRecording(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    room_id = Column(String(64), nullable=False)
    started_by = Column(Integer, ForeignKey("users.id", ondelete="RESTRICT"), nullable=False, index=True)
