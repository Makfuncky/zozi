...
class MapService:
    ...
    def get_cities_for_map(
        ...
        cities = query.limit(SAFE_QUERY_LIMIT).all()
        return [
            {
                "id": c.id,
                "name": c.name,
    ...
    def get_warehouses_for_map(
        ...
        warehouses = query.limit(SAFE_QUERY_LIMIT).all()
        return [
            {
                "id": w.id,
                "name": w.name,
    ...
    def get_delivery_zones(
        ...
        from data.models_country_enhancements import OmanDeliveryZone

        query = self.db.query(OmanDeliveryZone).filter(
            OmanDeliveryZone.is_active == True
        )

        zones = query.limit(SAFE_QUERY_LIMIT).all()
        return [
            {
    ...
    def generate_map_markers(
        ...
        if include_parcels:
            ...
            for pt in parcel_trackers:
                if pt.parcel_id not in latest_locations or pt.timestamp_at > latest_locations[pt.parcel_id].timestamp_at:
