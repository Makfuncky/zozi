...
from services.geography.legal_contract_service import generate_all_legal_documents
...
def first_currency(data: dict) -> tuple[list[str], str | None, str | None]:
...
@time_it
def normalize_rest_country(data: dict) -> dict:
    currencies, currency_symbol, currency_name = first_currency(data)
    ...
    async def _fetch():
        ...
        async with httpx.AsyncClient(timeout=DEFAULT_TIMEOUT) as client:
            ...
            if isinstance(data, list):
                data = data[0]
            return normalize_rest_country(data)
    try:
...
async def auto_populate_country(country_code: str) -> dict:
    ...
    rest_data = (rest_result or {}).get("data")
    wb_data = (wb_result or {}).get("data") or {}
    cities = (cities_result or {}).get("data") or []
    holidays = (holidays_result or {}).get("data") or []
    vat_rate = (vat_result.get("data") if isinstance(vat_result, dict) else vat_result) or 0.05
    
    if (rest_result or {}).get("error"):
        warnings.append((rest_result or {}).get("error"))
        degraded = True
    if (wb_result or {}).get("error"):
        warnings.append((wb_result or {}).get("error"))
        degraded = True
    if (cities_result or {}).get("error"):
        warnings.append((cities_result or {}).get("error"))
        degraded = True
    if (holidays_result or {}).get("error"):
        warnings.append((holidays_result or {}).get("error"))
        degraded = True
    if isinstance(vat_result, dict) and vat_result.get("error"):
        warnings.append(vat_result.get("error"))
        degraded = True
...
def _normalize_rest_country(data: dict) -> dict:
    ...
    if not isinstance(name_obj, dict):
        name_obj = {}
    currencies = data.get("currencies") or {}
    cur_codes = list(currencies.keys()) if isinstance(currencies, dict) else []
    first_cur = currencies[cur_codes[0]] if cur_codes else {}
    ...
    if isinstance(idd, dict):
        root = idd.get("root") or ""
        suffixes = idd.get("suffixes") or []
        phone = f"{root}{suffixes[0]}" if suffixes else (root or "")
    ...
    return {
        ...
        "currencies": cur_codes,
        "currency_symbol": (first_cur or {}).get("symbol"),
        "currency_name": (first_cur or {}).get("name"),
        "region": data.get("region"),
        "subregion": data.get("subregion"),
        "population": data.get("population"),
        "timezone": timezones[0] if timezones else "UTC",
