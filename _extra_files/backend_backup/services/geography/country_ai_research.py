...
class CountryAIResearchService:
    """Builds an AI-assisted research dossier for a country.

    Aggregates supplied demographics, economy, news and web evidence into a
    structured report and merges generated AI modules. Construct with the
    country name, a base report template, and the pre-fetched data dicts; call
    ``build``/``merge_ai_output`` to produce the final payload. Side effects are
    limited to outbound AI/web research calls.
    """
    ...
    async def enrich(self) -> Dict[str, Any]:
        if not getattr(settings, "country_ai_enabled", True):
            ...
            return self.fallback("AI enrichment disabled by configuration.")

        web_evidence = await self._fetch_web_evidence()
        ai_modules = await self._generate_ai_modules(web_evidence)
        merged = self.merge_ai_output(ai_modules, web_evidence)
        return merged
    ...
    def _build_ai_payload(self, evidence: Dict[str, List[Dict[str, Any]]]) -> Dict[str, Any]:
        ai_input = self.build_ai_input(evidence)
    ...
    def build_ai_input(self, evidence: Dict[str, List[Dict[str, Any]]]) -> Dict[str, Any]:
        ...
        if isinstance(gdp_per_capita, dict):
            try:
                ...
                logger.debug("Failed to parse GDP per capita: %s", exc)
        income_tier = "low"
        if gdp_pc:
            if gdp_pc > 30000:
                income_tier = "high"
        ...
        compact_evidence = self.compact_evidence(evidence)
        return {
    ...
    def compact_evidence(self, evidence: Dict[str, List[Dict[str, Any]]]) -> Dict[str, List[Dict[str, Any]]]:
    ...
    def parse_json_text(self, text: str) -> Optional[Dict[str, Any]]:
        if not text:
            return None
        try:
            return json.loads(text)
        except Exception as exc:
            logger.debug("Primary AI JSON parse failed: %s", exc)
            match = re.search(r"\{.*\}", text, re.DOTALL)
            if match:
                try:
                    return json.loads(match.group(0))
                except Exception as exc:
                    logger.debug("Fallback AI JSON parse failed: %s", exc)
                    return None
        return None
    ...
    def fallback(self, message: str) -> Dict[str, Any]:
