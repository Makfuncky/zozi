...
def run_treasury_sync():
    ...
    from services.country_rls_service import CountryRLSService
    
    with get_db_context() as db:
        countries = db.query(CountryConfig).filter(CountryConfig.is_active == True).all()
        
        for country in countries:
            settings = CountryRLSService.get_payout_settings(country.code)
            logger.info(f"Treasury sync for {country.code}: hold_days={settings.get('settlement_hold_days', 0)}, min_payout={settings.get('minimum_payout_amount', 0)}")
