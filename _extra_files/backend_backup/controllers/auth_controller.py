...
`routers/auth.py` so that all existing `from routers.public_auth_access import get_current_user`
...
from services.auth_write_service import (
    claim_share_points,
...
from utils.cache import cache_delete, cache_get_json, cache_set_json
...
def _normalize_customer_verification_when_gate_disabled(user: User, db: Session) -> None:
    ...
    if is_customer_email_verification_required() or user.email_verified:
        return
    update_user_email_verified(db, user)
...
def _record_device_fingerprint(request: Request | None, user_id: int, db: Session) -> None:
    ...
    try:
        update_user_device_fingerprint(db, user_id, fp, ip, ua)
    except Exception:
        db.rollback()
...
def _resolve_or_create_social_user(email: str, name: str | None, db: Session, profile: dict[str, Any] | None = None) -> User:
    ...
    if existing:
        update_user_email_verification(db, existing, email_verified=True)
        ...
        if updates:
            update_user(db, existing, updates)
        return existing
...
def handle_google_id_token_login(
    ...
    try:
        ...
        logger.error("Google ID credential verification failed: %s", exc)
...
def verify_email_token(token: str, db: Session) -> dict:
    ...
    if cast(datetime, getattr(ev, "expires_at")) < datetime.now(timezone.utc).replace(tzinfo=None):
        mark_email_verification_token_used(db, ev)
    ...
    update_user_email_verification(db, user)
    mark_email_verification_token_used(db, ev)
...
def _ensure_referral_code(user: User, db: Session) -> str:
    return ensure_referral_code(db, user, _generate_unique_referral_code)
...
def claim_share_points(body: ReferralShareRequest, current_user: dict, db: Session) -> dict[str, Any]:
    ...
    updated_user = claim_share_points(db, user, channel, REFERRAL_SHARE_DAILY_BONUS)
    ...
    return {
        ...
        "channel": channel,
        "total_points": _total_referral_points(updated_user),
        "referral_points": int(cast(int | None, getattr(updated_user, "referral_points", 0)) or 0),
        "sharing_points": int(cast(int | None, getattr(updated_user, "sharing_points", 0)) or 0),
    }
...
def update_profile(body: ProfileUpdate, current_user: dict, db: Session) -> User:
    ...
    for field_name in (
        ...
        if field_name in provided_fields:
            updates[field_name] = getattr(body, field_name, None)
...
async def upload_avatar(file: UploadFile, current_user: dict, db: Session) -> dict:
    ...
    if not user:
        raise HTTPException(status_code=404, detail="User not found")
    update_user(db, user, {"profile_image": url})
...
def change_password(body: ChangePasswordRequest, current_user: dict, db: Session) -> dict:
    ...
    update_user(db, user, {"hashed_password": get_password_hash(body.new_password)})
...
def forgot_password(body: ForgotPasswordRequest, db: Session) -> dict:
    ...
    try:
        ...
        logger.error("Failed to send account reset email: %s", exc)
...
def reset_password(body: ResetPasswordRequest, db: Session) -> dict:
    ...
    if cast(datetime, getattr(db_token, "expires_at")) < datetime.now(timezone.utc).replace(tzinfo=None):
        mark_password_reset_token_used(db, db_token)
    ...
    execute_password_reset(db, user, db_token, get_password_hash(body.new_password))
...
def update_user_preferences(body: PreferencesUpdate, current_user: dict, db: Session) -> dict:
    ...
    if body.preferred_currency is not None:
        ...
        updates["preferred_currency"] = body.preferred_currency.upper()
    if body.preferred_country is not None:
        updates["preferred_country"] = body.preferred_country.upper()
    update_user(db, user, updates)
...
def setup_totp(current_user: dict, db: Session) -> dict:
    ...
    update_user(db, user, {"totp_secret": secret})
    return {
        "secret": secret,
...
def enable_totp(current_user: dict, db: Session, code: str) -> dict:
    ...
    update_user_totp(db, user, secret, code)
    update_user(db, user, {"totp_recovery_codes": recovery_codes})
    return {
...
def disable_totp(current_user: dict, db: Session, password: str) -> dict:
    ...
    disable_user_totp(db, user, password, verify_password)
