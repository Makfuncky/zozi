...
async def auto_populate_async(search_term: str) -> dict[str, Any]:
    ...
    from routers.public_country_auto_populate_access import auto_populate_country
...
def _create_draft_version(
    ...
    *,
    ...
    return create_country_config_version(
        db=db,
...
def create_admin_country(payload: dict[str, Any], current_user: dict, db: Session) -> dict[str, Any]:
    ...
    if payload.get("suggested_gateways"):
        country.suggested_gateway_rankings_json = _to_json(payload["suggested_gateway_rankings"])
        ...
        if isinstance(rates, list):
            ...
            for item in rates:
                ...
                if entry is None:
                    ...
                    add_supplier_commission_entry(db, entry)
        ...
        if isinstance(feature_flags, dict):
            ...
            for feature_key, value in feature_flags.items():
                ...
                if flag is None:
                    ...
                    add_country_feature_flag(db, flag)
...
def rollback_country_to_version(code: str, version_id: int, current_user: dict, db: Session) -> dict[str, Any]:
    ...
    _apply_version_payload(row, db)
    rollback_row = create_rollback_version(
        db=db,
        country_code=row.country_code,
        config_type=row.config_type,
...
def create_payment_gateways_draft(code: str, payload: dict[str, Any], current_user: dict, db: Session) -> dict[str, Any]:
    ...
    record_admin_change(db, actor_id=current_user.get("id"), action="create_draft", entity="country_payment_gateways", entity_key=f"{country.code}:v{version.version}", before=None, after=draft_payload)
    commit_and_refresh(db)
...
def create_logistics_providers_draft(code: str, payload: dict[str, Any], current_user: dict, db: Session) -> dict[str, Any]:
    ...
    record_admin_change(db, actor_id=current_user.get("id"), action="create_draft", entity="country_logistics_providers", entity_key=f"{country.code}:v{version.version}", before=None, after=draft_payload)
    commit_and_refresh(db)
...
def create_legal_rules_draft(code: str, payload: dict[str, Any], current_user: dict, db: Session) -> dict[str, Any]:
    ...
    record_admin_change(db, actor_id=current_user.get("id"), action="create_draft", entity="country_legal_rules", entity_key=f"{country.code}:v{version.version}", before=existing, after=draft_payload)
    commit_and_refresh(db)
...
def create_regions_draft(code: str, payload: dict[str, Any], current_user: dict, db: Session) -> dict[str, Any]:
    ...
    record_admin_change(db, actor_id=current_user.get("id"), action="create_draft", entity="country_regions", entity_key=f"{country.code}:v{version.version}", before=None, after=draft_payload)
    commit_and_refresh(db)
...
def create_supplier_requirements_draft(code: str, payload: dict[str, Any], current_user: dict, db: Session) -> dict[str, Any]:
    ...
    record_admin_change(db, actor_id=current_user.get("id"), action="create_draft", entity="country_supplier_requirements", entity_key=f"{country.code}:v{version.version}", before=existing, after=draft_payload)
    commit_and_refresh(db)
...
def create_payout_settings_draft(code: str, payload: dict[str, Any], current_user: dict, db: Session) -> dict[str, Any]:
    ...
    record_admin_change(db, actor_id=current_user.get("id"), action="create_draft", entity="country_payout_settings", entity_key=f"{country.code}:v{version.version}", before=existing, after=draft_payload)
    commit_and_refresh(db)
...
def create_commission_tiers_draft(code: str, payload: dict[str, Any], current_user: dict, db: Session) -> dict[str, Any]:
    ...
    record_admin_change(db, actor_id=current_user.get("id"), action="create_draft", entity="country_commission_tiers", entity_key=f"{country.code}:v{version.version}", before=None, after=draft_payload)
    commit_and_refresh(db)
...
def assign_staff_to_country(country_code: str, user_id: int, role_in_country: str, current_user: dict, db: Session) -> dict:
    ...
    assignment = create_country_staff_assignment(
        db=db,
        user_id=user_id,
...
def unassign_staff_from_country(country_code: str, user_id: int, current_user: dict, db: Session) -> dict:
    ...
    a.is_active = False
    record_admin_change(db, actor_id=current_user.get("id"), action="unassign_staff", entity="country_staff", entity_key=f"{country_code}:{a.user_id}", before={"active": True}, after={"active": False})
    commit_and_refresh(db)
...
def send_country_communication(country_code: str, payload: dict, current_user: dict, db: Session) -> dict:
    ...
    _require_country_access(country_code, current_user)
    _get_country_or_404(country_code, db)
    comm = create_country_communication(
        db=db,
        country_code=country_code.upper(),
...
def mark_communication_read(comm_id: int, current_user: dict, db: Session) -> dict:
    ...
    comm.read_at = utcnow()
    mark_communication_read_at(db, comm)
...
def update_country_cities_bulk(code: str, payload: dict, current_user: dict, db):
    ...
    db.query(CountryCity).filter(CountryCity.country_code == cc).delete(synchronize_session=False)
