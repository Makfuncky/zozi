...
from services.promotion_engine_service import (
    ensure_promotion_tables as _ensure_promotion_tables,
)
from services.promotion_engine_service import (
    get_or_create_config as _get_or_create_config,
)
from services.promotions_write_service import (
    create_promotion_ledger_entry as _create_promotion_ledger_entry,
    create_promotion_order_tier as _create_promotion_order_tier,
    delete_promotion_order_tier as _delete_promotion_order_tier,
    update_promotion_engine_config as _update_promotion_engine_config,
    update_promotion_order_tier as _update_promotion_order_tier,
)
...
def update_promotion_config(payload: dict[str, Any], acting_user: dict[str, Any], db: Session) -> dict[str, Any]:
    ...
    updates = {"updated_by": acting_user.get("id")}
    ...
    row = _update_promotion_engine_config(db, row, updates)
...
def create_promotion_tier(payload: dict[str, Any], acting_user: dict[str, Any], db: Session) -> dict[str, Any]:
    ...
    row = _create_promotion_order_tier(
        db=db,
...
def update_promotion_tier(tier_id: int, payload: dict[str, Any], acting_user: dict[str, Any], db: Session) -> dict[str, Any]:
    ...
    if "tier_name" in payload:
        ...
        updates["tier_name"] = str(payload["tier_name"]).strip()
    if "discount_type" in payload:
        setattr(row, "discount_type", str(payload["discount_type"]).strip().lower())
        updates["discount_type"] = str(payload["discount_type"]).strip().lower()
    if "min_order" in payload:
        setattr(row, "min_order_amount", to_decimal(payload["min_order"]))
        updates["min_order_amount"] = to_decimal(payload["min_order"])
    if "max_order" in payload:
        ...
        updates["max_order_amount"] = to_decimal(payload["max_order"]) if payload["max_order"] is not None else None
    ...
    if "discount_value" in payload:
        ...
        updates["discount_value"] = to_decimal(payload["discount_value"])
    if "stacking_allowed" in payload:
        setattr(row, "stacking_allowed", bool(payload["stacking_allowed"]))
        updates["stacking_allowed"] = bool(payload["stacking_allowed"])
    ...
    row = _update_promotion_order_tier(db, row, updates)
...
def delete_promotion_tier(tier_id: int, acting_user: dict[str, Any], db: Session) -> dict[str, Any]:
    ...
    tier_name = str(getattr(row, "tier_name", "") or "")
    try:
        _delete_promotion_order_tier(db, row)
...
def record_order_tier_ledger(
    *,
    ...
    _ensure_promotion_tables(db)
    _create_promotion_ledger_entry(
        db=db,
        order_id=order_id,
        user_id=user_id,
        tier=tier,
