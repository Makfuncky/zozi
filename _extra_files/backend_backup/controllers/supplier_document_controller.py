...
from services.suppliers_write_service import (
    add_notification as create_notification,
    create_supplier_document as service_create_supplier_document,
    delete_supplier_document as service_delete_supplier_document,
    update_supplier_document as service_update_supplier_document,
    update_supplier_profile as service_update_supplier_profile,
)
...
def submit_document(data: dict, current_user: dict, db: Session) -> dict:
    ...
    doc = service_create_supplier_document(
        db,
...
def delete_my_document(doc_id: int, current_user: dict, db: Session) -> dict:
    ...
    service_delete_supplier_document(db, doc)
...
def admin_review_document(
    ...
    service_update_supplier_document(db, doc, doc_updates)
    ...
    if new_status == "approved":
        ...
        if profile:
            ...
            profile_updates = {
                "verified_documents": json.dumps(existing),
                "updated_at": _utcnow(),
            }
            if profile.verification_status in (None, "pending", "documents_submitted"):
                profile_updates["verification_status"] = "verified"
            service_update_supplier_profile(db, profile, profile_updates)
