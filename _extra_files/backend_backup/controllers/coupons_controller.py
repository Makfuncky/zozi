...
from services.coupons_write_service import (
    create_coupon as create_coupon_record,
    delete_coupon as delete_coupon_record,
...
def create_coupon(
    ...
    coupon = create_coupon_record(db, **coupon_data)
    audit_log(
        db,
...
def _delete_coupon_record(coupon: Coupon, current_user: dict, db: Session) -> dict:
    usage_count = get_coupon_usage_count(db, coupon.id)
    if usage_count:
        raise HTTPException(
            status_code=409,
    ...
    try:
        delete_coupon_record(db, coupon)
