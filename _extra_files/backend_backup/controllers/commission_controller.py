...
def set_supplier_commission(
    ...
    new_agreement = _create_commission_agreement(
        db=db,
        supplier_id=supplier_id,
...
def delete_supplier_commission_override(
    ...
    for agreement in active_agreements:
        _delete_commission_agreement(db, agreement)
...
def set_product_commission_override(
    ...
    if existing:
        ...
        _update_product_commission_override(db, existing, updates)
    else:
        override = _create_product_commission_override(
            db=db,
            product_id=product_id,
...
def delete_product_commission_override(
    ...
    _delete_product_commission_override(db, override)
...
def list_product_commission_overrides(
    ...
    *,
    ...
    q = (
        ...
        .order_by(ProductCommissionOverride.created_at.desc(), ProductCommissionOverride.id.desc())
    )
...
def _serialize_override(o: ProductCommissionOverride | None) -> Optional[dict]:
    ...
    return {
        ...
        "rate": float(getattr(o, "rate_percent")),
...
def update_global_config(payload: dict, acting_user: dict, db: Session) -> dict:
    ...
    updates["updated_by"] = acting_user["id"]
    updates["updated_at"] = datetime.now(timezone.utc)
    
    _update_commission_global_config(db, config, updates)
...
def update_category_rate(
    ...
    if "rate" in payload:
        ...
        if not (0.0 <= r <= 1.0):
            raise HTTPException(status_code=422, detail="Rate must be between 0.0 and 1.0")
        updates["rate_percent"] = Decimal(str(r))
    if "is_active" in payload:
        updates["is_active"] = bool(payload["is_active"])
    if "category_display_name" in payload:
        updates["category_display_name"] = payload["category_display_name"]

    _update_commission_category_rate(db, row, updates)
...
def update_badge_tier(
    ...
    if "min_monthly_revenue" in payload:
        updates["min_monthly_revenue"] = (
            None if payload["min_monthly_revenue"] is None 
            else Decimal(str(payload["min_monthly_revenue"]))
        )

    updates["updated_by"] = acting_user["id"]
    updates["updated_at"] = datetime.now(timezone.utc)
    
    _update_commission_badge_tier(db, row, updates)
...
def create_ledger_adjustment(
    ...
    updates = {
        "is_adjusted": True,
        "adjusted_by": acting_user["id"],
        "adjusted_at": now,
        "adjustment_reason": reason,
        "original_commission_amount": getattr(original, "commission_amount"),
        "commission_amount": Decimal(str(new_amount)),
    }
    _update_commission_ledger_entry(db, original, updates)
...
from services.commission_write_service import (
    create_commission_agreement as _create_commission_agreement,
    create_product_commission_override as _create_product_commission_override,
    delete_commission_agreement as _delete_commission_agreement,
    delete_product_commission_override as _delete_product_commission_override,
    update_commission_badge_tier as _update_commission_badge_tier,
    update_commission_category_rate as _update_commission_category_rate,
    update_commission_global_config as _update_commission_global_config,
    update_commission_ledger_entry as _update_commission_ledger_entry,
    update_product_commission_override as _update_product_commission_override,
)
