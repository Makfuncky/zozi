...
async def upload_verification_documents(
    ...
    update_supplier_profile(db, profile, verified_documents=json.dumps(existing_docs), document_expires_at=None)
...
def accept_supplier_terms(current_user: dict, db: Session) -> dict:
    ...
    profile = db.query(SP).filter(SP.user_id == current_user["id"]).first()
    if not profile:
        profile = create_supplier_profile(db=db, user_id=current_user["id"])
...
def upsert_supplier_bank_account(body: dict, current_user: dict, db: Session) -> dict:
    ...
    if is_new:
        record = create_supplier_bank_account(db=db, supplier_id=supplier_id)
