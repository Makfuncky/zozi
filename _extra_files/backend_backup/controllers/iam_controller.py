...
from services.iam_write_service import (
    create_employee_biometric as service_create_employee_biometric,
    create_geo_fence_log as service_create_geo_fence_log,
    create_physical_id_card as service_create_physical_id_card,
    update_employee_biometric as service_update_employee_biometric,
    update_physical_id_card as service_update_physical_id_card,
)
...
def generate_physical_card(employee_id: int, db: Session) -> dict:
    ...
    card = service_create_physical_id_card(db, employee_id=employee_id, card_number=card_number)
...
def enroll_biometric(employee_id: int, biometric_type: str, data: str, db: Session) -> dict:
    ...
    if existing:
        service_update_employee_biometric(
            db,
            existing,
            biometric_type=biometric_type,
            fingerprint_hash=data if biometric_type == "fingerprint" else existing.fingerprint_hash,
            face_encoding=data if biometric_type == "face" else existing.face_encoding,
        )
        return {"biometric_id": existing.id, "type": biometric_type, "active": True}
    bio = service_create_employee_biometric(
        db,
...
def revoke_physical_card(employee_id: int, reason: str, db: Session) -> dict:
    ...
    if card:
        service_update_physical_id_card(db, card, is_revoked=True)
