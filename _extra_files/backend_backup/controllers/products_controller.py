...
from data.schemas import Product as ProductSchema
...
from services.suppliers_write_service import add_to_session, commit_only
from utils.cache import build_versioned_cache_key, cache_or_compute
...
def update_product(product_id: int, product: ProductCreate, current_user: dict, db: Session) -> Product:
    from services.products_write_service import update_product as update_product_model
    ...
    for _k, _v in data.items():
        setattr(db_product, _k, _v)
    add_to_session(db, db_product)
    commit_only(db)
    return db_product
    audit_log(
        db,
...
def create_supplier_product_with_upload(
    ...
    from services.products_write_service import create_product as create_product_model
    ...
    if category_match is not None:
        product_data["category_id"] = cast(int, getattr(category_match, "id"))
        product_data["category"] = cast(str, getattr(category_match, "name"))
    _bump_product_cache_version()
