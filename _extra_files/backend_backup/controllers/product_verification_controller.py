...
from models import Product, ProductVerification
from services.products_write_service import (
    create_product_verification as svc_create_verification,
    update_product_verification as svc_update_verification,
)
...
def create_verification(data: dict, current_user: dict, db: Session) -> dict:
    ...
    verification = svc_create_verification(
        db,
...
def update_verification(v_id: int, data: dict, current_user: dict, db: Session) -> dict:
    ...
    if updates:
        verification = svc_update_verification(db, verification, updates)
    return _serialize(verification)
...
def bulk_update_verifications(data: dict, current_user: dict, db: Session) -> dict:
    ...
    for verification_id in list(dict.fromkeys(verification_ids)):
        ...
        if notes is not None:
            updates["notes"] = notes
        svc_update_verification(db, verification, updates)
