...
from services.cart_write_service import (
    commit_cart_items as commit_cart_items_db,
    create_cart_item as create_cart_item_db,
    delete_cart_item as delete_cart_item_db,
    delete_cart_items_by_user as delete_cart_items_by_user_db,
    update_cart_item as update_cart_item_db,
)
...
def sync_cart(user_id: int, body: CartSyncRequest, db: Session) -> List[dict]:
    ...
    """
    if not body.items:
        delete_cart_items_by_user_db(db, user_id)
        return []
    ...
    for existing in existing_items:
        if _variant_key(cast(Any, existing).product_id, cast(Any, existing).selected_size, cast(Any, existing).selected_color) not in incoming_keys:
            delete_cart_item_db(db, existing)
    ...
    for variant_key, item in normalized_items.items():
        ...
        if existing:
            ...
            update_cart_item_db(db, existing, {"quantity": qty})
        else:
            create_cart_item_db(
                db,
                user_id=user_id,
