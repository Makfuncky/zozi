...
from services.payments_write_service import (
    create_payment_provider_config as create_payment_provider_config_model,
    update_payment_provider_config as update_payment_provider_config_model,
    create_or_update_gateway_connection as create_or_update_gateway_connection_model,
    update_gateway_connection as update_gateway_connection_model,
    update_payment_provider_config_no_refresh,
    add_processed_webhook_event,
    add_notification,
    flush_model,
    commit_only,
    create_payment as create_payment_model,
    update_payment as update_payment_model,
)
...
async def _paypal_get_access_token(client_id: str, secret: str, base_url: str) -> str:
    ...
    if resp.status_code != 200:
        logger.error("PayPal auth request failed (%s): %s", resp.status_code, resp.text[:200])
...
def update_payment_provider_runtime_config(
    ...
    record = _get_payment_provider_config_record(db)
    if record is None:
        record = create_payment_provider_config_model(
            db,
            online_provider=payload.online_provider,
            updated_by=current_user.get("id")
        )
    else:
        update_payment_provider_config_model(
            db, record,
            {
                "online_provider": payload.online_provider,
                "updated_by": current_user.get("id")
...
def upsert_payment_gateway_connection(
    ...
    connection_data = {
        ...
        "supports_customer_checkout": payload.supports_customer_checkout,
        ...
        "supported_currencies_json": json.dumps(payload.supported_currencies),
        "extra_config_json": json.dumps(payload.extra_config),
        "notes": payload.notes,
        "fee_percent": payload.fee_percent,
        "fixed_fee_amount": payload.fixed_fee_amount,
        "payout_fee_percent": payload.payout_fee_percent,
        "payout_fixed_fee_amount": payload.payout_fixed_fee_amount,
        "pass_fee_to_customer": payload.pass_fee_to_customer,
        "settlement_cycle": payload.settlement_cycle,
        "updated_by": current_user.get("id"),
    }
    
    if payload.secret_key is not None:
        connection_data["secret_key"] = payload.secret_key
    if payload.webhook_secret is not None:
        connection_data["webhook_secret"] = payload.webhook_secret

    if record is None:
        record = create_or_update_gateway_connection_model(db, **connection_data)
    else:
        update_gateway_connection_model(db, record, connection_data)
...
async def handle_stripe_webhook(request: Request, db: Session) -> dict:
    ...
    if stripe_event_id:
        add_processed_webhook_event(db, event_id=stripe_event_id, processor="stripe")
...
async def handle_tap_webhook(request: Request, db: Session) -> dict:
    ...
    add_processed_webhook_event(db, event_id=tap_event_id, processor="tap")
...
async def handle_paytabs_callback(request: Request, db: Session) -> dict:
    ...
    add_processed_webhook_event(db, event_id=paytabs_event_id, processor=PAYTABS_PAYMENT_METHOD)
...
async def handle_paypal_webhook(request: Request, db: Session) -> dict:
    ...
    if event_id:
        add_processed_webhook_event(db, event_id=event_id, processor="paypal")
...
async def handle_thawani_webhook(request: Request, db: Session) -> dict:
    ...
    if invoice_id or session_id_field:
        add_processed_webhook_event(db, event_id=idempotency_key, processor=THAWANI_PAYMENT_METHOD)
...
async def handle_generic_gateway_callback(request: Request, provider_code: str, db: Session) -> dict:
    ...
    if success and order.paid_at is None and order.status not in INVENTORY_RELEASE_STATUSES:
        ...
        add_processed_webhook_event(db, event_id=event_id, processor=normalized_code, payload_hash=_payload_hash)
    ...
    add_processed_webhook_event(db, event_id=event_id, processor=normalized_code)
