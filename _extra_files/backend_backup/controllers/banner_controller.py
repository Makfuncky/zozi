...
def _seed_defaults(db: Session) -> None:
    ...
    if count == 0:
        bulk_add_banners(db, _DEFAULT_BANNERS)
        return

    for default_banner in _DEFAULT_BANNERS:
        add_banner_if_missing(db, default_banner)
...
def update_banner(banner_id: int, payload: BannerUpdate, current_admin: dict, db: Session) -> dict:
    ...
    banner = update_banner_model(db, banner, updates)
    audit_log(
        db,
...
async def upload_banner_image(
    ...
    ext = validate_upload_image(img_content, file.filename or "banner")
    suffix = ext if ext.startswith(".") else f".{ext}"
    filename = f"banner_{uuid.uuid4().hex[:8]}{suffix}"
    key = f"banners/{filename}"
    url = _storage.save(key, img_content, content_type=file.content_type)

    banner = update_banner_image_model(db, banner, url, filename)
    audit_log(
        db,
...
def reorder_banners(banner_ids: list[int], current_admin: dict, db: Session) -> list[dict]:
    ...
    reorder_banners_model(db, banner_ids)
    audit_log(
        db,
...
from services.banner_write_service import (
    add_banner_if_missing,
    bulk_add_banners,
    create_banner as create_banner_model,
    delete_banner as delete_banner_model,
    reorder_banners as reorder_banners_model,
    update_banner as update_banner_model,
    update_banner_image as update_banner_image_model,
)
