...
from services.orders.orders_write_service import (
    create_order as create_order_model,
    update_order as update_order_model,
    create_order_item as create_order_item_model,
)
from services.logistics_partner_write_service import (
    create_shipment_event as create_shipment_event_model,
    create_notification as create_notification_model,
    update_order as update_order_logistics,
    update_shipment as update_shipment_model,
)
...
from utils.order_tracking import (
    build_order_tracking_payload,
    derive_order_financials,
    normalize_shipment_event_type,
    order_status_label,
...
def get_orders(current_user: dict, db: Session, *, skip: int = 0, limit: int = 50) -> List[Order]:
    ...
    for order in orders_to_update:
        update_order_logistics(db, order, {"status": order.status})
    return orders
...
def get_order(order_id: int, current_user: dict, db: Session) -> Order:
    ...
    reconciled_status = reconcile_order_status(order, shipments)
    if order.status != reconciled_status:
        update_order_logistics(db, order, {"status": reconciled_status})
    setattr(order, "status_label", order_status_label(reconciled_status, shipments, events))
    return order
...
def confirm_order_scan_receipt(order_id: int, data: dict, current_user: dict, db: Session) -> dict:
    ...
    event = create_shipment_event_model(
    ...
    reconciled_status = reconcile_order_status(order, shipments)
    update_order_logistics(db, order, {"status": reconciled_status, "delivered_at": now if reconciled_status == "delivered" and not getattr(order, "delivered_at", None) else getattr(order, "delivered_at", None)})
...
def respond_to_shipment_confirmation(
    ...
    if decision == "accepted":
        ...
        if confirmation.current_hub:
            shipment_updates["current_hub"] = confirmation.current_hub
        if confirmation.tracking_number:
            shipment_updates["tracking_number"] = confirmation.tracking_number
        if not cast(str | None, getattr(shipment, "scan_code", None)):
            shipment_updates["scan_code"] = f"SHIP-{shipment.id}"
        if requested_status == "shipped" and not cast(datetime | None, getattr(shipment, "shipped_at", None)):
            shipment_updates["shipped_at"] = _utcnow()
        if requested_status == "delivered":
            shipment_updates["delivery_signature_name"] = confirmation.delivery_signature_name
            shipment_updates["delivery_signature_data_url"] = confirmation.delivery_signature_data_url
            shipment_updates["delivery_signature_captured_at"] = confirmation.responded_at
            if not cast(datetime | None, getattr(shipment, "actual_delivery", None)):
                shipment_updates["actual_delivery"] = confirmation.responded_at

        update_shipment_model(db, shipment, shipment_updates)

        event_notes = confirmation.notes or response_notes
        create_shipment_event_model(
        ...
        if order is not None:
            ...
            update_order_logistics(db, order, {"status": new_order_status})
    ...
    commit_only(db)
    refresh_model(db, confirmation)
    if decision == "accepted":
        try:
            from services.transactional_email_service import (
                enqueue_shipment_status_email,
            )
...
def confirm_order_receipt_scan(order_id: int, data: dict, current_user: dict, db: Session) -> dict:
    ...
    matched_updates = {
        "status": "delivered",
        "actual_delivery": matched.actual_delivery or _utcnow(),
        "updated_at": matched.actual_delivery or _utcnow(),
    }
    update_shipment_model(db, matched, matched_updates)
    order_shipments = db.query(Shipment).filter(Shipment.order_id == order_id).all()
    new_status = reconcile_order_status(order, order_shipments)
    update_order_logistics(db, order, {"status": new_status})
