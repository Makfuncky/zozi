...
from services.logistics_partner_pricing import (
    ...
    partner_can_service_order,
    quote_shipping_for_destination,
...
from services.logistics_partner_write_service import (
    create_logistics_partner as create_partner_model,
    create_logistics_partner_bank_account,
    create_logistics_partner_document,
    create_logistics_partner_payout,
    create_logistics_partner_service_area,
    create_notification as create_notification_model,
    create_shipment,
    create_shipment_confirmation,
    create_shipment_event,
    create_transaction_ledger_entry,
    create_city_distance_matrix,
    create_pricing_rule,
    create_coding_remittance_receipt,
    create_pricing_profile,
    create_settlement,
    create_vehicle_rule,
    update_logistics_partner as update_partner_model,
    update_logistics_partner_bank_account,
    update_logistics_partner_document,
    update_logistics_partner_payout,
    update_logistics_partner_service_area,
    update_transaction_ledger,
    update_shipment,
    update_order,
    update_order_logistics_allocation,
    update_city_distance_matrix,
    update_pricing_rule,
    update_pricing_profile,
    update_settlement,
    update_vehicle_rule,
    update_notification as update_notification_model,
    delete_pricing_rule,
    delete_vehicle_rule,
    delete_shipment,
    delete_logistics_partner_bank_account,
    delete_logistics_partner_document,
    delete_logistics_partner_payout,
    delete_logistics_partner_service_area,
    delete_logistics_partner,
    delete_notification,
    delete_pricing_profile,
    delete_category_pricing_rule,
    delete_city_distance_matrix,
)
...
def update_my_partner_profile(data: dict, current_user: dict, db: Session) -> dict:
    ...
    return _serialize_partner(update_partner_model(db, partner, {}))
...
def accept_partner_terms(current_user: dict, db: Session) -> dict:
    ...
    updates = {
        ...
        "terms_version": CURRENT_LOGISTICS_TERMS_VERSION,
        "terms_accepted_at": _utcnow(),
        "updated_at": _utcnow(),
    }
    update_partner_model(db, partner, updates)
...
def submit_partner_profile_for_review(current_user: dict, db: Session) -> dict:
    ...
    return _serialize_partner(update_partner_model(db, partner, updates))
...
def upsert_my_partner_service_area(area_id: int | None, data: dict, current_user: dict, db: Session) -> dict[str, Any]:
    ...
    if role in ("admin", "sub_admin"):
        ...
        payload["review_note"] = "Set directly by admin"
        payload["reviewed_by"] = current_user["id"]
        payload["reviewed_at"] = _utcnow()
    else:
        payload["approval_status"] = "pending"
        payload["review_note"] = "Awaiting admin approval"
        payload["reviewed_by"] = None
        payload["reviewed_at"] = None
    payload["updated_at"] = _utcnow()

    if area_id is None:
        area = create_logistics_partner_service_area(db, partner_id=partner.id, **payload)
    else:
        area = (
            db.query(LogisticsPartnerServiceArea)
            .filter(
        ...
        area = update_logistics_partner_service_area(db, area, payload)
...
def upsert_my_partner_pricing_profile(profile_id: int | None, data: dict, current_user: dict, db: Session) -> dict[str, Any]:
    ...
    if profile_id is None:
        profile = create_pricing_profile(db, partner_id=partner.id, **payload)
    else:
        profile = (
        ...
        profile = update_pricing_profile(db, profile, payload)
...
def upsert_my_partner_category_rule(rule_id: int | None, data: dict, current_user: dict, db: Session) -> dict[str, Any]:
    ...
    if rule_id is None:
        rule = create_pricing_rule(db, partner_id=partner.id, **payload)
    else:
        rule = (
        ...
        rule = update_pricing_rule(db, rule, payload)
...
def upsert_my_partner_vehicle_rule(rule_id: int | None, data: dict, current_user: dict, db: Session) -> dict[str, Any]:
    ...
    if rule_id is None:
        rule = create_vehicle_rule(db, partner_id=partner.id, **payload)
    else:
        rule = (
        ...
        rule = update_vehicle_rule(db, rule, payload)
...
def delete_my_partner_pricing_profile(profile_id: int, current_user: dict, db: Session) -> dict[str, Any]:
    ...
    if not profile:
        raise HTTPException(status_code=404, detail="Pricing profile not found")
    delete_pricing_profile(db, profile)
...
def delete_my_partner_category_rule(rule_id: int, current_user: dict, db: Session) -> dict[str, Any]:
    ...
    if not rule:
        raise HTTPException(status_code=404, detail="Category pricing rule not found")
    delete_category_pricing_rule(db, rule)
...
def delete_my_partner_service_area(area_id: int, current_user: dict, db: Session) -> dict[str, Any]:
    ...
    if not area:
        raise HTTPException(status_code=404, detail="Service area not found")
    delete_logistics_partner_service_area(db, area)
...
def review_partner_profile(partner_id: int, data: dict, current_user: dict, db: Session) -> dict:
    ...
    updates = {
        ...
        "verification_note": note or ("Approved" if decision == "approved" else "Rejected" if decision == "rejected" else "Awaiting review"),
        "verified_by": current_user["id"] if decision == "approved" else None,
        "verified_at": _utcnow() if decision == "approved" else None,
    }
...
def review_partner_service_area(area_id: int, data: dict, current_user: dict, db: Session) -> dict[str, Any]:
    ...
    updates = {
        ...
        "review_note": note or ("Approved" if decision == "approved" else "Rejected" if decision == "rejected" else "Awaiting review"),
        "reviewed_by": current_user["id"] if decision in {"approved", "rejected"} else None,
        "reviewed_at": _utcnow() if decision in {"approved", "rejected"} else None,
        "updated_at": _utcnow(),
    }
    return serialize_service_area(update_logistics_partner_service_area(db, area, updates))
...
def review_partner_pricing_profile(profile_id: int, data: dict, current_user: dict, db: Session) -> dict[str, Any]:
    ...
    return serialize_pricing_profile(update_pricing_profile(db, profile, updates))
...
def review_partner_category_rule(rule_id: int, data: dict, current_user: dict, db: Session) -> dict[str, Any]:
    ...
    return serialize_category_pricing_rule(update_pricing_rule(db, rule, updates))
...
def review_partner_vehicle_rule(rule_id: int, data: dict, current_user: dict, db: Session) -> dict[str, Any]:
    ...
    return serialize_vehicle_rule(update_vehicle_rule(db, rule, updates))
...
def create_partner(data: dict, current_user: dict, db: Session) -> dict:
    ...
    if linked_user_id is not None:
        ...
        if existing_placeholder:
            ...
            return _serialize_partner(update_partner_model(db, existing_placeholder, updates))
...
def delete_partner(partner_id: int, current_user: dict, db: Session) -> dict:
    ...
    try:
        delete_logistics_partner(db, partner)
...
def bulk_manage_partners(
    ...
    for partner_id in ordered_ids:
        ...
        if normalized_action == "approve":
            update_partner_model(db, partner, {
                "verification_status": "approved",
                "verification_note": normalized_note or "Approved",
                "verified_by": current_user["id"],
                "verified_at": _utcnow(),
                "status": "active" if cast(str | None, getattr(partner, "status", None)) == "pending_onboarding" else partner.status,
            })
            processed.append({
                "id": partner_id,
            ...
            update_partner_model(db, partner, {
                ...
                "verification_note": normalized_note or "Rejected",
                "verified_by": None,
                "verified_at": None,
                "updated_at": _utcnow(),
            })
            ...
            update_partner_model(db, partner, {"status": "active", "updated_at": _utcnow()})
            processed.append({
                "id": partner_id,
                "status": getattr(partner, "status", None),
                "verification_status": getattr(partner, "verification_status", None),
            ...
            update_partner_model(db, partner, {"status": "suspended", "updated_at": _utcnow()})
            processed.append({
                "id": partner_id,
                "status": getattr(partner, "status", None),
                "verification_status": getattr(partner, "verification_status", None),
...
def request_partner_payout(data: dict, current_user: dict, db: Session) -> dict:
    ...
    payout = create_logistics_partner_payout(
        db=db,
        partner_id=partner.id,
    ...
    update_logistics_partner_payout(db, payout, {"reference_id": payout.reference_id})
...
def verify_partner_payout(payout_id: int, data: dict, current_user: dict, db: Session) -> dict:
    ...
    reference_id = str(data.get("reference", "")).strip() or payout.reference_id
    if not reference_id:
        reference_id = build_transfer_reference(
            db,
            kind="logistics_payout",
            entity_id=int(payout.partner_id),
            record_id=int(payout.id),
    ...
    updates = {
        ...
        "notes": str(data.get("notes", "")).strip() or payout.notes,
    }
    if new_status in {"completed", "rejected"}:
        updates["processed_at"] = _utcnow()
    ...
    payout = update_logistics_partner_payout(db, payout, updates)
    audit_log(
        db=db,
...
def create_shipment_confirmation_request_partner(
    ...
    confirmation = create_shipment_confirmation(
        db=db,
        shipment_id=shipment.id,
        order_id=shipment.order_id,
        supplier_id=shipment.supplier_id,
    ...
    update_shipment(db, shipment, {"updated_at": _utcnow()})
...
def update_shipment_status_partner(
    ...
    if order is not None:
        ...
        update_order(db, order, {"status": new_order_status})
        if new_order_status == "delivered":
            try:
                from services.cash_management_service import (
                    create_settlements_on_delivery,
                )
    ...
    shipment = update_shipment(db, shipment, {"updated_at": _utcnow()})
...
def upsert_partner_bank_account(body: dict, current_user: dict, db: Session) -> dict:
    ...
    if not is_new and getattr(record, "verification_status", "pending") != "pending":
        ...
        updates["verification_note"] = "Resubmitted by partner — awaiting re-verification."
        updates["provider"] = None
        updates["provider_recipient_id"] = None
        updates["provider_status"] = None
        updates["provider_last_synced_at"] = None
        updates["verified_at"] = None
        updates["verified_by"] = None

    if is_new:
        record = create_logistics_partner_bank_account(db=db, partner_id=partner_id, **updates)
    else:
        record = update_logistics_partner_bank_account(db=db, account=record, updates=updates)
...
async def upload_partner_document(
    ...
    partner = _get_partner_for_user(current_user, db)
    doc = create_logistics_partner_document(
        db=db,
        partner_id=cast(int, partner.id),
...
def delete_partner_document(doc_id: int, current_user: dict, db: Session) -> dict:
    ...
    delete_logistics_partner_document(db, doc)
...
def admin_review_lp_document(doc_id: int, data: dict, current_user: dict, db: Session) -> dict:
    ...
    doc = update_logistics_partner_document(
        db=db,
        doc=doc,
        updates={
            "status": new_status,
...
def create_city_distance(data: dict[str, Any], current_user: dict, db: Session) -> dict[str, Any]:
    ...
    entry = create_city_distance_matrix(
        db=db,
...
def update_city_distance(matrix_id: int, data: dict[str, Any], current_user: dict, db: Session) -> dict[str, Any]:
    ...
    if distance_km <= 0:
        raise HTTPException(status_code=422, detail="distance_km must be greater than 0")
    entry = update_city_distance_matrix(
        db=db,
        matrix=entry,
        updates={
            "distance_km": distance_km,
            "notes": _sanitize_optional_string(data.get("notes"), max_length=1000) if "notes" in data else None,
            "updated_by": current_user["id"],
            "updated_at": _utcnow(),
        },
    )
