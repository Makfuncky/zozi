...
async def update_event_gps(
    ...
    update_shipment_event_model(db, event, {"latitude": latitude, "longitude": longitude})
...
async def create_carrier(data: dict, current_user: dict, db: Session) -> dict:
    ...
    carrier = create_shipping_carrier_model(
        db,
...
async def delete_carrier(carrier_id: int, current_user: dict, db: Session) -> dict:
    ...
    update_shipping_carrier_model(db, carrier, {"is_active": False})
...
async def upsert_shipping_zone(data: dict, current_user: dict, db: Session) -> dict:
    ...
    if zone_id:
        ...
        zone = update_shipping_zone_model(db, zone, updates)
    else:
        zone = create_shipping_zone_model(
            db,
            supplier_id=supplier_id,
            name=name,
            countries=json.dumps(countries),
...
async def delete_shipping_zone(zone_id: int, current_user: dict, db: Session) -> dict:
    ...
    if not zone:
        raise HTTPException(status_code=404, detail="Zone not found")
    delete_shipping_zone_model(db, zone)
...
async def scan_shipment_event(shipment_id: int, data: dict, current_user: dict, db: Session) -> dict:
    ...
    event = ShipmentEvent(
        shipment_id=shipment.id,
        order_id=shipment.order_id if shipment.order else None,
        supplier_id=shipment.supplier_id,
        actor_user_id=current_user.get("id"),
        actor_role=current_user.get("role", "supplier"),
        event_type=event_type,
...
from services.logistics_write_service import (
    add_and_flush,
    commit_only,
    create_shipping_carrier as create_shipping_carrier_model,
    create_shipping_zone as create_shipping_zone_model,
    delete_shipping_zone as delete_shipping_zone_model,
    refresh_model,
    update_shipping_carrier as update_shipping_carrier_model,
    update_shipment as update_shipment_model,
    update_shipment_event as update_shipment_event_model,
    update_shipping_zone as update_shipping_zone_model,
)
