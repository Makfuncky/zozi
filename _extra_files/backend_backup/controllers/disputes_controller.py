...
from models import SupplierDispute, SupplierNotificationPreference
...
def _get_or_create_preferences(supplier_id: int, db: Session) -> SupplierNotificationPreference:
    ...
    return _create_supplier_notification_preference(db, supplier_id=supplier_id)
...
def update_supplier_notification_preferences(
    ...
    for field in _PREF_FIELDS:
        if field in payload:
            updates[field] = bool(payload.get(field))

    if updates:
        _update_supplier_notification_preference(db, prefs, updates)
    return _serialize_preferences(prefs)
...
def create_supplier_dispute(payload: dict[str, Any], current_user: dict, db: Session) -> dict[str, Any]:
    ...
    dispute = _create_supplier_dispute(
        db,
...
def update_admin_dispute(dispute_id: int, payload: dict[str, Any], current_admin: dict, db: Session) -> dict[str, Any]:
    ...
    if status is not None:
        ...
        if normalized_status in {"resolved", "rejected", "closed"}:
            updates["resolved_at"] = datetime.now(timezone.utc).replace(tzinfo=None)
            updates["resolved_by"] = _as_int(current_admin.get("id"))
    ...
    if "admin_notes" in payload:
        updates["admin_notes"] = (str(payload.get("admin_notes") or "").strip() or None)
    if "resolution_notes" in payload:
        updates["resolution_notes"] = (str(payload.get("resolution_notes") or "").strip() or None)

    if updates:
        _update_supplier_dispute(db, dispute, updates)

    _create_dispute_notification_for_update(
        db,
...
def bulk_update_admin_disputes(
    ...
    if updates:
        _bulk_update_disputes(db, rows, updates)
...
from services.disputes_write_service import (
    bulk_update_disputes as _bulk_update_disputes,
    create_dispute_notification as _create_dispute_notification,
    create_dispute_notification_for_update as _create_dispute_notification_for_update,
    create_supplier_dispute as _create_supplier_dispute,
    create_supplier_notification_preference as _create_supplier_notification_preference,
    update_supplier_dispute as _update_supplier_dispute,
    update_supplier_notification_preference as _update_supplier_notification_preference,
)
