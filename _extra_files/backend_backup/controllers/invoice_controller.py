...
from models import Invoice, InvoiceItem, Order, OrderItem
from utils.audit import AuditAction, audit_log
from services.invoice_write_service import create_invoice_with_items, update_invoice
...
def create_invoice_from_order(data: dict, current_user: dict, db: Session) -> dict:
    ...
    items_data = [
        ...
        for line_total in [float(item.price) * item.quantity]
    ]

    inv = create_invoice_with_items(db, invoice_data, items_data)
...
def update_invoice_status(invoice_id: int, data: dict, current_user: dict, db: Session) -> dict:
    ...
    if new_status == "in_transit":
        if not inv.picked_at:
            updates["picked_at"] = data.get("picked_at") or now
        if not inv.dispatched_at:
            updates["dispatched_at"] = data.get("dispatched_at") or now
    if new_status == "delivered" and not inv.delivered_at:
        updates["delivered_at"] = data.get("delivered_at") or now
    if "notes" in data:
    ...
    inv = update_invoice(db, inv, updates)
