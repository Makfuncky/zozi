...
from models import Order, OrderItem, Product, User, Wishlist
from services.users_write_service import (
    create_chatbot_query_event as svc_create_chatbot_query_event,
    update_user_browsing_history as svc_update_user_browsing_history,
)
...
def _record_chatbot_event(
    ...
    *,
    ...
    svc_create_chatbot_query_event(
        db=db,
...
def record_product_view(db: Session, user_id: int, product_id: int) -> None:
    ...
    if not user:
        return
    svc_update_user_browsing_history(db=db, user=user, product_id=product_id)
