...
class Announcement(VersionMixin, Base):
...
class HelpCategory(VersionMixin, Base):
...
class ProxyChannel(VersionMixin, Base):
...
class ProxySession(VersionMixin, Base):
...
class ProxyMessage(VersionMixin, Base):
...
class ProxyCallLog(VersionMixin, Base):
...
class EmployeeCommunicationThread(Base, TenantMixin):
    ...
    __table_args__ = (
        ...
        Index("ix_emp_comm_country_created", "country_code", "created_at"), {"schema": "communication"})
...
class ExternalContactMasking(VersionMixin, Base):
...
class CommunicationAuditTrail(VersionMixin, Base):
...
class InternalChannelMember(VersionMixin, Base):
...
class InternalMessage(VersionMixin, Base):
...
class ChatReadReceipt(VersionMixin, Base):
...
class ChatAttachment(VersionMixin, Base):
...
class InternalEmail(Base, TenantMixin):
    ...
    in_reply_to = Column(Integer, ForeignKey("communication.internal_emails.id", ondelete="RESTRICT"), nullable=True, index=True)
...
class EmailFolder(VersionMixin, Base):
