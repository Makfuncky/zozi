...
class CountryConfig(VersionMixin, Base):
...
class CountryCommunication(VersionMixin, Base):
    __tablename__ = "country_communications"
    __table_args__ = (
        Index("ix_country_communications_recipient", "to_user_id", "status"),
        Index("ix_country_communications_country_created", "country_code", "created_at"),
        {"schema": "country"})
...
class CountryGatewayCredentials(VersionMixin, Base):
    __tablename__ = "country_gateway_credentials"
    __table_args__ = (
        Index("ix_country_gateway_credentials_country_created", "country_code", "created_at"),
        {"schema": "country"},
    )
    id = Column(Integer, primary_key=True, index=True)
    country_code = Column(String(3), ForeignKey("country.country_configs.code", ondelete="RESTRICT"), nullable=False, index=True)
...
class PayoutRule(VersionMixin, Base):
    __tablename__ = "payout_rules"
    __table_args__ = (
        Index("ix_payout_rules_country_created", "country_code", "created_at"),
        {"schema": "treasury"},
    )
    id = Column(Integer, primary_key=True, index=True)
...
class TaxRule(VersionMixin, Base):
    __tablename__ = "tax_rules"
    __table_args__ = (
        Index("ix_tax_rules_country_created", "country_code", "created_at"),
        {"schema": "country"},
    )
    id = Column(Integer, primary_key=True, index=True)
    country_code = Column(String(3), ForeignKey("country.country_configs.code", ondelete="RESTRICT"), nullable=False, index=True)
...
class ShippingRule(VersionMixin, Base):
    __tablename__ = "shipping_rules"
    __table_args__ = (
        Index("ix_shipping_rules_country_created", "country_code", "created_at"),
        {"schema": "logistics"},
    )
    id = Column(Integer, primary_key=True, index=True)
    country_code = Column(String(3), nullable=False)
...
class PayoutRuleCategory(VersionMixin, Base):
    __tablename__ = "payout_rule_categories"
    __table_args__ = (
        UniqueConstraint("country_code", "category_slug", name="uq_payout_rule_category"),
        Index("ix_payout_rule_categories_country_created", "country_code", "created_at"),
        {"schema": "country"})
...
class PayoutRuleProduct(VersionMixin, Base):
    __tablename__ = "payout_rule_products"
    __table_args__ = (
        UniqueConstraint("country_code", "product_id", name="uq_payout_rule_product"),
        Index("ix_payout_rule_products_country_created", "country_code", "created_at"),
        {"schema": "country"})
...
Index("ix_country_gateway_credentials_credentials", text("(credentials::jsonb)"), postgresql_using="gin")
