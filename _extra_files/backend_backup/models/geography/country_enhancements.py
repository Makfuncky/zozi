...
class CountryFeatureFlag(VersionMixin, Base):
    __tablename__ = "country_feature_flags"
    __table_args__ = (
        UniqueConstraint("country_code", "feature_key", name="uq_country_feature"),
        Index("ix_country_feature_flags_country_created", "country_code", "created_at"),
...
class CountryStaffAssignment(VersionMixin, Base):
    ...
    __table_args__ = (
        ...
        Index("ix_country_staff_assignments_country_created", "country_code", "created_at"),
        {"schema": "configuration"})
...
class CrossCountryCustomerSession(VersionMixin, Base):
...
class OmanDeliveryZone(VersionMixin, Base):
...
class CountryConfigVersion(Base):
    ...
    __table_args__ = (
        ...
        Index("ix_country_config_versions_country_created", "country_code", "created_at"),
        {"schema": "configuration"})
...
class SupplierKYCRequirement(VersionMixin, Base):
    __tablename__ = 'supplier_kyc_requirements'
    __table_args__ = (
        Index("ix_supplier_kyc_requirements_country_created", "country_code", "created_at"),
        {"schema": "country"},
    )
    id = Column(Integer, primary_key=True, index=True)
...
class LogisticsPartnerKYCRequirement(VersionMixin, Base):
    __tablename__ = 'logistics_partner_kyc_requirements'
    __table_args__ = (
        Index("ix_logistics_partner_kyc_requirements_country_created", "country_code", "created_at"),
        {"schema": "country"},
    )
    id = Column(Integer, primary_key=True, index=True)
    country_code = Column(String(3), ForeignKey("country.country_configs.code", ondelete="RESTRICT"), nullable=False, unique=True, index=True)
...
class CountryCommissionRate(VersionMixin, Base):
...
class CountryLocalization(VersionMixin, Base):
...
class CountryPaymentAlias(VersionMixin, Base):
    __tablename__ = 'country_payment_aliases'
    __table_args__ = (
        UniqueConstraint('country_code', 'alias_type', name='uq_country_payment_alias'),
        Index("ix_country_payment_aliases_country_created", "country_code", "created_at"),
        {"schema": "configuration"})
...
class CountryLegalContract(Base):
    ...
    __table_args__ = (
        ...
        Index("ix_country_legal_contracts_country_created", "country_code", "created_at"),
        {"schema": "configuration"})
...
class CountryCategoryTaxRate(VersionMixin, Base):
    __tablename__ = 'country_category_tax_rates'
    __table_args__ = (
        UniqueConstraint('country_code', 'category_id', name='uq_country_category_tax'),
        Index("ix_country_category_tax_rates_country_created", "country_code", "created_at"),
        {"schema": "configuration"})
...
class CountryHolidayCalendar(VersionMixin, Base):
    __tablename__ = 'country_holiday_calendars'
    __table_args__ = (
        Index('ix_country_holiday_date', 'holiday_date'),
        Index("ix_country_holiday_calendars_country_created", "country_code", "created_at"),
        {"schema": "configuration"})
...
class CountryGatewayConfig(VersionMixin, Base):
    __tablename__ = 'country_gateway_configs'
    __table_args__ = (
        UniqueConstraint('country_code', 'gateway_id', name='uq_country_gateway'),
        Index("ix_country_gateway_configs_country_created", "country_code", "created_at"),
        {"schema": "configuration"})
...
class CountryCommunicationThread(VersionMixin, Base):
    __tablename__ = 'country_communication_threads'
    __table_args__ = (
        Index('ix_comm_thread_entity', 'entity_type', 'entity_id'),
        Index("ix_country_communication_threads_country_created", "country_code", "created_at"),
        {"schema": "configuration"})
...
class CountryCommissionRateHistory(VersionMixin, Base):
    __tablename__ = 'country_commission_rate_history'
    __table_args__ = (
        Index('ix_comm_rate_effective', 'effective_from'),
        Index("ix_country_commission_rate_history_country_created", "country_code", "created_at"),
        {"schema": "configuration"})
...
class CountryLogisticsZone(VersionMixin, Base):
    __tablename__ = 'country_logistics_zones'
    __table_args__ = (
        UniqueConstraint('country_code', 'zone_code', name='uq_zone_code'),
        Index("ix_country_logistics_zones_country_created", "country_code", "created_at"),
        {"schema": "configuration"})
...
class CountryPayoutRule(VersionMixin, Base):
    __tablename__ = 'country_payout_rules'
    __table_args__ = (
        Index('ix_payout_supplier', 'supplier_tier'),
        Index("ix_country_payout_rules_country_created", "country_code", "created_at"),
        {"schema": "configuration"})
