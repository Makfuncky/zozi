...
@dataclass
class _Br13ProcessingConfig:
    max_rembg_dimension: int = 1024
    max_output_dimension: int = 2048
CONFIG = _Br13ProcessingConfig()

class _Br13MemoryManager:

    @staticmethod
    def cleanup():
        gc.collect()

class _Br13AISegmenter:
    ...
    @classmethod
    def generate_alpha(cls, image_bytes: bytes, orig_size: Tuple[int, int], config: _Br13ProcessingConfig) -> np.ndarray:
        from rembg import remove
        models_to_try = ['birefnet-general-lite', 'u2net_cloth_seg', 'briaai-rmbg-1.4', 'isnet-general-use']
...
class _Br13GlobalBackgroundBleeder:
...
class _Br13FloatingArtifactRemover:
...
class _Br13Exporter:
    ...
    @staticmethod
    def process_and_save(input_bytes: bytes, output_path: str, config: _Br13ProcessingConfig):
        img = Image.open(io.BytesIO(input_bytes)).convert('RGB')
        orig_size = img.size
        input_np = np.array(img)
        alpha_map = _Br13AISegmenter.generate_alpha(input_bytes, orig_size, config)
        alpha_map = _Br13FloatingArtifactRemover.remove_floating_objects(alpha_map)
        alpha_map = _Br13BottomTextEraser.erase_bottom_text(alpha_map, input_np)
        alpha_map = _Br13EdgeShaver.shave_trailing_edges(alpha_map)
        alpha_map = _Br13GlobalBackgroundBleeder.remove_background_in_gaps(input_np, alpha_map)
...
if __name__ == '__main__':
    ...
    logger.info('🚀 Testing variants: birefnet-lite, u2net_cloth_seg, briaai-rmbg-1.4\n')
    (success, fail) = (0, 0)
    for (idx, img_path) in enumerate(image_files, 1):
        ...
        try:
            ...
            _Br13Exporter.process_and_save(img_bytes, str(out_path), CONFIG)
