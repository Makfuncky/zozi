...
from utils.logging_config import setup_structlog
...
from routers.public_ws_chat_access import websocket_user  # noqa: E402
...
from utils.websocket_manager import BACKGROUND_JOBS_ROOM, manager  # noqa: E402
...
def _load_routers():
    ...
    for name, prefix in router_names:
        try:
            ...
            failed_routers.append((name, type(e).__name__, str(e)))
            logger.error(
    ...
    if failed_routers:
        summary = "; ".join(f"{n} ({t}: {m})" for n, t, m in failed_routers)
        logger.error(
            "Failed to load/register %d router(s): %s",
            len(failed_routers),
            summary,
        )
    ...
    try:
        from routers.admin_promotions_governance import country_router as promotions_country_router
    ...
    try:
        countries_module = importlib.import_module("routers.public_countries_access")
