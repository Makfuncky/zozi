...
def _find_backend():
    ...
    for candidate in (here.parent / "backend", here.parent.parent / "backend"):
        if (candidate / "controllers" / "orders" / "orders_controller.py").exists():
            return candidate
    raise FileNotFoundError("backend/controllers/orders/orders_controller.py not found")


MODULE = _find_backend() / "controllers" / "orders" / "orders_controller.py"
LIMIT = 120
PINNED = [
