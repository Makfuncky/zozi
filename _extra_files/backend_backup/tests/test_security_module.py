...
def test_admin_auth_rename_is_atomic():
    """The auth.py shadow was renamed to admin_auth.py and every importer updated."""
    import pathlib

    admin_dir = pathlib.Path(__file__).resolve().parents[1] / "controllers" / "admin"
    assert (admin_dir / "admin_auth.py").exists(), "admin_auth.py must exist"
    assert not (admin_dir / "auth.py").exists(), "old auth.py must be gone"

    init_src = (admin_dir / "__init__.py").read_text(encoding="utf-8")
    assert "from .admin_auth import" in init_src
    assert "\nfrom .auth import" not in init_src

    users_src = (admin_dir / "users.py").read_text(encoding="utf-8")
    assert "from controllers.admin.admin_auth import _require_admin" in users_src
