...
def test_payments_folder_merged_into_finance():
    """DOM7: the non-canonical `services/payments/` folder is gone; its
    modules now live under `services/finance/`."""
    assert not (BACKEND / "services" / "payments").exists(), (
        "services/payments should have been merged into services/finance"
    )
    assert (BACKEND / "services" / "finance" / "payment_engine.py").exists()
    assert (BACKEND / "services" / "finance" / "webhook_processor.py").exists()
    assert (BACKEND / "services" / "finance" / "events" / "payment_events.py").exists()
...
def test_finance_functions_within_qual3_limit():
    ...
    commission = BACKEND / "services" / "finance" / "commission_engine.py"
    transfer = BACKEND / "services" / "finance" / "finance_transfer_service.py"
