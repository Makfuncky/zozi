...
> **Generated:** 2026-08-05T09:35:42.781206+00:00  
> **Repo:** `D:\Projects\10- E-COMMERCE WEBSITE\zozi`  
> **Result:** 🔴 2 violations · 🟡 988 advisories · 🟢 41 info  
> **Architecture Debt Score:** **19799**  
...
graph TD
    ...
    controllers_flat["19 flat .py files"]
    ...
    services_flat["13 flat .py files"]
...
| A2 | 14 | 🟡 ADVISORY | possibly dead/orphan module (no inbound imports; not an entrypoint) |
...
| DG2 | 17 | 🟡 ADVISORY | circular dependency detected |
...
| DOM2 | 10 | 🟡 ADVISORY | file is inside the wrong domain folder |
| DOM6 | 12 | 🟢 INFO | new domain candidate auto-detected |
...
| F4 | 4 | 🟡 ADVISORY | committed cache/build/artifact present (bloat) |
| F9 | 10 | 🟡 ADVISORY | repo-root note outside allow-list / banned dir |
| FE3 | 3 | 🟡 ADVISORY | frontend flat folder scaling warning |
| FE6 | 44 | 🟡 ADVISORY | frontend console/debugger statement left in source |
...
| MV1 | 17 | 🟡 ADVISORY | flat layer file should be moved into its detected domain folder |
...
| PERF4 | 24 | 🟡 ADVISORY | unbounded query detected (no limit clause) |
| PF1 | 1 | 🟢 INFO | required project file missing |
| PF2 | 7 | 🟡 ADVISORY | required scope document missing |
| Q1 | 17 | 🟡 ADVISORY | controller/router reads via db.query (delegate) |
| QUAL1 | 38 | 🟡 ADVISORY | weak exception handling (bare except / swallowed exception) |
...
| REG1 | 4 | 🟢 INFO | domain missing from architecture registry |
...
|---|---|---|---|---|---|
| 🔴 | DG3 | backend | `backend\services\write_helpers.py:7` | cross-domain import write_helpers -> core violates explicit ownership rules | declare allowed imports in layer_rules.yaml or route via core service facade |
...
| 🟡 | API2 | backend | `backend\controllers\country\country_controller.py:93` | private symbol '_require_country_access' used in 3 external module(s) | make it public (remove _) or keep internal and refactor external usages |
...
| 🟡 | BC1 | backend | `backend\services\write_helpers.py:7` | cross-domain import write_helpers → core bypasses event/facade boundary | route through events/ or a core service facade; declare in layer_rules.yaml if intentional |
...
| 🟡 | DG2 | backend | `backend\models\__init__.py` | circular module dependency: models -> models._exports -> models.country.country_economics -> models.country -> models | break the cycle by extracting shared logic into a lower layer (utils/service interface) |
...
| 🟡 | DG2 | backend | `backend\models\__init__.py` | circular module dependency: models -> models._exports -> models.logistics.imports -> models.logistics -> models | break the cycle by extracting shared logic into a lower layer (utils/service interface) |
...
| 🟡 | DOM2 | controllers | `backend/controllers/` | 2 file(s) are in the wrong backend/controllers/ sub-folder; detected domain: 'treasury' | mkdir -p backend/controllers/treasury; move: backend\controllers\finance\treasury_query_controller.py, backend\controllers\supplier\payouts.py (detected from treasury) |
...
| 🟢 | DOM6 | backend | `backend/services|models/cash` | new domain candidate auto-detected: 'cash' | create backend/<layer>/cash/ and group related files; or merge into nearest existing domain if this is not a real bounded context. Examples: backend\services\cash_management_service.py, backend\services\cash_write_service.py |
...
| 🟢 | DOM6 | backend | `backend/services|models/ledger` | new domain candidate auto-detected: 'ledger' | create backend/<layer>/ledger/ and group related files; or merge into nearest existing domain if this is not a real bounded context. Examples: backend\controllers\sub_ledger_controller.py, backend\services\general_ledger_service.py |
...
| 🟢 | DOM6 | backend | `backend/services|models/service` | new domain candidate auto-detected: 'service' | create backend/<layer>/service/ and group related files; or merge into nearest existing domain if this is not a real bounded context. Examples: backend\services\bank_transaction_service.py, backend\services\cash_management_service.py, backend\services\cash_write_service.py, backend\services\commission_write_service.py, backend\services\country_read_service.py, backend\services\credit_control_service.py, backend\services\customer\customer_health_service.py, backend\services\customer\customer_router_service.py |
| 🟢 | DOM6 | backend | `backend/services|models/write` | new domain candidate auto-detected: 'write' | create backend/<layer>/write/ and group related files; or merge into nearest existing domain if this is not a real bounded context. Examples: backend\services\cash_write_service.py, backend\services\commission_write_service.py, backend\services\supplier\suppliers_write_service.py, backend\services\write_helpers.py |
...
| 🟢 | MET1 | repo | `architecture-debt` | architecture debt score = 19799 | track this number down over time; lower is healthier |
...
| 🟡 | MV1 | services | `backend/services/` | 3 'finance' domain file(s) at backend/services/ root should be moved to backend/services/finance/ | mkdir -p backend/services/finance; move: backend\services\commission_write_service.py, backend\services\general_ledger_service.py, backend\services\refund_posting_service.py (detected from commission) |
...
| 🟡 | MV1 | services | `backend/services/` | 4 'treasury' domain file(s) at backend/services/ root should be moved to backend/services/treasury/ | mkdir -p backend/services/treasury; move: backend\services\auto_payout_scheduler.py, backend\services\bank_transaction_service.py, backend\services\cash_management_service.py, backend\services\cash_write_service.py (detected from payout) |
...
| 🟡 | PERF4 | services | `backend\services\finance\je_reversal_service.py:53` | unbounded query: .all() without .limit() | add .limit() to prevent loading entire tables into memory |
...
| 🟢 | REG1 | backend | `domain:cash_write_service` | domain 'cash_write_service' exists in code but not in architecture registry | add 'cash_write_service' to domains.yaml registry |
...
| 🟢 | REG1 | backend | `domain:general_ledger_service` | domain 'general_ledger_service' exists in code but not in architecture registry | add 'general_ledger_service' to domains.yaml registry |
| 🟢 | REG1 | backend | `domain:refund_posting_service` | domain 'refund_posting_service' exists in code but not in architecture registry | add 'refund_posting_service' to domains.yaml registry |
...
| 🟡 | SYM2 | backend | `services.general_ledger_service:60, services.treasury.treasury_service:119, services.finance.treasury_query_service:101, routers.api_finance_domain:68, routers.api_finance_routes:98` | public function 'create_journal_entry' defined in 7 modules | consolidate or rename to avoid confusion |
...
| 🟡 | W4 | backend | `backend\controllers\supplier\__init__.py:39` | controller imports another controller ('controllers.supplier.payouts') | extract shared logic into a service or util; controllers stay thin |
...
- 🟢 **I1** `.` — backend models=51 routers=152 controllers=83 services=283 middleware=22
- 🟢 **I2** `documents/scope/` — rules loaded from: YAML policy (documents/scope/ or governance/)
- 🟢 **I3** `backend/` — module graph: modules=919, edges=2510, classes=1163
- 🟢 **MET1** `architecture-debt` — architecture debt score = 19799 → *track this number down over time; lower is healthier*

### BACKEND (627 findings)
...
- 🟢 **DOM8** `backend/` — 203 scanned file(s) are already in the correct domain folder → *keep these placements; do not move them*
- 🟢 **DOM6** `backend/services|models/badge` — new domain candidate auto-detected: 'badge' → *create backend/<layer>/badge/ and group related files; or merge into nearest existing domain if this is not a real bounded context. Examples: backend\controllers\supplier\badge.py, backend\services\supplier\supplier_badge_service.py*
- 🟢 **DOM6** `backend/services|models/cash` — new domain candidate auto-detected: 'cash' → *create backend/<layer>/cash/ and group related files; or merge into nearest existing domain if this is not a real bounded context. Examples: backend\services\cash_management_service.py, backend\services\cash_write_service.py*
...
- 🟡 **W4** `backend\controllers\supplier\__init__.py:39` — controller imports another controller ('controllers.supplier.payouts') → *extract shared logic into a service or util; controllers stay thin*
...
- 🔴 **DG3** `backend\services\write_helpers.py:7` — cross-domain import write_helpers -> core violates explicit ownership rules → *declare allowed imports in layer_rules.yaml or route via core service facade*
...
- 🟡 **DG2** `backend\models\__init__.py` — circular module dependency: models -> models._exports -> models.country.country_economics -> models.country -> models → *break the cycle by extracting shared logic into a lower layer (utils/service interface)*
...
- 🟡 **DG2** `backend\models\__init__.py` — circular module dependency: models -> models._exports -> models.logistics.imports -> models.logistics -> models → *break the cycle by extracting shared logic into a lower layer (utils/service interface)*
...
- 🟡 **SYM2** `services.general_ledger_service:60, services.treasury.treasury_service:119, services.finance.treasury_query_service:101, routers.api_finance_domain:68, routers.api_finance_routes:98` — public function 'create_journal_entry' defined in 7 modules → *consolidate or rename to avoid confusion*
...
- 🟡 **API2** `backend\controllers\country\country_controller.py:93` — private symbol '_require_country_access' used in 3 external module(s) → *make it public (remove _) or keep internal and refactor external usages*
...
- 🟡 **BC1** `backend\services\write_helpers.py:7` — cross-domain import write_helpers → core bypasses event/facade boundary → *route through events/ or a core service facade; declare in layer_rules.yaml if intentional*
...
- 🟢 **REG1** `domain:cash_write_service` — domain 'cash_write_service' exists in code but not in architecture registry → *add 'cash_write_service' to domains.yaml registry*
...
- 🟢 **REG1** `domain:general_ledger_service` — domain 'general_ledger_service' exists in code but not in architecture registry → *add 'general_ledger_service' to domains.yaml registry*
- 🟢 **REG1** `domain:refund_posting_service` — domain 'refund_posting_service' exists in code but not in architecture registry → *add 'refund_posting_service' to domains.yaml registry*
...
- 🟡 **DOM2** `backend/controllers/` — 2 file(s) are in the wrong backend/controllers/ sub-folder; detected domain: 'treasury' → *mkdir -p backend/controllers/treasury; move: backend\controllers\finance\treasury_query_controller.py, backend\controllers\supplier\payouts.py (detected from treasury)*
...
- 🟡 **MV1** `backend/services/` — 3 'finance' domain file(s) at backend/services/ root should be moved to backend/services/finance/ → *mkdir -p backend/services/finance; move: backend\services\commission_write_service.py, backend\services\general_ledger_service.py, backend\services\refund_posting_service.py (detected from commission)*
...
- 🟡 **MV1** `backend/services/` — 4 'treasury' domain file(s) at backend/services/ root should be moved to backend/services/treasury/ → *mkdir -p backend/services/treasury; move: backend\services\auto_payout_scheduler.py, backend\services\bank_transaction_service.py, backend\services\cash_management_service.py, backend\services\cash_write_service.py (detected from payout)*
...
- 🟡 **PERF4** `backend\services\finance\je_reversal_service.py:53` — unbounded query: .all() without .limit() → *add .limit() to prevent loading entire tables into memory*
...
| 4 | `backend\services\cash_management_service.py` | `backend/services/treasury/cash_management_service.py` | domain | 55% |
| 5 | `backend\services\cash_write_service.py` | `backend/services/treasury/cash_write_service.py` | domain | 86% |
...
| 10 | `backend\services\country_read_service.py` | `backend/services/geography/country_read_service.py` | domain | 86% |
| 11 | `backend\services\customer\customer_router_service.py` | `backend/services/commerce/customer_router_service.py` | domain | 94% |
| 12 | `backend\services\finance\automation_read_service.py` | `backend/services/ai/automation_read_service.py` | domain | 86% |
| 13 | `backend\services\finance\payouts_write_service.py` | `backend/services/treasury/payouts_write_service.py` | domain | 86% |
| 14 | `backend\services\general_ledger_service.py` | `backend/services/finance/general_ledger_service.py` | domain | 97% |
| 15 | `backend\services\orders\cart_shipping_service.py` | `backend/services/logistics/cart_shipping_service.py` | domain | 74% |
| 16 | `backend\services\orders\order_payment_functions.py` | `backend/services/finance/order_payment_functions.py` | domain | 67% |
| 17 | `backend\services\refund_posting_service.py` | `backend/services/finance/refund_posting_service.py` | domain | 92% |
| 18 | `backend\services\supplier\supplier_finance_service.py` | `backend/services/finance/supplier_finance_service.py` | domain | 86% |
| 19 | `backend\services\supplier\supplier_payouts_service.py` | `backend/services/treasury/supplier_payouts_service.py` | domain | 86% |
| 20 | `backend\services\supplier\supplier_profile_service.py` | `backend/services/customer/supplier_profile_service.py` | domain | 86% |
| 21 | `backend\services\video_conferencing.py` | `backend/services/comms/video_conferencing.py` | domain | 74% |
| 22 | `backend\models\_exports.py` | `backend/models/geography/_exports.py` | domain | 59% |
| 23 | `backend\controllers\admin_controller.py` | `backend/controllers/security/admin_controller.py` | domain | 53% |
| 24 | `backend\controllers\banner_controller.py` | `backend/controllers/core/banner_controller.py` | domain | 93% |
| 25 | `backend\controllers\cart_controller.py` | `backend/controllers/orders/cart_controller.py` | domain | 93% |
| 26 | `backend\controllers\chatbot_controller.py` | `backend/controllers/ai/chatbot_controller.py` | domain | 91% |
| 27 | `backend\controllers\comm_controller.py` | `backend/controllers/comms/comm_controller.py` | domain | 93% |
| 28 | `backend\controllers\coupons_controller.py` | `backend/controllers/commerce/coupons_controller.py` | domain | 93% |
| 29 | `backend\controllers\disputes_controller.py` | `backend/controllers/orders/disputes_controller.py` | domain | 93% |
| 30 | `backend\controllers\finance\treasury_query_controller.py` | `backend/controllers/treasury/treasury_query_controller.py` | domain | 67% |
| 31 | `backend\controllers\flash_sale_controller.py` | `backend/controllers/catalog/flash_sale_controller.py` | domain | 62% |
| 32 | `backend\controllers\logistics_partner_controller.py` | `backend/controllers/logistics/logistics_partner_controller.py` | domain | 91% |
| 33 | `backend\controllers\product_verification_controller.py` | `backend/controllers/catalog/product_verification_controller.py` | domain | 96% |
| 34 | `backend\controllers\products_controller.py` | `backend/controllers/catalog/products_controller.py` | domain | 96% |
| 35 | `backend\controllers\promotion_controller.py` | `backend/controllers/commerce/promotion_controller.py` | domain | 93% |
| 36 | `backend\controllers\returns_controller.py` | `backend/controllers/orders/returns_controller.py` | domain | 93% |
| 37 | `backend\controllers\search_controller.py` | `backend/controllers/catalog/search_controller.py` | domain | 93% |
| 38 | `backend\controllers\sub_ledger_controller.py` | `backend/controllers/finance/sub_ledger_controller.py` | domain | 93% |
| 39 | `backend\controllers\supplier\payouts.py` | `backend/controllers/treasury/payouts.py` | domain | 86% |
| 40 | `backend\providers\bg_remover.py` | `backend/providers/media/bg_remover.py` | domain | 89% |
| 41 | `backend\providers\catalog\text.py` | `backend/providers/ai/text.py` | domain | 91% |
| 42 | `backend\providers\image.py` | `backend/providers/media/image.py` | domain | 93% |
| 43 | `backend\providers\ocr.py` | `backend/providers/ai/ocr.py` | domain | 91% |
| 44 | `backend\providers\vision.py` | `backend/providers/ai/vision.py` | domain | 91% |
| 45 | `backend\providers\voice_to_text.py` | `backend/providers/ai/voice_to_text.py` | domain | 95% |
| 46 | `backend\_fix_syntax.py` | `backend/utils/_fix_syntax.py` | backend-root | 60% |
| 47 | `backend\check_app.py` | `backend/utils/check_app.py` | backend-root | 60% |
...
- **Architecture Debt Score:** 19799
- **Modules scanned:** 919
- **Dependency edges:** 2510
- **Classes found:** 1163
- **Layer counts:** `_fix_syntax=1`, `_triage=1`, `alembic=33`, `check_app=1`, `controllers=83`, `data=84`, `db=10`, `dependencies=3`, `events=1`, `lifespan=1`, `main=1`, `middleware=22`, `models=51`, `providers=44`, `routers=152`, `run_server=1`, `scripts=3`, `services=283`, `tests=74`, `utils=70`
