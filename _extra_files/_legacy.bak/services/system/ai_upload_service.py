"""AI upload pipeline service (Phase 4).

Holds the durable, resumable, audited flow that was previously inline in
``routers/system_ai_upload.py``:

    create_ai_upload_job  -> create job (pending) + save media
    get_ai_upload_job      -> inspect job + staging + logs
    publish_ai_upload_job  -> commit reviewed staging -> products/variants
    cancel_ai_upload_job   -> mark job cancelled

The worker ``process_ai_upload_job`` runs outside the request context (RLS
restricted flag is False, so it can read/write across the job's country).
"""
...
def _save_upload(file, job_dir: str) -> tuple[str, str, bytes]:
...
def create_ai_upload_job(
    ...
    """Create a pending job, persist uploaded media, return a summary dict."""
    job = AIUploadJob(
...
def get_ai_upload_job(db: Session, job_id: int) -> dict:
    """Return job + staging + logs as a serializable dict."""
    job = db.get(AIUploadJob, job_id)
    if job is None:
...
def publish_ai_upload_job(db: Session, job_id: int, overrides: Optional[dict] = None) -> dict:
    """Commit reviewed staging into products/variants. Returns a summary dict."""
    job = db.get(AIUploadJob, job_id)
    if job is None:
        raise HTTPException(status_code=404, detail="Job not found.")
...
def cancel_ai_upload_job(db: Session, job_id: int) -> dict:
    """Mark a job cancelled. Returns a summary dict."""
    job = db.get(AIUploadJob, job_id)
    if job is None:
        raise HTTPException(status_code=404, detail="Job not found.")
