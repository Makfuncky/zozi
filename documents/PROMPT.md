...
# ZOZI Prompt Archive (Audit + Build Workstreams)

This is the canonical archive of prompts used to audit, plan, and construct features in the ZOZI project.

- Keep history: do **not** delete older prompt sections; add new prompts under a dated heading.
- Convert prompts into specs under `documents/02-FEATURES/` (and link them back from here).
- Before implementing, always cross-reference:
    - `documents/CODEBASE_STATUS_MATRIX_DETAILED.md`
    - `documents/CODEBASE_FILE_INDEX.md`
    - `documents/DESIGN_SYSTEM.md`

## New prompt template (copy/paste)
...
1. Admin Panel -> Countries Page layout is must be change to be understandable and manageable like a ledger.
  a.  list of all the countries and cities needed as dropdown
  b. New country system is not correctly integrated and should be come into same ledger when have to add new country there should be new form should to come.
  c. automatically should to the currency, country code, Tax rate, cities and legals rules by searching the web by itself. means completely automatic.
  d. commision system you integrate into countries page which is good, but integrate properly and give all the list of catagory and complete system.
  e. I am thinking to integrate one own made payment gateway for all countries which will make easier. need your comment on it.
  f. give properly editable and addable row which will expand and collapse of the table of the countries.
  g. payout system will be handle category to category and product to product and countries page will have initial rules and in country head who is handling will make changes which will be 'country-head' the employee from the 2nd layer which will come from Finance. the Finance Page and Admin Role page will integrate this later properly.
  h. promotion, finance, discount, banner, email, order, product, user, dispute, ticket and other all function will be handle by 'country-manager' separately for better operation. but admin can see everything, and according to the admin can assign the roles which is more working according to this system.
2. do the complete test, do the playwright test, integrate backend and frontend and database properly.
3. read again and make changes
