...
interface SearchBarProps {
  ...
  inputStyle?: object;
  theme?: {
    colors?: { surface2?: string; text?: string; textMuted?: string };
...
export default function SearchBar({
  ...
  return (
    ...
    >
      {leftComponent ? (
        <View style={[styles.sideSlot]}>{leftComponent}</View>
      ) : null}

      <View style={[styles.container, { backgroundColor: bgColor }, containerStyle]}>
        <Text style={[styles.icon, { color: placeholderColor, fontSize: size }]}>🔍</Text>
        <TextInput
          value={value}
      ...
      {rightComponent ? (
        <View style={[styles.sideSlot]}>{rightComponent}</View>
      ) : null}
    </View>
  );
}
...
const styles = StyleSheet.create({
  ...
  sideSlot: {
    // reserved space for compact buttons next to the input
    minWidth: 44,
    alignItems: "center",
