...
/* ── Celebration / season background effect layer ──────────────────────────
   ...
   Pure CSS animations; rules live in styles/banner-effects.css. ───────────── */
...
/* Banner effect styles (keyframes + .bcu-* classes) live in ./banner-effects.css,
   imported above — keeps presentation in the design system (DS02). */
                  ...
                  <Fragment key={el.id}>
                    ...
                    {!preview && selectedIds.includes(el.id) && selectedEls.length === 1 && (
                      <div
                        ...
                        style={{ left: `${el.x}%`, top: `${el.y}%`, width: `${el.w}%`, height: `${el.h}%`, zIndex: "var(--z-canvas)" }}
                      >
                        {HANDLES.map((h) => (
                          <span
                            key={h}
                            onPointerDown={(e) => onPointerDownHandle(e, el, h)}
                            className="absolute z-[var(--z-canvas)] h-3 w-3 rounded-full border border-primary bg-white shadow"
