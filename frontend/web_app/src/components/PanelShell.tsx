          ...
          <nav className="flex-1 overflow-y-auto px-3 py-2">
            {sections.map((section) => (
              <div key={section.key} className="mb-3 last:mb-0">
                <p className="px-2.5 pb-1 text-xs font-semibold uppercase tracking-[0.18em] text-text-faint">
                  {section.label}
                </p>
      ...
      >
            ...
            <div className="mt-1 rounded-lg border border-border bg-surface shadow-lg overflow-hidden">
              {filteredNav.slice(0, 5).map((item) => (
                <Link key={item.href} href={item.href} onClick={() => setNavSearch("")}
                  ...
                  <span className="text-text-faint ml-auto text-xs">{item.desc.slice(0, 30)}</span>
                </Link>
              ))}
            </div>
          )}
        ...
        <nav className="flex-1 overflow-y-auto p-2 space-y-1">
          {sections.map((section) => (
            <div key={section.key} className="mb-3 last:mb-0">
              {!collapsed && (
                <p className="px-2.5 pb-1 text-xs font-semibold uppercase tracking-[0.2em] text-text-faint">
                  {section.label}
                </p>
              )}
                ...
                <div className="min-w-0">
                  {headerMode === "default" && (
                    <div className="flex flex-wrap items-center gap-1.5">
                      <span className="rounded-full border px-2 py-0.5 text-3xs font-semibold uppercase tracking-[0.16em]"
                        style={{ borderColor: "var(--color-border)", backgroundColor: "var(--color-glass-mid)" }}>
                        {panelBadgeLabel}
                      </span>
                      {activeSection && (
                        <span className="rounded-full border border-border bg-surface-1 px-2 py-0.5 text-3xs font-semibold uppercase tracking-[0.16em] text-text-faint">
                          {activeSection.label}
                        </span>
                      )}
                    </div>
                  ...
                  <div className="hidden items-center gap-1.5 lg:flex">
                    {quickLinks.map((item) => (
                      <Link key={item.href} href={item.href} prefetch
                        className="theme-panel inline-flex items-center gap-1.5 rounded-full border px-2.5 py-1 text-xs font-semibold
