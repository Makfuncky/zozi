...
export default function PerformanceTab({ employees, addToast }: PerformanceTabProps) {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      const [okrRes, riskRes] = await Promise.allSettled([
        apiFetch("/hr/kpi/employee/0"),  // Get KPI dashboard (falls back if employee ID needed)
        apiFetch("/hr/health/0"),         // Get health score
      ]);
  ...
  const handleCreateOkr = async () => {
    ...
    try {
      const res = await apiFetch("/hr/okr", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          owner_employee_id: parseInt(okrForm.employee_id),
          title: okrForm.title,
          description: okrForm.description,
          cascade_level: "individual",
          key_results: [],
        }),
      });
      if (res.ok) {
