...
const CATEGORY_COLORS: Record<string, string> = {
  ...
  furniture: "from-success to-brand",
    ...
    <div className="min-h-screen bg-(--color-bg)">
      ...
      <main className="max-w-11xl mx-auto px-4 sm:px-6 pb-20">
        ...
        <section className="py-12 md:py-16 text-center">
          ...
          >
            <span className="inline-flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-[0.3em] text-primary bg-brand/10 border border-brand/20 px-3 py-1 rounded-full mb-4">
              <Sparkles className="w-3 h-3" />
              GCC&apos;s Premier Marketplace
            </span>

            <h1 className="text-4xl sm:text-5xl md:text-7xl font-bold text-(--color-text) tracking-tight leading-[0.92] mb-5">
              THE{" "}
              <span className="italic font-heading theme-hero-gradient">MARKETPLACE.</span>
            </h1>

            <p className="text-sm md:text-base text-(--color-text-muted) max-w-xl mx-auto mb-8 leading-relaxed">
            ...
            <div className="flex flex-col sm:flex-row gap-3 justify-center items-center mb-4">
              <Link
                href="/products"
                className="px-6 py-3 bg-primary text-on-brand text-sm font-bold rounded-2xl hover:bg-primary-dark transition-all flex items-center gap-2 shadow-lg shadow-brand/25"
              >
              ...
              <Link
                href="/offers"
                className="px-6 py-3 border border-(--color-border-light) text-(--color-text-muted) text-sm font-bold rounded-2xl hover:border-primary hover:text-primary transition-all flex items-center gap-2"
              >
            ...
            <p className="text-[10px] text-(--color-text-faint) uppercase tracking-widest">
        ...
        <section className="mb-12">
          <div className="flex items-center justify-between mb-5">
            <div>
              <h2 className="text-xl font-bold text-(--color-text)">Shop by Category</h2>
              <p className="text-xs text-(--color-text-faint) mt-0.5">Browse our full collection</p>
            </div>
            <Link
              href="/products"
              ...
              return (
                ...
                >
                  <Link
                    ...
                    className="flex flex-col items-center gap-2 p-4 rounded-2xl border border-(--color-border-light) bg-(--color-surface) hover:border-primary/50 hover:bg-(--color-surface-2) transition-all group"
                  >
                    ...
                    <span className="text-[10px] font-semibold text-(--color-text-muted) group-hover:text-(--color-text) transition-colors text-center leading-tight">
                      {cat.name}
                    </span>
                  </Link>
                </motion.div>
          ...
          <section className="mb-12">
            <div className="flex items-center justify-between mb-5">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <h2 className="text-xl font-bold text-(--color-text)">Trending Now</h2>
                  <p className="text-xs text-(--color-text-faint) mt-0.5">Top-rated this week</p>
                </div>
              </div>
              <Link
        ...
        <section className="mb-12 grid grid-cols-2 md:grid-cols-4 gap-3">
          {[
            ...
            <div
              key={title}
              className="flex flex-col gap-2 p-4 rounded-2xl border border-(--color-border-light) bg-(--color-surface)"
            >
              <Icon className="w-5 h-5 text-primary" />
              <p className="text-xs font-bold text-(--color-text)">{title}</p>
              <p className="text-[11px] text-(--color-text-faint)">{desc}</p>
            </div>
          ))}
        </section>
        ...
        <section className="mb-12">
          <div className="flex items-center justify-between mb-5">
            <div>
              <h2 className="text-xl font-bold text-(--color-text)">New Arrivals</h2>
              <p className="text-xs text-(--color-text-faint) mt-0.5">Latest additions to the marketplace</p>
            </div>
          </div>
        ...
        <section className="mb-12">
          <motion.div
            ...
            className="relative overflow-hidden rounded-3xl bg-linear-to-br from-primary via-primary-dark to-accent p-8 md:p-12 text-center"
          >
        ...
        <section className="mb-4">
          <div className="rounded-2xl border border-(--color-border-light) bg-(--color-surface) p-8 text-center">
            <h3 className="text-lg font-bold text-(--color-text) mb-2">Stay in the loop</h3>
            ...
            {subscribed ? (
              ...
              <form onSubmit={handleSubscribe} className="flex flex-col sm:flex-row gap-2 max-w-sm mx-auto">
                <input
                  ...
                  required
                  className="flex-1 px-4 py-2.5 rounded-xl text-sm bg-(--color-surface-2) border border-(--color-border-light) text-(--color-text) placeholder:text-(--color-text-faint) focus:outline-none focus:border-primary"
                />
