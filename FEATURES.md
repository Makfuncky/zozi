...
everything must be country to country sepearate and Admin manageable completely.

1. referral program and earn ZOZI coin (discount) for more audience to convert into customer.
2. if you buy more then 15 OMR you will get 500 ZOZI coin which you can claim into next order which makes them to buy next time also and according to that repeat order will come.

Designing a promotion system around **ZOZI coins** and the **OMR** currency is a great strategy. Your initial ideas (Referral and Spend-Threshold Rewards) are excellent foundations for acquisition and retention. 

To drive **faster engagement**, create urgency, and increase the average order value, here are additional, highly effective promotion system options categorized by the psychological trigger they use:

### ⚡ 1. Instant Engagement & Urgency (To Convert Faster)
*   **Gamified "Spin & Win" Onboarding:** Instead of a static "10% off" code, give new users a daily "Spin the Wheel" chance upon downloading the app or signing up. They can win anywhere from 100 to 1,000 ZOZI coins. This creates instant dopamine, hooks them immediately, and gets them to the checkout page faster.
*   **"Flash Coin" Happy Hours:** Announce random 2-hour "Happy Hours" via push notifications (e.g., "Double ZOZI coins on all Groceries from 2 PM to 4 PM!"). This trains users to check the Zozi app frequently and creates massive FOMO (Fear Of Missing Out) for quick conversions.
*   **Cart Abandonment "Coin Rescue":** If a user leaves items in their cart, trigger an automated push notification or WhatsApp message 1 hour later: *"Your cart is expiring! Complete your purchase in the next 30 minutes and we’ll instantly drop 150 bonus ZOZI coins into your wallet."*

### 🔁 2. Habit Building & Retention (To Keep Them Coming Back)
*   **Daily Check-In Streaks:** Reward users with small amounts of ZOZI coins (e.g., 20 coins/day) just for opening the app and clicking "Check-In." If they maintain a 7-day streak, give them a massive 500-coin bonus. This builds a daily habit of opening Zozi.
*   **Tiered VIP Status (Zozi Elite):** Create loyalty tiers (e.g., Bronze, Silver, Gold) based on lifetime OMR spent. 
    *   *Bronze:* Earn 1x ZOZI coins.
    *   *Silver (Spend > 50 OMR):* Earn 1.5x ZOZI coins + Free Shipping on orders over 10 OMR.
    *   *Gold (Spend > 150 OMR):* Earn 2x ZOZI coins + Early access to sales. 
    *   *Why it works:* Customers will artificially inflate their basket size just to hit the next VIP tier for the bragging rights and better coin-earning rates.
*   **Post-Purchase "Scratch Cards":** Immediately after a successful order, give the user a digital "Scratch Card" in the app. They scratch it to reveal ZOZI coins that *must be used within 48 hours*. This drastically speeds up the time between their first and second order.

### 📣 3. Social & Viral Growth (To Expand the Audience Organically)
*   **"Zozi Squad" Group Buying:** Oman has a heavy WhatsApp culture. Allow a user to generate a "Squad Link" for a product. If they get 2 friends to buy the same item via their link within 24 hours, the whole squad gets a 15% discount + 1,000 ZOZI coins each. This turns your current customers into your marketing team.
*   **Social Media Unboxing Bounties:** Offer 200 ZOZI coins if a customer records a quick unboxing video, posts it on their Instagram/Snapchat/TikTok story, tags Zozi, and sends a screenshot to customer support. This generates massive social proof for new audiences.
*   **User-Generated Content (UGC) Reviews:** Don't just ask for reviews; pay for them. Offer 50 ZOZI coins for a text review, but **200 ZOZI coins if they upload a photo or video of the product**. Photo reviews drastically increase conversion rates for future customers.

### 🛒 4. Increasing Basket Size (Upselling & Cross-selling)
*   **Category-Specific Coin Boosts:** If you want to push slow-moving inventory, use dynamic coin multipliers. For example: *"Add any 2 items from the Electronics category to your cart and get a 500 ZOZI coin bonus."*
*   **Mystery Coin Bundles:** At checkout, offer an upsell: *"Add a 1 OMR Mystery Coin Box to your order!"* The customer pays 1 OMR but is guaranteed to receive at least 150 ZOZI coins back in their wallet, with a 5% chance to win a "Jackpot" of 5,000 coins. 
*   **"Bundle & Boost" Deals:** "Buy a Shampoo and Conditioner together, and get 300 ZOZI coins." This forces customers to buy complementary items rather than just single products.

### 💡 Pro-Tips for Managing the ZOZI Coin Economy:
*   **Set an Expiry Date:** ZOZI coins should **never** last forever. Give them a 60-day or 90-day expiry. Send an automated email/push notification: *"You have 450 ZOZI coins expiring in 3 days! Use them now before they vanish."* This forces repeat purchases.
*   **Clear Exchange Rate:** Make sure the customer easily understands the value. For example, clearly display: *1000 ZOZI Coins = 1 OMR* (or whatever your margin allows) right on the checkout page so they know exactly how much they are saving.
*   **Partial Redemptions:** Allow users to pay "Part Cash, Part ZOZI". If an item is 5 OMR, let them pay 4.5 OMR in cash and use 500 ZOZI coins to cover the remaining 0.5 OMR. This makes them feel like they are getting a "discount" while still spending cash.
...
- **Admin Promotion Builder panel** in web admin (`/admin/promotions?section=builder`) with:
  ...
  - promotion rules set by the Admin and every rule will convert into coin means discount 
...
### Order Tier Discounts (Your thresholds tuned) - but this one set by the admin so keep it open to rule set and it can be vary according to the country.
...
┌──────────────────────────┐   ┌───────────────┬──────────────────────────┬──────────────┐
│ Search + voice + chips   │   │ FILTER RAIL   │ Banner + Search + voice  │ RIGHT RAIL   │
│ Banner (auto-scheduled)  │   │ price hist    ├──────────────────────────┤ related vids │
...
# _____________________________________________________________________________________________ Inbox 
Based on the ZOZI codebase, here are examples using actual project domains:

```markdown
Surface: Admin, Employee, Customer, Supplier, Logistic, 
Domain: Treasury, 
...
```markdown
Domain: Logistics
Feature: Partner Service Areas
Module: logistics
```

```markdown
Domain: Users
Feature: Admin Identity
Module: core
```
...
The existing `_progress` files use this pattern implicitly — the file title states the domain (e.g., `TREASURY_REMEDIATION_PLAN.md` → Domain: Treasury), and the header contains **Module:** with the technical module name. `Feature` would be the specific capability being worked on within that domain.
