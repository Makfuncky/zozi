"""Q1 rescue test — admin_payouts router.
...
_MODULE = "routers.admin_payouts_governance"
_ROUTER_FILE = _BACKEND_ROOT / "routers" / "admin_payouts_governance.py"
...
def test_router_has_no_session_reads() -> None:
    ...
    assert not violations, f"admin_payouts still reads via db: {violations}"
...
def test_router_delegates_to_db_read() -> None:
    ...
    assert imported, "admin_payouts does not import from services.db_read"
