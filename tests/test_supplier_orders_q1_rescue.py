"""Q1 rescue test — routers/supplier_orders.py.
...
_MODULE = "routers.supplier_orders"
_ROUTER_FILE = _BACKEND_ROOT / "routers" / "supplier_orders.py"
...
def test_router_delegates_to_db_read() -> None:
    ...
    assert {"all_rows", "first", "rows", "scalars"} <= imported


def test_shared_lookup_helpers_exist() -> None:
    """The de-duplicated supplier/order lookups stay in the router layer."""
    mod = importlib.import_module(_MODULE)
    assert callable(getattr(mod, "_supplier_for_user", None))
    assert callable(getattr(mod, "_supplier_order", None))


def test_db_read_helpers_are_callable() -> None:
    db_read = importlib.import_module("services.db_read")
    for name in ("all_rows", "first", "rows", "scalars"):
