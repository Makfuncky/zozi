"""Q1 rescue test — supplier_payouts router.
...
_MODULE = "routers.supplier_payouts"
_ROUTER_FILE = _BACKEND_ROOT / "routers" / "supplier_payouts.py"
...
def test_router_has_no_session_reads() -> None:
    ...
    assert not violations, f"supplier_payouts still reads via db: {violations}"
...
def test_router_delegates_to_db_read() -> None:
    ...
    assert imported, "supplier_payouts does not import from services.db_read"
