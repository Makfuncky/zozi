"""Q1 rescue test — country_maps router.
...
_MODULE = "routers.public_country_maps_access"
_ROUTER_FILE = _BACKEND_ROOT / "routers" / "public_country_maps_access.py"
...
def test_router_has_no_session_reads() -> None:
    ...
    assert not violations, f"country_maps still reads via db: {violations}"
...
def test_router_delegates_to_db_read() -> None:
    ...
    assert imported, "country_maps does not import from services.db_read"
