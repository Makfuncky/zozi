"""Q1 rescue test — finance_automation router.
...
_MODULE = "routers.public_finance_automation_access"
_ROUTER_FILE = _BACKEND_ROOT / "routers" / "public_finance_automation_access.py"
...
def test_router_has_no_session_reads() -> None:
    ...
    assert not violations, f"finance_automation still reads via db: {violations}"
...
def test_router_delegates_to_db_read() -> None:
    ...
    assert imported, "finance_automation does not import from services.db_read"
