"""Q1 rescue test — routers/finance_erp.py.
...
_MODULE = "routers.public_finance_erp_access"
_ROUTER_FILE = _BACKEND_ROOT / "routers" / "public_finance_erp_access.py"
...
def test_router_delegates_to_db_read() -> None:
    ...
    assert {"all_rows", "count", "rows", "scalar", "scalars"} <= imported


def test_db_read_helpers_are_callable() -> None:
    db_read = importlib.import_module("services.db_read")
    for name in ("all_rows", "count", "rows", "scalar", "scalars"):
