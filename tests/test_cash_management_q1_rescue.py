"""Q1 rescue test — cash_management router.
...
_MODULE = "routers.public_cash_management_access"
_ROUTER_FILE = _BACKEND_ROOT / "routers" / "public_cash_management_access.py"
...
def test_router_has_no_session_reads() -> None:
    ...
    assert not violations, f"cash_management still reads via db: {violations}"
...
def test_router_delegates_to_db_read() -> None:
    ...
    assert imported, "cash_management does not import from services.db_read"
