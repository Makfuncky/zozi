"""Q1 rescue test — payout_approval router.
...
_MODULE = "routers.public_payout_approval_access"
_ROUTER_FILE = _BACKEND_ROOT / "routers" / "public_payout_approval_access.py"
...
def test_router_has_no_session_reads() -> None:
    ...
    assert not violations, f"payout_approval still reads via db: {violations}"
...
def test_router_delegates_to_db_read() -> None:
    ...
    assert imported, "payout_approval does not import from services.db_read"
