...
_MODULE = "routers.public_tickets_management"
_ROUTER_FILE = _BACKEND_ROOT / "routers" / "public_tickets_management.py"
...
def test_router_has_no_session_reads() -> None:
    ...
    assert not violations, f"tickets still reads via db: {violations}"
...
def test_router_delegates_to_db_read() -> None:
    ...
    assert imported, "tickets does not import from services.db_read"
