"""Q1 rescue test — routers/admin.py.
...
_MODULE = "routers.admin_core_management"
_ROUTER_FILE = _BACKEND_ROOT / "routers" / "admin_core_management.py"
...
def test_router_delegates_to_db_read() -> None:
    ...
    assert {"all_rows", "count", "first", "rows", "scalar_with_filters"} <= imported


def test_db_read_helpers_are_callable() -> None:
    db_read = importlib.import_module("services.db_read")
    for name in ("all_rows", "count", "first", "rows", "scalar_with_filters"):
