"""Q1 rescue test — ai_upload router.
...
_MODULE = "routers.public_ai_upload_access"
_ROUTER_FILE = _BACKEND_ROOT / "routers" / "public_ai_upload_access.py"
...
def test_router_has_no_session_reads() -> None:
    ...
    assert not violations, f"ai_upload still reads via db: {violations}"
...
def test_router_delegates_to_db_read() -> None:
    ...
    assert imported, "ai_upload does not import from services.db_read"
