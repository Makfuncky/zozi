...
def _coerce_failed_login_entry(entry: object, *, now: float) -> tuple[int, float]:
    if isinstance(entry, tuple) and len(entry) == 2:
        try:
            ...
            logger.exception("_coerce_failed_login_entry_failed", error=str(e))
            count = 0
        try:
...
def _get_redis():
    ...
    try:
        ...
        logger.exception("_get_redis_failed", error=str(e))
        return None
...
def blacklist_token(jti: str, ttl_seconds: int) -> None:
    ...
    if client is not None:
        try:
            ...
            logger.warning('auth.py:91 handler (except Exception) failed; degrading', exc_info=True)

    app_env = os.environ.get("APP_ENV", "").lower()
    if app_env == "production":
        logger.error("Redis unavailable for JWT revocation store in production - revocation may fail")
        raise RuntimeError("Redis unavailable - cannot blacklist token")
    
    logger.warning("Redis unavailable - falling back to in-memory revocation store (not shared across workers)")
...
def is_token_blacklisted(jti: str) -> bool:
    ...
    if client is not None:
        try:
            ...
            logger.warning('auth.py:109 handler (except Exception) failed; degrading', exc_info=True)
...
def record_failed_login(identifier: str) -> int:
    ...
    if client is not None:
        try:
            ...
            logger.warning('auth.py:130 handler (except Exception) failed; degrading', exc_info=True)
...
def is_account_locked(identifier: str) -> bool:
    ...
    if client is not None:
        try:
            ...
            logger.warning('auth.py:153 handler (except Exception) failed; degrading', exc_info=True)
...
def clear_failed_logins(identifier: str) -> None:
    ...
    if client is not None:
        try:
            ...
            logger.warning('auth.py:174 handler (except Exception) failed; degrading', exc_info=True)
    _memory_failed_logins.pop(identifier, None)
...
def verify_password(plain_password: str, hashed_password: str) -> bool:
    try:
        ...
        logger.exception("verify_password_failed", error=str(e))
        return False
...
def mark_refresh_token_used(family_id: str, jti: str) -> None:
    ...
    if client is not None:
        try:
            ...
            logger.warning('auth.py:224 handler (except Exception) failed; degrading', exc_info=True)
...
def is_refresh_token_used(family_id: str, jti: str) -> bool:
    ...
    if client is not None:
        try:
            ...
            logger.warning('auth.py:235 handler (except Exception) failed; degrading', exc_info=True)
...
def revoke_refresh_family(family_id: str) -> None:
    ...
    if client is not None:
        try:
            ...
            logger.warning('auth.py:253 handler (except Exception) failed; degrading', exc_info=True)
...
def is_refresh_family_revoked(family_id: str) -> bool:
    ...
    if client is not None:
        try:
            ...
            logger.warning('auth.py:264 handler (except Exception) failed; degrading', exc_info=True)
...
def verify_temp_token(token: str) -> dict[str, Any]:
    ...
    try:
        ...
        logger.exception("verify_temp_token_failed", error=str(e))
...
def _decode_and_validate(token: str, token_type: str) -> dict[str, Any]:
    try:
        ...
        logger.exception("_decode_and_validate_failed", error=str(exc))
...
def decode_token(token: str) -> dict[str, Any]:
    try:
        ...
        logger.exception("decode_token_failed", error=str(exc))
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Invalid token") from exc
