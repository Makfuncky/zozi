"""Q1 rescue test — admin_commission router.
...
_MODULE = "routers.admin_commission_governance"
_ROUTER_FILE = _BACKEND_ROOT / "routers" / "admin_commission_governance.py"
...
def test_router_has_no_session_reads() -> None:
    ...
    assert not violations, f"admin_commission still reads via db: {violations}"
...
def test_router_delegates_to_db_read() -> None:
    ...
    assert imported, "admin_commission does not import from services.db_read"
