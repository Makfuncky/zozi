"""Q1 rescue test — logistics_orders router.
...
_MODULE = "routers.logistics_orders"
_ROUTER_FILE = _BACKEND_ROOT / "routers" / "logistics_orders.py"
...
def test_router_has_no_session_reads() -> None:
    ...
    assert not violations, f"logistics_orders still reads via db: {violations}"
...
def test_router_delegates_to_db_read() -> None:
    ...
    assert imported, "logistics_orders does not import from services.db_read"
