"""Q1 rescue test — onboarding router.
...
_MODULE = "routers.public_onboarding_access"
_ROUTER_FILE = _BACKEND_ROOT / "routers" / "public_onboarding_access.py"
...
def test_router_has_no_session_reads() -> None:
    ...
    assert not violations, f"onboarding still reads via db: {violations}"
...
def test_router_delegates_to_db_read() -> None:
    ...
    assert imported, "onboarding does not import from services.db_read"
