"""Q1 rescue test — flash_sales router.
...
_MODULE = "routers.public_flash_sales_management"
_ROUTER_FILE = _BACKEND_ROOT / "routers" / "public_flash_sales_management.py"
...
def test_router_has_no_session_reads() -> None:
    ...
    assert not violations, f"flash_sales still reads via db: {violations}"
...
def test_router_delegates_to_db_read() -> None:
    ...
    assert imported, "flash_sales does not import from services.db_read"
