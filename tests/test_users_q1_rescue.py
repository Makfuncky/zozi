...
_MODULE = "routers.public_users_access"
_ROUTER_FILE = _BACKEND_ROOT / "routers" / "public_users_access.py"
...
def test_router_has_no_session_reads() -> None:
    ...
    assert not violations, f"users still reads via db: {violations}"
...
def test_router_delegates_to_db_read() -> None:
    ...
    assert imported, "users does not import from services.db_read"
