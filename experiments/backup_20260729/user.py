...
from sqlalchemy import Boolean, Column, DateTime, ForeignKey, Index, Integer, JSON, String, Text, CheckConstraint
...
class User(Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint("role IN ('customer','supplier','admin','employee','logistics_partner')", name="chk_user_role_valid"),
    )
    ...
    phone = Column(EncryptedString(length=20), nullable=True)
    ...
    staff_permissions = Column(EncryptedString(), nullable=True)
...
class Referral(Base):
    ...
    __table_args__ = (
        CheckConstraint("status IN ('pending','completed','expired','cancelled')", name="chk_referral_status_valid"),
    )
    id = Column(Integer, primary_key=True, index=True)
...
class UserPoints(Base):
    """Current points balance per user — one row per user.

    Loyalty tier is derived from the points balance:
    - Bronze: 0–500 points
    ...
    loyalty_tier = Column(String(20), default="bronze", nullable=False)  # bronze | silver | gold
...
class PointsTransaction(Base):
    """Immutable audit trail for every points movement."""
    __tablename__ = "points_transactions"
    id = Column(Integer, primary_key=True, index=True)
    user_id = Column(Integer, ForeignKey("users.id"), nullable=False, index=True)
    points = Column(Integer, nullable=False)  # positive = earned, negative = redeemed/expired
    transaction_type = Column(String(30), nullable=False)  # earn_order | earn_referral | redeem | expire | adjust
