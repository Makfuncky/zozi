...
class SystemAlert(Base):
    ...
    acknowledged_by = Column(Integer, ForeignKey("users.id"), nullable=True, index=True)
...
class LogisticsCODRemittanceReceipt(Base):
    ...
    settlement_id = Column(Integer, ForeignKey("logistics_settlements.id"), nullable=True, index=True)
    amount = Column(Numeric(12, 2), nullable=False)
