...
celery_app.conf.update(
    ...
    # Task routing
    task_routes={
        "tasks.ai_tasks.*": {"queue": "ml"},
        "tasks.periodic_tasks.*": {"queue": "periodic"},
    ...
    beat_schedule={
        ...
        "auto-payout-sweep": {
            "task": "tasks.periodic_tasks.run_auto_payout_sweep",
            "schedule": 3600.0,  # Every hour
        },
        # Finance reconciliation daily at 2 AM UTC
        "finance-reconciliation": {
            "task": "tasks.periodic_tasks.run_finance_reconciliation",
            "schedule": crontab(hour=2, minute=0),
        },
        # VAT remittance calculation on 1st of month
        "vat-remittance": {
            "task": "tasks.periodic_tasks.compute_vat_remittance",
            "schedule": crontab(day_of_month=1, hour=3, minute=0),
        },
        # Supplier statement generation monthly
        "supplier-statements": {
            "task": "tasks.periodic_tasks.generate_supplier_statements",
            "schedule": crontab(day_of_month=1, hour=4, minute=0),
        },
        # Distributor statement generation monthly
        "distributor-statements": {
            "task": "tasks.periodic_tasks.generate_distributor_statements",
            "schedule": crontab(day_of_month=1, hour=5, minute=0),
        },
        # Alert engine every 30 minutes
        "alert-engine": {
            "task": "tasks.periodic_tasks.run_alert_engine",
            "schedule": 1800.0,  # Every 30 minutes
        },
        # Clean up old background job records daily
        "cleanup-jobs": {
            "task": "tasks.periodic_tasks.cleanup_old_jobs",
            "schedule": crontab(hour=1, minute=0),
        },
        # Clean up expired tokens daily
        "cleanup-tokens": {
            "task": "tasks.periodic_tasks.cleanup_expired_tokens",
    ...
    task_annotations={
        "tasks.ai_tasks.remove_background": {
            "rate_limit": "10/m",
            "time_limit": 300,
            "soft_time_limit": 240,
        },
        "tasks.ai_tasks.analyze_product_image": {
            "rate_limit": "20/m",
            "time_limit": 180,
            "soft_time_limit": 120,
        },
        "tasks.ai_tasks.generate_angles": {
            "rate_limit": "5/m",
            "time_limit": 600,
            "soft_time_limit": 540,
        },
