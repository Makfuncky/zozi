...
@dataclass
class _Br12ProcessingConfig:
    max_rembg_dimension: int = 1024
    max_output_dimension: int = 2048
CONFIG = _Br12ProcessingConfig()

class _Br12MemoryManager:

    @staticmethod
    def cleanup():
        gc.collect()

class _Br12AISegmenter:
    ...
    @classmethod
    def generate_alpha(cls, image_bytes: bytes, orig_size: Tuple[int, int], config: _Br12ProcessingConfig) -> np.ndarray:
        from rembg import remove
        models_to_try = ['birefnet-massive', 'birefnet-hrsod', 'u2net_cloth_seg', 'isnet-general-use']
...
class _Br12FloatingArtifactRemover:
...
class _Br12GlobalBackgroundBleeder:
...
class _Br12Exporter:
    ...
    @staticmethod
    def process_and_save(input_bytes: bytes, output_path: str, config: _Br12ProcessingConfig):
        img = Image.open(io.BytesIO(input_bytes)).convert('RGB')
        orig_size = img.size
        input_np = np.array(img)
        alpha_map = _Br12AISegmenter.generate_alpha(input_bytes, orig_size, config)
        alpha_map = _Br12FloatingArtifactRemover.remove_floating_objects(alpha_map)
        alpha_map = _Br12BottomTextEraser.erase_bottom_text(alpha_map, input_np)
        alpha_map = _Br12EdgeShaver.shave_trailing_edges(alpha_map)
        alpha_map = _Br12GlobalBackgroundBleeder.remove_background_in_gaps(input_np, alpha_map)
...
if __name__ == '__main__':
    ...
    logger.info('🚀 Using specialized models: birefnet-massive, birefnet-hrsod, u2net_cloth_seg\n')
    (success, fail) = (0, 0)
    for (idx, img_path) in enumerate(image_files, 1):
        ...
        try:
            ...
            _Br12Exporter.process_and_save(img_bytes, str(out_path), CONFIG)
