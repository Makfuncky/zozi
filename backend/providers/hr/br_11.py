...
@dataclass
class _Br11ProcessingConfig:
    max_rembg_dimension: int = 1024
    max_output_dimension: int = 2048
CONFIG = _Br11ProcessingConfig()

class _Br11MemoryManager:

    @staticmethod
    def cleanup():
        gc.collect()

class _Br11AISegmenter:
    ...
    @classmethod
    def generate_alpha(cls, image_bytes: bytes, orig_size: Tuple[int, int], config: _Br11ProcessingConfig) -> np.ndarray:
...
class _Br11GlobalBackgroundBleeder:
...
class _Br11Exporter:
    ...
    @staticmethod
    def process_and_save(input_bytes: bytes, output_path: str, config: _Br11ProcessingConfig):
        img = Image.open(io.BytesIO(input_bytes)).convert('RGB')
        orig_size = img.size
        input_np = np.array(img)
        alpha_map = _Br11AISegmenter.generate_alpha(input_bytes, orig_size, config)
        alpha_map = _Br11EdgeShaver.shave_trailing_edges(alpha_map)
        alpha_map = _Br11GlobalBackgroundBleeder.remove_background_in_gaps(input_np, alpha_map)
        alpha_map = _Br11ArtifactIsolator.remove_floating_dust(alpha_map)
...
if __name__ == '__main__':
    ...
    for (idx, img_path) in enumerate(image_files, 1):
        ...
        try:
            ...
            _Br11Exporter.process_and_save(img_bytes, str(out_path), CONFIG)
