...
CONFIG = _Br08ProcessingConfig()

class LegacySubjectCategory(str, Enum):
    PRODUCT = 'product'
    HUMAN = 'human'
    CLOTHING = 'clothing'
    FOOD = 'food'
...
class _FusionStrategy(str, Enum):
    UNION = 'union'

class _Br08ColorSpaceUtils:
    ...
    def detect_skin_regions(image: np.ndarray) -> np.ndarray:
        hsv = _Br08ColorSpaceUtils.rgb_to_hsv(image)
...
class _Br08QualityAnalyzer:

    @staticmethod
    def analyze(image_np: np.ndarray) -> Dict[str, float]:
        gray = _Br08ColorSpaceUtils.rgb_to_gray(image_np)
        laplacian = cv2.Laplacian(gray, cv2.CV_64F)
        return {'texture_complexity': float(np.var(laplacian)), 'brightness': float(np.mean(gray))}

class _Br08SubjectDetector:

    @staticmethod
    def detect(image_np: np.ndarray, initial_mask: np.ndarray) -> Tuple[str, Dict[str, float]]:
        metrics = {}
        try:
            skin_mask = _Br08ColorSpaceUtils.detect_skin_regions(image_np)
            ...
            gray_full = _Br08ColorSpaceUtils.rgb_to_gray(image_np)
            ...
            if metrics.get('skin_ratio', 0) < 0.05:
                subject_type = LegacySubjectCategory.CLOTHING if metrics.get('edge_density', 0) > 0.1 else LegacySubjectCategory.PRODUCT
            elif metrics.get('skin_ratio', 0) > 0.15:
                if metrics.get('edge_density', 0) < 0.15:
                    subject_type = LegacySubjectCategory.HUMAN
                else:
                    subject_type = LegacySubjectCategory.CLOTHING
            else:
                subject_type = LegacySubjectCategory.PRODUCT
...
class _Br08ModelSelector:

    def __init__(self):
        self.model_configs = {LegacySubjectCategory.PRODUCT: {'primary': ['birefnet-general', 'isnet-general-use'], 'fallback': ['u2net', 'silueta']}, LegacySubjectCategory.HUMAN: {'primary': ['birefnet-general', 'isnet-general-use'], 'fallback': ['silueta']}, LegacySubjectCategory.CLOTHING: {'primary': ['birefnet-general', 'isnet-general-use'], 'fallback': ['silueta']}, LegacySubjectCategory.UNKNOWN: {'primary': ['isnet-general-use', 'birefnet-general'], 'fallback': ['u2net', 'silueta']}}

    def select(self, subject_type: str) -> Dict[str, Any]:
        return self.model_configs.get(subject_type, self.model_configs[LegacySubjectCategory.UNKNOWN])

class _Br08MultiModelSegmenter:
    ...
    def _generate_probability_map(self, image_bytes: bytes, model_name: str, orig_size: Tuple[int, int], config: _Br08ProcessingConfig) -> Optional[np.ndarray]:
    ...
    def segment_with_comparison(self, image_bytes: bytes, model_config: Dict[str, Any], orig_size: Tuple[int, int], config: _Br08ProcessingConfig) -> Tuple[np.ndarray, str]:
        ...
        for model_name in available_models[:config.max_models_to_try]:
            ...
            mem_usage = _Br08MemoryManager.get_available_memory_mb()
            if mem_usage < 1024:
...
class _Br08WoodBackgroundRemover:

    @staticmethod
    def remove(image_rgba: np.ndarray, prob_map: np.ndarray) -> np.ndarray:
        gray = _Br08ColorSpaceUtils.rgb_to_gray(image_rgba[:, :, :3])
        ...
        corner_samples = np.concatenate([image_rgba[:corner_size, :corner_size, :3].reshape(-1, 3), image_rgba[:corner_size, -corner_size:, :3].reshape(-1, 3), image_rgba[-corner_size:, :corner_size, :3].reshape(-1, 3), image_rgba[-corner_size:, -corner_size:, :3].reshape(-1, 3)])
...
class _Br08Exporter:
    ...
    @staticmethod
    def create_canvas(final_img: Image.Image, background: str, config: _Br08ProcessingConfig) -> Image.Image:
    ...
    @staticmethod
    def encode(canvas: Image.Image, output_format: str, config: _Br08ProcessingConfig) -> Tuple[bytes, str]:
...
class _Br08ZoziBackgroundRemover:

    def __init__(self, config: _Br08ProcessingConfig=None):
        self.config = config or CONFIG
        self.segmenter = _Br08MultiModelSegmenter()
        self.model_selector = _Br08ModelSelector()
    ...
    def process(self, image_bytes: bytes, background: str='transparent', output_format: str='PNG') -> Dict[str, Any]:
        try:
            ...
            (input_img, input_np) = _Br08ImageLoader.load_from_bytes(image_bytes)
            orig_size = input_img.size
            logger.info(f'✓ Image: {orig_size[0]}x{orig_size[1]}')
            logger.info('\n[STAGE 2] Pre-processing (CLAHE)...')
            input_np_enhanced = _Br08ImageLoader.preprocess_for_segmentation(input_np)
            quality_metrics = _Br08QualityAnalyzer.analyze(input_np)
            logger.info('\n[STAGE 3] Subject detection...')
            initial_prob_map = self.segmenter._generate_probability_map(image_bytes, 'isnet-general-use', orig_size, self.config)
            if initial_prob_map is None:
                return {'error': 'Initial segmentation failed'}
            initial_mask = (initial_prob_map > 0.3).astype(np.uint8) * 255
            (subject_type, subject_metrics) = _Br08SubjectDetector.detect(input_np_enhanced, initial_mask)
            ...
            (fused_prob_map, best_model) = self.segmenter.segment_with_comparison(image_bytes, model_config, orig_size, self.config)
            ...
            final_rgba = _Br08WoodBackgroundRemover.remove(final_rgba, fused_prob_map)
            logger.info('\n[STAGE 8] Hole filling...')
            final_rgba = _Br08HoleFiller.fill(final_rgba, subject_type)
            logger.info('\n[STAGE 9] Exporting...')
            final_img = Image.fromarray(final_rgba, mode='RGBA')
            canvas = _Br08Exporter.create_canvas(final_img, background, self.config)
            if background.lower() == 'transparent':
                output_format = 'PNG'
            (encoded, final_format) = _Br08Exporter.encode(canvas, output_format, self.config)
            ...
            logger.info('=' * 70)
            return {'base64': base64.b64encode(encoded).decode('utf-8'), 'format': final_format, 'width': canvas.width, 'height': canvas.height, 'processing_time_seconds': elapsed, 'subject_type': subject_type, 'best_model': best_model, 'file_size_kb': round(len(encoded) / 1024, 1)}
        finally:
            _Br08MemoryManager.cleanup()

def process_product_image(image_bytes: bytes, background: str='transparent', output_format: str='PNG') -> Dict[str, Any]:
    remover = _Br08ZoziBackgroundRemover()
...
if __name__ == '__main__':
    ...
    logger.info(f'📐 Resolution: {args.resolution}px')
    logger.info(f"🔄 Model comparison: {('Enabled' if CONFIG.enable_model_comparison else 'Disabled')}\n")
    successful = 0
    failed = 0
    for (idx, img_path) in enumerate(image_files, 1):
        ...
        try:
            ...
            logger.info(f"✅ Success! Subject: {result.get('subject_type')} | Model: {result.get('best_model')}")
