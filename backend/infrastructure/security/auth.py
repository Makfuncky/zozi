...
def verify_refresh_token(token: str) -> tuple[str, str]:
    """Verify refresh token and return (subject, family_id)."""
    payload = _decode_and_validate(token, "refresh")
    subject = payload.get("sub")
    family_id = payload.get("family_id")
    if subject is None or family_id is None:
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Invalid token")
...
def rotate_refresh_token(token: str, device_fp: str | None = None) -> tuple[str, str]:
    """
    Rotate a refresh token: verify the old one, mark it used, and issue new access+refresh tokens.
    ...
    token_data = {"sub": subject}
    new_access = create_access_token(token_data, device_fp=device_fp)
