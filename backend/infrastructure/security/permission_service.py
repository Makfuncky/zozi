"""Permission service — canonical infrastructure-level implementation.

This module owns the permission-checking primitives used by auth dependencies
and module routers. Domain layers call these functions; they must NOT depend
on domain service modules (Law 1: modules → domains → infrastructure).
"""
...
from infrastructure.utils.cache import cached_call
...
# ── Lazy-loaded cross-domain models (resolved at call time, not import time) ──
...
def list_permissions(db: Session, category_id: Optional[int] = None) -> list[dict]:
    Permission = _get_model("Permission")
...
def get_role_permissions(role_name: str, country_code: Optional[str] = None, db: Session = None) -> dict:
    RolePermissionAssignment = _get_model("RolePermissionAssignment")
    Permission = _get_model("Permission")
...
def assign_permission_to_role(role_name: str, permission_id: int, actor_id: int, db: Session, country_code: Optional[str] = None):
    RolePermissionAssignment = _get_model("RolePermissionAssignment")
...
def set_user_permission_override(user_id: int, permission_id: int, actor_id: int, db: Session, country_code: Optional[str] = None, is_granted: bool = True, expires_at: Optional[datetime] = None):
    UserPermissionOverride = _get_model("UserPermissionOverride")
...
def _invalidate_role_cache(role_name: str) -> None:
    ...
    try:
        from infrastructure.utils.cache import invalidate_role_permissions_cache as _invalidate
...
def _invalidate_user_cache(user_id: int) -> None:
    ...
    try:
        from infrastructure.utils.cache import invalidate_user_permissions_cache as _invalidate
        _invalidate(user_id)
    except Exception as exc:
...
def check_user_permission(user_id: int, permission_slug: str, db: Session, country_code: Optional[str] = None, user_role: Optional[str] = None) -> bool:
    """Return True if *user_id* holds *permission_slug* (via override or role assignment)."""
    Permission = _get_model("Permission")
    UserPermissionOverride = _get_model("UserPermissionOverride")
    RolePermissionAssignment = _get_model("RolePermissionAssignment")
...
def _log_audit(actor_id: int, action: str, target_user_id: Optional[int] = None, target_role: Optional[str] = None, permission_id: Optional[int] = None, country_code: Optional[str] = None, details: Optional[str] = None, db: Session = None):
    PermissionAuditLog = _get_model("PermissionAuditLog")
