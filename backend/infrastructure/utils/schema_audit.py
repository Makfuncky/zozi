...
Usage::
    ...
    python -m infrastructure.utils.schema_audit

    # Summary only (no per-column detail)
    python -m infrastructure.utils.schema_audit --summary

    # JSON output (for pipelines / dashboards)
    python -m infrastructure.utils.schema_audit --json

    # Filter by table name pattern (SQL ``LIKE`` syntax)
    python -m infrastructure.utils.schema_audit --table "orders%"

    # Fix mode — generate SQL to sync DB to ORM (dry-run by default)
    python -m infrastructure.utils.schema_audit --fix

    # Actually execute fix SQL against the database
    python -m infrastructure.utils.schema_audit --fix --apply

    # Importable — call from your own code:
    #
    #   from infrastructure.utils.schema_audit import audit_schema
    #   report = audit_schema()
...
def check_alembic(
    ...
    if engine is None:
        try:
            from infrastructure.database.database import engine as _engine
            engine = _engine
        except Exception:
            pass
