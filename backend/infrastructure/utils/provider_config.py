...
Real implementation relocated from ``providers.config`` (P13 root-leak cleanup).
``providers/config.py`` is now a backward-compatible re-export shim.
"""
import os
...
__all__ = ["ProviderConfig", "settings"]
