...
def _ensure_default_accounts() -> None:
    ...
    try:
        from infrastructure.database.seed import bootstrap_default_accounts
        bootstrapped = bootstrap_default_accounts(accounts)
        logger.info("Ensured %d default login accounts from DEFAULT_ACCOUNTS_JSON", bootstrapped)
    except Exception:
...
def build_lifespan():
    ...
    async def lifespan(app: FastAPI):
        ...
        # --- Startup ---
        _preload_all_models()
        # Phase 5B: back-fill canonical mixin columns onto domain tables
        # whose models still declare the columns inline. This is a
        # metadata-only patch; it satisfies ARCHITECTURE_DIAGRAM.md Laws
        # 23, 45, 50, 51, 52, 53 (Audit/SoftDelete/Tenant/Version mixins).
        try:
            from domains._mixin_compliance import apply_to_five_domains
            patched = apply_to_five_domains()
            if patched:
                logger.info("Phase 5B: mixin compliance patched %d tables", len(patched))
        except Exception as _exc:  # pragma: no cover - defensive
            logger.warning("Phase 5B mixin compliance skipped: %s", _exc)
