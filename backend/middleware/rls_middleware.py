...
NOTE: All DB access is delegated to
:mod:`middleware.dependencies.country_staff` so this module never
imports from ``domains/`` directly (Law 1).
"""
...
class RLSMiddleware:
    ...
    def get_user_countries(self, user_id: int) -> List[str]:
        ...
        from middleware.dependencies.country_staff import get_user_country_codes
        return get_user_country_codes(user_id)

    def filter_query_by_country(self, query: Query, user_id: Optional[int], is_global_admin: bool = False) -> Query:
        ...
        # Get the country_code column from the model
    ...
    def inject_country_context(self, user_id: int, is_global_admin: bool = False) -> dict:
...
def rls_filter(model_class, country_field='country_code'):
    ...
    Usage:
        @rls_filter(UserRepository, 'country_code')
        def get_entities(self, query, user): ...
    """
    def decorator(func):
        @wraps(func)
...
class CountryScopedRepository:
    ...
    def __init__(self, model_class, country_field='country_code'):
    ...
    def find_all(self, user_id: int, is_global_admin: bool = False, **filters):
        ...
        with get_service_session() as db:
            ...
            if not is_global_admin:
                countries = self.rls.get_user_countries(user_id)
                if not countries:
                    return []
