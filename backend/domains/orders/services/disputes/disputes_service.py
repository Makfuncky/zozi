"""
Orders — Disputes Service (canonical).

Merged from:
  - domains/orders/services/disputes/disputes_service.py
  - domains/orders/services/_parked/disputes_service.py
...
_PREF_FIELDS = (
    "notify_new_order", "notify_low_stock", "notify_payout_processed",
    "notify_doc_expiry", "notify_return_updates", "notify_dispute_updates",
...
def _supplier_id_from_user(current_user: dict) -> int:
    ...
    if str(current_user.get("role") or "").strip().lower() != "supplier":
...
def _serialize_dispute(row: SupplierDispute) -> dict[str, Any]:
    return {
        ...
        "dispute_type": row.dispute_type, "priority": row.priority,
        ...
        "supplier_notes": row.supplier_notes, "admin_notes": row.admin_notes,
        "resolution_notes": row.resolution_notes, "resolved_by": row.resolved_by,
...
def _get_or_create_preferences(supplier_id: int, db: Session) -> SupplierNotificationPreference:
    prefs = db.query(SupplierNotificationPreference).filter(SupplierNotificationPreference.supplier_id == supplier_id).first()
    if prefs:
        return prefs
...
# ── Dispute CRUD (legacy API) ──────────────────────────────────────────────────

def create_dispute(db: Session, supplier_id: int, order_id: int, dispute_type: str, priority: str, subject: str, description: str) -> SupplierDispute:
    """Create a dispute (legacy API)."""
    if dispute_type not in _ALLOWED_DISPUTE_TYPES:
        raise HTTPException(status_code=422, detail=f"dispute_type must be one of: {', '.join(sorted(_ALLOWED_DISPUTE_TYPES))}")
    if priority not in _ALLOWED_PRIORITIES:
        raise HTTPException(status_code=422, detail=f"priority must be one of: {', '.join(sorted(_ALLOWED_PRIORITIES))}")
    dispute = SupplierDispute(supplier_id=supplier_id, order_id=order_id, dispute_type=dispute_type,
...
def update_dispute_status(db: Session, dispute_id: int, status: str, resolution_notes: str | None = None) -> SupplierDispute:
    """Update dispute status (legacy API)."""
    if status not in _ALLOWED_STATUSES:
        raise HTTPException(status_code=422, detail=f"status must be one of: {', '.join(sorted(_ALLOWED_STATUSES))}")
...
def list_disputes(db: Session, supplier_id: int | None = None, status: str | None = None) -> list[SupplierDispute]:
    """List disputes (legacy API)."""
    query = db.query(SupplierDispute)
    if supplier_id is not None:
        query = query.filter(SupplierDispute.supplier_id == supplier_id)
    if status:
        query = query.filter(SupplierDispute.status == status)
    return query.order_by(SupplierDispute.created_at.desc()).all()


def get_dispute(db: Session, dispute_id: int) -> SupplierDispute:
    """Get a dispute by ID (legacy API)."""
    dispute = db.query(SupplierDispute).filter(SupplierDispute.id == dispute_id).first()
    if not dispute:
        raise HTTPException(status_code=404, detail="Dispute not found")
    return dispute
...
def update_supplier_notification_preferences(payload: dict[str, Any], current_user: dict, db: Session) -> dict[str, Any]:
...
def create_supplier_dispute(payload: dict[str, Any], current_user: dict, db: Session) -> dict[str, Any]:
    """Create a supplier dispute with validation."""
    supplier_id = _supplier_id_from_user(current_user)
    ...
    dispute = SupplierDispute(
        supplier_id=supplier_id, created_by=supplier_id, dispute_type=dispute_type,
        priority=priority, status="pending", title=title[:255], description=description,
    ...
    db.commit()
    db.refresh(dispute)
    db.add(Notification(user_id=supplier_id, type="dispute", title="Dispute submitted",
                        message=f"Dispute #{dispute.id} has been submitted for admin review.",
                        link="/supplier/disputes", is_read=False))
    db.commit()
    return _serialize_dispute(dispute)


def list_supplier_disputes(current_user: dict, db: Session, status: Optional[str] = None,
                           priority: Optional[str] = None, limit: int = 100, offset: int = 0) -> dict[str, Any]:
    """List supplier disputes with filtering."""
    supplier_id = _supplier_id_from_user(current_user)
    ...
    total = query.count()
    items = query.order_by(SupplierDispute.created_at.desc()).offset(max(offset, 0)).limit(min(max(limit, 1), 500)).all()
...
def get_supplier_dispute(dispute_id: int, current_user: dict, db: Session) -> dict[str, Any]:
    """Get a supplier dispute by ID."""
    supplier_id = _supplier_id_from_user(current_user)
    dispute = db.query(SupplierDispute).filter(SupplierDispute.id == dispute_id, SupplierDispute.supplier_id == supplier_id).first()
    if dispute is None:
        raise HTTPException(status_code=404, detail="Dispute not found")
    return _serialize_dispute(dispute)
...
def list_admin_disputes(db: Session, status: Optional[str] = None, priority: Optional[str] = None,
                        supplier_id: Optional[int] = None, limit: int = 100, offset: int = 0) -> dict[str, Any]:
...
def update_admin_dispute(dispute_id: int, payload: dict[str, Any], current_admin: dict, db: Session) -> dict[str, Any]:
    ...
    db.commit()
    db.refresh(dispute)
    db.add(Notification(user_id=dispute.supplier_id, type="dispute_update",
...
def bulk_update_admin_disputes(dispute_ids: list[int], action: str, value: Optional[str],
                               current_admin: dict, db: Session) -> dict[str, Any]:
    """Bulk update disputes (admin)."""
    ...
    return {"updated": len(rows), "total_requested": len(ids), "missing_ids": missing_ids}
