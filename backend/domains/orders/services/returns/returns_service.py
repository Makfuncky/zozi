"""
Orders — Returns Service (canonical).

Merged from:
  - domains/orders/services/returns_service.py
  - domains/orders/services/_parked/returns_controller_service.py
  - domains/orders/services/_parked/returns_write_service.py

Owns all return-request business logic: creation, listing, status updates,
supplier review workflows, refunds, and notifications.
"""
...
from domains.orders.models.orders import Order, OrderItem, ReturnRequest
...
from domains.finance.services.payments.payments import _order_holds_inventory, apply_order_status_change
...
def _default_supplier_review_entry(timestamp: datetime | None = None) -> dict[str, Any]:
    ...
    return {"decision": "pending", "notes": None, "updated_at": iso_timestamp, "restocked_at": None, "restock_applied": False}


def _build_supplier_review_state(order: Order, timestamp: datetime) -> dict[str, dict[str, Any]]:
    return {str(supplier_id): _default_supplier_review_entry(timestamp) for supplier_id in _supplier_ids_for_order(order)}
...
def _serialize_supplier_return_request(req: ReturnRequest, supplier_id: int) -> dict[str, Any] | None:
    ...
    return {
        "id": req.id, "order_id": req.order_id,
        ...
        "intent": req.intent, "reason": req.reason, "status": req.status,
...
def create_return_request(current_user: dict, payload: ReturnRequestCreate, db: Session) -> ReturnRequest:
    ...
    if created_at > return_deadline:
        raise HTTPException(status_code=422, detail=f"Return window expired for this order after {return_window_days} days from delivery")

    return_request = ReturnRequest(
        order_id=payload.order_id, order_item_id=payload.order_item_id,
        user_id=current_user["id"], intent=payload.intent, reason=payload.reason.strip(),
        status="pending",
    ...
    audit_log(user_id=current_user.get("id"), username=current_user.get("username"),
              user_role=current_user.get("role"), action=AuditAction.RETURN_REQUEST_CREATED,
              resource_type="order", resource_id=str(payload.order_id),
              details={"reason": payload.reason, "intent": payload.intent, "order_item_id": payload.order_item_id}, db=db)
    try:
...
def list_return_requests(current_user: dict, db: Session, *, limit: int = 50, offset: int = 0) -> List[ReturnRequest]:
    """List return requests for the current user (or all for admin/support)."""
...
def update_return_request(return_id: int, payload: ReturnRequestUpdate, current_user: dict, db: Session) -> ReturnRequest:
    """Update a return request status (admin/support only)."""
    if current_user.get("role") not in ("admin", "support"):
    ...
    # Auto-issue refund for returns (not replacements)
    if payload.status == "completed" and intent == "return":
        ...
        if order is not None and payment_id:
            if payment_id.startswith("pi_") or payment_id.startswith("py_"):
                _issue_stripe_refund(order, req, payment_id, db)
            elif payment_id.startswith("chg_"):
                _issue_tap_refund(order, req, payment_id, db)
    elif payload.status == "completed" and intent == "replacement":
        order = db.query(Order).filter(Order.id == req.order_id).first()
        if order is not None:
            db.add(Notification(user_id=req.user_id, type="order_update", title="Replacement Request Completed",
    ...
    db.refresh(req)
    audit_log(user_id=current_user.get("id"), username=current_user.get("username"),
              user_role=current_user.get("role"), action=AuditAction.RETURN_REQUEST_UPDATED,
              resource_type="return_request", resource_id=str(return_id),
              details={"status": payload.status, "notes": resolution_notes, "intent": intent}, db=db)
    try:
...
def update_return_request_status(return_id: int, status: str, notes: str | None, db: Session) -> Dict[str, Any]:
    """Legacy status-update shim."""
...
def bulk_update_return_requests(return_ids: list[int], payload: ReturnRequestUpdate, current_user: dict, db: Session) -> dict[str, Any]:
    """Bulk update return request status (admin/support only)."""
    if current_user.get("role") not in ("admin", "support"):
        raise HTTPException(status_code=403, detail="Admin access required")
    if not return_ids:
    ...
    return {"processed": len(processed), "skipped": len(skipped), "status": payload.status, "details": processed, "skipped_details": skipped}


# ── Supplier return operations ─────────────────────────────────────────────────

def list_supplier_return_requests(current_user: dict, db: Session, limit: Optional[int] = None, offset: int = 0) -> dict[str, Any]:
    """List return requests for a supplier."""
...
def update_supplier_return_request(return_id: int, payload: SupplierReturnReviewUpdate, current_user: dict, db: Session) -> dict[str, Any]:
    """Update supplier review for a return request."""
    if current_user.get("role") != "supplier":
        raise HTTPException(status_code=403, detail="Supplier access required")
    req = db.query(ReturnRequest).filter(ReturnRequest.id == return_id).first()
    if req is None:
    ...
    if decision == "restocked":
        ...
        if not current_state.get("restock_applied"):
            for item in owned_items:
                product = db.query(Product).filter(Product.id == item["product_id"], Product.supplier_id == supplier_id).first()
                if product is not None:
    ...
    audit_log(user_id=current_user.get("id"), username=current_user.get("username"),
              user_role=current_user.get("role"), action=AuditAction.RETURN_REQUEST_UPDATED,
              resource_type="return_request", resource_id=str(return_id),
              details={"supplier_id": supplier_id, "supplier_decision": decision, "supplier_notes": notes, "intent": intent}, db=db)
    serialized = _serialize_supplier_return_request(req, supplier_id)
...
def create_return_notification(
    db: Optional[Session] = None, *, return_request=None, user_id: Optional[int] = None,
    country_code: str = "OM", type: Optional[str] = None, title: str = "", message: str = "",
    link: Optional[str] = None, is_read: bool = False, **kw,
) -> Notification:
    ...
    if db is None:
        ...
        own_session = True
    try:
        record = Notification(user_id=user_id, title=title, message=message, link=link, type=type,
                              is_read=bool(is_read), country_code=country_code, **kw)
        db.add(record)
        db.commit()
        db.refresh(record)
        return record
...
def _issue_stripe_refund(order: Order, req: ReturnRequest, payment_id: str, db: Session) -> None:
    """Issue a Stripe refund for a return."""
    ...
    if refund is not None:
        try:
            ...
            try:
                ...
                log_refund_bank_transaction(order, db, source="stripe_refund", transaction_ref=refund.id,
            ...
            db.add(Notification(user_id=req.user_id, type="order_update", title="Return Refund Issued",
                                message=f"Your return for Order #{order.id} has been processed and a refund has been issued via Stripe.",
                                link=f"/orders/{order.id}"))
            logger.info("Stripe auto-refund issued for return %s: refund %s", req.id, refund.id)
        except Exception as exc:
            logger.error("Stripe auto-refund failed for return %s: %s", req.id, exc)
            _capture_exc(exc)


def _issue_tap_refund(order: Order, req: ReturnRequest, payment_id: str, db: Session) -> None:
    """Issue a Tap Payments refund for a return."""
    ...
    if tap_key:
        try:
            ...
            if tap_data.get("status") in ("REFUNDED", "CAPTURED"):
                ...
                try:
                    from domains.finance.services.treasury.cash_management_service import log_refund_bank_transaction
                    log_refund_bank_transaction(order, db, source="tap_refund", transaction_ref=tap_data.get("id") or f"{payment_id}:return-refund",
                ...
                db.add(Notification(user_id=req.user_id, type="order_update", title="Return Refund Issued",
                                    ...
                                    link=f"/orders/{order.id}"))
                logger.info("Tap auto-refund issued for return %s: %s", req.id, tap_data.get("id"))
            else:
                logger.error("Tap refund returned unexpected status for return %s: %s", req.id, tap_data)
        except Exception as exc:
            logger.error("Tap auto-refund failed for return %s: %s", req.id, exc)
            _capture_exc(exc)
