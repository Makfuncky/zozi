        ...
        try:
            for (idx, media) in enumerate(parsed):
                ...
                try:
                    ...
                    db.add(AIGenerationLog(job_id=job.id, field='error', model_used='worker', prompt_hash=None, tokens_used=None, cost=None, confidence=None, country_code=job.country_code))
                    continue
                db.add(staging)
                db.flush()
                for v in variants:
...
def _publish_staging(db: Session, staging: AIStagingProduct, overrides: Optional[dict], job: AIUploadJob) -> Product:
    ...
    product = Product(name=name, slug=slug, description=description, ai_description=staging.ai_description, price=price, stock=0, category=category, subcategory=staging.subcategory, color=color, brand=brand, tags=staging.tags, materials=staging.materials, sizes=staging.sizes, image_url=staging.image_url, images=[staging.image_url] if staging.image_url else None, supplier_id=job.supplier_id, country_code=job.country_code, is_active=True, is_verified=True, moderation_status='approved', variant_axes=staging.variant_axes)
    db.add(product)
    db.flush()
    ...
    for sv in variants:
        ...
        total_stock += v_stock
        variant = ProductVariant(product_id=product.id, size=v_size, color=v_color, material=sv.material, pattern=sv.pattern, gender=sv.gender, sku=sv.sku, barcode=sv.barcode, product_code=sv.product_code, price=v_price, stock=v_stock, media_url=sv.media_url, attributes_json=sv.attributes_json, is_active=True, country_code=job.country_code, variant_key=compute_variant_key(product.id, v_size, v_color, sv.material, sv.pattern, sv.gender))
        db.add(variant)
...
def _enrich_one(img_bytes: bytes, idx: int, job: AIUploadJob, image_url: str) -> tuple[AIStagingProduct, list[AIStagingVariant], list[AIGenerationLog]]:
    ...
    for field in ('name', 'category', 'tags', 'description', 'color'):
        logs.append(AIGenerationLog(job_id=job.id, field=field, model_used=ai_service.HF_API_TOKEN and 'huggingface-inference' or 'rule-based-fallback', prompt_hash=None, tokens_used=None, cost=None, confidence=None, country_code=job.country_code))
    staging = AIStagingProduct(job_id=job.id, name=name, description=description, price=None, stock=0, category=category, subcategory=None, color=color, brand=None, tags=tags, sizes=sizes, materials=materials, image_url=image_url, ai_description=description, variant_axes={'size': sizes, 'color': [color] if color else [], 'material': materials}, attributes=None, confidence_score=confidence_score, requires_human_review=requires_review, country_code=job.country_code)
    variants: list[AIStagingVariant] = []
    color_list = [color] if color else [None]
    for size in sizes or [None]:
        for c in color_list:
            variants.append(AIStagingVariant(job_id=job.id, staging_product_id=0, variant_key=None, size=size, color=c, material=(materials or [None])[0] if materials else None, pattern=None, gender=None, sku=None, barcode=None, product_code=None, price=None, stock=0, media_url=image_url, attributes_json=None, is_active=True, confidence_score=confidence_score, requires_human_review=requires_review, country_code=job.country_code))
    return (staging, variants, logs)

async def create_ai_upload_job(background_tasks: BackgroundTasks, images: list[UploadFile]=File(default=[]), country_code: str=Form(...), model_used: Optional[str]=Form(None), prompt_hash: Optional[str]=Form(None), current_user: dict=_AUTH, db: Session=Depends(get_db)):
    if not images:
        raise HTTPException(status_code=422, detail='At least one image is required.')
    user_id = current_user.get('id') or current_user.get('user_id')
    if user_id is None and isinstance(current_user.get('user'), dict):
        user_id = current_user['user'].get('id')
    job = AIUploadJob(supplier_id=int(user_id), status='pending', model_used=model_used, prompt_hash=prompt_hash, source_media_json='[]', country_code=country_code)
    db.add(job)
    db.flush()
    media_list = []
    for img in images:
    ...
    return {'job_id': job.id, 'status': job.status, 'country_code': job.country_code, 'media_count': len(media_list)}
...
def get_ai_upload_job(job_id: int, current_user: dict=_AUTH, db: Session=Depends(get_db)):
    ...
    return {'job': {'id': job.id, 'status': job.status, 'model_used': job.model_used, 'tokens_used': float(job.tokens_used) if job.tokens_used is not None else None, 'error_log': job.error_log, 'country_code': job.country_code, 'created_at': job.created_at.isoformat() if job.created_at else None, 'created_product_id': job.created_product_id}, 'staging_products': [{'id': s.id, 'name': s.name, 'description': s.description, 'category': s.category, 'color': s.color, 'brand': s.brand, 'price': float(s.price) if s.price is not None else None, 'tags': s.tags, 'sizes': s.sizes, 'materials': s.materials, 'image_url': s.image_url, 'confidence_score': float(s.confidence_score) if s.confidence_score is not None else None, 'requires_human_review': s.requires_human_review} for s in staging], 'logs': [{'field': lg.field, 'model_used': lg.model_used, 'confidence': float(lg.confidence) if lg.confidence is not None else None} for lg in logs]}

def publish_ai_upload_job(job_id: int, overrides: Optional[dict]=None, current_user: dict=_AUTH, db: Session=Depends(get_db)):
    job = db.get(AIUploadJob, job_id)
    if job is None:
        raise HTTPException(status_code=404, detail='Job not found.')
