...
# domain. The proper fix is to publish ``accounts.events.chat_*`` events
# and have accounts consume them. Until then the imports are routed
# through ``accounts.ports`` (read surface) but the write surface
# remains a violation — tracked in a follow-up.
...
# Notification lives in comms; TicketReply lives in governance.
...
class ConnectionManager:
    ...
    async def connect(self, websocket: WebSocket, room_id: str, user_id: int, user_name: str=''):
        ...
        self._user_info[user_id] = {'user_id': user_id, 'name': user_name, 'status': 'online', 'last_seen': datetime.now(timezone.utc).isoformat(), 'rooms': set()}
    ...
    def get_room_users(self, room_id: str) -> list[dict]:
        ...
        for uid in self._rooms.get(room_id, {}):
            ...
            users.append({'user_id': uid, 'name': info.get('name', f'User {uid}'), 'status': info.get('status', 'offline'), 'last_seen': info.get('last_seen')})
        return users
...
# Single shared in-memory connection registries (created after the classes above).
...
def _mark_messages_read(db: Session, room_id: str, user_id: int):
    ...
    if room_id.startswith('dm_'):
        ...
        if room:
            msgs = db.query(DirectChatMessage).filter(DirectChatMessage.room_id == room.id, DirectChatMessage.sender_id != user_id, DirectChatMessage.read_at.is_(None)).all()
            for m in msgs:
                m.read_at = now
                count += 1
        ...
        if room:
            msgs = db.query(GroupChatMessage).filter(GroupChatMessage.room_id == room.id, GroupChatMessage.sender_id != user_id, GroupChatMessage.read_at.is_(None)).all()
            for m in msgs:
                m.read_at = now
                count += 1
    else:
        msgs = db.query(EntityChatMessage).filter(EntityChatMessage.thread_id == int(room_id), EntityChatMessage.sender_id != user_id, EntityChatMessage.read_at.is_(None)).all()
        for m in msgs:
            m.read_at = now
            count += 1
    db.commit()
...
async def websocket_chat(websocket: WebSocket, room_id: str, token: str=Query(...)):
    ...
    await manager.broadcast(room_id, {'type': 'user_joined', 'room_id': room_id, 'user_id': user_id, 'user_name': user_name, 'users': room_users})
    try:
        while True:
            ...
            if event_type == 'message':
                ...
                try:
                    ...
                    db.close()
                await manager.broadcast(room_id, {'type': 'message', 'room_id': room_id, 'sender_id': user_id, 'sender_name': user_name, 'content': content, 'message_type': msg_type, 'message_id': msg_id, 'created_at': created_at})
                ...
                await manager.broadcast(room_id, {'type': 'typing', 'room_id': room_id, 'user_id': user_id, 'user_name': user_name, 'is_typing': is_typing, 'typing_user_ids': typing_users, 'typing_user_names': typing_names}, exclude_user_id=user_id)
                ...
                try:
                    ...
                    db.close()
                await manager.broadcast(room_id, {'type': 'read_receipt', 'room_id': room_id, 'user_id': user_id, 'user_name': user_name, 'count': count}, exclude_user_id=user_id)
                ...
                for rid in user_rooms:
                    await manager.broadcast(rid, {'type': 'presence', 'room_id': rid, 'user_id': user_id, 'user_name': user_name, 'status': status, 'users': manager.get_room_users(rid)}, exclude_user_id=user_id)
            elif event_type == 'ping':
                await websocket.send_json({'type': 'pong'})
    except WebSocketDisconnect:
        manager.disconnect(websocket, room_id, user_id)
        room_users = manager.get_room_users(room_id)
        await manager.broadcast(room_id, {'type': 'user_left', 'room_id': room_id, 'user_id': user_id, 'user_name': user_name, 'users': room_users})
