...
# ---------------------------------------------------------------------------
# Event-bus integration (bounded context decoupling)
#
# Order-domain code no longer imports this comms service. Instead it publishes
# intents on the circuit-exempt ``data.event_bus``; we subscribe here so email
# side-effects fire without the order domain depending on comms.
# ---------------------------------------------------------------------------

_email_handlers_registered = False


def register_email_event_handlers() -> None:
    """Subscribe this service's email side-effects to order-domain events."""
    global _email_handlers_registered
    if _email_handlers_registered:
        return
    from domains.comms.services.shared.utility.shared_utils import EVENT_ORDER_REFUNDED
    from domains.comms.services.shared.utility.shared_utils import EVENT_ORDER_STATUS_CHANGED
    from domains.comms.services.shared.utility.shared_utils import subscribe
    ...
    def _on_order_refunded(payload: dict) -> None:
        enqueue_refund_processed_email(
            int(payload["order_id"]),
            source=str(payload.get("source", "admin")),
        )

    subscribe(EVENT_ORDER_STATUS_CHANGED, _on_order_status_changed)
    subscribe(EVENT_ORDER_REFUNDED, _on_order_refunded)
    _email_handlers_registered = True
