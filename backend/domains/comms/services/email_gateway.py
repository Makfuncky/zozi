...
from models.comms import Notification, InternalEmail, EmailFolder
...
class EmailGateway:
    ...
    def send_internal_email(self, to_user_ids: List[int], subject: str,
        ...
        users = self.db.query(User).filter(User.id.in_(to_user_ids)).limit(SAFE_QUERY_LIMIT).all()
        ...
        # N+1 removal: resolve employee rows and inbox folders in bulk before the loops.
        employee_rows = (
            self.db.query(Employee)
            .filter(Employee.user_id.in_([u.id for u in users]))
            .limit(SAFE_QUERY_LIMIT)
            .all()
        ) if users else []
        employees_by_user_id: dict = {}
        for row in employee_rows:
            employees_by_user_id.setdefault(row.user_id, row)
        ...
        for u in users:
            ...
            if not emp:
                emp = employees_by_user_id.get(u.id)
            if not emp:
                continue
            resolved_employees.append(emp)

        folders_by_employee_id: dict = {}
        target_employee_ids = [emp.id for emp in resolved_employees]
        if target_employee_ids:
            folder_rows = (
                self.db.query(EmailFolder)
                .filter(EmailFolder.employee_id.in_(target_employee_ids), EmailFolder.name == "inbox")
                .limit(SAFE_QUERY_LIMIT)
                .all()
            )
            for f in folder_rows:
                folders_by_employee_id.setdefault(f.employee_id, f)

        for emp in resolved_employees:
            folder = folders_by_employee_id.get(emp.id)
            if not folder:
                folder = EmailFolder(employee_id=emp.id, name="inbox", folder_type="inbox", is_system=True)
                self.db.add(folder)
                self.db.flush()
                folders_by_employee_id[emp.id] = folder
    ...
    def send_from_alias(self, alias: str, to_email: str, subject: str, body: str,
        ...
        try:
            ...
            logger.exception("Handled Exception in email_gateway.py:244")
            return {
                "status": "failed",
                "error": str(e)
            }
...
def _enqueue_email_delivery(email_id: int, body_html: str, subject: str) -> None:
    ...
    try:
        ...
        def _deliver() -> dict:
            ...
            from models.comms import InternalEmail
            ...
            try:
                ...
                # N+1 removal: resolve every recipient user in a single query before the loop.
                recipient_ids = [
                    entry.get("user_id") for entry in recipients if entry.get("user_id")
                ]
                users_by_id: dict = {}
                if recipient_ids:
                    recipient_rows = (
                ...
                for to_user_id in recipient_ids:
                    user = users_by_id.get(to_user_id)
