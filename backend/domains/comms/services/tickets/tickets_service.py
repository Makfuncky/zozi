...
from domains.governance.ports import TicketAttachment
import structlog
...
def get_order_first(db: Session, **filters) -> Optional[Order]:
    conditions = [getattr(Order, key) == value for key, value in filters.items()]
    query = db.query(Order).filter(*conditions)
    return query.limit(1).first()
...
def db_invoice_query_0(db: Session) -> Optional[Any]:
    """Read-only query delegated from controller."""
    result = db.query(Invoice)
    return result

def db_invoice_query_1(db: Session) -> Optional[Any]:
    """Read-only query delegated from controller."""
    return db.query(Invoice)

def db_order_first_2(db: Session, id: Any, inv: Any, order_id: Any, uid: Any, user_id: Any) -> Optional[Any]:
    """Read-only query delegated from controller."""
    result = db.query(Order).filter(Order.id == inv.order_id, Order.user_id == uid).first()
    return result

def db_order_query_3(db: Session) -> Optional[Any]:
    """Read-only query delegated from controller."""
    return db.query(Order)

def db_invoice_first_4(db: Session, invoice_type: Any, order_id: Any, sale: Any, supplier_id: Any) -> Optional[Any]:
    """Read-only query delegated from controller."""
    result = db.query(Invoice).filter( Invoice.order_id == order_id, Invoice.supplier_id == supplier_id, Invoice.invoice_type == "sale", ).first()
    return result

def db_invoice_count_5(db: Session) -> Optional[Any]:
    """Read-only query delegated from controller."""
    result = db.query(Invoice).count()
    return result

def db_invoice_all_6(db: Session) -> Optional[Any]:
    """Read-only query delegated from controller."""
