...
from domains.audit.ports import AuditLog  # Law 3
from domains.comms.models.communication_schema_models import SupportTicket  # noqa: NS8 fix
...
def _create_realtime_redis_client():
    ...
    try:
        ...
        logger.debug("Redis client unavailable; realtime features disabled", exc_info=True)
        return None
    return client
...
class _RedisRealtimeBridge:
    ...
    def publish(self, message: dict[str, Any]) -> bool:
        ...
        try:
            ...
            logger.warning("Redis publish failed for channel %s", self._channel, exc_info=True)
            self._publisher_client = None
            return False
    ...
    async def _broadcast(self, sockets: set[WebSocket], payload: dict[str, Any]) -> None:
        ...
        for socket in list(sockets):
            try:
                ...
                logger.debug("WebSocket broadcast send failed, marking socket as stale", exc_info=True)
                stale.append(socket)
        for socket in stale:
    ...
    async def broadcast_user(self, user_id: object, payload: dict[str, Any]) -> None:
        ...
        for socket in list(sockets):
            try:
                ...
                logger.debug("WebSocket send failed, marking socket as stale", exc_info=True)
                stale.append(socket)
        for socket in stale:
            sockets.discard(socket)
