...
# ── Notification Service (merged from notifications/notification_service.py) ───
...
class NotificationService:
    ...
    def send_notification(self, user_id: int, title: str, message: str, type: str = "info", priority: str = "medium") -> dict:
        return self.send_alert(alert_type=type, data={"user_id": user_id, "title": title, "message": message}, priority=priority)

    def send_fulfillment_issues_notification(self, user_id: int, order_id: int, payment_id: int, issues: list[str]) -> dict:
        return self.send_notification(user_id=user_id, title="Order Fulfillment Issue", message=f"Order #{order_id} has fulfillment issues: {'; '.join(issues)}", type="fulfillment_issue", priority="high")

    def send_fulfillment_success_notification(self, user_id: int, order_id: int, payment_id: int, amount: float) -> dict:
        return self.send_notification(user_id=user_id, title="Order Confirmed", message=f"Your order #{order_id} for ${amount} has been confirmed and is being processed.", type="fulfillment_success", priority="medium")
...
def mark_notification_read(notification_id):
    ...
    if notification:
        notification.is_read = True; notification.read_at = utcnow(); db.commit(); return True
    return False


def mark_all_notifications_read(user_id):
    db = next(get_db())
    db.query(Notification).filter(Notification.user_id == user_id, Notification.is_read == False).update({"is_read": True, "read_at": utcnow()})
    db.commit()
...
def delete_notification(notification_id):
    db = next(get_db())
    notification = db.query(Notification).filter(Notification.id == notification_id).first()
    if notification: db.delete(notification); db.commit(); return True
    return False
