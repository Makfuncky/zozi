    ...
    def refine(image_np: np.ndarray, alpha: np.ndarray) -> np.ndarray:
        ...
        if _HAS_GUIDED_FILTER:
            try:
                ...
                logger.debug("guidedFilter refinement failed, using raw alpha", exc_info=True)
...
def _load_rembg_session():
    ...
    if not hasattr(_load_rembg_session, "_session"):
        try:
            ...
            logger.debug("rembg isnet-general-use model unavailable, falling back to u2net", exc_info=True)
            try:
...
def magic_erase(data: bytes, max_dim: int = MAX_DIMENSION) -> bytes:
    ...
    """
    try:
        # TODO: Use finance.ports for cross-domain read
    ...
    try:
        ...
        if brightness != 1.0:
            try:
                ...
                logger.debug("skimage gamma adjustment failed, using PIL fallback", exc_info=True)
