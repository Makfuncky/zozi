...
from sqlalchemy import Column, Integer, String, DateTime, Boolean, Text, Numeric, ForeignKey, UniqueConstraint, Index, JSON, text as sa_text, func
...
class Category(Base):
    ...
    parent_id = Column(Integer, ForeignKey("catalog.categories.id", ondelete='CASCADE'), nullable=True, index=True)
...
class Product(Base):
    ...
    category_id = Column(Integer, ForeignKey("catalog.categories.id", ondelete='RESTRICT'), nullable=True, index=True)
...
class ProductFilterMetadata(Base):
    __tablename__ = "product_filter_metadatas"
    __table_args__ = ({"schema": "catalog"},)
    id = Column(Integer, primary_key=True, index=True)
    category_id = Column(Integer, ForeignKey("catalog.categories.id", ondelete='CASCADE'), nullable=True, index=True)
...
class ProductFilterOption(Base):
    ...
    __table_args__ = ({"schema": "catalog"},)
    id = Column(Integer, primary_key=True, index=True)
    filter_metadata_id = Column(Integer, ForeignKey("catalog.product_filter_metadatas.id", ondelete='CASCADE'), nullable=False, index=True)
