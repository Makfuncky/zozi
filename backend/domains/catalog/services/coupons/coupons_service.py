"""
Coupons Controller — coupon validation and admin CRUD logic.
"""
import logging
...
# ── Lookups (folded from coupons_read_service) ───────────────────────────────


def get_coupon_by_id(coupon_id: int, db: Session) -> Optional[Coupon]:
    """Return a coupon by primary key (or None)."""
    return db.query(Coupon).filter(Coupon.id == coupon_id).first()


def get_coupon_by_code_active(code: str, db: Session) -> Optional[Coupon]:
    """Return the active, non-deleted coupon for ``code`` (case-insensitive)."""
...
def get_coupon_usage_count(coupon_id: int, db: Session) -> int:
    """Return the number of usage records for a coupon."""
    return int(
...
# ── Update (folded from coupons_write_service) ────────────────────────────────


def update_coupon(coupon: Coupon, updates: dict, db: Session) -> Coupon:
    """Apply ``updates`` to an existing coupon and persist them."""
