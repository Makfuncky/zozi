# Live Points & Loyalty service (merged from _parked/promotion_points_service.py).
# Historical archive retained at domains/_parked/promotion_points_service.py.
# Repointed internal import to live catalog promotion_service (was orders.services).
# See RESOLVER.md PART 5 / Sec 37.5 (PARKED-POINTS merge).
...
from domains.catalog.services.promotions.promotion_service import _get_or_create_config as _get_promotion_config
...
def get_points_transactions(
    ...
    rows = (
        keyset_offset_window(db.query(PointsTransaction)
        .filter(PointsTransaction.user_id == user_id)
        .order_by(PointsTransaction.created_at.desc()), offset=offset, limit=limit)
    )
    return [
        {
            "id": tx.id,
