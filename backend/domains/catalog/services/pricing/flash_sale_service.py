"""
Catalog — Pricing sub-domain: Flash Sale Service (canonical).
...
Time-limited discount campaigns with caching, audit logging, and product targeting.
"""
...
def _bump_product_cache_version() -> None:
    """Bump product cache version after flash sale changes."""
    try:
        from domains.catalog.services.products_service import _bump_product_cache_version as _bump
        _bump()
    except ImportError:
        pass
...
def get_active_flash_sales(db: Session) -> List[FlashSaleOut]:
    ...
    sales = (
        db.query(FlashSale)
        .filter(FlashSale.is_active.is_(True), FlashSale.starts_at <= now, FlashSale.ends_at >= now)
...
def get_all_flash_sales(db: Session, limit: Optional[int] = None, offset: int = 0, search: Optional[str] = None) -> dict[str, Any]:
...
def create_flash_sale(body: FlashSaleCreate, current_admin: dict, db: Session) -> FlashSaleOut:
    ...
    db.commit()
    db.refresh(sale)
    audit_log(db, action=AuditAction.FLASH_SALE_CREATED, user_id=current_admin.get("id"),
              username=current_admin.get("username"), user_role=current_admin.get("role"),
              resource_type="flash_sale", resource_id=cast(int, getattr(sale, "id")),
...
def create_flash_sale_with_items(db: Session, payload: FlashSaleCreate) -> FlashSale:
    """Persist a new flash sale and its items (without admin audit)."""
...
def update_flash_sale(sale_id: int, body: FlashSaleCreate, current_admin: dict, db: Session) -> FlashSaleOut:
    ...
    db.commit()
    db.refresh(sale)
    audit_log(db, action=AuditAction.FLASH_SALE_UPDATED, user_id=current_admin.get("id"),
              username=current_admin.get("username"), user_role=current_admin.get("role"),
              resource_type="flash_sale", resource_id=cast(int, getattr(sale, "id")),
              details={"title": sale.title, "discount_pct": sale.discount_pct})
    bump_cache_version("flash-sales")
...
def delete_flash_sale(sale_id: int, current_admin: dict, db: Session) -> dict:
    ...
    db.delete(sale)
    db.commit()
    audit_log(db, action=AuditAction.FLASH_SALE_DELETED, user_id=current_admin.get("id"),
              username=current_admin.get("username"), user_role=current_admin.get("role"),
              resource_type="flash_sale", resource_id=sale_id, details={"title": sale_title})
    bump_cache_version("flash-sales")
    _bump_product_cache_version()
...
# ---------------------------------------------------------------------------
# Low-level persist primitives (for use by other services)
# ---------------------------------------------------------------------------
