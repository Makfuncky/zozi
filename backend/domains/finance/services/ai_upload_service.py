"""AI upload service — DB and business logic for routers/ai_upload.py."""
...
def create_ai_upload_job(current_user: dict, country_code: str, model_used: Optional[str], prompt_hash: Optional[str], images: list, db: Session) -> dict:
...
def get_ai_upload_job(job_id: int, db: Session) -> dict:
    job = db.get(AIUploadJob, job_id)
    if job is None:
...
def publish_ai_upload_job(job_id: int, overrides: Optional[dict], db: Session) -> dict:
    job = db.get(AIUploadJob, job_id)
    if job is None:
        raise ValueError("Job not found.")
...
def cancel_ai_upload_job(job_id: int, db: Session) -> dict:
    job = db.get(AIUploadJob, job_id)
    if job is None:
        raise ValueError("Job not found.")
