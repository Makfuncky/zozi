...
delegator. Behavior (route paths, methods, auth, response shapes) is
preserved exactly.

Country-scoped routes still perform ``get_country_or_404`` + RLS context
wrapping in the router; these service functions assume the RLS context is
...
def get_treasury_ledger(db: Session, start_date: date, end_date: date, limit: int) -> list:
    from infrastructure.utils.constants import DEFAULT_PAGE_SIZE
...
def get_trial_balance(db: Session) -> list:
    engine = TreasuryEngine(db)
    return engine.get_trial_balance()


def get_cash_position(db: Session) -> list:
    accounts = db.execute(
...
def approve_payout_batch(db: Session, batch_id: int, current_user: dict) -> dict:
    batch = db.execute(
...
def dispatch_payout_batch(db: Session, batch_id: int, current_user: dict) -> dict:
    batch = db.execute(
        select(PayoutBatch).where(PayoutBatch.id == batch_id).with_for_update()
    ).scalar_one_or_none()
...
def get_country_gateway_exceptions(db: Session, cc: str) -> list:
    issues = db.execute(
        select(GatewaySettlementSchedule).where(
            GatewaySettlementSchedule.status.in_(["pending", "flagged"]),
            GatewaySettlementSchedule.country_code == cc,
        ).order_by(GatewaySettlementSchedule.settlement_date.desc()).limit(100)
    ).scalars().all()
    return [
        {
...
def get_consolidated_ledger(db: Session, limit: int) -> list:
...
def get_consolidated_payout_batches(db: Session, limit: int) -> list:
...
def get_consolidated_vat_liability(db: Session) -> list:
...
def get_consolidated_cod_remittances(db: Session, limit: int) -> list:
    ...
    return [
        {
            ...
            "order_id": (db.query(Shipment.order_id).filter(Shipment.id == r.shipment_id).scalar() if r.shipment_id else None),
...
def get_consolidated_gateway_summary(db: Session) -> list:
...
def get_consolidated_cash_forecasts(db: Session) -> list:
    forecasts = db.execute(
        select(CashFlowForecast).order_by(CashFlowForecast.forecast_date.desc()).limit(12)
    ).scalars().all()
    return [
...
def get_consolidated_reconciliation_pipeline(db: Session, limit: int) -> dict:
    pipeline = []
...
def get_country_treasury_ledger(db: Session, cc: str, start_date: date, end_date: date, limit: int) -> list:
    entries = db.execute(
        select(JournalEntry)
        .where(
...
def get_country_cash_position(db: Session, cc: str) -> list:
    accounts = db.execute(
...
def get_country_payout_batches(db: Session, cc: str) -> list:
    batches = db.execute(
        select(PayoutBatch)
        .options(joinedload(PayoutBatch.creator), joinedload(PayoutBatch.approver))
...
def get_country_vat_liability(db: Session, country_code: str, period: str) -> dict:
    output_vat = db.execute(
        select(func.coalesce(func.sum(AccountBalance.balance), 0))
        .join(Account, AccountBalance.account_id == Account.id)
        .where(Account.code == OUTPUT_VAT_ACCOUNT)
...
def get_country_cod_remittances(db: Session, cc: str, status: Optional[str]) -> list:
    query = (
        select(LogisticsCODRemittanceReceipt, LogisticsPartner.name)
        .outerjoin(LogisticsPartner, LogisticsCODRemittanceReceipt.partner_id == LogisticsPartner.id)
...
def get_country_gateway_summary(db: Session, cc: str) -> list:
    schedules = db.execute(
        select(GatewaySettlementSchedule)
...
def settle_supplier(
    db: Session, cc: str, order_id: int, supplier_id: int, net_amount: float,
...
def approve_settlement(db: Session, cc: str, settlement_id: int) -> dict:
    settlement = db.query(SupplierSettlement).filter(
...
def get_payment_transactions(
    ...
    gateway: Optional[str], status: Optional[str],
) -> list:
    query = (
        select(Payment)
...
def get_country_payment_transactions(
    db: Session, cc: str, start_date: date, end_date: date,
    gateway: Optional[str], status: Optional[str],
) -> list:
    query = (
        select(Payment)
        .where(
            Payment.country_code == cc,
...
def get_supplier_payouts(db: Session, status: Optional[str]) -> list:
    ...
    return [_shape_supplier_payout(p, s) for p, s in rows]
...
def _shape_logistics_payout(p) -> dict:
    return {
        "id": p.id,
...
def get_logistics_payouts(db: Session, status: Optional[str]) -> list:
...
def get_country_logistics_payouts(db: Session, cc: str, status: Optional[str]) -> list:
    query = (
        select(LogisticsPartnerPayout)
        .where(LogisticsPartnerPayout.country_code == cc)
...
def get_supplier_earnings(db: Session) -> list:
    rows = db.execute(
        select(
...
def get_country_supplier_earnings(db: Session, cc: str) -> list:
    rows = db.execute(
        select(
            SupplierSettlement.supplier_id,
            func.sum(SupplierSettlement.gross_amount).label("gross"),
            func.sum(SupplierSettlement.commission_amount).label("commission"),
            func.sum(SupplierSettlement.net_amount).label("net"),
        ).where(SupplierSettlement.country_code == cc)
        .group_by(SupplierSettlement.supplier_id)
    ).all()
    return [_shape_earnings(r) for r in rows]
...
def get_country_liabilities_exposure(db: Session, cc: str) -> dict:
    ...
    for code, label in codes.items():
        bal = db.execute(
            select(func.coalesce(func.sum(AccountBalance.balance), 0))
            .join(Account, AccountBalance.account_id == Account.id)
            .where(Account.code == code, AccountBalance.country_code == cc)
        ).scalar() or Decimal("0")
        exposure[label] = float(bal)
    return exposure
...
def get_pending_entries(db: Session) -> dict:
    engine = TreasuryEngine(db)
    return {"entries": engine.list_pending_entries()}


def get_country_pending_entries(db: Session, cc: str) -> dict:
    engine = TreasuryEngine(db)
    all_entries = engine.list_pending_entries()
    filtered = [e for e in all_entries if e.get("country_code") == cc]
    return {"entries": filtered}


def approve_pending(db: Session, pending_id: int, approver_id: int) -> dict:
    engine = TreasuryEngine(db)
    return engine.approve_pending_entry(pending_id, approver_id)


def reject_pending(db: Session, pending_id: int, rejected_by: int, reason: str) -> dict:
    engine = TreasuryEngine(db)
...
def detect_orphans(db: Session, cc: Optional[str] = None) -> dict:
    ...
    if cc:
        alerts = [a for a in alerts if a.get("country_code") == cc]
...
def get_country_payroll(db: Session, country_code: str) -> dict:
    rows = (
