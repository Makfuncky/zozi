...
from domains.finance.models.finance import FinancialReport
...
def generate_cash_flow_statement(
    ...
    op_codes = list(operating_codes.keys())
    acct_by_code = {}
    if op_codes:
        acct_rows = db.query(Account).filter(Account.code.in_(op_codes)).all()
        for a in acct_rows:
            acct_by_code[a.code] = a

    acct_ids = [a.id for a in acct_by_code.values() if a]
    change_by_acct = {}
    if acct_ids:
        placeholders = ", ".join(f":aid{i}" for i in range(len(acct_ids)))
        ...
        for r in change_rows:
            change_by_acct[r["account_id"]] = Decimal(str(r["change"]))

    for code, (label, kind) in operating_codes.items():
        acct = acct_by_code.get(code)
        if not acct:
            continue
        change = change_by_acct.get(acct.id, Decimal("0"))
        if change != 0:
            adjusted = -change if kind == "asset" else change
