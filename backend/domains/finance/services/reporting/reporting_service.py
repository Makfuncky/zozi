...
def admin_cod_remittances(status: Optional[str], db: Session, current_user: dict):
    from domains.logistics.ports import logistics_partner_model
    from domains.governance.models.admin import LogisticsCODRemittanceReceipt
    LP = logistics_partner_model()
    query = select(LogisticsCODRemittanceReceipt, LP.name).outerjoin(LP, LogisticsCODRemittanceReceipt.partner_id == LP.id)
    if status:
...
def consolidated_cod_remittances(limit: int, db: Session, current_user: dict):
    from domains.logistics.ports import shipment_model
    SM = shipment_model()
    ...
    return [{'id': r.id, 'shipment_id': r.shipment_id, 'order_id': db_read_query(db, SM.order_id).filter(SM.id == r.shipment_id).scalar() if r.shipment_id else None, 'partner_id': r.partner_id, 'amount': float(r.amount), 'bank_reference': r.bank_reference, 'status': r.status, 'country_code': r.country_code, 'created_at': r.created_at.isoformat() if r.created_at else None} for r in receipts]
...
def consolidated_reconciliation_pipeline(limit: int, db: Session, current_user: dict):
    ...
    P = payment_model()
    pipeline = []
    orders = order_query(db).filter(O.status.in_(['shipped', 'delivered', 'completed', 'dispatched'])).order_by(O.updated_at.desc()).limit(limit).all()
    for order in orders:
        payment = payment_query(db).filter(P.order_id == order.id).first()
        settlement = db_read_query(db, SupplierSettlement).filter(SupplierSettlement.order_id == order.id).first()
        payout = payout_query(db).filter_by(id=settlement.payout_id).first() if settlement and settlement.payout_id else None
...
def country_cod_remittances(country_code: str, status: Optional[str], db: Session, current_user: dict):
    ...
    LP = logistics_partner_model()
    cc = country_code.upper()
    query = select(LogisticsCODRemittanceReceipt, LP.name).outerjoin(LP, LogisticsCODRemittanceReceipt.partner_id == LP.id).where(LogisticsCODRemittanceReceipt.country_code == cc)
    if status:
        query = query.where(LogisticsCODRemittanceReceipt.status == status)
...
def admin_reconciliation_pipeline(country_code: str, status: Optional[str], limit: int, db: Session, current_user: dict):
    ...
    try:
        ...
        S = shipment_model()
        pipeline = []
        orders = order_query(db).filter(O.country_code == cc, O.status.in_(['shipped', 'delivered', 'completed', 'dispatched'])).order_by(O.updated_at.desc()).limit(limit).all()
        for order in orders:
            order_total = float(getattr(order, 'total_amount', None) or getattr(order, 'total', 0) or 0)
            payment = payment_query(db).filter(P.order_id == order.id).first()
            shipment = shipment_query(db).filter(S.order_id == order.id).first()
            ...
            if payment and payment.payment_method == 'cod':
                cod_receipt = logistics_cod_remittance_receipt_query(db).filter_by(order_id=order.id).first()
            settlement = db_read_query(db, SupplierSettlement).filter(SupplierSettlement.order_id == order.id).first()
            payout = None
            if settlement:
                payout = payout_query(db).filter_by(id=settlement.payout_id).first() if settlement.payout_id else None
            supplier_id = None
            first_item = order_item_query(db).filter(OI.order_id == order.id).first()
            if first_item:
...
def admin_record_cod_remittance(country_code: str, order_id: int, partner_id: int, amount: float, bank_reference: str, db: Session, current_user: dict):
    ...
    try:
        from domains.orders.ports import order_query, order_model
        from domains.logistics.ports import shipment_query, shipment_model
        from domains.governance.ports import logistics_cod_remittance_receipt_query
        from domains.governance.models.admin import LogisticsCODRemittanceReceipt
        O = order_model()
        S = shipment_model()
        shipment = shipment_query(db).filter(S.order_id == order_id).first()
        ...
        db_write.flush(db)
        order = order_query(db).filter(O.id == order_id).first()
        if order:
...
def admin_settle_supplier(country_code: str, order_id: int, supplier_id: int, net_amount: float, gross_amount: Optional[float], commission_amount: Optional[float], currency: Optional[str], payout_id: Optional[int], db: Session, current_user: dict):
    ...
    try:
        from domains.orders.ports import order_query, order_model
        from domains.country.ports import country_config_query
        O = order_model()
        gross = gross_amount if gross_amount is not None else net_amount
        resolved_currency = currency or 'USD'
        ccfg = country_config_query(db).filter_by(code=cc).first()
