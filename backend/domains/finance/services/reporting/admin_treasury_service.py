...
def admin_trial_balance(as_of_date: Optional[str] = Query(None), country_code: Optional[str] = Query(None), db: Session = Depends(get_db), current_user: dict = Depends(require_treasury_access)):
...
def country_trial_balance(country_code: str = Path(..., description='ISO country code'), as_of_date: Optional[str] = Query(None), db: Session = Depends(get_db), current_user: dict = Depends(require_treasury_access)):
...
def admin_manual_adjustment(debit_account: str = FastAPIBody(...), credit_account: str = FastAPIBody(...), amount: float = FastAPIBody(...), reason: str = FastAPIBody(...), created_by: int = FastAPIBody(...), db: Session = Depends(get_db), current_user: dict = Depends(require_treasury_access)):
...
def country_manual_adjustment(country_code: str = Path(..., description='ISO country code'), debit_account: str = FastAPIBody(...), credit_account: str = FastAPIBody(...), amount: float = FastAPIBody(...), reason: str = FastAPIBody(...), created_by: int = FastAPIBody(...), db: Session = Depends(get_db), current_user: dict = Depends(require_treasury_access)):
...
def admin_pending_entries(db: Session = Depends(get_db), current_user: dict = Depends(require_treasury_access)):

    engine = TreasuryEngine(db)
    return {"entries": engine.list_pending_entries()}

def country_pending_entries(country_code: str = Path(..., description='ISO country code'), db: Session = Depends(get_db), current_user: dict = Depends(require_treasury_access)):
...
def admin_approve_pending(pending_id: int = Path(...), approver_id: int = FastAPIBody(...), db: Session = Depends(get_db), current_user: dict = Depends(require_treasury_access)):

    engine = TreasuryEngine(db)
    result = engine.approve_pending_entry(pending_id, approver_id)
    return result

def country_approve_pending(country_code: str = Path(..., description='ISO country code'), pending_id: int = Path(...), approver_id: int = FastAPIBody(...), db: Session = Depends(get_db), current_user: dict = Depends(require_treasury_access)):
...
def admin_reject_pending(pending_id: int = Path(...), rejected_by: int = FastAPIBody(...), reason: str = FastAPIBody(...), db: Session = Depends(get_db), current_user: dict = Depends(require_treasury_access)):

    engine = TreasuryEngine(db)
    result = engine.reject_pending_entry(pending_id, rejected_by, reason)
    return result

def country_reject_pending(country_code: str = Path(..., description='ISO country code'), pending_id: int = Path(...), rejected_by: int = FastAPIBody(...), reason: str = FastAPIBody(...), db: Session = Depends(get_db), current_user: dict = Depends(require_treasury_access)):
...
def admin_detect_orphans(db: Session = Depends(get_db), current_user: dict = Depends(require_treasury_access)):

    engine = TreasuryEngine(db)
    alerts = engine.run_orphan_detector()
    return {"alerts": alerts, "count": len(alerts)}

def country_detect_orphans(country_code: str = Path(..., description='ISO country code'), db: Session = Depends(get_db), current_user: dict = Depends(require_treasury_access)):
