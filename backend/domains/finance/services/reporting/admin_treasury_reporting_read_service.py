...
def admin_treasury_root(db: Session):
...
def admin_treasury_metrics(db: Session):
...
def admin_treasury_ledger(start_date, end_date, limit, db: Session):
    entries = db.execute(
        select(JournalEntry)
...
def admin_cash_position(db: Session):
    accounts = db.execute(
...
def admin_payout_batches(db: Session):
    batches = db.execute(
        select(PayoutBatch)
...
def admin_vat_liability(country_code, period, db: Session):
...
def admin_cod_remittances(status, db: Session):
    from domains.logistics.models.logistics import LogisticsPartner
...
def admin_gateway_summary(db: Session):
...
def admin_cash_forecasts(db: Session):
...
def consolidated_treasury_metrics(db: Session):
...
def consolidated_treasury_ledger(limit, db: Session):
...
def consolidated_trial_balance(page, page_size, db: Session):
    query = db.query(Account).filter(Account.is_active == True)
    total = query.count()
    accounts = keyset_offset_window(query.order_by(Account.code), offset=(page - 1) * page_size, limit=page_size)
    return {
        "data": [
            {
                "id": a.id,
...
def consolidated_cash_position(page, page_size, db: Session):
    query = db.query(TreasuryAccount).filter(TreasuryAccount.is_active == True)
    total = query.count()
    accounts = keyset_offset_window(query, offset=(page - 1) * page_size, limit=page_size)
...
def consolidated_payout_batches(limit, db: Session):
...
def consolidated_vat_liability(db: Session):
...
def consolidated_cod_remittances(limit, db: Session):
...
def consolidated_gateway_summary(db: Session):
...
def consolidated_cash_forecasts(db: Session):
    forecasts = db.execute(
        select(CashFlowForecast).order_by(CashFlowForecast.forecast_date.desc()).limit(12)
    ).scalars().all()
    return [
...
def country_treasury_metrics(country_code, db: Session):
...
def country_treasury_ledger(country_code, start_date, end_date, limit, db: Session):
    cc = country_code.upper()
    entries = db.execute(
        select(JournalEntry)
        .where(
...
def country_cash_position(country_code, db: Session):
    cc = country_code.upper()
    accounts = db.execute(
...
def country_payout_batches(country_code, db: Session):
    cc = country_code.upper()
    batches = db.execute(
        select(PayoutBatch)
        .options(joinedload(PayoutBatch.creator), joinedload(PayoutBatch.approver))
...
def country_vat_liability(country_code, period, db: Session):
    output_vat = db.execute(
        select(func.coalesce(func.sum(AccountBalance.balance), 0))
        .join(Account, AccountBalance.account_id == Account.id)
        .where(Account.code == OUTPUT_VAT_ACCOUNT)
...
def country_cod_remittances(country_code, status, db: Session):
    from domains.logistics.models.logistics import LogisticsPartner
...
def country_gateway_summary(country_code, db: Session):
    cc = country_code.upper()
    schedules = db.execute(
        select(GatewaySettlementSchedule)
...
def admin_reconciliation_pipeline(country_code, status, limit, db: Session):
    ...
    try:
        ...
        for order in orders:
            ...
            supplier_id = None
            first_item = get_order_item_by_order_id(db, order.id)
            if first_item:
...
def admin_gateway_exceptions(db: Session):
    issues = db.execute(
...
def country_gateway_exceptions(country_code, db: Session):
    get_country_or_404(country_code.upper(), db)
    set_rls_context({country_code.upper()}, is_restricted=True)
    try:
        issues = db.execute(
...
def admin_payment_transactions(start_date, end_date, gateway, status, db: Session):
    query = (
        select(Payment)
...
def country_payment_transactions(country_code, start_date, end_date, gateway, status, db: Session):
    get_country_or_404(country_code.upper(), db)
    set_rls_context({country_code.upper()}, is_restricted=True)
    try:
        query = (
...
def admin_supplier_payouts(status, db: Session):
...
def country_supplier_payouts(country_code, status, db: Session):
    get_country_or_404(country_code.upper(), db)
    set_rls_context({country_code.upper()}, is_restricted=True)
    try:
        from domains.suppliers.ports import SupplierProfile
...
def admin_logistics_payouts(status, db: Session):
...
def country_logistics_payouts(country_code, status, db: Session):
    get_country_or_404(country_code.upper(), db)
    set_rls_context({country_code.upper()}, is_restricted=True)
    try:
        query = (
...
def admin_supplier_earnings(db: Session):
    rows = db.execute(
        select(
...
def country_supplier_earnings(country_code, db: Session):
    get_country_or_404(country_code.upper(), db)
    set_rls_context({country_code.upper()}, is_restricted=True)
    try:
        rows = db.execute(
...
def admin_liabilities_exposure(db: Session):
...
def country_liabilities_exposure(country_code, db: Session):
    get_country_or_404(country_code.upper(), db)
    set_rls_context({country_code.upper()}, is_restricted=True)
    try:
...
def country_payroll(country_code, db: Session):
