...
from models import JournalEntry, JournalEntryLine, Account, AccountBalance, User, PayoutBatch, PayoutBatchItem, TreasuryAccount, VATRemittance, GatewaySettlementSchedule, CashPositionSnapshot, CashFlowForecast, BankTransaction, Invoice, SupplierSettlement, TransactionLedger
...
from services.common.write_helpers import commit_only
...
def admin_treasury_metrics(db: Session, current_user: dict):
    total_debits = db_read_execute(db, select(func.coalesce(func.sum(JournalEntryLine.amount), 0)).where(JournalEntryLine.side == 'debit')).scalar() or Decimal('0')
    total_credits = db_read_execute(db, select(func.coalesce(func.sum(JournalEntryLine.amount), 0)).where(JournalEntryLine.side == 'credit')).scalar() or Decimal('0')
    total_entries = db_read_query(db, JournalEntry).count()
...
def admin_treasury_ledger(start_date: date, end_date: date, limit: int, db: Session, current_user: dict):
    entries = db_read_execute(db, select(JournalEntry).where(JournalEntry.entry_date >= start_date, JournalEntry.entry_date <= end_date).options(joinedload(JournalEntry.lines)).order_by(JournalEntry.entry_date.desc()).limit(min(limit, MAX_PAGE_SIZE))).unique().scalars().all()
    result = []
    for e in entries:
...
def admin_payout_batches(db: Session, current_user: dict):
    batches = db_read_execute(db, select(PayoutBatch).options(joinedload(PayoutBatch.creator), joinedload(PayoutBatch.approver)).order_by(PayoutBatch.created_at.desc()).limit(MAX_PAGE_SIZE)).unique().scalars().all()
...
def admin_generate_payout_batch(country_code: str, cutoff_date: date, db: Session, current_user: dict):
    ...
    pending_payouts = db_read_execute(db, select(Payout).where(Payout.country_code == country_code, Payout.status == 'pending', Payout.created_at <= cutoff_date)).scalars().all()
    ...
    for payout in pending_payouts:
        ...
        db_write.add(db, item)
        payout.status = 'batched'
    commit_only(db)
    db_write.refresh(db, batch)
...
def admin_approve_payout_batch(batch_id: int, db: Session, current_user: dict):
    batch = db_read_execute(db, select(PayoutBatch).where(PayoutBatch.id == batch_id).with_for_update()).scalar_one_or_none()
    if not batch:
...
def admin_vat_liability(country_code: Optional[str], period: str, db: Session, current_user: dict):
    output_vat = db_read_execute(db, select(func.coalesce(func.sum(AccountBalance.balance), 0)).join(Account, AccountBalance.account_id == Account.id).where(Account.code == OUTPUT_VAT_ACCOUNT)).scalar() or Decimal('0')
    input_vat = db_read_execute(db, select(func.coalesce(func.sum(AccountBalance.balance), 0)).join(Account, AccountBalance.account_id == Account.id).where(Account.code == INPUT_VAT_ACCOUNT)).scalar() or Decimal('0')
...
def admin_cod_remittances(status: Optional[str], db: Session, current_user: dict):
    ...
    rows = db_read_execute(db, query).all()
...
def admin_gateway_summary(db: Session, current_user: dict):
    schedules = db_read_execute(db, select(GatewaySettlementSchedule).order_by(GatewaySettlementSchedule.settlement_date.desc()).limit(100)).scalars().all()
...
def admin_cash_forecasts(db: Session, current_user: dict):
    forecasts = db_read_execute(db, select(CashFlowForecast).order_by(CashFlowForecast.forecast_date.desc()).limit(12)).scalars().all()
...
def consolidated_treasury_metrics(db: Session, current_user: dict):
    total_accounts = db_read_query(db, TreasuryAccount).count()
    total_je = db_read_query(db, JournalEntry).count()
    total_batches = db_read_query(db, PayoutBatch).count()
    total_invoices = db_read_query(db, Invoice).count()
...
def consolidated_treasury_ledger(limit: int, db: Session, current_user: dict):
    entries = db_read_query(db, JournalEntry).order_by(JournalEntry.created_at.desc()).limit(limit).all()
...
def consolidated_payout_batches(limit: int, db: Session, current_user: dict):
    batches = db_read_query(db, PayoutBatch).order_by(PayoutBatch.created_at.desc()).limit(limit).all()
...
def consolidated_vat_liability(db: Session, current_user: dict):
    vats = db_read_query(db, VATRemittance).order_by(VATRemittance.period_start.desc()).limit(12).all()
...
def consolidated_cod_remittances(limit: int, db: Session, current_user: dict):
    ...
    receipts = db_read_query(db, LogisticsCODRemittanceReceipt).order_by(LogisticsCODRemittanceReceipt.created_at.desc()).limit(limit).all()
    return [{'id': r.id, 'shipment_id': r.shipment_id, 'order_id': db_read_query(db, ShipmentModel.order_id).filter(ShipmentModel.id == r.shipment_id).scalar() if r.shipment_id else None, 'partner_id': r.partner_id, 'amount': float(r.amount), 'bank_reference': r.bank_reference, 'status': r.status, 'country_code': r.country_code, 'created_at': r.created_at.isoformat() if r.created_at else None} for r in receipts]

def consolidated_gateway_summary(db: Session, current_user: dict):
    schedules = db_read_query(db, GatewaySettlementSchedule).order_by(GatewaySettlementSchedule.settlement_date.desc()).limit(12).all()
...
def consolidated_reconciliation_pipeline(limit: int, db: Session, current_user: dict):
    ...
    pipeline = []
    orders = db_read_query(db, OrderModel).filter(OrderModel.status.in_(['shipped', 'delivered', 'completed', 'dispatched'])).order_by(OrderModel.updated_at.desc()).limit(limit).all()
    for order in orders:
        payment = db_read_query(db, PaymentModel).filter(PaymentModel.order_id == order.id).first()
        settlement = db_read_query(db, SupplierSettlement).filter(SupplierSettlement.order_id == order.id).first()
        payout = db_read_query(db, Payout).filter(Payout.id == settlement.payout_id).first() if settlement and settlement.payout_id else None
...
def country_treasury_metrics(country_code: str, db: Session, current_user: dict):
    ...
    total_debits = db_read_execute(db, select(func.coalesce(func.sum(JournalEntryLine.amount), 0)).join(JournalEntry, JournalEntryLine.entry_id == JournalEntry.id).where(JournalEntryLine.side == 'debit', JournalEntry.country_code == cc)).scalar() or Decimal('0')
    total_credits = db_read_execute(db, select(func.coalesce(func.sum(JournalEntryLine.amount), 0)).join(JournalEntry, JournalEntryLine.entry_id == JournalEntry.id).where(JournalEntryLine.side == 'credit', JournalEntry.country_code == cc)).scalar() or Decimal('0')
    total_entries = db_read_query(db, JournalEntry).filter(JournalEntry.country_code == cc).count()
...
def country_treasury_ledger(country_code: str, start_date: date, end_date: date, limit: int, db: Session, current_user: dict):
    cc = country_code.upper()
    entries = db_read_execute(db, select(JournalEntry).where(JournalEntry.entry_date >= start_date, JournalEntry.entry_date <= end_date, JournalEntry.country_code == cc).options(joinedload(JournalEntry.lines)).order_by(JournalEntry.entry_date.desc()).limit(limit)).unique().scalars().all()
    result = []
    for e in entries:
...
def country_cash_position(country_code: str, db: Session, current_user: dict):
    cc = country_code.upper()
    accounts = db_read_execute(db, select(TreasuryAccount).where(TreasuryAccount.is_active == True, TreasuryAccount.country_code == cc)).scalars().all()
    if not accounts:
        accounts = db_read_execute(db, select(TreasuryAccount).where(TreasuryAccount.is_active == True, TreasuryAccount.country_code.is_(None))).scalars().all()
    return [{'account_name': a.name, 'balance': float(a.balance), 'gl_code': a.gl_account_code or a.slug} for a in accounts]

def country_payout_batches(country_code: str, db: Session, current_user: dict):
    cc = country_code.upper()
    batches = db_read_execute(db, select(PayoutBatch).options(joinedload(PayoutBatch.creator), joinedload(PayoutBatch.approver)).where(PayoutBatch.country_code == cc).order_by(PayoutBatch.created_at.desc()).limit(100)).unique().scalars().all()
    return [{'id': b.id, 'batch_number': b.batch_number, 'country_code': b.country_code, 'total_amount': float(b.total_amount), 'status': b.status, 'created_at': b.created_at.isoformat(), 'created_by': b.created_by, 'created_by_name': b.creator.full_name if b.creator else None, 'approved_by': b.approved_by, 'approved_by_name': b.approver.full_name if b.approver else None} for b in batches]
...
def country_gateway_summary(country_code: str, db: Session, current_user: dict):
    cc = country_code.upper()
    schedules = db_read_execute(db, select(GatewaySettlementSchedule).where(GatewaySettlementSchedule.country_code == cc).order_by(GatewaySettlementSchedule.settlement_date.desc()).limit(100)).scalars().all()
    by_gateway = defaultdict(lambda : {'total_settled': 0, 'total_expected': 0, 'count': 0, 'last_date': None})
    for s in schedules:
        key = str(s.gateway_id)
        by_gateway[key]['total_expected'] += float(s.amount or 0)
...
def admin_reconciliation_pipeline(country_code: str, status: Optional[str], limit: int, db: Session, current_user: dict):
    ...
    try:
        ...
        pipeline = []
        orders = db_read_query(db, OrderModel).filter(OrderModel.country_code == cc, OrderModel.status.in_(['shipped', 'delivered', 'completed', 'dispatched'])).order_by(OrderModel.updated_at.desc()).limit(limit).all()
        for order in orders:
            order_total = float(getattr(order, 'total_amount', None) or getattr(order, 'total', 0) or 0)
            payment = db_read_query(db, PaymentModel).filter(PaymentModel.order_id == order.id).first()
            from models import Shipment
            shipment = db_read_query(db, Shipment).filter(Shipment.order_id == order.id).first()
            ...
            if payment and payment.payment_method == 'cod':
                cod_receipt = db_read_query(db, LogisticsCODRemittanceReceipt).filter(LogisticsCODRemittanceReceipt.order_id == order.id).first()
            settlement = db_read_query(db, SupplierSettlement).filter(SupplierSettlement.order_id == order.id).first()
            payout = None
            if settlement:
                payout = db_read_query(db, Payout).filter(Payout.id == settlement.payout_id).first() if settlement.payout_id else None
            supplier_id = None
            first_item = db_read_query(db, OrderItem).filter(OrderItem.order_id == order.id).first()
            if first_item:
...
def admin_record_cod_remittance(country_code: str, order_id: int, partner_id: int, amount: float, bank_reference: str, db: Session, current_user: dict):
    ...
    try:
        from models.orders import Order as OrderModel
        from models import Shipment as ShipmentModel
        shipment = db_read_query(db, ShipmentModel).filter(ShipmentModel.order_id == order_id).first()
        ...
        db_write.add(db, receipt)
        db_write.flush(db)
        order = db_read_query(db, OrderModel).filter(OrderModel.id == order_id).first()
        if order:
            setattr(order, 'settlement_status', 'cod_remitted')
        commit_only(db)
        db_write.refresh(db, receipt)
        try:
...
def admin_settle_supplier(country_code: str, order_id: int, supplier_id: int, net_amount: float, gross_amount: Optional[float], commission_amount: Optional[float], currency: Optional[str], payout_id: Optional[int], db: Session, current_user: dict):
    ...
    try:
        ...
        ccfg = db_read_query(db, CountryConfig).filter(CountryConfig.code == cc).first()
        ...
        db_write.add(db, settlement)
        db_write.flush(db)
        order = db_read_query(db, OrderModel).filter(OrderModel.id == order_id).first()
        if order:
            setattr(order, 'settlement_status', 'settled')
        commit_only(db)
        db_write.refresh(db, settlement)
...
def admin_approve_settlement(country_code: str, settlement_id: int, db: Session, current_user: dict):
    ...
    set_rls_context({country_code.upper()}, is_restricted=True)
    try:
        settlement = db_read_query(db, SupplierSettlement).filter(SupplierSettlement.id == settlement_id, SupplierSettlement.country_code == country_code.upper()).first()
        if not settlement:
...
def admin_gateway_exceptions(db: Session, current_user: dict):
    issues = db_read_execute(db, select(GatewaySettlementSchedule).where(GatewaySettlementSchedule.status.in_(['pending', 'flagged'])).order_by(GatewaySettlementSchedule.settlement_date.desc()).limit(100)).scalars().all()
...
def country_gateway_exceptions(country_code: str, db: Session, current_user: dict):
    ...
    set_rls_context({country_code.upper()}, is_restricted=True)
    try:
        issues = db_read_execute(db, select(GatewaySettlementSchedule).where(GatewaySettlementSchedule.status.in_(['pending', 'flagged']), GatewaySettlementSchedule.country_code == country_code.upper()).order_by(GatewaySettlementSchedule.settlement_date.desc()).limit(100)).scalars().all()
...
def admin_payment_transactions(start_date: date, end_date: date, gateway: Optional[str], status: Optional[str], db: Session, current_user: dict):
    ...
    rows = db_read_execute(db, query.limit(200)).scalars().all()
...
def admin_supplier_payouts(status: Optional[str], db: Session, current_user: dict):
    ...
    rows = db_read_execute(db, query.limit(200)).all()
...
def admin_supplier_earnings(db: Session, current_user: dict):
    rows = db_read_execute(db, select(SupplierSettlement.supplier_id, func.sum(SupplierSettlement.gross_amount).label('gross'), func.sum(SupplierSettlement.commission_amount).label('commission'), func.sum(SupplierSettlement.net_amount).label('net')).group_by(SupplierSettlement.supplier_id)).all()
...
def admin_liabilities_exposure(db: Session, current_user: dict):
    ...
    for (code, label) in codes.items():
        bal = db_read_execute(db, select(func.coalesce(func.sum(AccountBalance.balance), 0)).join(Account, AccountBalance.account_id == Account.id).where(Account.code == code)).scalar() or Decimal('0')
...
def country_liabilities_exposure(country_code: str, db: Session, current_user: dict):
    ...
    try:
        ...
        exposure = {}
        for (code, label) in codes.items():
            bal = db_read_execute(db, select(func.coalesce(func.sum(AccountBalance.balance), 0)).join(Account, AccountBalance.account_id == Account.id).where(Account.code == code, AccountBalance.country_code == country_code.upper())).scalar() or Decimal('0')
            exposure[label] = float(bal)
        return exposure
    finally:
        clear_rls_context()
...
def admin_manual_adjustment(debit_account: str, credit_account: str, amount: float, reason: str, created_by: int, db: Session, current_user: dict):
    ...
    lines = [{'account_code': debit_account, 'debit': float(amount_dec), 'description': reason}, {'account_code': credit_account, 'credit': float(amount_dec), 'description': reason}]
    if amount_dec > threshold:
        pending = engine.submit_pending_entry(lines=lines, description=reason, created_by=created_by, source='manual_adjustment')
        return {'status': 'pending_approval', 'pending_id': pending['pending_id']}
    entry = engine.post_journal_entry(lines=lines, description=reason, source='manual_adjustment', created_by=created_by)
...
def country_manual_adjustment(country_code: str, debit_account: str, credit_account: str, amount: float, reason: str, created_by: int, db: Session, current_user: dict):
    ...
    try:
        ...
        lines = [{'account_code': debit_account, 'debit': float(amount_dec), 'description': reason}, {'account_code': credit_account, 'credit': float(amount_dec), 'description': reason}]
        if amount_dec > threshold:
            pending = engine.submit_pending_entry(lines=lines, description=reason, created_by=created_by, source='manual_adjustment', country_code=country_code.upper())
            return {'status': 'pending_approval', 'pending_id': pending['pending_id']}
        entry = engine.post_journal_entry(lines=lines, description=reason, source='manual_adjustment', created_by=created_by, country_code=country_code.upper())
        return {'entry_id': entry.id, 'reference_number': entry.reference_number}
    finally:
        clear_rls_context()
...
def payroll_equity(db: Session):
    ...
    rows = db_read_query(db, Employee.department, Employee.gender, func.avg(Employee.salary)).filter(Employee.salary.isnot(None), Employee.department.isnot(None)).group_by(Employee.department, Employee.gender).all()
    by_dept = {}
...
def country_payroll(country_code: str, db: Session):
    ...
    rows = db_read_query(db, func.count(Employee.id), func.coalesce(func.sum(Employee.salary), 0)).filter(Employee.country_code == country_code.upper()).all()
