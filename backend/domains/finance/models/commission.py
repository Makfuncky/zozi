...
class CommissionAgreement(Base):
    ...
    set_by_admin_id = Column(Integer, ForeignKey("accounts.users.id", ondelete="SET NULL"), nullable=True, index=True)
...
class CommissionLedgerEntry(Base):
    ...
    order_item_id = Column(Integer, ForeignKey("orders.order_items.id", ondelete="SET NULL"), nullable=True, index=True)
    ...
    adjusted_by_id = Column(Integer, ForeignKey("accounts.users.id", ondelete="SET NULL"), nullable=True, index=True)
...
class CommissionCategoryRate(Base):
    ...
    __table_args__ = (
        ...
        {"schema": "finance"})
    id = Column(Integer, primary_key=True, index=True)
    category_id = Column(Integer, ForeignKey('catalog.categories.id', ondelete="SET NULL"), nullable=True, index=True)
    category_slug = Column(String(100), nullable=True)
