...
class CommissionAgreement(Base):
    ...
    set_by_admin_id = Column(Integer, ForeignKey("governance.users.id"), nullable=True, index=True)
...
class CommissionLedgerEntry(Base):
    ...
    adjusted_by = Column(Integer, ForeignKey("governance.users.id"), nullable=True, index=True)
