...
def _ensure_promotion_tables(db: Session) -> None:
    bind = db.get_bind()
    Base.metadata.create_all(
        ...
        tables=[
            ...
            PromotionLedgerEntry.__table__,
        ],
        checkfirst=True,
    )
...
# Public aliases (stable API for recovery tests / external callers).
ensure_promotion_tables = _ensure_promotion_tables
get_or_create_config = _get_or_create_config
