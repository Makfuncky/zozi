"""
Promotions — Banner Service (canonical).
...
DB-backed banner management with multi-instance-safe PostgreSQL storage,
caching, audit logging, and image upload support.
"""
...
_DEFAULT_BANNERS = [
    {
        ...
        "image_url": None, "cta_label": "Explore the edit", "cta_url": "/products",
        "banner_type": "hero", "sort_order": 0, "bg_color": "#22c55e", "text_color": "#ffffff",
        "subtitle_color": "rgba(255,255,255,0.86)", "btn_bg_color": "rgba(255,255,255,0.16)",
        "btn_text_color": "#ffffff", "badge_text": "Marketplace Edit", "badge_color": "rgba(255,255,255,0.14)",
        "effect": "aurora",
    },
    {
        ...
        "image_url": None, "cta_label": "View offers", "cta_url": "/products?deals=1",
        "banner_type": "flash", "sort_order": 1, "bg_color": "#0f172a", "text_color": "#f8fafc",
        "subtitle_color": "rgba(248,250,252,0.82)", "btn_bg_color": "#ffd400", "btn_text_color": "#111111",
        "badge_text": "Flash Deals", "badge_color": "rgba(255,255,255,0.14)", "effect": "poppers",
    },
    {
        ...
        "image_url": None, "cta_label": "View seasonal picks", "cta_url": "/products?newArrivals=1",
        "banner_type": "seasonal", "sort_order": 2, "bg_color": "#14532d", "text_color": "#ffffff",
        "subtitle_color": "rgba(255,255,255,0.84)", "btn_bg_color": "rgba(255,255,255,0.16)",
        "btn_text_color": "#ffffff", "badge_text": "Ramadan Highlights", "badge_color": "rgba(255,255,255,0.14)",
        "effect": "ramadan",
    },
    {
        ...
        "image_url": None, "cta_label": "Celebrate with deals", "cta_url": "/products?deals=1",
        "banner_type": "seasonal", "sort_order": 3, "bg_color": "#4338ca", "text_color": "#ffffff",
        "subtitle_color": "rgba(255,255,255,0.84)", "btn_bg_color": "#f8fafc", "btn_text_color": "#111111",
        "badge_text": "Eid Celebration", "badge_color": "rgba(255,255,255,0.14)", "effect": "eid",
    },
    {
        ...
        "image_url": None, "cta_label": "Shop best sellers", "cta_url": "/products?bestSellers=1",
        "banner_type": "hero", "sort_order": 4, "bg_color": "#0f172a", "text_color": "#f8fafc",
        "subtitle_color": "rgba(248,250,252,0.82)", "btn_bg_color": "rgba(255,255,255,0.16)",
        "btn_text_color": "#ffffff", "badge_text": "Trust & Safety", "badge_color": "rgba(255,255,255,0.14)",
        "effect": "aurora",
    },
]
...
def _banner_to_dict(banner: Banner) -> dict:
    ...
    return {
        ...
        "image_url": image_url, "cta_label": banner.cta_label, "cta_url": banner.cta_url,
        "banner_type": banner_type, "position": banner_type, "is_active": banner.is_active,
        "sort_order": banner.sort_order,
        "starts_at": starts_at.isoformat() if starts_at else None,
        "ends_at": ends_at.isoformat() if ends_at else None,
        "bg_color": banner.bg_color, "text_color": banner.text_color,
        "subtitle_color": banner.subtitle_color, "btn_bg_color": banner.btn_bg_color,
        "btn_text_color": banner.btn_text_color, "badge_text": banner.badge_text,
        "badge_color": banner.badge_color, "effect": banner.effect,
        "video_url": banner.video_url, "country_code": banner.country_code,
...
def get_banners(db: Session, banner_type: Optional[str] = None, active_only: bool = False, country_code: Optional[str] = None) -> list[dict]:
    ...
    if active_only:
        ...
        q = q.filter((Banner.starts_at == None) | (Banner.starts_at <= now))
        q = q.filter((Banner.ends_at == None) | (Banner.ends_at >= now))
...
def get_banners_page(db: Session, banner_type: Optional[str] = None, active_only: bool = False, limit: Optional[int] = None, offset: int = 0) -> dict:
    ...
    if active_only:
        ...
        query = query.filter((Banner.starts_at == None) | (Banner.starts_at <= now))
        query = query.filter((Banner.ends_at == None) | (Banner.ends_at >= now))
    total = query.count()
...
def create_banner(payload: BannerCreate, admin_id: int, current_admin: dict, db: Session) -> dict:
    banner = Banner(
        title=payload.title, subtitle=payload.subtitle, image_url=payload.image_url,
        cta_label=payload.cta_label, cta_url=payload.cta_url, banner_type=payload.banner_type,
        is_active=payload.is_active, sort_order=payload.sort_order, starts_at=payload.starts_at,
        ends_at=payload.ends_at, bg_color=payload.bg_color, text_color=payload.text_color,
        subtitle_color=payload.subtitle_color, btn_bg_color=payload.btn_bg_color,
        btn_text_color=payload.btn_text_color, badge_text=payload.badge_text,
        badge_color=payload.badge_color, effect=payload.effect, video_url=payload.video_url,
        country_code=payload.country_code, layout_json=payload.layout_json, created_by=admin_id,
    )
    db.add(banner)
    db.commit()
    db.refresh(banner)
    audit_log(db, action=AuditAction.BANNER_CREATED, user_id=current_admin.get("id"),
              username=current_admin.get("username"), user_role=current_admin.get("role"),
              resource_type="banner", resource_id=cast(int, getattr(banner, "id")),
...
def update_banner(banner_id: int, payload: BannerUpdate, current_admin: dict, db: Session) -> dict:
    ...
    db.commit()
    db.refresh(banner)
    audit_log(db, action=AuditAction.BANNER_UPDATED, user_id=current_admin.get("id"),
              username=current_admin.get("username"), user_role=current_admin.get("role"),
              resource_type="banner", resource_id=cast(int, getattr(banner, "id")),
...
def delete_banner(banner_id: int, current_admin: dict, db: Session) -> dict:
    ...
    db.delete(banner)
    db.commit()
    audit_log(db, action=AuditAction.BANNER_DELETED, user_id=current_admin.get("id"),
              username=current_admin.get("username"), user_role=current_admin.get("role"),
              resource_type="banner", resource_id=banner_id, details={"title": banner_title})
    bump_cache_version("banners")
    return {"detail": "Banner deleted"}


async def upload_banner_image(banner_id: int, file: UploadFile, current_admin: dict, db: Session) -> dict:
    ...
    db.commit()
    db.refresh(banner)
    audit_log(db, action=AuditAction.BANNER_IMAGE_UPLOADED, user_id=current_admin.get("id"),
              username=current_admin.get("username"), user_role=current_admin.get("role"),
              resource_type="banner", resource_id=cast(int, getattr(banner, "id")),
...
def reorder_banners(banner_ids: list[int], current_admin: dict, db: Session) -> list[dict]:
    ...
    db.commit()
    audit_log(db, action=AuditAction.BANNER_REORDERED, user_id=current_admin.get("id"),
              username=current_admin.get("username"), user_role=current_admin.get("role"),
              resource_type="banner", details={"banner_ids": banner_ids})
    bump_cache_version("banners")
    return get_banners(db)


# ---------------------------------------------------------------------------
# Legacy compatibility (for backward compatibility with old field names)
# ---------------------------------------------------------------------------
...
def add_banner_if_missing(db: Session, banner=None, **kw) -> Banner:
    ...
    existing = db.query(Banner).filter(
        Banner.title == title, Banner.is_deleted.is_(False), Banner.country_code == country_code,
    ).first()
...
def update_banner_image(db: Session, banner_or_id, image_url: str, filename: Optional[str] = None, **kw) -> Banner:
    """Update a banner's image."""
    if isinstance(banner_or_id, Banner):
        record = banner_or_id
    else:
