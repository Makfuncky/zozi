"""
Promotions — Coin / Points Service (canonical).
...
from domains.comms.ports import UserPoints, PointsTransaction
...
def award_points_for_order(
    *,
    ...
    loyalty tier bonus. Only awarded once per order.
    """
...
def award_referral_points(
    *,
    ...
    if referrer_points > 0:
        ...
        db.add(tx)
        result["referrer"] = {"user_id": referrer_id, "points": referrer_points, "balance_after": row.balance}
    if referee_points > 0:
        ...
        db.add(tx)
        result["referee"] = {"user_id": referee_id, "points": referee_points, "balance_after": row.balance}
    db.flush()
