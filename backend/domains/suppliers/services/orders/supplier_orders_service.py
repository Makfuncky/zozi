"""Supplier orders service — DB and business logic for routers/supplier_orders.py."""
...
def _get_user_id(current_user: User | dict) -> int:
    if isinstance(current_user, dict):
        ...
        if not uid:
            raise ValueError("Invalid user session: missing user ID")
        return int(uid)
...
def list_supplier_orders(current_user: User | dict, db: Session) -> list[Any]:
...
def get_supplier_label(current_user: User | dict, order_id: int, db: Session) -> dict[str, Any]:
    ...
    if not order:
        raise ValueError("Order not found for this supplier")
...
def upload_parcel_proof(current_user: User | dict, order_id: int, file_content: bytes, file_filename: str | None, content_type: str, notes: str, db: Session) -> dict[str, Any]:
    user_id = _get_user_id(current_user)
    supplier = (
        db.query(SupplierProfile)
        .filter(SupplierProfile.user_id == user_id)
    ...
    ext = os.path.splitext(file_filename or ".jpg")[1] or ".jpg"
    proof_filename = f"proof_{timestamp}{ext}"
    proof_key = f"parcel_proofs/{order_id}/{proof_filename}"
    proof_url = _storage.save(proof_key, file_content, content_type=content_type)
    ...
    if is_first_proof:
        ...
        ref_url = _storage.save(ref_key, file_content, content_type=content_type)
    else:
...
def verify_parcel_proof(current_user: User | dict, order_id: int, db: Session) -> dict[str, Any]:
    user_id = _get_user_id(current_user)
    supplier = (
        db.query(SupplierProfile)
        .filter(SupplierProfile.user_id == user_id)
    ...
    if not proof_keys:
        raise ValueError("No parcel proof image found for this order.")
    ...
    try:
        ...
        result["reference_used"] = bool(reference_image_bytes)
        ...
        logger.warning("Parcel verification provider not available — returning basic check")
        result = {
            "status": "pending",
            "match_score": 0.0,
        ...
        logger.exception("Failed to verify parcel proof for order %s", order_id)
        return {
            "status": "error",
            "match_score": 0.0,
...
def replace_reference_image(current_user: User | dict, order_id: int, file_content: bytes, file_filename: str | None, content_type: str, db: Session) -> dict[str, Any]:
    user_id = _get_user_id(current_user)
    supplier = (
        db.query(SupplierProfile)
        .filter(SupplierProfile.user_id == user_id)
...
def get_reference_image(current_user: User | dict, order_id: int, db: Session) -> str:
    user_id = _get_user_id(current_user)
    supplier = (
        db.query(SupplierProfile)
        .filter(SupplierProfile.user_id == user_id)
    ...
    if not refs:
        raise ValueError("No reference image set for this order.")

    latest_key = refs[0]
    return _storage.url(latest_key)


def get_parcel_verification_history(current_user: User | dict, limit: int, db: Session) -> dict[str, Any]:
    user_id = _get_user_id(current_user)
    supplier = (
        db.query(SupplierProfile)
        .filter(SupplierProfile.user_id == user_id)
...
# ── ORM-based Order Operations (merged from supplier_order_service.py) ─────────
...
def list_supplier_orders_orm(db: Session, supplier_id: int) -> list[Order]:
