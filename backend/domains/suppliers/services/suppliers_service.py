...
def bulk_supplier_verification(
    ...
    for sid in supplier_ids:
        user = first(db, User, [User.id == sid, User.role == "supplier"])
        if not user:
            skipped.append({"id": sid, "reason": "Supplier not found"})
            continue

        profile = first(db, SP, [SP.user_id == sid])
        if profile is None:
        ...
        if action == "verify":
            ...
            add_to_session(db, Notification(
                user_id=user.id,
                type="account",
...
def bulk_manage_suppliers(
    ...
    for supplier_id in list(dict.fromkeys(supplier_ids)):
        user = first(db, User, [User.id == supplier_id, User.role == "supplier"])
        if not user:
            skipped.append({"id": supplier_id, "reason": "Supplier not found"})
            continue

        profile = first(db, SupplierProfile, [SupplierProfile.user_id == supplier_id])
        if profile is None:
...
def get_supplier_comparison(db: Session, limit: Optional[int] = None, offset: int = 0) -> dict[str, Any]:
    ...
    supplier_filters = [User.role == "supplier"]
    total = count(db, User, supplier_filters)
    suppliers = all_rows(
        db,
        User,
        supplier_filters,
    ...
    product_count_rows = aggregate_rows(
        db,
        [Product.supplier_id, func.count(Product.id).label("product_count")],
        [Product.supplier_id.in_(supplier_ids), Product.is_deleted.is_(False)],
    ...
    avg_price_rows = aggregate_rows(
        db,
        [Product.supplier_id, func.avg(Product.price).label("avg_price")],
        [Product.supplier_id.in_(supplier_ids), Product.is_deleted.is_(False)],
        group_by=[Product.supplier_id],
    )
    avg_prices = {
...
def get_pending_suppliers(db: Session, limit: Optional[int] = None, offset: int = 0) -> dict[str, Any]:
    ...
    resolved_limit = _ADMIN_DEFAULT_PAGE_SIZE if limit is None else max(1, min(limit, _ADMIN_MAX_PAGE_SIZE))
    supplier_filters = [User.role == "supplier", User.is_verified.is_(False)]
    total = count(db, User, supplier_filters)
    suppliers = all_rows(
        db,
        User,
...
def verify_supplier(user_id: int, note: Optional[str], acting_user: dict, db: Session) -> dict:
    ...
    profile = first(db, SupplierProfile, [SupplierProfile.user_id == user_id])
    if profile is None:
    ...
    if supplier_country:
        country_config = first(db, CountryConfig, [
        ...
        if country_config:
            ...
            if req_raw:
                try:
                    ...
                    logger.exception("verify_supplier_failed", error=str(e))
                    requirements = {}
...
def get_all_suppliers(
    ...
    *,
    ...
    supplier_outerjoins = [(SupplierProfile, SupplierProfile.user_id == User.id)]
    supplier_filters: list = [User.role == "supplier"]

    normalized_query = (q or "").strip()
    if normalized_query:
        like = f"%{normalized_query}%"
        supplier_filters.append(
            or_(
    ...
    if normalized_status and normalized_status != "all":
        if normalized_status == "pending":
            supplier_filters.append(User.is_active.in_([True, 1]))
            supplier_filters.append(
                or_(
            ...
            supplier_filters.append(SupplierProfile.verification_status == "rejected")
        elif normalized_status in {"suspended", "archived"}:
            supplier_filters.append(User.is_active.in_([False, 0]))
        elif normalized_status == "active":
            supplier_filters.append(User.is_active.in_([True, 1]))

    normalized_badge = (badge or "").strip().lower()
    if normalized_badge and normalized_badge != "all":
        supplier_filters.append(SupplierProfile.badge_level == normalized_badge)

    total = count(db, User, supplier_filters, outerjoins=supplier_outerjoins)
    suppliers = all_rows(
        db,
        User,
        supplier_filters,
        outerjoins=supplier_outerjoins,
    ...
    profiles = {
        ...
        for profile in all_rows(db, SupplierProfile, [SupplierProfile.user_id.in_(supplier_ids)])
    } if supplier_ids else {}

    product_metric_rows = aggregate_rows(
        db,
        [
            Product.supplier_id,
    ...
    top_product_rows = aggregate_rows(
        db,
        [Product.supplier_id, Product.name, Product.sales_count],
        [Product.supplier_id.in_(supplier_ids), Product.is_deleted.is_(False)],
        ...
        "active_suppliers": int(
            (
                scalar_with_filters(
                    db,
                    func.count(User.id),
                    [User.role == "supplier", User.is_active.in_([True, 1])],
                )
            ) or 0
        ),
        "suspended_suppliers": int(
            (
                scalar_with_filters(
                    db,
                    func.count(User.id),
                    [User.role == "supplier", User.is_active.in_([False, 0])],
                )
            ) or 0
        ),
        "total_revenue": round(
            float(
                (
                    scalar_with_filters(
                        ...
                        [User.role == "supplier", Product.is_deleted.is_(False)],
                        joins=[
