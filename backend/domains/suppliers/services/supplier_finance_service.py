"""Supplier finance read/write operations.

Owns the DB reads/writes for supplier payment-status and payout-status
endpoints so the router stays free of ``db.query``/``db.add``/``db.commit``.
"""
...
def get_supplier_payout_summary(db: Session, user_id: int) -> dict:
    ...
    paid_payouts = (
        db.query(Payout)
        .filter(Payout.user_id == user_id, Payout.status == "completed")
        .all()
    )
    ...
    bank_account = (
        ...
        .filter(SupplierBankAccount.supplier_id == user_id, SupplierBankAccount.is_active == True)  # noqa: E712
        .first()
    )
...
def get_order_payment_status(db: Session, order_id: int, user_id: int) -> dict:
    """Return detailed payment + payout status for a single supplier order."""
    order = (
        db.query(Order)
    ...
    settlement = (
        db.query(SupplierSettlement)
        .filter(SupplierSettlement.supplier_id == user_id, SupplierSettlement.order_id == order_id)
        .first()
    )
    ...
    ledger = (
        ...
        .filter(TransactionLedger.order_id == order_id, TransactionLedger.supplier_id == user_id)
        .first()
    )
...
def list_supplier_orders_with_payout_status(
    ...
    """Return paginated supplier orders with payment + payout status."""
    base = (
        db.query(Order)
        .join(OrderItem)
        .filter(OrderItem.supplier_id == user_id)
    ...
    settlements = {
        ...
        .filter(SupplierSettlement.supplier_id == user_id, SupplierSettlement.order_id.in_(order_ids))
        .all()
    }
    payout_ids = [s.payout_id for s in settlements.values() if s.payout_id]
    payouts = (
        {p.id: p for p in db.query(Payout).filter(Payout.id.in_(payout_ids)).all()}
        if payout_ids
        else {}
    )
...
def get_supplier_bank_account(db: Session, user_id: int) -> dict:
...
def upsert_supplier_bank_account(db: Session, user_id: int, payload: dict) -> dict:
    """Create or update the supplier's bank account."""
    account = db.query(SupplierBankAccount).filter(SupplierBankAccount.supplier_id == user_id).first()
