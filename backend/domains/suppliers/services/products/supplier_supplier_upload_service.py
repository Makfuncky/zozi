...
_CATEGORY_RECO_TTL_S = 3600
_CATEGORY_RECO_CACHE: Dict[str, tuple] = {}

async def ab_test_bg_strategies(image: UploadFile=File(...), strategies: Optional[str]=Form(None), current_user: dict=Depends(require_roles('supplier', 'admin'))):
...
# from domains.finance.services.shared.bg_removal_service import VALID_STRATEGIES
        ...
        for strategy in test_strategies:
            try:
                ...
                scores[strategy] = {'edge_clarity': 0.0, 'alpha_confidence': 0.0, 'coverage': 0.0, 'overall': 0.0, 'edge_pixels_pct': 0.0}
                timing[strategy] = 0.0
                results[strategy] = ''
        gc.collect()
        return {'winner': winner, 'scores': scores, 'results': results, 'timing_ms': timing, 'strategies_tested': test_strategies, 'image_dimensions': {'width': img.width, 'height': img.height}}
    job = enqueue_ml_job(owner_user_id=owner_id, owner_role=owner_role, func=_run_ab_test, metadata={'strategies': strategies}, max_retries=1)
...
def _compute_quality_score(image_np: np.ndarray, alpha_map: np.ndarray) -> Dict[str, float]:
    ...
    if _HAS_CV2:
        try:
            ...
            return {'edge_clarity': 0.0, 'alpha_confidence': round(alpha_confidence, 4), 'coverage': round(coverage, 4), 'overall': round(alpha_confidence * 0.4 + coverage * 0.2, 4), 'edge_pixels_pct': 0.0}
