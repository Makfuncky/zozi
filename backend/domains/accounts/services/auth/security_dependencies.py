...
either same-domain (``domains.governance``) or cross-domain (``domains.governance``)
...
def require_permissions(slugs: list):
    ...
    from domains.governance.ports import check_permission
...
def start_otp(user, purpose: str, channel: str = "email", destination: str | None = None, db: Session | None = None):
    ...
    """
    from domains.accounts.ports import start_otp as _start_otp
...
def verify_otp(user, purpose: str, code: str, db: Session | None = None) -> bool:
    ...
    """
    from domains.accounts.ports import verify_otp as _verify_otp
