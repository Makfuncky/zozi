...
from infrastructure.utils.auth import blacklist_token, create_access_token, create_refresh_token, decode_token, get_password_hash, is_refresh_family_revoked, is_refresh_token_used, mark_refresh_token_used, revoke_refresh_family, verify_password
...
if TYPE_CHECKING:
    from domains.accounts.models.user import User, UserLoginHistory
...
def _record_login_history(db: Session, user: "User", request: Request | None=None, success: bool=True) -> None:
    ...
    UserLoginHistory = _get_cross_domain_model("UserLoginHistory")
    try:
...
def register(payload: RegisterRequest, db: Session=Depends(get_db)):
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Registration could not be completed. If this persists, contact support.")
...
def refresh(payload: RefreshRequest | None=None, request: Request=None, db: Session=Depends(get_db)):
    ...
    if is_refresh_family_revoked(family_id):
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail='Refresh token family has been revoked')
    if is_refresh_token_used(family_id, jti):
        revoke_refresh_family(family_id)
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail='Refresh token has been reused. All sessions revoked.')
    mark_refresh_token_used(family_id, jti)
    user = db.query(User).filter(User.id == int(decoded.get('sub'))).first()
    if not user or not user.is_active:
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail='User not found')
    access = create_access_token({'sub': str(user.id), 'role': user.role})
    new_refresh = create_refresh_token({'sub': str(user.id), 'role': user.role}, family_id=family_id)
    resp = JSONResponse(jsonable_encoder(TokenResponse(access_token=access, refresh_token=new_refresh, token_type='bearer', user=UserOut.model_validate(user))))
    resp.set_cookie(key='access_token', value=access, httponly=True, samesite='lax' if str(settings.app_env).lower() != 'production' else 'none', secure=str(settings.app_env).lower() == 'production', max_age=settings.access_token_expire_minutes * 60, path='/')
    resp.set_cookie(key='refresh_token', value=new_refresh, httponly=True, samesite='lax' if str(settings.app_env).lower() != 'production' else 'none', secure=str(settings.app_env).lower() == 'production', max_age=settings.refresh_token_expire_days * 86400, path='/auth/refresh')
    return resp
