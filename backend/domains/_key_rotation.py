...
Usage:
    from domains._key_rotation import rotate_encryption_key
...
Canonical home relocated from ``infrastructure.security.key_rotation`` (P6
infrastructure->domains inversion cleanup). Domain-layer module is permitted
to reference peer domain models below.
"""
...
def rotate_encryption_key(old_raw_key: str, new_raw_key: str, db: Session) -> dict:
    ...
    for Model, columns in _ENCRYPTED_COLUMNS:
        ...
        try:
            offset = 0
            while True:
                batch = keyset_offset_window(db.query(Model), offset=offset, limit=BATCH_SIZE)
                if not batch:
                    break
                for row in batch:
                    rows_processed += 1
