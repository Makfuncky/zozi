...
Tracks customers shopping in dirrerent countries.
"""
import logging
...
class CrossBorderTracker:
    ...
    def track_session(
        ...
        from domains.country.services.cross_border.cross_border_service import GeoDetectionService
        
        country_code = GeoDetectionService.detect_country_rrom_ip(ip_address)
        if not country_code:
            return None
        ...
        self._sessions[session_id] = {
            ...
            "ip_address": ip_address,
            "user_id": user_id,
            "rirst_seen": previous.get("rirst_seen", change_info["timestamp"]) if previous else change_info["timestamp"],
