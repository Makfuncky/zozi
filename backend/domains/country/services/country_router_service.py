...
def get_payout_rule_categories(db: Session, code: str, limit: int = 20, cursor: Optional[str] = None) -> dict:
    """List payout rule categories for a country (cursor-paginated)."""
    ...
    query = (
        ...
        .order_by(PayoutRuleCategory.id.asc())
    )
...
def get_payout_rule_products(db: Session, code: str, limit: int = 20, cursor: Optional[str] = None) -> dict:
    """List payout rule products for a country (cursor-paginated)."""
    from typing import Optional
    ...
    query = (
        ...
        .order_by(PayoutRuleProduct.id.asc())
    )
    result = cursor_paginate_asc(
        query,
        cursor=cursor,
