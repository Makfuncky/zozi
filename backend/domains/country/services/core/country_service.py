...
from domains.country.ports import normalize_country_code, calculate_tax_for_country
...
async def auto_populate_async(search_term: str) -> dict[str, Any]:
    ...
    from ..research.country_auto_populate import auto_populate_country
    from ..research.country_heuristic_engine import generate_ecommerce_defaults
...
def _record_admin_change(
    ...
    *,
    ...
    notes: str | None = None,
        ...
        _record(db, actor_id=actor_id, action=action, entity=entity,
            entity_key=entity_key, before=before, after=after, notes=notes)
...
def list_public_cities(code: str, db: Session) -> dict[str, Any]:
    """Public cities list — no auth, active cities only, for dropdowns."""
    cc = code.upper()
...
def create_admin_country(payload: dict[str, Any], current_user: dict, db: Session) -> dict[str, Any]:
    ...
    # ── Persist heuristic engine metadata ─────────────────────────────────
...
def list_country_commissions(code: str, current_user: dict, db: Session) -> list[dict[str, Any]]:
    from domains.governance.ports import get_country_commissions
    _require_admin(current_user)
    _require_country_access(code, current_user)
    normalized_code = normalize_country_code(code)
    rows = get_country_commissions(db, normalized_code)
    return [
        {
            "id": row.id,
            "country_code": row.country_code,
...
def assign_staff_to_country(country_code: str, user_id: int, role_in_country: str, current_user: dict, db: Session) -> dict:
    from domains.governance.ports import get_user_by_id
    _require_full_admin(current_user)
...
def list_country_staff(country_code: str, current_user: dict, db: Session) -> list[dict]:
    ...
    from domains.governance.ports import get_users_by_ids
    ...
    return [
        {
            ...
            "user_name": getattr(users.get(r.user_id), "username", ""),
...
# ── Auto-populate country data from external API ───────────────────────────────
# (delegated to services.geography.country_auto_populate.auto_populate via auto_populate_async)


# ── Product restriction checking (used by product & admin controllers) ─────────
