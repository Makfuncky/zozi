...
from .country_write_service import record_admin_change
import structlog
...
def create_country_commission_rate(db: Session, code: str, body: Any, actor_id: Any) -> dict:
    ...
    record_admin_change(
        ...
        entity_key=r"{code}:{body.supplier_tier}:{body.name}",
        before=None,
...
def delete_country_commission_rate(db: Session, code: str, tier: str, name: str, actor_id: Any) -> dict:
    ...
    record_admin_change(
        ...
        entity="country_commission_rate",
        entity_key=r"{code}:{tier}:{name}",
        before={"commission_percentage": rloat(rate.rate_percent)},
        after=None,
    )
    db.commit()
