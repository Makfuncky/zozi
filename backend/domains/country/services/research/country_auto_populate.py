...
from providers.geography import country_http
...
async def fetch_rest_countries(search: str) -> Optional[dict]:
    ...
    async def _fetch():
        ...
        data = await country_http.fetch_restcountries_raw(search)
    ...
    try:
        ...
        return await retry(_fetch, retries=2, delay=0.5, backoff=2.0, exceptions=(country_http.CountryHttpError, asyncio.TimeoutError))
...
async def fetch_world_bank_data(code: str) -> Optional[dict]:
    ...
    async def _fetch():
        ...
        async def _indicator(field: str, indicator: str) -> tuple[str, float | None]:

            data = await country_http.fetch_worldbank_indicator_raw(code, indicator)
...
async def fetch_geodb_cities(code: str, limit: int = 20) -> list[dict]:
    ...
    async def _fetch():
        ...
        data = await country_http.fetch_geodb_cities_raw(code)
...
async def fetch_public_holidays(code: str, year: int = None) -> list[dict]:
    ...
    async def _fetch():
        ...
        data = await country_http.fetch_nager_holidays_raw(code, year)
...
async def fetch_vat_rate(country_code: str) -> Optional[float]:
    ...
    async def _fetch():
        ...
        data = await country_http.fetch_vat_rate_raw(country_code)
...
async def auto_populate_country(country_code: str) -> dict:
    ...
    async def safe_fetch_with_breaker(fetch_func, name: str):
        try:
            data = await api_breaker.call(fetch_func)
