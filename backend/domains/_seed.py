...
from domains.orders.ports import Order, order_item_query
...
from domains.orders.ports import order_logistics_allocations_query
...
def _ensure_demo_pickup_ready_shipment(
    ...
    *,
    ...
    order = get_order_by_id(db, shipment.order_id) if shipment else None
    if order is None:
        order = Order(
    ...
    allocation = order_logistics_allocations_query(
        db, order_id=order.id, supplier_id=supplier_user.id
    ).first()
    if not allocation:
