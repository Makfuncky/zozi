    ...
    async def broadcast_to_room(self, room: str, message: dict) -> None:
        ...
        for ws in list(self.rooms[room]):
            try:
                ...
                logger.debug("WebSocket send failed, removing socket from room %s", room, exc_info=True)
...
class CommandCenterService:
    ...
    def _get_avg_response_time(self) -> float:
        try:
            ...
            logger.warning("Failed to get API response time, using default", exc_info=True)
            return 0.15
    ...
    def _get_cpu_usage(self) -> float:
        try:
            ...
            logger.warning("Failed to get CPU usage, using default", exc_info=True)
            return 0.0
    ...
    def _get_memory_usage(self) -> float:
        try:
            ...
            logger.warning("Failed to get memory usage, using default", exc_info=True)
            return 0.0
