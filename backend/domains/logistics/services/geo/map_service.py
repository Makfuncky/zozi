...
from domains.logistics.models.logistics_country_sync import ShopWarehouseLocation
...
class MapService:
    ...
    def get_cities_for_map(self, country_code: str, region: Optional[str] = None, limit: int = 100) -> List[Dict[str, Any]]:
        """Get cities with coordinates for map markers."""
        query = (
            self.db.query(CountryCity)
            .filter(CountryCity.country_code == country_code.upper(), CountryCity.is_active == True)
            .limit(limit)
        )
        if region:
    ...
    def get_warehouses_for_map(self, country_code: str, include_inactive: bool = False) -> List[Dict[str, Any]]:
        """Get warehouse locations for map."""
        query = self.db.query(ShopWarehouseLocation).filter(ShopWarehouseLocation.country_code == country_code.upper())
    ...
    def get_delivery_zones(self, country_code: str, zone_type: Optional[str] = None) -> List[Dict[str, Any]]:
        """Get delivery zones for a country."""
        from domains.country.models.country_enhancements import OmanDeliveryZone
        query = self.db.query(OmanDeliveryZone).filter(OmanDeliveryZone.is_active == True)
        zones = query.all()
        return [
            {
    ...
    def generate_map_markers(self, country_code: str, include_parcels: bool = False) -> Dict[str, Any]:
            ...
            "cities": [
                {
                    ...
                    "lat": c["latitude"],
                    "lng": c["longitude"],
                    "popup": f"<strong>{html.escape(c['name'])}</strong><br/>Population: {html.escape(str(c['population'] or 'N/A'))}",
                }
                for c in cities
            ...
            "warehouses": [
                {
                    ...
                    "lat": w["latitude"],
                    "lng": w["longitude"],
                    "popup": f"<strong>{html.escape(w['name'])}</strong><br/>{html.escape(w['warehouse_code'])}",
                }
                for w in warehouses
        ...
        if include_parcels:
            from domains.logistics.models.logistics import Shipment, ParcelLocationTracker
            shipments = self.db.query(Shipment).filter(Shipment.country_code == country_code.upper()).all()
            parcel_trackers = self.db.query(ParcelLocationTracker).filter(ParcelLocationTracker.country_code == country_code.upper()).all()
            ...
            markers["parcels"] = [
                {
                    ...
                    "lat": loc.latitude,
                    "lng": loc.longitude,
                    "popup": f"Parcel {html.escape(str(pid))}",
                }
    ...
    def get_region_bounds(self, country_code: str, region_name: str) -> Optional[Dict[str, float]]:
