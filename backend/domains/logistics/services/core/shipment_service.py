...
"""Shipment service — shipment operations and fulfillment."""
...
from domains.orders.models.order_entities import Order, OrderItem, OrderLogisticsAllocation
...
async def get_orders_to_fulfil(current_user: dict, db: Session, limit: int = 200, offset: int = 0, cursor: Optional[int] = None) -> list[dict]:
    ...
    orders = orders.limit(min(max(1, limit), 200)).all()
...
async def create_shipment(data: dict, current_user: dict, db: Session) -> dict:
    ...
    try:
        ...
        from domains.finance.ports import create_invoice_from_order
...
async def get_shipment_history(current_user: dict, db: Session, page: int = 1, per_page: int = 30, cursor: Optional[int] = None) -> dict:
