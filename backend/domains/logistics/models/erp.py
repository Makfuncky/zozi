"""logistics domain — ERP/procurement models previously housed under finance.

These models were originally defined in ``domains/finance/models/erp.py``.
They describe logistics-domain concepts (warehouses, purchase orders, stock movements)
and have been relocated here to remove cross-domain pollution.
...
class PurchaseOrder(Base):
    ...
    country_code = Column(String(2), nullable=True, index=True)
    created_by_id = Column(Integer, ForeignKey('governance.users.id', ondelete='RESTRICT'), nullable=True, index=True)
...
class GoodsReceiptNote(Base):
    ...
    notes = Column(Text, nullable=True)
    received_by_id = Column(Integer, ForeignKey('governance.users.id', ondelete='RESTRICT'), nullable=True, index=True)
    country_code = Column(String(2), nullable=True, index=True)
    created_at = Column(DateTime, server_default=func.now())
    updated_at = Column(DateTime, server_default=func.now(), onupdate=func.now())
...
class ImportShipment(Base):
    ...
    shipment_date = Column(DateTime, server_default=func.now())
