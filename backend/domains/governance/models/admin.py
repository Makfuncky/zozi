...
class SystemAlert(Base):
    ...
    acknowledged_by_id = Column(Integer, ForeignKey("accounts.users.id", ondelete="SET NULL"), nullable=True, index=True)
...
class AdminChangeAuditLog(Base):
    ...
    __table_args__ = ({"schema": "governance"},)
    id = Column(Integer, primary_key=True, index=True)
    admin_id = Column(Integer, ForeignKey("accounts.users.id", ondelete="SET NULL"), nullable=False, index=True)
...
class BadgeBillingRecord(Base):
    ...
    bank_transaction_id = Column(Integer, ForeignKey("finance.bank_transactions.id", ondelete="SET NULL"), nullable=True, index=True)
    created_at = Column(DateTime, server_default=func.now(), nullable=False)
    updated_at = Column(DateTime, server_default=func.now(), onupdate=func.now(), nullable=True)
    country_code = Column(String(2), nullable=True, index=True)
...
class LogisticsCODRemittanceReceipt(Base):
    ...
    __table_args__ = ({"schema": "governance"},)
    id = Column(Integer, primary_key=True, index=True)
    partner_id = Column(Integer, ForeignKey("logistics.logistics_partners.id", ondelete="SET NULL"), nullable=True, index=True)
    shipment_id = Column(Integer, ForeignKey("logistics.shipments.id", ondelete="SET NULL"), nullable=True, index=True)
    settlement_id = Column(Integer, ForeignKey("governance.logistics_settlements.id", ondelete="SET NULL"), nullable=True, index=True)
    amount = Column(Numeric(12, 2), nullable=False)
...
class LogisticsSettlement(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    partner_id = Column(Integer, ForeignKey("logistics.logistics_partners.id", ondelete="SET NULL"), nullable=False, index=True)
    order_id = Column(Integer, ForeignKey("orders.orders.id", ondelete="SET NULL"), nullable=True, index=True)
    ...
    currency = Column(String(3), nullable=True)
    is_deleted = Column(Boolean, default=False, server_default='false', nullable=False, index=True)
    payout_id = Column(Integer, ForeignKey("finance.payouts.id", ondelete="SET NULL"), nullable=True, index=True)
...
class ShipmentConfirmation(Base):
    ...
    __table_args__ = ({"schema": "governance"},)
    id = Column(Integer, primary_key=True, index=True)
    shipment_id = Column(Integer, ForeignKey("logistics.shipments.id", ondelete="SET NULL"), nullable=False, index=True)
    order_id = Column(Integer, ForeignKey("orders.orders.id", ondelete="SET NULL"), nullable=True, index=True)
    supplier_id = Column(Integer, ForeignKey("accounts.users.id", ondelete="SET NULL"), nullable=True, index=True)
    requester_user_id = Column(Integer, ForeignKey("accounts.users.id", ondelete="SET NULL"), nullable=True, index=True)
    requester_role = Column(String(255), nullable=True)
    target_user_id = Column(Integer, ForeignKey("accounts.users.id", ondelete="SET NULL"), nullable=True, index=True)
...
class ChatbotQueryEvent(Base):
    ...
    clicked_product_id = Column(Integer, ForeignKey("catalog.products.id", ondelete="SET NULL"), nullable=True, index=True)
    is_deleted = Column(Boolean, default=False, nullable=False, index=True)
    created_at = Column(DateTime, server_default=func.now(), nullable=False)
    updated_at = Column(DateTime, server_default=func.now(), onupdate=func.now(), nullable=True)
    country_code = Column(String(2), nullable=True, index=True)
...
class ProductVerification(Base):
    ...
    __table_args__ = ({"schema": "governance"},)
    id = Column(Integer, primary_key=True, index=True)
    product_id = Column(Integer, ForeignKey("catalog.products.id", ondelete="SET NULL"), nullable=False, index=True)
    status = Column(String(255), default="pending")
