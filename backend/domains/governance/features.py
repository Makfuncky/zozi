...
# === Merged from accounts/features.py ===

"""Accounts domain — AXIS 3 feature atoms (permission catalog seed).

Single-sourced here; aggregated by ``rbac/catalog.py``. The strings below are the
canonical permission atoms for the accounts domain. CI must fail on any
``require_feature("accounts.*")`` literal that is not present in this map.
"""
...
FEATURES: dict[str, dict] = {
    ...
    "accounts.user.read": {
        ...
        "risk": "medium",
        "actions": ["read"],
        "description": "View user account details and profile.",
    },
    ...
    "accounts.auth.login": {
        ...
        "risk": "high",
        "actions": ["read", "write"],
        "description": "Perform login, token refresh, and logout operations.",
    },
    "accounts.auth.mfa.manage": {
        "label": "Manage MFA",
        "risk": "high",
        "actions": ["read", "write"],
        "description": "Enable, disable, and enforce multi-factor authentication.",
    },
    "accounts.identity.verify": {
        "label": "Verify Identity",
        "risk": "high",
        "actions": ["read", "write", "approve"],
        "description": "Verify user identity, email, and linked social identities.",
    },
    "accounts.session.manage": {
        "label": "Manage Sessions",
        "risk": "medium",
        "actions": ["read", "write", "delete"],
        "description": "View and revoke active user sessions and tokens.",
    },
    "accounts.address.manage": {
        "label": "Manage Addresses",
        "risk": "low",
        "actions": ["read", "write", "delete"],
        "description": "Manage a user's saved delivery addresses.",
    },
}
...
def is_known(feature: str) -> bool:
    if feature in FEATURES:
        return True
    # Support wildcard form, e.g. "accounts.*"
    if feature.endswith(".*"):
        prefix = feature[:-1]
        return any(f.startswith(prefix) for f in FEATURES)
    return False
