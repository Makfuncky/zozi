...
def list_rates(country_code: str, page: int, page_size: int, db: Session) -> dict:
    get_country_or_404(country_code.upper(), db)
    set_rls_context({country_code.upper()}, is_restricted=True)
    try:
        q = db.query(CommissionCategoryRate).filter(CommissionCategoryRate.country_code == country_code.upper())
        total = q.count()
        rows = q .offset(page_size).limit(page_size).all()
        return {"data": rows, "total": total, "page": page, "page_size": page_size}
    finally:
        clear_rls_context()


def create_rate(country_code: str, payload: CommissionCategoryRateCreate, db: Session) -> CommissionCategoryRate:
    get_country_or_404(country_code.upper(), db)
    set_rls_context({country_code.upper()}, is_restricted=True)
    try:
...
def update_rate(country_code: str, rate_id: int, payload: CommissionCategoryRateCreate, db: Session) -> CommissionCategoryRate:
    get_country_or_404(country_code.upper(), db)
    ...
    if not r:
        raise ValueError("Category rate not found")
    data = payload.model_dump() if payload else {}
...
def list_badge_tiers(country_code: str, page: int, page_size: int, db: Session) -> dict:
    get_country_or_404(country_code.upper(), db)
    set_rls_context({country_code.upper()}, is_restricted=True)
    try:
...
def create_badge_tier(country_code: str, payload: CommissionBadgeTierCreate, db: Session) -> CommissionBadgeTier:
    get_country_or_404(country_code.upper(), db)
    set_rls_context({country_code.upper()}, is_restricted=True)
    try:
...
def update_badge_tier(country_code: str, tier_id: int, payload: CommissionBadgeTierCreate, db: Session) -> CommissionBadgeTier:
    get_country_or_404(country_code.upper(), db)
    ...
    if not t:
        raise ValueError("Badge tier not found")
    data = payload.model_dump() if payload else {}
