"""Business logic extracted from routers/auth.py."""
...
def issue_auth_response(user: User):
    """Issue the standard auth response (access + refresh tokens) for a user.

    Used by social/OIDC sign-in after the local User has been found or created.
    Mirrors the token-issuing logic in ``login``/``register``.
    """
    family_id = uuid.uuid4().hex
    access = create_access_token({"sub": str(user.id), "role": user.role})
    refresh = create_refresh_token({"sub": str(user.id), "role": user.role}, family_id=family_id)
