...
from domains.finance.ports import build_transfer_export_payload
from infrastructure.utils.background_jobs import enqueue_job, get_job

logger = logging.getLogger(__name__)

ADMIN_ROLES = {"admin", "superadmin"}
EXPORTS_DIR = Path(__file__).resolve().parents[2] / "artifacts" / "exports"
...
def export_orders_csv(current_user: dict, db: Session) -> StreamingResponse:
    ...
    return _csv_streaming_response(
        _csv_stream(_iter_rows(db, query, _map_order), ORDER_FIELDS),
        "orders_export.csv",
    )
...
def export_products_csv(current_user: dict, db: Session) -> StreamingResponse:
    ...
    return _csv_streaming_response(
        _csv_stream(_iter_rows(db, query, _map_product), PRODUCT_FIELDS),
        "products_export.csv",
    )
...
def export_coupons_csv(current_user: dict, db: Session) -> StreamingResponse:
    ...
    return _csv_streaming_response(
        _csv_stream(_iter_rows(db, query, _map_coupon), COUPON_FIELDS),
        "coupons_export.csv",
    )
...
def export_audit_logs_csv(current_user: dict, db: Session, days: int = 30) -> StreamingResponse:
    ...
    return _csv_streaming_response(
        _csv_stream(_iter_rows(db, query, _map_audit_log), AUDIT_FIELDS),
...
def export_pay_equity(db: Session) -> list[dict]:
    ...
    from finance.ports import payroll_equity
    return payroll_equity(db)