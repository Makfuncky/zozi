...
from comms.ports import get_video_conference
from comms.ports import list_all_video_rooms

def list_video_rooms(db: Session, country: Optional[str] = None):
    q = db.query(VideoRoom)
    if country:
        q = q.filter(VideoRoom.country_code == country)
    return q.all()

def ensure_video_room_country(db: Session, room_id: int, country_code: str):
    room = db.query(VideoRoom).filter(VideoRoom.id == room_id).first()
    if room and country_code:
        room.country_code = country_code
        db.commit()
        db.refresh(room)
    return room

def list_video_rooms_for_country(db: Session, country_code: str):
    return db.query(VideoRoom).filter(VideoRoom.country_code == country_code).all()

def video_room_metrics(db: Session):
    total = db.query(VideoRoom).count()
    active = db.query(VideoRoom).filter(VideoRoom.is_active == True).count()
    return {"total_rooms": total, "active_rooms": active}
...
# _resolve_country: local implementation below (admin_comms_messaging_service does not exist)
...
def _resolve_country(request, default=""):
    """Extract country code from request headers (X-Country-Code) or return default."""
    if request is None:
        return default
    return request.headers.get("X-Country-Code") or default
