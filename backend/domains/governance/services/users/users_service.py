...
def update_user_role(user_id: int, role: str, acting_user: dict, db: Session) -> dict:
    ...
    if role in STAFF_ROLES and acting_user.get("role") not in ("admin", "super_admin"):
...
def toggle_user_active(user_id: int, acting_user: dict, db: Session) -> dict:
    ...
    setattr(user, "is_active", not current_active)
    db.commit()
    audit_log(
        db=db,
...
def _build_user_delete_blocker(user, acting_user, db, delete_orders=False, order_count=0):
    """Return (status_code, detail) tuple if deletion should be blocked, None otherwise."""
    user_role = getattr(user, "role", None)
    if user_role == "super_admin" and acting_user.get("role") != "super_admin":
        return (403, "Cannot delete super_admin accounts")
    if user.email in _PROTECTED_EMAILS and acting_user.get("role") != "super_admin":
        return (403, "Protected account cannot be deleted")
    if not delete_orders and order_count > 0:
        return (409, f"User has {order_count} order(s). Delete orders first or pass delete_orders=True.")
    if delete_orders:
        return None
    # Check blocking supplier records
    for model, field, label in _DELETE_BLOCKING_SUPPLIER_MODELS:
        count = db.query(model).filter(field == user.id).count()
        if count > 0:
            return (409, f"User has {count} {label} that must be removed first.")
    return None


def _hard_delete_user_record(user, db):
    """Nullify references then delete the user record."""
    user_id = user.id
    # Nullify nullable user references
    for model, field, _label in _NULLABLE_USER_REFERENCE_UPDATES:
        try:
            db.query(model).filter(field == user_id).update({field: None}, synchronize_session=False)
        except Exception:
            pass
    # Delete user-owned records
    for model, field in _DELETABLE_USER_OWNED_MODELS:
        try:
...
def update_user_profile(user_id: int, updates: dict, db: Session) -> Optional[User]:
...
def list_all_users_paginated(skip: int = 0, limit: int = 50, db: Session = None) -> list:
