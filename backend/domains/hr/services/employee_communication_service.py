...
def send_internal_email(
    ...
    """
    from _legacy.models import User
    from _legacy.models.employee_models import InternalEmail, EmailFolder, Employee
    ...
    email = InternalEmail(
        ...
        body_html=body_html,
        body_text=body_html,
        recipients=recipients_json + cc_json,
    ...
    all_uids = list(recipient_ids) + list(cc_ids or [])
    users_by_id = {}
    emps_by_user = {}
    if all_uids:
        user_rows = db.query(User).filter(User.id.in_(all_uids)).all()
        for u in user_rows:
            users_by_id[u.id] = u
        emp_rows = db.query(Employee).filter(Employee.user_id.in_(all_uids)).all()
        for e in emp_rows:
            emps_by_user[e.user_id] = e

    emp_ids = [e.id for e in emps_by_user.values() if e]
    folders_by_emp = {}
    if emp_ids:
        folder_rows = db.query(EmailFolder).filter(
            EmailFolder.employee_id.in_(emp_ids),
            EmailFolder.name == "inbox",
        ).all()
        for f in folder_rows:
            folders_by_emp[f.employee_id] = f
    ...
    for uid in all_uids:
        ...
        if not emp:
            emp = emps_by_user.get(uid)
        if not emp:
            continue
        folder = folders_by_emp.get(emp.id)
        if not folder:
            folder = EmailFolder(employee_id=emp.id, name="inbox", folder_type="inbox", is_system=True)
            db.add(folder)
            db.flush()
            folders_by_emp[emp.id] = folder
...
def get_inbox(
    ...
    sender_names = {}
    if sender_ids:
        placeholders = ", ".join(f":sid{i}" for i in range(len(sender_ids)))
        params = {f"sid{i}": sid for i, sid in enumerate(sender_ids)}
        srows = db.execute(
            text(f"""
                SELECT e.id, u.full_name FROM users u
        ...
        for s in srows:
            sender_names[s["id"]] = s["full_name"]

    emails = []
    for r in rows:
        sender = sender_names.get(r["sender_id"])
