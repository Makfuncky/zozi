...
from controllers.analytics.analytics import ROLE_PERMISSION_MAP
import structlog
...
def load_role_permission_settings(db: Session) -> dict[str, set[str]]:
    ...
    rows = all_rows(db, RolePermissionSetting)
    for row in rows:
        ...
        if isinstance(raw_permissions, str):
            ...
            if serialized_permissions:
                try:
                    ...
                    logger.exception("load_role_permission_settings_failed", error=str(e))
...
def update_role_permissions(role: str, new_permissions: list[str], db: Session, current_user: dict) -> dict:
    ...
    _upsert_role_permission_setting(
        db=db,
        role=role,
