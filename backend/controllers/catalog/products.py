...
def get_all_products(
    ...
    if filter_value == "deleted":
        filters.append(Product.is_deleted.is_(True))
    else:
        filters.append(Product.is_deleted.is_(False))
        if filter_value == "pending":
            filters.append(Product.is_approved.is_(False))
        elif filter_value == "approved":
            filters.append(Product.is_approved.is_(True))
        elif filter_value == "rejected":
            filters.append(Product.is_approved.is_(False))
    total = count(db, Product, filters, options=options)
    products = all_rows(
        db,
        Product,
        filters,
...
def delete_product_admin(product_id: int, acting_user: dict, db: Session) -> dict:
    product = first(db, Product, [Product.id == product_id])
    if not product:
    ...
    affected_orders = all_rows(
        ...
        joins=[(OrderItem, OrderItem.order_id == Order.id)],
    )
...
def get_pending_products(db: Session, limit: Optional[int] = None, offset: int = 0) -> dict[str, Any]:
    ...
    resolved_limit = _ADMIN_DEFAULT_PAGE_SIZE if limit is None else max(1, min(limit, _ADMIN_MAX_PAGE_SIZE))
    product_filters = [Product.is_approved.is_(False), Product.is_deleted.is_(False)]
    total = count(db, Product, product_filters)
    products = all_rows(
        db,
        Product,
        product_filters,
