...
from services.country_read_service import (
    count_active_cities_for_country,
    get_active_country_by_code,
    get_communication_by_id,
    get_country_by_code,
    get_country_staff_assignment,
    get_country_version,
    get_latest_version_number,
    get_published_country_version,
    get_user_by_id,
    get_users_by_ids,
    list_active_cities_for_country,
    list_active_countries,
    list_all_oman_delivery_zones,
    list_country_commissions as _list_country_commissions_service,
    list_country_commissions_ordered,
    list_country_communications,
    list_country_configs,
    list_country_feature_flags as _list_country_feature_flags_service,
    list_country_feature_flags_ordered,
    list_country_staff_assignments,
    list_country_versions,
    list_oman_delivery_zones_ordered,
    list_recent_cross_country_sessions,
    search_cities_for_country,
    list_cities_for_country_ordered,
)
...
def _from_json(raw: str | None, *, default: Any) -> Any:
    ...
    try:
        ...
        logger.exception("_from_json_failed", error=str(e))
        return default
    return parsed
...
def _to_decimal(value: Any, *, field: str) -> Decimal:
    try:
        ...
        logger.exception("_to_decimal_failed", error=str(exc))
...
def _next_version(db: Session, country_code: str, config_type: str) -> int:
    return get_latest_version_number(db, country_code, config_type)
...
def _country_public_payload(country: CountryConfig, db: Session | None = None) -> dict[str, Any]:
    ...
    if db is not None:
        try:
            city_count = count_active_cities_for_country(db, country.code)
        except (ValueError, TypeError, KeyError, IndexError, AttributeError, RuntimeError, OSError, IOError, EOFError, ImportError, NameError, StopIteration, ArithmeticError, AssertionError, UnicodeError, NotImplementedError, RecursionError, ReferenceError, SystemError, BufferError, LookupError) as exc:
            logger.debug("Failed to count active cities for %s: %s", country.code, exc)
            city_count = 0
...
def list_public_cities(code: str, db: Session) -> dict[str, Any]:
    ...
    cc = code.upper()
    country = get_active_country_by_code(db, cc)
    if not country:
        raise HTTPException(status_code=404, detail="Country not found or inactive")

    cities = list_active_cities_for_country(db, cc)
    return {
        "code": cc,
        "cities": [
            {
...
def list_admin_countries(current_user: dict, db: Session) -> list[dict[str, Any]]:
    ...
    rows = list_country_configs(db, assigned if assigned else None)
...
def create_admin_country(payload: dict[str, Any], current_user: dict, db: Session) -> dict[str, Any]:
    ...
    existing = get_country_by_code(db, normalized_code)
    if existing:
...
def list_country_versions(code: str, current_user: dict, db: Session, config_type: str | None = None) -> list[dict[str, Any]]:
    ...
    rows = list_country_versions(db, normalized_code, config_type)
    return [
        {
            "id": row.id,
...
def approve_country_version(code: str, version_id: int, current_user: dict, db: Session) -> dict[str, Any]:
    ...
    _require_country_access(code, current_user)
    normalized_code = normalize_country_code(code)
    row = get_country_version(db, version_id, normalized_code)
    if not row:
...
def _apply_version_payload(row: CountryConfigVersion, db: Session) -> None:
    ...
    if row.config_type == "tax":
        ...
        if isinstance(zones, list) and country.logistics_model == "zone":
            existing = {zone.zone_code: zone for zone in list_all_oman_delivery_zones(db)}
        ...
        if isinstance(rates, list):
            existing_rows = {
                ...
                for entry in _list_country_commissions_service(db, row.country_code)
            }
            for item in rates:
        ...
        if isinstance(feature_flags, dict):
            existing_flags = {
                ...
                for flag in _list_country_feature_flags_service(db, row.country_code)
            }
...
def rollback_country_to_version(code: str, version_id: int, current_user: dict, db: Session) -> dict[str, Any]:
    ...
    _require_country_access(code, current_user)
    normalized_code = normalize_country_code(code)
    row = get_published_country_version(db, version_id, normalized_code)
    if not row:
...
def list_country_commissions(code: str, current_user: dict, db: Session) -> list[dict[str, Any]]:
    ...
    _require_country_access(code, current_user)
    normalized_code = normalize_country_code(code)
    rows = list_country_commissions_ordered(db, normalized_code)
    return [
        {
            "id": row.id,
            "country_code": row.country_code,
...
def get_country_feature_flags(code: str, current_user: dict, db: Session) -> list[dict[str, Any]]:
    ...
    _require_country_access(code, current_user)
    normalized_code = normalize_country_code(code)
    rows = list_country_feature_flags_ordered(db, normalized_code)
    return [
        {
            "id": row.id,
            "country_code": row.country_code,
...
def list_country_delivery_zones(code: str, current_user: dict, db: Session) -> list[dict[str, Any]]:
    ...
    rows = list_oman_delivery_zones_ordered(db)
    return [
        {
...
def test_gateway_connection(code: str, gateway_id: str, environment: str, current_user: dict, db: Session) -> dict[str, Any]:
    ...
    response = payment_service.test_payment_gateway_connection(gateway_id, db)
    latency_ms = (time.perf_counter() - start) * 1000
    return {
        "success": response.test_status == "passed",
...
def list_country_cities(
    ...
    *,
    ...
    db_cities = search_cities_for_country(db, cc, include_inactive, query, limit)
    if db_cities:
        return {
            "code": cc,
            "cities": [
    ...
    if not cities:
        ...
        try:
            ...
            logger.debug("Fallback city parsing failed for %s: %s", cc, exc)
...
def assign_staff_to_country(country_code: str, user_id: int, role_in_country: str, current_user: dict, db: Session) -> dict:
    ...
    existing = get_country_staff_assignment(db, user_id, country_code)
    if existing:
...
def list_country_staff(country_code: str, current_user: dict, db: Session) -> list[dict]:
    ...
    rows = list_country_staff_assignments(db, country_code)
...
def unassign_staff_from_country(country_code: str, user_id: int, current_user: dict, db: Session) -> dict:
    require_full_admin(current_user)
    a = get_country_staff_assignment(db, user_id, country_code)
    if not a:
...
def list_country_communications(country_code: str, current_user: dict, db: Session, category: str | None = None) -> list[dict]:
    require_admin(current_user)
    _require_country_access(country_code, current_user)
    rows = list_country_communications(db, country_code, category)
    return [
        {
            "id": r.id,
...
def mark_communication_read(comm_id: int, current_user: dict, db: Session) -> dict:
    require_admin(current_user)
    comm = get_communication_by_id(db, comm_id)
    if not comm:
...
def list_cross_country_sessions(country_code: str, current_user: dict, db: Session) -> list[dict]:
    require_admin(current_user)
    _require_country_access(country_code, current_user)
    q = list_recent_cross_country_sessions(db, country_code)
    return [
        {
            "id": r.id,
            "user_id": r.user_id,
...
def list_payout_rules_categories(country_code: str, current_user: dict, db: Session) -> list[dict]:
    ...
    country = get_country_by_code(db, normalized_code)
    if not country:
        return []
...
def update_country_cities_bulk(code: str, payload: dict, current_user: dict, db):
    ...
    all_cities = list_cities_for_country_ordered(db, cc, include_inactive=True)
    return {
        'code': cc,
