...
def _refresh_order_finance_settlement_status(order_id: int, db: Session) -> None:
    entries = all_rows(db, TransactionLedger, [TransactionLedger.order_id == order_id], limit=1000)
    if not entries:
        return
...
def _sync_supplier_settlements_for_payout(
    ...
    settlements = all_rows(db, SupplierSettlement, [SupplierSettlement.payout_id == payout_id])
    if not settlements:
        return
...
def verify_payout(
    ...
    payout = first(db, Payout, [Payout.id == payout_id])
    if not payout:
