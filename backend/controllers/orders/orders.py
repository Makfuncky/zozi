...
from data.controllers_payments_controller import apply_order_status_change
...
def bulk_update_order_status_admin(
    ...
    for oid in order_ids:
        order = first(db, Order, [Order.id == oid])
        if not order:
    ...
    if updated:
        for o in updated:
            order_obj = first(db, Order, [Order.id == o["id"]])
            if order_obj:
                update_order_service(db, order_obj, {"status": o["new_status"]})
        audit_log(
            db=db,
...
def bulk_delete_orders_admin(order_ids: List[int], acting_user: dict, db: Session) -> dict:
    ...
    for oid in order_ids:
        ...
        if not order:
            ...
            continue
        try:
            info = delete_order_with_savepoint(db, order)
            ...
            logger.exception("bulk_delete_orders_admin_failed", error=str(e))
            info = None
        if info is None:
    ...
    if deleted:
        ...
        discard_bulk_delete_transaction(db)
    return {
        "deleted": len(deleted),
        "skipped": len(skipped),
        "details": deleted,
...
def delete_order_admin(order_id: int, acting_user: dict, db: Session) -> dict:
    ...
    order = first(db, Order, [Order.id == order_id])
    if not order:
...
def get_all_orders(
    ...
    if date_range and date_range != "all":
        ...
        if date_range == "7d":
            filters.append(Order.created_at >= now - timedelta(days=7))
        elif date_range == "30d":
            filters.append(Order.created_at >= now - timedelta(days=30))
        elif date_range == "month":
            month_start = now.replace(day=1, hour=0, minute=0, second=0, microsecond=0)
            filters.append(Order.created_at >= month_start)
    if missing_tracking_only:
        filters.append(Order.status.in_(["shipped", "delivered"]))
        filters.append(~exists().where(Shipment.order_id == Order.id))
    if search and search.strip():
        term = f"%{search.strip()}%"
        outerjoins.append((User, User.id == Order.user_id))
        filters.append(
            or_(
    ...
    total = count(db, Order, filters, outerjoins=outerjoins, options=options)
    orders = all_rows(
        db,
        Order,
        filters,
    ...
    if all_shipment_ids:
        all_events = all_rows(
            db,
            ShipmentEvent,
            [ShipmentEvent.shipment_id.in_(all_shipment_ids)],
...
def update_order_status(order_id: int, status: str, acting_user: dict, db: Session) -> dict:
    ...
    apply_order_status_change(order, status, db)
    update_order_service(db, order, {"status": status})
    try:
        from data.event_bus import publish_order_status_changed

        publish_order_status_changed(cast(int, order.id), status=status, old_status=old_status)
    except (ValueError, TypeError, KeyError, IndexError, AttributeError, RuntimeError, OSError, IOError, EOFError, ImportError, NameError, StopIteration, ArithmeticError, AssertionError, UnicodeError, NotImplementedError, RecursionError, ReferenceError, SystemError, BufferError, LookupError) as e:
...
def refund_order(order_id: int, acting_user: dict, db: Session) -> dict:
    ...
    from data.controllers_payments_controller import _apply_stripe_runtime_key
    ...
    try:
        ...
        try:
            from data.event_bus import publish_order_refunded

            publish_order_refunded(cast(int, order.id), source="admin")
        except (ValueError, TypeError, KeyError, IndexError, AttributeError, RuntimeError, OSError, IOError, EOFError, ImportError, NameError, StopIteration, ArithmeticError, AssertionError, UnicodeError, NotImplementedError, RecursionError, ReferenceError, SystemError, BufferError, LookupError) as e:
...
def bulk_update_order_status(ids: List[int], status: str, db: Session) -> dict:
    """Admin bulk order status update (thin pass-through to the service)."""
    return _bulk_update_order_status(ids, status, db)
