...
def _database_health_snapshot(db: Session) -> tuple[bool, str]:
    try:
        ...
        logger.exception("_database_health_snapshot_failed", error=str(e))
...
def _table_row_count(db: Session, table_name: str) -> int | None:
    try:
        if not table_name or not table_name.replace("_", "").isalnum():
            return None
        count_value = scalar(db, select(func.count()).select_from(table(table_name)))
        return 0 if count_value is None else int(count_value)
    except (ValueError, TypeError, KeyError, IndexError, AttributeError, RuntimeError, OSError, IOError, EOFError, ImportError, NameError, StopIteration, ArithmeticError, AssertionError, UnicodeError, NotImplementedError, RecursionError, ReferenceError, SystemError, BufferError, LookupError) as e:
        logger.exception("_table_row_count_failed", error=str(e))
        return None
...
def get_database_overview(db: Session) -> dict[str, Any]:
    ...
    if "alembic_version" in table_names:
        try:
            version_value = scalar(db, sql="SELECT version_num FROM alembic_version LIMIT 1")
            alembic_version = str(version_value) if version_value else None
        except (ValueError, TypeError, KeyError, IndexError, AttributeError, RuntimeError, OSError, IOError, EOFError, ImportError, NameError, StopIteration, ArithmeticError, AssertionError, UnicodeError, NotImplementedError, RecursionError, ReferenceError, SystemError, BufferError, LookupError) as e:
            logger.exception("get_database_overview_failed", error=str(e))
