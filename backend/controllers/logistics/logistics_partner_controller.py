...
from services.logistics_partner_write_service import (
    ...
    create_logistics_payout_request,
    create_pricing_profile,
    create_pricing_rule,
    create_shipment_confirmation_request,
    create_sla_breach_notifications,
    create_vehicle_rule,
    ...
    delete_pricing_rule,
    delete_vehicle_rule,
    persist_shipment_status_update,
    stage_notification,
    stage_shipment_event,
    try_delete_logistics_partner,
    ...
    update_logistics_payout_status,
...
def _get_partner_for_user(user_id: int, db: Session) -> LogisticsPartner:
    ...
    if not partner:
        ...
        partner = create_logistics_partner(
            db,
...
def upsert_my_partner_service_area(area_id: int | None, data: dict, current_user: dict, db: Session) -> dict[str, Any]:
    ...
    else:    setattr(area, "approval_status", "pending")
...
def upsert_my_partner_pricing_profile(profile_id: int | None, data: dict, current_user: dict, db: Session) -> dict[str, Any]:
    ...
    else:    setattr(profile, "approval_status", "pending")
...
def upsert_my_partner_category_rule(rule_id: int | None, data: dict, current_user: dict, db: Session) -> dict[str, Any]:
    ...
    else:    setattr(rule, "approval_status", "pending")
...
def review_partner_profile(partner_id: int, data: dict, current_user: dict, db: Session) -> dict:
    ...
    update_logistics_partner(db, partner, updates)
    return _serialize_partner(partner)
...
def review_partner_service_area(area_id: int, data: dict, current_user: dict, db: Session) -> dict[str, Any]:
    ...
    note = _sanitize_optional_string(data.get("note"), max_length=2000)
    area = update_logistics_partner_service_area(
        db,
        area,
        {
...
def review_partner_pricing_profile(profile_id: int, data: dict, current_user: dict, db: Session) -> dict[str, Any]:
    ...
    if decision not in {"approved", "rejected", "pending"}:
        raise HTTPException(status_code=422, detail="status must be one of: pending, approved, rejected")
    note = _sanitize_optional_string(data.get("note"), max_length=2000)
    profile = update_pricing_profile(
        db,
        profile,
        {
            "approval_status": decision,
...
def bulk_manage_partners(
    ...
    for partner_id in ordered_ids:
        ...
        if normalized_action == "approve":
            ...
            if not try_delete_logistics_partner(db, partner):
                skipped.append(
                    {
                        "id": partner_id,
...
def request_partner_payout(data: dict, current_user: dict, db: Session) -> dict:
    ...
    payout = create_logistics_payout_request(
        db,
        partner,
...
def verify_partner_payout(payout_id: int, data: dict, current_user: dict, db: Session) -> dict:
    ...
    update_logistics_payout_status(
        ...
        notes=str(data.get("notes", "")).strip() or cast(Optional[str], getattr(payout, "notes", None)),
        partner_user_id=partner_user_id,
    )
    audit_log(
        db=db,
...
def create_shipment_confirmation_request_partner(
    ...
    confirmation = create_shipment_confirmation_request(
        db,
        confirmation_fields={
            "shipment_id": shipment.id,
            "order_id": shipment.order_id,
...
def update_shipment_status_partner(
    ...
    shipment, event = persist_shipment_status_update(
        db,
        shipment,
...
def upsert_partner_bank_account(body: dict, current_user: dict, db: Session) -> dict:
    ...
    if not is_new and getattr(record, "verification_status", "pending") != "pending":
        updates.update({
            ...
            "verification_note": "Resubmitted by partner — awaiting re-verification.",
            "provider": None,
    ...
    if is_new:
        record = create_logistics_partner_bank_account(db, partner_id=partner_id, **updates)
    else:
        record = update_logistics_partner_bank_account(db, record, updates)
    return {
        "ok": True,
        "id": record.id,
        "verification_status": record.verification_status,
...
async def upload_partner_document(
    ...
    doc = create_logistics_partner_document(
        db,
        partner_id=cast(int, partner.id),        document_type=document_type,
...
def update_city_distance(matrix_id: int, data: dict[str, Any], current_user: dict, db: Session) -> dict[str, Any]:
    ...
    if "notes" in data:
        updates["notes"] = _sanitize_optional_string(data.get("notes"), max_length=1000)
    updates["updated_by"] = current_user["id"]
    updates["updated_at"] = _utcnow()
    entry = update_city_distance_matrix(db, entry, updates)
    return _serialize_city_distance(entry)
