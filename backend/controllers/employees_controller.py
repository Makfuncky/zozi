...
def check_in_employee(employee_id: int, data: dict, db: Session) -> dict:
    ...
    if not emp:
        raise HTTPException(status_code=404, detail="Employee not found")
    attendance = EmployeeAttendance(employee_id=employee_id, **data)
    db.add(attendance)
    db.commit()
...
def check_in_with_geo(employee_id: int, data: dict, db: Session) -> dict:
    ...
    attendance = EmployeeAttendance(**data)
    db.add(attendance)
    db.commit()
    db.refresh(attendance)
    return {"id": attendance.id, "status": attendance.status}
...
def generate_qr_login_token(employee_id: int, db: Session) -> dict:
    ...
    if not emp:
        raise HTTPException(status_code=404, detail="Employee not found")
    token = str(uuid4())
    session = DynamicQRSession(employee_id=employee_id, qr_token=token,
