...
async def analyze_product_image(
    ...
    ``generate_copy`` controls the slow (~60-90s CPU) LLM copy generation:
      - True  â†’ full EN/AR marketing copy via Ollama (use in background jobs).
      - False â†’ instant heuristic-only result (use for the synchronous request).

    ``use_vision`` overrides the ``AI_USE_VISION`` env default for this call:
      - None  â†’ follow the env flag (default off for VPS safety).
      - True  â†’ run moondream vision so the product is detected from the image
                itself (used by the background copy job so real detection never
                blocks the interactive request).
      - False â†’ force vision off regardless of env.
    """
    global _CONFIG
    cfg = _CONFIG
    ...
    # Best-effort Ollama vision â†’ text structuring (heuristic fallback if absent).
    # Vision is gated (heavy on CPU) â€” see AI_USE_VISION / the use_vision override.
    vision_prose = None
    ...
    if structured:
        ...
        # Preserve the filename-derived subcategory (e.g. "Perfume Bottle" â†’ "Bottle")
    ...
    # (liquids â†’ volume not weight, solids â†’ weight not volume, etc.).
    ...
    # â”€â”€ Variant suggestion refinement (the "better model") â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    # The raw config contains ~150 variant axes, many of which are product
    # *metadata* (brand, sku, hs_code, country_of_origin, warranty, â€¦) rather
    ...
    # ones â€” capped to keep the matrix usable.
    ...
    # â”€â”€ Photo-driven upgrades from the vision/text model â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    return {
        ...
        **suggest_price(product_type, product_name, category),
        ...
        # Photo-analysis signals (dominant colours, background complexity) â€”
...
def _refine_variant_suggestions(
    ...
    for electronics, volume/scent for beauty, plating/gemstone for jewelry, â€¦).
    ...
    # ai_rules-mandated axes (e.g. furniture â†’ dimensions) are ranked ahead of
