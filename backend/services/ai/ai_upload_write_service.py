"""AI upload pipeline write service (W1 remediation).

Owns every DB mutation of the AI upload flow that previously lived inline in
``routers/ai_upload.py``:
...
Every function that mutates takes the SQLAlchemy ``Session`` as its first
positional argument and owns the surrounding transaction (``commit``), so the
router/controller layers stay read-only orchestration.
"""
...
def save_upload(file: UploadFile, job_dir: str) -> tuple[str, str, bytes]:
    """Save uploaded file through storage abstraction.

    Returns ``(storage_key, public_url, content_bytes)``.
    """
    from data.services_storage import storage as _storage
...
def _publish_staging(
    ...
    slug = f"{slugify(name) or 'product'}-{uuid.uuid4().hex[:8]}"
    product = Product(
        name=name,
        slug=slug,
        description=description,
...
def create_ai_upload_job(
    ...
    *,
    ...
    for img in images:
        try:
            key, url, content = save_upload(img, str(job.id))
...
def run_ai_upload_job(db: Session, job_id: int) -> None:
    ...
    try:
        ...
        logger.exception("run_ai_upload_job_failed", error=str(e))
        parsed = []
    ...
    try:
        for idx, media in enumerate(parsed):
            ...
            if image_bytes_str:
                ...
                continue
            try:
                staging, variants, logs = enrich_one(img_bytes, idx, job, image_url)
            except (ValueError, TypeError, KeyError, IndexError, AttributeError, RuntimeError, OSError, IOError, EOFError, ImportError, NameError, StopIteration, ArithmeticError, AssertionError, UnicodeError, NotImplementedError, RecursionError, ReferenceError, SystemError, BufferError, LookupError) as exc:  # per-image isolation
...
def process_ai_upload_job(job_id: int) -> None:
    ...
    Runs outside the request context (RLS restricted flag is False, so it can
    read/write across the job's country).
    """
