...
def db_side_effects_enabled() -> bool:
...
def _parse_datetime(value: Any) -> datetime | None:
    ...
    try:
        ...
        logger.exception("_parse_datetime_failed", error=str(e))
        return None
...
def verify_resend_webhook_request(payload: str, headers: Mapping[str, str], secret: str) -> tuple[bool, str]:
    ...
    try:
        ...
        logger.exception("verify_resend_webhook_request_failed", error=str(e))
    ...
    try:
        ...
        logger.exception('verify_resend_webhook_request: caught broad Exception')
...
def _serialize_payload(payload: Any) -> str | None:
    ...
    try:
        ...
        logger.exception('_serialize_payload: caught broad Exception')
...
def record_email_delivery_event(
    *,
    ...
    if owns_session:
        if not db_side_effects_enabled():
            return None
        db = SessionLocal()
    ...
    try:
        ...
        logger.exception('unknown: caught broad Exception')
        if owns_session:
            db.rollback()
...
def process_resend_webhook(payload: dict[str, Any], headers: Mapping[str, str], db: Session) -> dict[str, Any]:
    ...
    occurred_at = _parse_datetime(payload.get("created_at")) or _parse_datetime(data.get("created_at")) or utcnow()
