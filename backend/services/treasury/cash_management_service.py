...
from services.treasury.bank_transaction_service import log_bank_transaction  # noqa: F401 â€” re-export (moved here to break DG2 cycle)
...
def run_scheduled_finance_cycle(db: Session) -> dict:
    ...
    from services.analytics.admin_analytics_service import refresh_admin_analytics_snapshots
    from services.commerce.retention_service import run_operational_retention_cycle
...
class CashManagementService:
    """Cash management service for treasury operations."""

    def __init__(self, db: Session):
        self.db = db

    def get_financial_summary(self) -> dict:
        return get_financial_summary(self.db)

    def get_reconciliation_summary(self) -> dict:
        return get_reconciliation_summary(self.db)

    def run_scheduled_cycle(self) -> dict:
        return run_scheduled_finance_cycle(self.db)
