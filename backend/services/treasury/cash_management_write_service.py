...
def execute_bank_transfer(db: Session, **transfer_data) -> CashTransaction:
    transaction = CashTransaction(**transfer_data)
    db.add(transaction)
    db.commit()
    db.refresh(transaction)
    return transaction
...
def create_payout_method(db: Session, **method_data) -> CashAccount:
    from models import PayoutMethod
    method = PayoutMethod(**method_data)
    db.add(method)
    db.commit()
    db.refresh(method)
    return method