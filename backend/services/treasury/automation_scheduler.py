...
from services.treasury.gateway_reconciliation_service import run_gateway_3way_reconciliation
...
# â”€â”€ Daily Cash Position Snapshot â”€â”€
...
# â”€â”€ Automated Financial Report Generation â”€â”€
...
# â”€â”€ Distributor Statement Generation â”€â”€
...
def generate_distributor_statements(db: Session, period_year: int, period_month: int,
    ...
    for customer in customers.all():
        ...
        try:
            from services.communication.transactional_email_service import enqueue_distributor_statement_email
...
# â”€â”€ Supplier Statement Generation â”€â”€
...
# â”€â”€ Automated Alerts Engine â”€â”€
...
def _check_orphan_journals(db: Session, country_code: str = None) -> list[dict]:
    alerts = []
    from services.finance.orphan_detector_service import run_orphan_detector
    try:
        orphans = run_orphan_detector(db)
        if orphans:
            alerts.append({
...
# â”€â”€ Master Automation Runner â”€â”€
...
def run_full_automation(db: Session, country_code: str = None,
    ...
    try:
        from services.orders.import_service import run_fx_revaluation
    ...
    try:
        from services.orders.trading_service import scan_unmatched_pos
    ...
    if today.weekday() == 0:  # Monday
        try:
            from services.orders.trading_service import run_dunning_engine
    ...
    try:
        from services.orders.trading_service import auto_invoice_ecommerce_orders
    ...
    try:
        from services.treasury.gateway_reconciliation_service import reconcile_all_cod_deposits
