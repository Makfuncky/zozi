...
def commit_and_refresh(db: Session, model) -> None:
    db.commit()
    db.refresh(model)
...
def stage_notification(db: Session, **notification_data) -> None:
    """Stage a Notification in the session without committing (caller commits later)."""
    db.add(Notification(**notification_data))
