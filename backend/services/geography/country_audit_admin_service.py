"""Service layer for the inline country-audit/administration database operations that
lived in ``routers/admin_geography_audit.py``.

The original router mixed HTTP concerns with direct ORM writes for cities, staff
assignments, category tax rates and internal communications. Those blocks are moved
here so the router stays a thin delegator. Return shapes and error behavior match the
...
def mark_communication_read(comm_id: int, db: Session) -> dict:
...
def list_cities(country_code: str, *, active: bool = True, limit: int = 100, db: Session) -> List[dict]:
...
def update_city(
    ...
    *,
    ...
    city = db.query(CountryCity).filter(
        CountryCity.id == city_id, CountryCity.country_code == country_code.upper()
    ).first()
    if not city:
...
def delete_city(country_code: str, city_id: int, db: Session) -> dict:
    city = db.query(CountryCity).filter(
        CountryCity.id == city_id, CountryCity.country_code == country_code.upper()
    ).first()
    if not city:
...
def list_staff(country_code: str, db: Session) -> List[dict]:
    assignments = (
...
def remove_staff(country_code: str, staff_id: int, db: Session) -> dict:
    assignment = (
        db.query(CountryStaffAssignment)
        .filter(
...
def list_tax_rates(country_code: str, db: Session) -> List[dict]:
    rates = (
