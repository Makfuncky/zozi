...
Also explicitly imports FulfillmentService from the submodule since it's not
re-exported at package level.
"""

import services.orders as _m
from services.orders.fulfillment_service import FulfillmentService as _FulfillmentService

_declared = set(getattr(_m, "__all__", ()) or ())

def _forward_names() -> None:
    globals()["FulfillmentService"] = _FulfillmentService
    for _n in vars(_m):
...
_forward_names()
__all__ = [n for n in globals() if not n.startswith("_") or n in _declared or n == "FulfillmentService"]