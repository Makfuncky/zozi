#!/usr/bin/env python3
"""Reconstruct function definitions corrupted by the first script.
The script removed 'def func_name(...)' lines from single-line function defs.
This script detects @router decorators followed by indented body code
    ...
    while i < len(lines):
        ...
        if decorator_match:
            ...
            if next_stripped.startswith('"""') or next_stripped.startswith("'''"):
                ...
                # Look for the function def pattern
                result.append(next_line)
                i += 1
                continue
...
def main():
    ...
    for rel_path in files:
        ...
        new_content, fixed = fix_broken_functions(str(filepath))
        if fixed > 0:
            print(f"{rel_path}: found {fixed} broken functions (cannot auto-fix)")
        else:
            print(f"{rel_path}: no broken functions found")
