"""
Permission Management Router — shared endpoints (no-delete refactor).
3-Layer Permission Matrix: Admin → Sub-Admin (Roles) → Employee (Override)

This module holds the handlers with NO prefix. ``admin_permissions_validation.py``
and ``public_permissions_validation.py`` include these routes under their
respective prefixes (``/api/v1/admin`` and ``/api/v1``). The original two files
were ~1.00 duplicates differing only by prefix; this consolidation keeps both
URL surfaces while removing the duplicated source.
"""
...
from modules.admin.auth import get_current_user
from infrastructure.security.dependencies import require_admin
from infrastructure.database.database import get_db
from domains.governance.services import permission_service as svc
from domains.country.utils.country_rls import get_country_or_404

from rbac.dependencies import require_feature, require_module
router = APIRouter(tags=["permissions"], dependencies=[Depends(require_module("admin"))])
