...
class DirectChatRoom(Base):
    ...
    participant_one = Column(Integer, ForeignKey("users.id"), nullable=False)
    participant_two = Column(Integer, ForeignKey("users.id"), nullable=False)
    country_code = Column(String(10), ForeignKey("country_configs.code"), nullable=True)
...
class ShiftHandoverSession(Base):
    ...
    incoming_employee_id = Column(Integer, ForeignKey("employees.id"), nullable=True)
...
class EscalationSLALog(Base):
    ...
    original_recipient_id = Column(Integer, ForeignKey("users.id"), nullable=True)
