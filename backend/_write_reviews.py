...
RAW = r'''"""Reviews controller (CONTROLLERS layer).
...
so the auto-router emits ``routers/public_commerce_reviews_controller.py``.
"""
from __future__ import annotations
...
@get("/api/v1/products/{product_id}", deps=["db"], query=["limit", "cursor"], response_model=List[ReviewOut], tags=["reviews"])
...
@get("/api/v1", deps=["db"], query=["product_id", "limit", "cursor"], response_model=List[ReviewOut], tags=["reviews"])
def list_reviews(
    db: Session, product_id: Optional[int] = None, limit: int = 50, cursor: Optional[int] = None
) -> List[dict]:
    """Mirrors the legacy router's GET /api/v1 (list reviews for a product supplied
...
@post("/api/v1/products/{product_id}", deps=["db", "user"], body=ReviewCreate, response_model=ReviewOut, status_code=201, tags=["reviews"])
...
@put("/api/v1/{review_id}", deps=["db", "user"], body=ReviewCreate, response_model=ReviewOut, tags=["reviews"])
