...
class Permission(Base):
    ...
    __table_args__ = ({"schema": "accounts"},)
    id = Column(Integer, primary_key=True, index=True)
    category_id = Column(Integer, ForeignKey("accounts.permission_categories.id"), nullable=False)
...
class RolePermissionAssignment(Base):
    ...
    __table_args__ = (
        UniqueConstraint("role_name", "permission_id", "country_code", name="uq_role_permission_country"), {"schema": "accounts"})
    id = Column(Integer, primary_key=True, index=True)
    role_name = Column(String(80), nullable=False)
    permission_id = Column(Integer, ForeignKey("accounts.permissions.id"), nullable=False)
    country_code = Column(String(2), ForeignKey("country.country_configs.code"), nullable=True)
    granted_by = Column(Integer, ForeignKey("accounts.users.id"), nullable=True)
...
class UserPermissionOverride(Base):
    ...
    __table_args__ = (
        UniqueConstraint("user_id", "permission_id", "country_code", name="uq_user_perm_override_country"), {"schema": "accounts"})
    id = Column(Integer, primary_key=True, index=True)
...
class PermissionAuditLog(Base):
    ...
    __table_args__ = ({"schema": "accounts"},)
    id = Column(Integer, primary_key=True, index=True)
    actor_id = Column(Integer, ForeignKey("accounts.users.id"), nullable=False)
    action = Column(String(50), nullable=False)
    target_user_id = Column(Integer, ForeignKey("accounts.users.id"), nullable=True)
    target_role = Column(String(80), nullable=True)
    permission_id = Column(Integer, ForeignKey("accounts.permissions.id"), nullable=True)
