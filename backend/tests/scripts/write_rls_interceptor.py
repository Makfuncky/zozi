...
def derive_tables() -> list[tuple[str, str]]:
    result: list[tuple[str, str]] = []
    for table in Base.metadata.tables.values():
        cols = {c.name for c in table.columns}
        if "country_code" in cols:
            result.append((table.name, "country_code"))
    return result
...
for name, col in sorted(derive_tables(), key=lambda x: x[0]):
...
print(f"Wrote {len(lines)} lines with {len([l for l in lines if 'country_code' in l])} table entries")
