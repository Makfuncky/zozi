"""Supplier module router law tests.

Verifies Laws 1, 2, 87, 88, 89, 90, 99, 132-139 for the supplier module.
"""
...
class TestSupplierRouterImports:
            ...
            for node in ast.walk(tree):
                if isinstance(node, ast.ImportFrom) and node.module:
                    if node.module.startswith("domains."):
                        domain_imports += 1
        assert domain_imports > 0, "supplier routers never import from domains"


class TestSupplierRouterLaw8788:
...
class TestSupplierRouterLaw290:
...
class TestSupplierRouterLaw139:
    """Law 139: route prefixes match /supplier/*."""
