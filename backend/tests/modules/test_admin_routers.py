"""Admin module router law tests.

Verifies Laws 1, 2, 87, 88, 89, 90, 99, 132-139 for the admin module.
AST-based so they run without importing the broken app.
"""
...
class TestAdminStructure:
    ...
    def test_auth_has_dependencies(self):
        deps = BACKEND_ROOT / "modules" / MODULE / "auth" / "dependencies.py"
        assert deps.exists()
...
class TestAdminRouterImports:
    ...
    def test_module_imports_domains(self):
        """Admin routers should import from domains (Law 1)."""
        domain_imports = 0
        for path in iter_router_files(MODULE):
            ...
            for node in ast.walk(tree):
                if isinstance(node, ast.ImportFrom) and node.module:
                    if node.module.startswith("domains."):
                        domain_imports += 1
        assert domain_imports > 0, "admin routers never import from domains"


class TestAdminRouterLaw8788:
...
class TestAdminRouterLaw139:
    """Law 139: route prefixes match /admin/*."""
...
class TestAdminModuleInit:
    ...
    def test_module_init_has_lazy_exports(self):
        init_path = BACKEND_ROOT / "modules" / MODULE / "__init__.py"
