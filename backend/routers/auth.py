...
from services.core.users_write_service import find_user_by_email_or_username, get_user_by_email, get_user_by_username, get_user_by_id
from services.security.auth_write_service import record_login_history, persist_last_login, create_user
router = APIRouter()
...
def _find_user(db: Session, email: str | None, username: str | None) -> User | None:
    return find_user_by_email_or_username(db, email, username)
...
def _record_login_history(db: Session, user: User, request: Request | None = None, success: bool = True) -> None:
    ...
    try:
        record_login_history(
            db, user.id, ip or "unknown", ua, success,
...
def refresh(payload: RefreshRequest | None = None, request: Request = None, db: Session = Depends(get_db)):
    ...
    user = get_user_by_id(db, int(decoded.get("sub")))
