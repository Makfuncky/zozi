...
async def ai_analyze(
    ...
    if generate_copy:
        from services.ai.ai_copy_jobs import enqueue_copy_job
...
async def voice_transcribe(
    ...
    from services.comms.video_conferencing import VideoConferenceRoom
...
async def ai_copy_status(
    ...
    """
    from services.ai.ai_copy_jobs import get_job
    job = get_job(job_id)
    if not job:
...
async def translate_text(
    ...
    from services.comms.content_service import translate_en_to_ar
...
async def process_tools(
    ...
    if not raw:
        raise HTTPException(status_code=400, detail="Empty image file")
    from controllers.supplier.supplier_controller import _process_image_with_tools
    tools = {
        "magic_erase": process_magic_erase,
        "smart_crop": process_smart_crop,
        "rotate": process_rotate,
...
def update_return_window(
    ...
    import controllers.products.products_controller as products_ctrl
