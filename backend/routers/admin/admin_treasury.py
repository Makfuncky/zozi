...
def admin_settle_supplier(
    ...
    try:
        ...
        add_and_flush(db, settlement)
        flush_only(db)
        order = db.query(OrderModel).filter(OrderModel.id == order_id).first()
        if order:
            setattr(order, "settlement_status", "settled")
        commit_and_refresh(db, settlement)
