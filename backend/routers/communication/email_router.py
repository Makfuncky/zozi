...
def move_email(
    ...
    email.folder_id = payload.folder_id
    commit_and_refresh(db, email)
...
def delete_folder(
    ...
    # Move emails back to inbox before deleting
    inbox = (
        db.query(EmailFolder)
        .filter(EmailFolder.employee_id == folder.employee_id, EmailFolder.name == "inbox")
        .first()
    )
    if inbox:
        db.query(InternalEmail).filter(InternalEmail.folder_id == folder_id).update(
            {"folder_id": inbox.id}
        )

    delete_only(db, folder)
    commit_only(db)
...
def list_dlp_violations(
    ...
    from models.security.fraud import DLPViolation
