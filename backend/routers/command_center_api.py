...
def _get_user_country_codes(current_user: User | None) -> list[str]:
    if current_user is None:
        return ["OM"]
    codes = getattr(current_user, "staff_country_codes", None)
    if not codes:
        return ["OM"]
...
def get_dashboard(
    ...
    user_country = _get_user_country_codes(current_user)
...
def get_comprehensive_dashboard(
    ...
    effective_country = country_code or _get_user_country_codes(current_user)[0]
