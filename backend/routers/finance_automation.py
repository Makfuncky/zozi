...
from models import Account, AccountGroup, AccountBalance, BankMappingRule, BankStatementLine
...
def list_rules(country_code: Optional[str] = Query(None, max_length=3), db: Session = Depends(get_db), _admin=Depends(require_finance_permission("finance.bank.mapping"))):
    ...
    return [
        ...
        for r in q.order_by(BankMappingRule.priority.asc()).all()
    ]
...
def statement_lines(import_id: int, db: Session = Depends(get_db), _admin=Depends(require_finance_permission("finance.bank.mapping"))):
    return [
        {"id": l.id, "description": l.description, "amount": float(l.amount),
         ...
         "status": l.status}
        for l in db.query(BankStatementLine).filter(BankStatementLine.import_id == import_id).all()
    ]
...
def list_scanned(country_code: Optional[str] = Query(None, max_length=3), db: Session = Depends(get_db), _admin=Depends(require_finance_permission("finance.expenses.scan"))):
    from models import ScannedExpense
...
def list_assets(country_code: Optional[str] = Query(None, max_length=3), db: Session = Depends(get_db), _admin=Depends(require_finance_permission("finance.assets"))):
    from models import FixedAsset
    ...
    return [
        ...
        for a in q.order_by(FixedAsset.id.desc()).all()
    ]
...
def list_accruals(country_code: Optional[str] = Query(None, max_length=3), db: Session = Depends(get_db), _admin=Depends(require_finance_permission("finance.accruals"))):
    ...
    return [
        ...
        for a in q.order_by(Accrual.id.desc()).all()
    ]
