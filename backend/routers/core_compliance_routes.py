...
@router.get("/core_compliance_routes/health")
def health():
...
try:
    import controllers.compliance_controller as _ctrl
    _HAS_CTRL = True
...
@router.get("/core_compliance_routes/status")
def status():
    """Report whether a backing controller is importable."""
    return {"router": "core_compliance_routes", "controller": "controllers.compliance_controller" if _HAS_CTRL else None,
