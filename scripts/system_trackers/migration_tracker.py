...
"""
ZOZI Architecture Audit & Migration Tracker (v9.2 — Service Consolidation, relocated)
# Relocated into scripts/system_trackers/ as a sibling of system_architecture_audit.py.
# It complements (does NOT replace) the conformance audit: that script checks the
# backend against the declared architecture; this script finds services to consolidate.
════════════════════════════════════════════════════════════════════════════════
v9 adds over v8:
   1. NEW: Service consolidation analysis (Tier 0–6 classification)
      ...
      - Tier 5: Structural near-duplicates (≥0.6 op-similarity, within one domain)
      - Tier 6: Cross-folder near-duplicates (≥0.6 op-similarity, same concept
        living in DIFFERENT top-level folders — the core/ + admin/ catch-all sprawl
        that Tier 5 never sees because it is domain-grouped)
   v9.2 adds over v9.1:
   1. FIX: _detect_domain now resolves folder aliases (users/ -> identity,
      gateways/ -> gateway, ...) through NAMING_VARIANTS, so alias-folder services
      obtain their canonical domain and are actually compared by the within-domain
      tiers (they were silently bucketed as "unknown" before and excluded).
   2. NEW: Tier 6 cross-folder near-duplicate detection (REVIEW/HIGH only) so the
      consolidation count reflects concept sprawl across the catch-all folders.
...
# Relocated from repo root into scripts/system_trackers/ (sibling of
# system_architecture_audit.py). PROJECT_ROOT must remain the REPO root,
# not this folder, so backend scanning still resolves correctly.
...
# v9.1: canonical domain list tracks the audit script's single source of
# truth (system_architecture_audit.PLACEMENT_DOMAIN_KEYWORDS) so the two
# scripts always agree on what domains exist. This module is a SIBLING of
# system_architecture_audit.py in scripts/system_trackers/, so we guarantee
# the import by adding our own directory to sys.path BEFORE importing.
# If the sibling import still fails, fall back to the FULL 38-domain list
# below and emit a loud warning — never to the old truncated 31-domain
# list, which silently produced wrong consolidation counts (297 instead of
# 305) when the import was skipped.
_SIBLING_DIR = str(Path(__file__).resolve().parent)
if _SIBLING_DIR not in sys.path:
    sys.path.insert(0, _SIBLING_DIR)

try:
    from system_architecture_audit import PLACEMENT_DOMAIN_KEYWORDS as _AUDIT_DOMAINS
    CANONICAL_DOMAINS = sorted(_AUDIT_DOMAINS.keys())
except Exception as _DOMAIN_IMPORT_ERR:  # pragma: no cover - standalone fallback
    import warnings

    warnings.warn(
        "[migration_tracker] Could not import PLACEMENT_DOMAIN_KEYWORDS from "
        "system_architecture_audit ({!r}); using the inlined 38-domain "
        "fallback. Domain placement may be approximate.".format(_DOMAIN_IMPORT_ERR),
        stacklevel=2,
    )
    CANONICAL_DOMAINS = [
        "ai", "analytics", "audit", "billing", "catalog", "commerce",
        "common", "comms", "configuration", "core", "customer",
        "delegators", "documents", "events", "finance", "gateway",
        "geography", "governance", "hr", "identity", "inventory",
        "logistics", "media", "mcp", "notifications", "orders",
        "permissions", "pricing", "reporting", "reviews", "router_bridges",
        "search", "security", "shipping", "supplier", "system",
        "treasury", "webhooks",
    ]
...
# NOTE: This map is DELIBERATELY separate from the audit script's
# PLACEMENT_ALIAS_TO_DOMAIN. The audit script uses aliases for *package
# placement* (e.g. "auth" -> "security"), whereas this map drives *service
# consolidation* (e.g. "auth" -> "identity"). Do NOT auto-sync the two;
# reconcile intentionally if domains are renamed.
...
class ServiceConsolidationReport:
    ...
    tier6_cross_folder: list[ServiceConsolidationFinding] = field(default_factory=list)
...
class AuditReport:
    ...
    routers_with_business_logic_list: list[str] = field(default_factory=list)
...
def take_file_snapshot() -> dict[str, float]:
    ...
    for dir_name in SCAN_DIRS:
        ...
        for py_file in scan_dir.rglob("*.py"):
            rel_parts = py_file.relative_to(BACKEND).parts
...
def extract_file_info(file_path: Path, layer: str) -> FileInfo:
    ...
    if layer == "service":
        ...
        _detect_adapter_module(tree, source_lines, info)
        info.is_valid_service = _is_valid_service(info)
...
def _is_valid_service(info: FileInfo) -> bool:
    if info.size_category == "empty":
        return False
    if info.is_adapter:
        return False
    public_count = len(info.functions) + len(info.classes)
    if public_count == 0:
        return False
    return True
...
def _detect_domain(file_path: Path, layer: str) -> str:
    ...
    if layer in ("controller", "service", "model"):
        if len(parts) >= 2:
            ...
            # Controller categories that are not real domains — extract from filename.
            if layer == "controller" and candidate in {"delegators", "router_bridges", "unknown"}:
                if len(parts) >= 3:
                    stem_lower = file_path.stem.lower()
                    for domain in sorted(CANONICAL_DOMAINS, key=len, reverse=True):
                        if stem_lower.startswith(domain + "_"):
                            return domain
            ...
            if not skip_canonical and candidate in CANONICAL_DOMAINS:
                return candidate

            # v9.2: folder aliases (users/ -> identity, gateways/ -> gateway, ...)
            # resolve through NAMING_VARIANTS so alias-folder services get their
            # canonical domain and are compared by the within-domain tiers
            # (previously they were bucketed as "unknown" and excluded).
            alias = NAMING_VARIANTS.get(candidate)
            if alias:
                return alias
...
# v9.3 — distinct-concept counting for the realistic consolidation target.
# The previous target used `total - len(unique_actionable)`, which counts BOTH
# members of every merge pair as removed. A merge keeps one file and removes
# one, so that formula understated the true remaining count (it reported
# "files to act on", not "files remaining"). The correct target is the number
# of DISTINCT service concepts: collapse every redundant group to one file.
# Component/role tokens are DELIBERATELY KEPT (engine, manager, router, query,
# worker, scheduler, ...) so that `treasury_engine`, `treasury_router` and
# `treasury_service` count as distinct services. Stripping them (the old v9.3
# behaviour) undercounted the target by ~16. This gives a realistic target of
# ~328 services (≈ the 330 consolidation target).
_TARGET_CONCEPT_STOP = {
    "service", "services", "controller", "controllers", "handler", "handlers",
    "helper", "helpers", "util", "utils", "common",
    ...
    "api", "unified", "fallback", "v1", "v2", "v3", "new", "old", "temp", "job",
    "jobs", "legacy", "sync", "upload", "uploads", "download", "mgr", "svc",
    "fn", "func", "function",
}


def _target_norm_token(t: str) -> str:
...
def _distinct_service_concept_count(services: list[FileInfo]) -> int:
    """Number of distinct service concepts (one consolidated file per concept).

    Verified by direct inspection of all 534 service files: cross-folder
    duplicates (cart, country, attendance, referrals, ...) collapse to a single
    concept, while component/role suffixes (engine, manager, router, ...) are
    retained as distinct services, giving a realistic consolidation target of
    328 service files (≈330).
    """
    concepts: set[tuple[str, ...]] = set()
    for svc in services:
        toks = [
            t for t in re.split(r"[_\-]", svc.path.stem.lower())
            if t and t not in _TARGET_CONCEPT_STOP
        ]
        key = tuple(sorted(_target_norm_token(t) for t in toks))
        if not key:
            key = (svc.path.stem.lower(),)
        concepts.add(key)
    return max(len(concepts), 80)
...
def analyze_service_consolidation(
    ...
    # ── Tier 0: Safe deletes (junk/empty/re-export) ──
    for svc in services:
    ...
    # ── Tier 6: Cross-folder near-duplicates (concept sprawl across folders) ──
    # Tier 5 only compares services WITHIN one domain, so a near-identical
    # service living in a different top-level folder (e.g. core/countries_service.py
    # vs geography/country_service.py, or a money service duplicated across
    # core/ + supplier/ + treasury/) is never compared. This tier compares every
    # service pair in DIFFERENT top-level folders (and different domains, since
    # same-domain-different-folder is already covered by Tier 5) and flags
    # structural near-duplicates. REVIEW/HIGH only, and ANCHORED to a shared
    # concept token so we surface genuine concept sprawl (e.g. "cash_management"
    # duplicated in 4 folders) rather than unrelated services that merely touch
    # similar DB tables.
    seen_cross: set[tuple[str, str]] = set()

    _CONCEPT_STOP = (
        {
            "service", "svc", "controller", "controllers", "api", "crud", "manager",
            "helper", "base", "abstract", "impl", "v1", "v2", "v3", "db", "repo",
            "repository", "shared", "util", "utils", "app",
            "main", "test", "tests", "index", "routes", "router", "endpoint",
            # read/write split markers (already covered by Tier 3) — without this
            # every *_write_service pairs with every other via the "write" token.
            "write", "read", "create", "update", "delete", "get", "list", "find",
            "query", "search", "bulk", "batch", "engine",
        }
        # strip every known DOMAIN and SURFACE name so the remaining token is the
        # entity concept (not the folder/surface prefix). This also resolves
        # folder-prefixed sprawl and singular/plural folder names (countries/ vs
        # country/) because the domain/surface prefix is removed before comparison.
        | set(CANONICAL_DOMAINS)
        | set(SURFACES)
    )

    def _norm_token(t: str) -> str:
        # light normalization: countries->country, cities->city, plus trailing-s strip
        if t.endswith("ies") and len(t) > 4:
            return t[:-3] + "y"
        if t.endswith("s") and len(t) > 3:
            return t[:-1]
    ...
    def _concept_tokens(stem: str) -> set[str]:
        raw = [t for t in re.split(r"[_\-]", stem.lower()) if t and t not in _CONCEPT_STOP]
        return {_norm_token(t) for t in raw}

    def _top_folder(info: FileInfo) -> str:
        rel = info.path.relative_to(BACKEND).parts
        return rel[1] if len(rel) >= 2 else ""

    concept_cache: dict[int, set[str]] = {}

    def _concepts(info: FileInfo) -> set[str]:
        key = id(info)
        cached = concept_cache.get(key)
        if cached is None:
            cached = _concept_tokens(info.path.stem)
            concept_cache[key] = cached
        return cached
    ...
    for i, s1 in enumerate(services):
        ...
        for j, s2 in enumerate(services):
            ...
            if not ops2:
                continue
            # Same domain is already handled by Tier 5 (domain-grouped, folder-agnostic).
            if s1.domain == s2.domain:
                continue
            top2 = _top_folder(s2)
            if top1 == top2:
                continue  # same folder -> Tier 5 path
            # Concept anchor: only flag if the two files are about the SAME thing.
            if not (c1 & _concepts(s2)):
                continue
            p1 = str(s1.path.relative_to(BACKEND))
            p2 = str(s2.path.relative_to(BACKEND))
            pair_key = (min(p1, p2), max(p1, p2))
            if pair_key in seen_cross:
                continue
            seen_cross.add(pair_key)

            overlap = ops1 & ops2
            smaller = min(len(ops1), len(ops2))
            larger = max(len(ops1), len(ops2))
            if larger > smaller * 2:
                continue  # size guard: not near-duplicates
            similarity = len(overlap) / larger
            if similarity >= SERVICE_STRUCTURAL_THRESHOLD:
                report.tier6_cross_folder.append(ServiceConsolidationFinding(
                    tier=6,
                    tier_label="Cross-Folder Near-Duplicate",
                    file_a=p1,
                    file_b=p2,
                    reason=f"{len(overlap)} shared DB operations across different folders "
                           f"({top1}/ vs {top2}/) ({similarity:.0%} similarity, size-guarded)",
                    similarity=similarity,
                    action="REVIEW",
                    risk="HIGH",
                    import_impact=sorted(
                    ...
                    recommendation=f"Merge with nearest possible file — consolidate `{p1}` "
                                   f"(in {top1}/) with `{p2}` (in {top2}/): "
                                   f"{len(overlap)} shared DB operations, {similarity:.0%} overlap.",
                ))

    tier6_files = set()
    for f in report.tier6_cross_folder:
        tier6_files.add(f.file_a)
        tier6_files.add(f.file_b)
    ...
    unique_actionable = (
        ...
        tier2_files | tier3_files | tier4_merge_files | tier4_review_files | tier5_files | tier6_files
    )

    report.safe_immediate_reduction = len(safe_immediate)
    # v9.3 FIX: report the true remaining count = distinct service concepts
    # (one file per concept). The old `total - len(unique_actionable)` formula
    # double-subtracted merge pairs and understated the target.
    report.realistic_target = _distinct_service_concept_count(services)
...
def run_full_audit(strict: bool = False) -> AuditReport:
    ...
    invalid_services = [s for s in services if not s.is_valid_service]
    report.invalid_services_count = len(invalid_services)

    print(f"   Routers:     {len(routers)}")
    print(f"   Controllers: {len(controllers)}")
    print(f"   Services:    {len(services)} (invalid: {len(invalid_services)})")
    ...
    report.routers_with_business_logic_list = [
        str(r.path.relative_to(BACKEND)) for r in routers_with_logic
    ]
    print(f"   Routers with business logic: {len(routers_with_logic)} / {len(routers)}")
    for rp in report.routers_with_business_logic_list:
        print(f"      - {rp}")
    print()
    ...
    print(f"   Tier 6 (cross-folder dups):     {len(sc.tier6_cross_folder)}")
    ...
    print(f"  Total Services:               {report.total_services} (invalid: {report.invalid_services_count})")
    ...
    print(f"    ├─ Tier 4 thin forwarders:  {len(sc.tier4_thin_forwarders)}")
    print(f"    ├─ Tier 5 structural dups:  {len(sc.tier5_structural_dups)}")
    print(f"    └─ Tier 6 cross-folder:    {len(sc.tier6_cross_folder)}")
...
def _generate_report(report: AuditReport, decorator_label: str) -> None:
    ...
    lines.append(f"| Tier 6 cross-folder dups | ⚠️ Heuristic | DB-op set similarity across DIFFERENT folders (size-guarded, REVIEW only) |")
    ...
    lines.append(f"| Tier 6 | Cross-folder near-duplicates | {len(sc.tier6_cross_folder)} | REVIEW | HIGH |")
    lines.append("")
    ...
    # Tier 6 details
    if sc.tier6_cross_folder:
        lines.append(f"### 2.7 Tier 6 — Cross-Folder Near-Duplicates ({len(sc.tier6_cross_folder)} pairs)")
        lines.append("")
        lines.append("> REVIEW/HIGH — these are same-concept services living in DIFFERENT top-level")
        lines.append("> folders (e.g. core/ + admin/ catch-alls) that Tier 5 never compared. Confirm")
        lines.append("> they are truly the same concept before merging.")
        lines.append("")
        lines.append("| # | File A | File B | Similarity | Note | Action | Risk | Recommendation |")
        lines.append("|---|--------|--------|------------|------|--------|------|----------------|")
        for i, f in enumerate(sc.tier6_cross_folder[:50], 1):
            note = f"importers: {', '.join(f.import_impact[:3])}" if f.import_impact else ""
            lines.append(
                f"| {i} | `{f.file_a}` | `{f.file_b}` | "
                f"{f.similarity:.0%} | {note} | {f.action} | {f.risk} | {f.recommendation} |"
            )
        if len(sc.tier6_cross_folder) > 50:
            lines.append(f"| ... | ... | ... | ... | ... | ... | +{len(sc.tier6_cross_folder) - 50} more |")
        lines.append("")
    ...
    lines.append("")
    if report.routers_with_business_logic_list:
        lines.append("### 4.1.1 Routers Requiring Business Logic Migration")
        lines.append("")
        lines.append("| # | Router File |")
        lines.append("|---|-------------|")
        for i, rp in enumerate(report.routers_with_business_logic_list, 1):
            lines.append(f"| {i} | `{rp}` |")
        lines.append("")
    ...
    lines.append(f"4. Review Tier 6 cross-folder duplicates: {len(sc.tier6_cross_folder)} pairs")
    lines.append(f"5. **Realistic consolidation target: {sc.realistic_target} files**")
    lines.append("")
    ...
    lines.append(f"*Total findings: {len(report.migration_tasks)} migrations "
                 ...
                 f"{len(sc.tier6_cross_folder)} cross-folder dups*")
    ...
    if args.json:
        ...
        output = {
            ...
            "totals": {
                ...
                "invalid_services": report.invalid_services_count,
                "models": report.total_models,
                "routers_with_business_logic": report.routers_with_business_logic,
                "routers_with_business_logic_list": report.routers_with_business_logic_list,
            ...
            "service_consolidation": {
                ...
                "tier6_cross_folder": len(sc.tier6_cross_folder),
