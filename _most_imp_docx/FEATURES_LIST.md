...
## BACKEND "circuit":  Client → Middleware → Routers (thin) → Controllers (orchestrate/auth) → Services (business logic + DB) → Models/Providers/Redis → PostgreSQL (PgBouncer) + External APIs.
   - backend/routers/      → Keeping the files [ backend/routers/{surface}_{domain}_{feature}.py ] - thin: accept request, call a service, return response_model. (Only the AUTO-GENERATED subset is thin + db-free; legacy fat routers stay hand-written and still contain logic + db writes — the W1 debt. New controllers auto-emit thin routers into routers/ TOP-LEVEL carrying the AUTO-GENERATED marker; routers/generated/ holds only the auto_router.py generator + AUTO_ROUTER.md + generated health endpoints.)
   - backend/controllers/  → Keeping the files [ backend/controllers/{domain}/{domain}_{feature}_{operation}.py ]. NEW controllers declare routes with @get/@post/... from routers.generated.auto_router (which auto-emits the thin routers). CURRENT STATE IS HYBRID: many controllers still import FastAPI (APIRouter/Depends/HTTPException) and a few perform db writes (core/ai_controller.py, treasury/cash_management_write_controller.py) — these violate the target. TARGET: controllers orchestrate only, delegate all DB to services, never import FastAPI. Domains: admin, ai, catalog, commerce, comms, core, customer, delegators, documents, finance, gateway, geography, governance, hr, identity.
   - backend/services/     → Keeping the files [ backend/services/{domain}/{surface}_{domain}_{feature}.py ] own DB access + transactions (services/<domain>/*.py already exists & is full: admin/, security/, and per-domain service modules). Single source of business logic + get_db() sessions.
   ...
   - backend/models/       → SQLAlchemy ORM, one module per domain (ai, catalog, comms, configuration, core, finance, gateway, geography, hr, identity, logistics, media, orders, permissions, security, supplier); all inherit db/base.Base; live in domain schemas (customer/supplier/logistic/admin/employee), NO core/platform/identity schemas.
   - backend/db/           → base.py (Base + metadata), database.py (engine + get_db() session factory), session.py, transaction.py, schemas.py (schema registry), models.py, create_tables.py / init_db.py (dev-only create_all, refuses prod), seed.py + treasury_seeder.py (idempotent), database_logging.py. get_db() is the ONLY session source; PgBouncer in front; keyset cursor helper for hot lists.
   ...
   - backend/middleware/   → orchestrator.py (ordered pipeline) + modules: api_version, country_context, csrf, database_security, device_binding, impossible_travel, ip_extraction, logging, pci_dss_compliance, rate_limit, request_id, rls_dependency, security_headers, webhook_ip_whitelist, webhook_verification, zero_trust_auth. Enforces security/RLS/geo before routers; imports only utils/settings/db(read-only).
   - utils/, middleware/   → cross-cutting only.

## Database "circuit":
   - Engine (QueuePool sync today / AsyncPG at scale) sits behind PgBouncer (transaction mode) → get_db() yields one session → ORM models across domain schemas; Alembic is the single schema source of truth (create_all dev-only); RLS enforced on every session via rls_dependency/country_context; Redis fronts catalog/session/cache reads.
   - Hot lists use keyset cursors (NEVER OFFSET); cross-domain FKs allowed and resolved from ORM metadata; check_connection_health() → SELECT 1 backs /health/ready; schemas are customer/supplier/logistic/admin/employee (no core/platform/identity).
...
# Task: Extend ZOZI's database + storage layer for media & conversational entities

## Stack
- Backend: FastAPI (Python) + SQLAlchemy. Dev = SQLite, Prod = PostgreSQL.
- Frontend: Next.js 15 (web_app) + Expo React Native (mobile_app), shared TS in frontend/shared.
- Storage of binary blobs is via provider abstraction, NOT in the DB.

## Mandatory architecture contract (verified against the repo)
1. Routers are THIN: accept request → call a service → return response_model. NO raw db.add/commit in routers (legacy fat routers are debt, do not extend them).
2. Controllers declare routes with @get/@post/... from routers.generated.auto_router (which auto-emits thin routers into backend/routers/ top-level with an AUTO-GENERATED marker). Controllers must orchestrate only — DELEGATE ALL DB to services. Controllers must NOT import FastAPI (APIRouter/Depends/HTTPException) and must NOT touch db directly.
3. Services own the DB session and all write logic (backend/services/{domain}/).
4. Binary media (videos, images) go to object storage via backend/providers/media/ and backend/providers/image/; the DB stores only metadata + storage refs (URLs/keys/ids).
5. Chat/streaming is handled by backend/providers/comms/, ai/, voice/; DB stores threads/messages/state, not the live stream.
6. All new tables MUST be covered by row-level security. RLS is currently scattered (rls_dependency, rls_middleware, rls_interceptor, rls_context, country_context) and a central db/security.py is NOT yet built — propose where new media/chat tables hook into RLS.

## Your task
Design how database management must change to support: videos, images, chats, and "a range of things" (e.g., attachments, thumbnails, transcripts, reactions, reactions, moderation flags).

Deliver:
A. The concrete SQLAlchemy model set (backend/db/ files) for these entities — table names, columns, FK relationships, indexes, and which are metadata-only vs storage-ref.
B. Storage boundary: how providers/media + providers/image are used; what metadata each upload stores.
C. Chat model design (threads/messages/participants/state) and how comms/ai/voice providers interface with it.
D. The service layer (backend/services/{domain}/) that owns writes for each new entity.
E. RLS coverage plan for every new table (and where db/security.py should centralize it).
F. A migration path (Alembic) for both SQLite (dev) and Postgres (prod), noting type differences (e.g., JSON/UUID/array handling).

Constraints: stay normalized, keep routers/controllers thin, never put blobs in the DB, and flag any place where current code violates the contract (controllers importing FastAPI or doing db writes) that your design depends on.	
...
# _____________________________________________________________________________________________[FEATUER-COMPLETION-TRACKER]

I need feature_definitions.yaml where i will give complete prompt and feature_tracker.py will track that how much percentage is complete. feature_definitions.yaml will have very basic things, feature_name, feature_discription, Section to Section detail of implementation.
...
## Goal
- Execute the "Production-Ready Module Setup": 
	AI made complete system without any gridline and keep the functions and files where AI wants without any proper system architecture understanding to keep the function.
	`SYSTEM_AUDIT_REPORT.md` is identifyng the architectural problems which AI made while generating.
	Your job is to read one by one problem and verify that is correct, and if that is correct, start to resolve that problem.

## Relevant Files
- Audit reports: `SYSTEM_AUDIT_REPORT.md` for reading and getting all the problem of the system to sort them out

## Constraints & Preferences
- NEVER modify files inside `scripts/` (audit scripts read-only).
- NEVER modify `SYSTEM_AUDIT_REPORT.md` and `system_architecture_audit` by yourself.
...
- NEVER modify `backend/main.py` router registration until module fully wired.
- Never touch `git add -A && git commit -m`, `git pull` and any other `git` request. 
- No hardcoded values; config from environment.
- ALWAYS run app + tests after each module; one module at a time.
- Use "D:\Projects\10- E-COMMERCE WEBSITE\zozi\_extra_files" for any temp files.
- Use "D:\Projects\10- E-COMMERCE WEBSITE\zozi\tests" for keeping any test files.
- You are not allowed to delete anything until you found duplication. Verify the duplication then check inside content also, if content is same then delete otherwise start mergeing both files.
- You job is to fix, resolve the architecture, structure according to circuit plan and also enhance and improve performance of the codebase, features, file to file flow and etc. 
- Always follow the `Architecture` and `Circuit` which is defined into `SYSTEM_AUDIT_REPORT.md` and `ARCHITECTURE_DIAGRAM.md`

## Process:
- Read complete `Backend` and `Frontend` in detail.
- Read `SYSTEM_AUDIT_REPORT.md` and check the problems and pick anyone Domain/Feature/Module to rescue
- Investigate the `codebase` means project to verify the problems relevant to the module are authentic.
- If you find those particular problems are authentic, then take majors actions to resolve those particular problems and some enhancement also.
- Use "D:\Projects\10- E-COMMERCE WEBSITE\zozi\tests" to keep the test file after resolving the problems.
- You can just run below files for generating updated `Audit Reports` to verify that particular problem is resolved or not which you were working on above. You are not allowed make changes in below script files.
	- `D:\Projects\10- E-COMMERCE WEBSITE\zozi\scripts\system_trackers\system_architecture_audit.py`,
- You must have to finish all the problems of that particular Domain/Feature/Module.
- You are allowed to make any `.py` file to resolve multiples problems at a time if you think you can handle efficiently, if you think you can't handle efficiently then go one by one, 
	OR, 
	accumulate problems of a `file` and `module` and resolve all together at once if you can do that efficiently.
- Once you will finish Module wise changes and fixes then do the test properly and make sure Module/Domain/Feature is working perfectly fine.	
- If you will get any problem and you get any false problem detection in the audit file, you must have to report me in detail instead of trying to editing into `script` files.
- proceed according to the plan and start resolving the problem into group of features.
- Do not avoid `advisories` and take them very seriously because `advisories` are playing most vital role to keep architecture aligned.
- If you are getting `False Positive` then you have dig-down what is actual reason for that `False Positive` and Identify that is that architectural problem or report problem
	- If there is `ARCHITECTURAL` or `Feature Structure` problem then resolve it. and if it is not going to be resolve then we have to make changes particular changes into the report but after double check.

- You job is to fix, resolve the architecture, structure according to circuit plan and also enhance and improve performance of the codebase, features, file to file flow and etc. 
- Do the complete `testing` after `resolving` and do the playwright test also to ensure everything is working according to the architecture and plan and what you fix is working correctly.

**pick highly demage module or feature for repair, resolve, enhance, improve, and align according to the architecture according to the `SYSTEM_AUDIT_REPORT.md` after making plan start to implementation**
**Have to Fix all the Controllers to use generate automatically all the routers**
**controller/router writes to DB (must be a service)**
---

Start fixing the problem after reading and investigation and understanding each and everything in detail:

---


- Fast Tracking to go to the Problem to resolve that.
- Module to Module (Like Finance related) all the problems list down together which will be easier for AI to track-down and resolve easily.


Read the "D:\Projects\10- E-COMMERCE WEBSITE\zozi\scripts\system_trackers\system_architecture_audit.py" in detail and make corrections
This script must have to work alone with any json & yaml and just one .md report.
It must be track all the problem of frontend, backend, design, database, architectural, work flow, files and functions flow and etc.
AI made complete system without any gridline and keep the functions and files where AI wants without any proper system architecture understanding to keep the function.
So we develop the audit file for getting structural problem which AI will resolve later.
D:\Projects\10- E-COMMERCE WEBSITE\zozi\GOVERNANCE_AUDIT_REPORT.md this is the audit report also which generated by the script and there are some little problem which have to be fix.
do the complete study and fix the problem of the scripts.
remove the .governance/scaffolding_contract.json and .governance/AI_AGENT_PROTOCOL.md and any other files which is getting input for this script. I told you this script must work itself.



There are some serious alligation by another AI on the the generated report of the script and you have to verify all of them are correct or not of the attached file 
read complete script in detail.
if you find also same then giving me corrected complete functions to be replace into the script.

- Now we have biggest challenge to manage which is have to update `correctly` the `system_architecture_audit.py` according to `ARCHITECTURE_DIAGRAM.md` to generate accurate audit suggestions.
- Check complete `ARCHITECTURE_DIAGRAM.md` in detail then read `system_architecture_audit.py` completely.
- Provide me bunch of code which is replacable easily.
- Remove all dead code which is generating the governance files because we just need one file `SYSTEM_AUDIT_REPORT.md`, means no `.json`, another `.md` file. just `SYSTEM_AUDIT_REPORT.md`
- Remove all the unnecessary mermaid, current, should be, and add these mermaid of `ARCHITECTURE_DIAGRAM.md` which is target.
- Remove all other dead codes also which you think is unnecessary to achieve the targeted architecture but be careful.
- One more important `SYSTEM_AUDIT_REPORT.md` should to show all the problems list but also `Feature wise` or `File wise` becasue it is easry to track to resolve and easy to understand the problem accumulatedly.

Now have to do final verifying and comparison and checking the `system_architecture_audit.py` is giving all the suggestion as same as `ARCHITECTURE_DIAGRAM.md` flows ?
...
- Investigate, what changes needed in the `controllers` codes for auto-generation of `routers`. Make a complete plan for making changes. and start implementation on changes the `controllers` for auto-generating `routers`
- Do the Advanced investigation in detail `module to module` for following of the architecture, wiring, db table, improvement of code, broken code to be repair, over all evrything.
**Do detail investigation for missing pieces of the backend to complete and what the problems in the backend, list down all of them**
...
**Do the complete investigation of wiring of everything to auto-generate the router & then complete running of the backend properly**
**Make complete plan where you list all the problem and todo list in detail to follow**
...
Do the complete investigation and Audit to identify below:
- Transfer all business logics the `routers` → `controllers` & `services` ?
- Confrim me all the `controllers` now have decorator and ready for auto-generate `routers` ?
- Confrim me all the `models`, `controllers` & `services` complete, there is no missing pieces remaining ?
- Confrim me all the wiring is completed between `models`, `controllers` & `services` ?

Make a report: 
- list down the file to file migration of business logics from `routers` to `services` & `controllers`. 
- list down file to file plan for wiring of the `models`, `services` and `controllers`. 
- list down all the missing files and folders according to the architecture. 
- list down `controllers` to convert for auto-generating `routers`.
---

`scripts\system_trackers\migration_tracker.py`
Read the `ARCHITECTURE_MIGRATION_REPORT.md` and resolve all the **Transfer all business logics the `routers` → `controllers` & `services` in the correct and relevant files** problem one by one after reading the backend and related folders and file for proper verification to avoid new problem and the duplication.

Read the ARCHITECTURE_MIGRATION_REPORT.md and resolve all the **Duplication problem by merging similar files and functions** problem one by one after reading the backend and related folders and file for proper verification to avoid new problem and the duplication.

Read the ARCHITECTURE_MIGRATION_REPORT.md and resolve all the **Missing items and wiring between models, services, controllers** problem one by one after reading the backend and related folders and file for proper verification to avoid new problem and the duplication.

Read the ARCHITECTURE_MIGRATION_REPORT.md and resolve all the **Put the decorators on the controllers to preparing them for auto-generate routers** problem one by one after reading the backend and related folders and file for proper verification to avoid new problem and the duplication.



pilot on admin_catalog_operations.py (fully covered). First read it to know its routes, then delete + regenerate + verify the generated routers take over.
...
Why are checking the git, that is not your work.
- you working is run the `scripts\system_trackers\migration_tracker.py`.
- check the result of `scripts\system_trackers\migration_tracker.py` and verify by checking codebase and identify what are correction needed to fix the `scripts\system_trackers\migration_tracker.py`
- your task is just fix the `scripts\system_trackers\migration_tracker.py` becasue we will make editing systematically.

---
 
- Now we will make complete feature to feature instruction of AI from `FEATURES.md` to `scripts\system_trackers\feature_definitions.yaml` according to above TEMPLATE system according to `P2` and RULES which we design into `P1`.
- Read [Order Tracker & Order Management] from `FEATURES.md` in detail and add feature into `scripts\system_trackers\feature_definitions.yaml` according to the TEMPLATE and RULES and keep all important points of the feature very carefully.
- Feature detail take from `FEATURES.md` only and some understanding of architecture for referance but convert into RULE of architecture of 'P1'.
- RULE of architecture will be 'P1' from `scripts\system_trackers\feature_definitions.yaml`.
- TEMPLATE of adding feature prompt will be `P2` from `scripts\system_trackers\feature_definitions.yaml`.
- be carefully at each point. make a plan properly then start implementation to add into `scripts\system_trackers\feature_definitions.yaml` becasue we can't afford any mistake becasue if you make mistake then AI will make mistake to implement feature into system.

---
let's script generate both type of file becasue i can see all properly also for providing gudiance.

```
scripts\system_trackers\feature_definitions.yaml (requirements seed)
        │
        ▼
scripts\system_trackers\feature_implementation_checker.py ──► FEATURE_IMPLEMENTATION_CHECKER.json, FEATURE_IMPLEMENTATION_CHECKER.md (structured: expected vs found per feature)
        │                               
        ▼
scripts\system_trackers\feature_tracker.py ─────────────────► FEATURE_TRACKER.json, FEATURE_TRACKER.md (structured: actual codebase state)
        │
        ▼
scripts\system_trackers\generate_corrections.py ────────────► FEATURE_CORRECTION.json, FEATURE_CORRECTION.md (actionable: what to fix, where, how)


scripts\system_trackers\system_architecture_audit.py ───────► SYSTEM_AUDIT_REPORT.md (root level investigation)


scripts\generate_data_dictionary.py ───────► SYSTEM_AUDIT_REPORT.md (root level investigation)

scripts\generate_scaffolding.py

scripts\generate_codebase.py
...
start to making the script and test also them and ensure everything is working 100% correctly.

---
- `scripts\system_trackers\feature_tracker.py` is not sorting properly according to the domain and feature which is most important part of the process and also not extracting independently becasue it must be work independently to complete the process.
- `scripts\system_trackers\feature_tracker.py` has job to track all the features and list down of the system, not just 17 which are mentioned into `scripts\system_trackers\feature_definitions.yaml`.
- `scripts\system_trackers\feature_tracker.py` must have to work independently to findout in complete codebase that how many domain, features and what are their files, and what files' operation/functions in words.


**make a plan for implementation documents\NEW_STRUCTURE.md into our codebase with detail reading of backend and frontend, step by step. I guess first step is scaffolding pipeline, second is copy the files from lagacy folders to new folder, these 2 will make 40% of work easier, then we will start merging the files according to features will reduce 10% work and then we will make changes in code according to new structure.**

services, models, controllers, db, routers & utils into domains, modules, infrastrcutures, rbac, kernel, and new folder.
and do not touch git

Plan 
| sno | services 														| domains | modules | infrastrcutures | rbac | kernel |
|	1 | "backend\services\admin\admin_catalog_operations_service.py"	|		  |			|				  |		 | 		  |

| sno | controllers | domains | modules | infrastrcutures | rbac | kernel |
...
-----------------------------------------------------
> GENERAL `investigation` PROMPT 
-----------------------------------------------------
- Read `ARCHITECTURE_DIAGRAM.md` -`Law` must be read top to bottom because it is $Benchmark$.
- Do the complete investigation and understand `backend`, `frontend`, `database`, complete `architecture` problems.
- After Investifation resolve the below problem and ensure all the problem correctly and accurately resolved.
...
- Read `ARCHITECTURE_DIAGRAM.md` -`Law` must be read top to bottom because it is $Benchmark$.
- Read in detail `CODEBASE_AUDIT_BY_ASHER.md` top to bottom and start audit for each phase and prompt.
- Use maximum sub-agent (10 to 20) for at a time for *accuracy* and and give them *clear instructions* for better and detail audit result.
...
-----------------------------------------------------
- Read `ARCHITECTURE_DIAGRAM.md` -`Law` must be read top to bottom because it is $Benchmark$. 
- Read the codebase in file to file and code to code and make a detail report in `_audit_results\09_09_26\cleaning\**` for cleaning the codebase, duplication to merge, alignment properly and accurately.
- check everything in detail before conculding report  
- Use maximum sub-agent at a time to identify the problem & solution `accurately` and `faster`and give them clear instructions to identify problem & solution correctly and in detail without escaping anything.

You need to do more detail investigate becasue I read the _audit_results\09_09_26\cleaning\CLEANING_ALIGNMENT_REPORT.md, _audit_results\09_09_26\cleaning\CORRECTION_LIST.md which are not correctly and complete implementation plan cleaning and alignment. use 50+ sub-agent because it large codebase and each agent will investigate specific area to do accurate audit. keep on eye on each agent working by mentaining log of each agent.

I need perfect alignment, wiring, codebase management, follow the laws, cleaning of duplicate code, file, unnecessary file and etc, wiring of the Neon Database, alignment of tables, completeness of tables and their fields and etc, all the logics and operational function must be return complete sensible output, not empty/dumy functions, country scope, 5-type of User scope, all their functions operations, everything must be check of each corner of the code and problem & correct solution.
...
- Use maximum sub-agent at a time to identify the problem & solution `accurately` and `faster`and give them clear instructions to identify problem & solution correctly and in detail without escaping anything.
      ...
      "line": 1,
      "function": "N/A",
      "violation": "Forbidden root-level __init__.py file",
      "severity": "High",
      "law": "Law 27",
      "proposed_fix": "Delete backend/__init__.py"



-----------------------------------------------------
> DETAIL AUDIT PROMPT - role-play test as human of each feature and invesitage `BIGENNER LEVEL`
-----------------------------------------------------
- Read `ARCHITECTURE_DIAGRAM.md` -`Law` must be read top to bottom because it is $Benchmark$.
...
-----------------------------------------------------
> DETAIL AUDIT PROMPT - role-play test as human of each feature and invesitage - 
-----------------------------------------------------
- Read `ARCHITECTURE_DIAGRAM.md` -`Law` must be read top to bottom because it is $Benchmark$.
- Do the complete browser playwright test of all pages one by one and all the features as each role-play (admin, customer, employee, supplier, logistic partner) of each page and check which page is not properly not connected with the `backend`, `database`.
- Do role-play test as human of each feature and invesitage which feature of which page is working correctly and which feature is not working. 
...
- Use properly seed data system which we have already.
- Do not forget two country's detail {emplyee, supplier, customer, logistic, product, banner, flash discount and other} into the seed database to check how the backend and frontend working on them for 2 countries becasue as we discussed both must be separated and for admin it should be both separate and consolidated.
- Make a report in `_audit_results\05_09_26`
	| Sno | Module | Domain | Page | Feature | Role-Play Check | What is in Feature | Status | Location of Problem | Problem | Solution |


- Make another report in `_audit_results\05_09_26` for detailed feature to feature completion and step by step what are operations.
	| Sno | Module | Domain | Feature | Discription of Feature (what is happening into feature step by step) | backend:modules | backend:domains | backend:infrastructure | backend:providers | backend:rbac | backend:kernel | backend:middleware | backend:jobs | backend:tests | backend:connection report | frontend:web_app | frontend:mobile_app | frontend:Shared / Utils | frontend: Web Tests | frontend: Mobile Tests | database table in neon | cache detail | security detail | Completion Status % | Remaining todo | Comments for reference |
...
- Read `ARCHITECTURE_DIAGRAM.md` -`Law` must be read top to bottom because it is $Benchmark$. 
- **AUDIT** | 
	- For Audit use the `D:\Projects\10- E-COMMERCE WEBSITE\zozi\_audit_results\**` 
	- Always do the detail investigation and scanning of problem from top to root for changes, and also deep level [code to code] and [file to file] for finding complete pipeline problems to resolve.
	- Use loging system to track all the agent performance and working properly of audit to ensure the audit is relevant and correct.
- **TEMPORARY TEST** | For temporary test use the `D:\Projects\10- E-COMMERCE WEBSITE\zozi\test\_temporary_test\**` 
- **PERMANENT TEST** | For permanent test use the `D:\Projects\10- E-COMMERCE WEBSITE\zozi\test\domains\**`, `D:\Projects\10- E-COMMERCE WEBSITE\zozi\test\modules\**`, `D:\Projects\10- E-COMMERCE WEBSITE\zozi\test\frontend\**` and etc.
- **TEMPORARY SCRIPT** | For temporary test use the `D:\Projects\10- E-COMMERCE WEBSITE\zozi\scripts\_temporary_script\**` 
- **PERMANENT SCRIPT** | For permanent test use the `D:\Projects\10- E-COMMERCE WEBSITE\zozi\scripts\domains\**`, `D:\Projects\10- E-COMMERCE WEBSITE\zozi\script\modules\**`, `D:\Projects\10- E-COMMERCE WEBSITE\zozi\script\frontend\**`
- **BROWSER TEST** |
	- Use the `D:\Projects\10- E-COMMERCE WEBSITE\zozi\test\_browser_test\**` or `D:\Projects\10- E-COMMERCE WEBSITE\zozi\test\_playwright_test\**` for browser test.
	- Do the complete broswer playwright test when you finish editing or changes to ensure the required edit and change working 100% correctly.
- **DEVEOPMENT LOGGING**| 
	- ALWAYS use `D:\Projects\10- E-COMMERCE WEBSITE\zozi\test\_development_logging\**` to keep all the changes into the project to track properly changes of development of project. 	
- ALWAYS ensure and think before any changes regrading `database`, `tables`, `security`, `follow architecture`, what will impact and how to handle that properly with edit.	
- ALWAYS use `Neon` Database, `Velkey` Cache, `R2` Object Database for seed data and etc (if any). 
- ALWAYS ensure to follow the `Law` for Architecture of `ARCHITECTURE_DIAGRAM.md` while planing of editing and changes.
- ALWAYS search existing files and structure for changes, editing, improvement rather then creating new file.
- ALWAYS do the scanning of the project to ensure all the files and functions are placed accurately in there designated places, if not then shift them correctly according to `ARCHITECTURE_DIAGRAM.md` `LAW`.
- ALWAYS run app + tests after each major edit and changes.
- NEVER rename/move a file without updating ALL imports atomically. never break the import chain and ensure all import correctly working while editing.
- NEVER touch `git add -A && git commit -m`, `git pull`, `git push` & any other `git` request. 
- No hardcoded values; config from environment.
- You are not allowed to delete anything until you found duplication. Verify the duplication then check inside content also, if content is same then delete otherwise prefer to merge files.
- Use maximum sub-agent at a time to identify the problem & solution `accurately` and `faster`and give them clear instructions to identify problem & solution correctly and in detail without escaping anything.
- ALWAYS do the test and verification at each step after completion, to avoid drifting of Agent and keep aware of the alignment and correction (it is essential).
- Our target is to achieve architecture clean and production-ready. do a deep investigation of database alignment, security, scalability, and missing functionality and etc.
-----------------------------------------------------
